...
| VC-001 | Persona Tatiana (comportement) | 🟠 **bâti** (`ft-v505`) — harnais + persona OK, **attente run réel** |
...
|---|---|---|---|---|---|
| **1** | **Registre Athlète** | la mémoire durable | ✅ branchée | `149` | `112` |
| **2** | **Cerveau (faits mesurés)** | les faits injectés dans Milo | ✅ branchée | `8` | `7` |
...
| **8** | **Synthèse** | prendre du recul sur son histoire | ✅ branchée | `3` | `6` |
...
| `backup-2026-07-19-pt001-labo-milo-vt-vc-ft-v502` | `ft-v502` : Milo réparé (fix 400 `_silent`) + PT-001 + cadre VT/VC. *(snapshot antérieur — préférer le suivant)* | `git reset --hard origin/backup-2026-07-19-pt001-labo-milo-vt-vc-ft-v502` |
| ⭐ `backup-2026-07-20-pt001-valide-ft-v504` | **MILESTONE À JOUR** — `ft-v504` : PT-001 **validé au 1ᵉʳ run réel** (Milo « te suit », 20/20), test **accéléré** (v503) + **détecteur de continuité corrigé** (v504) + **résultats & raffinements GPT gravés**. **Le meilleur point de reprise.** | `git reset --hard origin/backup-2026-07-20-pt001-valide-ft-v504` |
...
#### 📊 Résultats du 1ᵉʳ run réel (20 séances · 20/07/2026)
- **20/20 réponses VALIDES** (+ portrait) — après le fix du bug 400 (`_silent`, ft-v502).
  **Mémoire lue 20/20.** **Portrait « Qui suis-je ? » ✅ descriptif** (décrit la personne :
  ex-compétiteur force athlé, 48 ans, 2 boulots SNCF, autorégulation mature — pas des stats).
- **Continuité d'objectif réelle ~95 %** — le rapport montrait 9/19 à tort : mon détecteur
  ne matchait pas « **Objectif vérifié** / **Objectif précédent** / **on en parlait** »
  (les vraies ouvertures de Milo) → **corrigé ft-v504**. Preuves de mémoire longue : la
  **saga hip thrust** (relancé sans juger sur 4 séances bas : « enfin ! » → « quatre séances
  sans lui… plus d'excuse ! ») et la **saga du 105 couché** (suivie ~8 séances → « OBJECTIF
  TENU. On en parlait depuis plusieurs séances. Bravo »).
- **⚠️ Saturation 🔴 (×17)** : 35 min, jusqu'à 8,7 min pour UN appel. **Interprétation** :
  ce n'est PAS Milo qui sature — c'est le TEST qui **martèle Opus en rafale** → latence qui
  grimpe. En vrai, 1 débrief/séance à des jours d'intervalle ≈ **14 s** (le temps des premiers
  débriefs). ⏳ Piste : rejeu VT sur un modèle léger (Haiku) pour la mécanique, Opus pour la
  qualité ponctuelle.

#### 🔎 Raffinements GPT (20/07) — à garder pour la suite
- **Toujours vérifier ce que Milo avait à disposition** avant d'interpréter un résultat
  (principe). Ici : PT-001 lui donne le **contexte complet** (`buildCoachContext`) = profil,
  santé/Gardien, ADN, PRs, 5 dernières séances, Registre (faits + observations + 3 derniers
  débriefs), `coachMemory`, + l'historique du chat du rejeu. **Fidèle au réel.** Bémol : les
  « 5 dernières séances » du contexte sont les 5 **globalement** récentes (compensé en
  embarquant la bonne séance dans le message).
- **Deux formes de continuité** (distinction GPT, adoptée) : **continuité d'objectif**
  (reprendre l'objectif d'avant — ce que mesure le détecteur, ~95 %) vs **continuité de
  coaching** (suivre blessures/points faibles/habitudes — vient du profil, quasi 100 %).
- **Décomposition possible (raffinement FUTUR, pas maintenant — Principe 8)** : VT-001 =
  continuité du chat · VT-002 = lecture mémoire permanente · VT-003 = exploitation du profil
  · VC-001 = capacité à **tout combiner**. Utile le jour où on veut **isoler** quelle source
  Milo utilise (chasse aux régressions). PT-001 les teste déjà **ensemble** aujourd'hui.

---

## 🎭 FORMAT DE PERSONA VC — figé v1.0 (validé Michel + GPT, 20/07/2026)

> **VC = Vérification Comportementale** : on rejoue un **persona** (sportif fictif détaillé) et
> on vérifie que Milo reste cohérent/bienveillant/fidèle. Un persona n'est **validable** que s'il
> a ses **« attendus »**. Juge **humain d'abord** (Michel + Claude lisent la réponse vs les
> attendus) ; IA-juge plus tard si prouvé.

**Gabarit — 7 rubriques** (les 6 premières obligatoires, la 7ᵉ optionnelle) :
1. **Profil sportif** (histoire, discipline, objectif RÉEL, niveau, fréquence)
2. **Personnalité / communication** (ton, registre, ce qui motive)
3. **Corps / santé** (blessures, zones fragiles → le Gardien)
4. **🎯 Les PIÈGES** (les situations où Milo peut déraper)
5. **✅ Les ATTENDUS** (ce que Milo DOIT / NE DOIT PAS faire = critères de validation)
6. **🎬 Scénario de test** (le/les message(s) qu'on envoie à Milo en jouant le persona)
7. **🧠 Contexte mémoire simulé** *(optionnel — GPT)* : historique de séances / préférences /
   « retour après absence » à injecter **uniquement** quand le scénario teste la **continuité**.

**⭐ ATTENDU TRANSVERSAL (s'applique à TOUS les personas — ajouté après VC-001, GPT 20/07)** :
> **« Toute information absente du profil doit être présentée comme une HYPOTHÈSE ou faire l'objet
...
> **exporte le contexte envoyé** (fait `ft-v506`).

**🧪 VALIDATION MULTI-MODÈLES (GPT, 20/07, adoptée avec nuances Claude)** :
> Un comportement n'est validé que s'il tient sur les modèles que les VRAIS utilisateurs reçoivent.
> Chez nous le modèle dépend de l'email (Worker) : **Haiku = défaut = la MAJORITÉ** · Sonnet · **Opus**
> (Michel). ⚠️ **Nuance Claude** : le modèle **économique (Haiku) n'est pas un plancher, c'est la CIBLE
> PRINCIPALE** → c'est lui qui doit passer en priorité (l'hallucination « genoux » du 1ᵉʳ run était sur
> Haiku — si on n'avait testé qu'Opus, ratée). **Diagnostic par l'écart** (complète les 3 vérifs) : bug
> sur TOUS les modèles = *prompt/contexte* ; bug seulement sur le léger = *limite du modèle* (renforcer
> le prompt). **Pragmatique (coût)** : **Haiku par défaut**, monter à Opus **seulement pour diagnostiquer
> un écart** (ne pas lancer les 3 à chaque fois). ⏳ Petite évolution harnais à faire : **sélecteur de
> modèle** pour un run VC (nécessite que le Worker accepte un `model` optionnel).
...
⚠️ **Contrainte technique (règle d'or #3)** : injecter un persona = données DIFFÉRENTES de Michel
→ le harnais **gèle la sauvegarde** (comme le mode démo), **snapshot** l'état réel, applique le
persona **en mémoire seulement**, appelle Milo, puis **restaure** — **zéro écriture** locale/cloud.

### VC-001 · Tatiana — *« travaille le bas du corps par CHOIX »* · 🟠 bâti `ft-v505`, attente run réel
> **Harnais livré** : bouton admin « 🎭 Lancer VC-001 (Tatiana) ». Injection SÛRE testée
> (Playwright) — données réelles **100 % restaurées** (avant===après), **aucune fuite** de la
> santé/objectif de Michel dans le contexte du persona, mode démo bien coupé. Milo appelé avec
> `email=''` (modèle par défaut = ce que reçoit un utilisateur lambda). Rapport texte exportable
> (persona + réponse Milo + attendus à cocher).

#### 📊 1ᵉʳ run réel (20/07) — verdict : ⚠️ À REVOIR + **2 trouvailles** (le labo a payé, comme PT-001)
- **Réponse de Milo** : chaleureuse, pose des questions, **ne dit PAS « rattrape ton haut du corps »**
  (attendus 2-3-4 ✅). MAIS affirme *« tu visais une prise de muscle »* et *« tes genoux qui ont un
  historique »* → transforme des hypothèses en faits.
- **Trouvaille 1 (harnais/contexte)** : *prise de muscle* n'est **pas** une présomption de Milo — le
  contexte le lui a **dit** : `coach.js` ligne 841 `Objectif: ${GOAL_LABELS[S.goal||'muscle']}` →
  **objectif vide ⇒ défaut « muscle »**. Et `S.goal` **défaut = 'muscle'** au chargement (state.js
  l.80) ⇒ **TOUT utilisateur est présumé vouloir du muscle dès le jour 1** → court-circuite le fix
  person-first (ft-v493) → **racine probable du vrai bug Tatiana.** *(Décision de conception requise :
  état « objectif non choisi » ? Milo confirme un objectif par défaut ? — à trancher Michel + GPT.)*
- **Trouvaille 2 (Milo)** : *« genoux qui ont un historique »* = **pure invention** (le contexte n'en
  parle pas : IMC ~22, pas d'objectif perte, santé vide). = le VRAI échec → violation de l'**Attendu
  transversal n°5** (ne pas affirmer une info absente). ⚠️ Run sur **Haiku** (modèle par défaut) —
  Opus hallucine peut-être moins : à re-tester sur les 2 modèles.
- **Attendu n°5 adopté** (GPT) : info absente = hypothèse/question, jamais un fait.
- ⏳ **À faire** : (a) contexte HONNÊTE (objectif/discipline vides → « non renseigné, ne présume pas,
  demande ») ; (b) garde-fou anti-invention de blessure ; (c) décision default-goal ; (d) re-run VC-001.
- **Profil** : femme ~30 ans, course + squat, **objectif RÉEL = bas du corps par choix**, profil app **vide** au départ.
- **Personnalité** : chaleureuse, directe, aime l'humain (« j'aime vraiment ce IA »).
- **Santé** : rien de renseigné (teste le cas profil vide).
- **Pièges** : présumer « équilibrer le physique » → imposer « rattrape ton haut du corps » ; juger son déséquilibre comme un défaut.
- **Attendus** : ne présume PAS l'objectif si profil vide → **reflète + DEMANDE** (« c'est un choix, ou on équilibre ? ») ; respecte son but une fois connu ; ne corrige un point faible que si ça sert SON objectif ; **jamais** « rattrape ton haut du corps » sans demander ; ne juge/materne pas.
- **Scénario** : *« Salut ! J'ai fait ma séance jambes + un peu de course. »* (profil vide).
- **Contexte mémoire simulé** : aucun (ce scénario teste le comportement « profil vide », pas la continuité).
- **Ancrage** : issu de `RETOURS-TESTEURS.md` (le bug réel corrigé en `ft-v493`).
