  ...
  try {
    ...
    const userContent = [
      ...
      { type:'text', text:
        ...
        + '- "Poids" → weight (kg)\n'
        + '- "Taux de graisse corporelle" ou "Taux de graisse" (%) → bf ; "Graisse corporelle" ou "Masse graisseuse" (kg) → fatMass\n'
        + '- "Masse osseuse" (kg) → bone ; "Protéine" ou "Masse protéique" (kg) → protein ; "Eau corporelle" ou "Masse hydrique" (kg) → water\n'
        + '- "Muscle" ou "Masse musculaire" (kg) → muscle ; "Muscle squelettique" (kg) → skMuscle\n'
        + '- "Indice de graisse viscérale" ou "Graisse viscérale" → visceral (petit nombre entier, ex. 6)\n'
        + '- "Taux Métabolique de Base", "BMR" ou "TMB" (kcal) → bmr\n'
        + '- "Âge corporel" (années) → metaAge ; "IMC" ou "BMI" → imc\n'
        + '- "Score corporel" (sur 100) → bodyScore ; "Masse maigre" ou "Masse corporelle maigre" (kg) → leanMass\n'
        + '- "Graisse sous-cutanée" (%) → subFat ; "Indice de masse musculaire squelettique" (kg/m²) → smi\n'
        + '- "Date des mesures" ou "Détail des mesures" → date (format YYYY-MM-DD)\n'
        + '⚠️ ATTENTION aux "Taux ...%" (pourcentages) : ils ne remplissent PAS les champs en kg. Ex. "Taux de muscle %", "Taux de masse hydrique %", "Taux de masse musculaire squelettique %" → NE PAS les mettre dans muscle/water/skMuscle (qui sont en kg). Ne remplis skMuscle que si un "Muscle squelettique" en kg est donné, sinon null. Ignore aussi "Poids idéal" et "Niveau d\'obésité".\n'
        + 'DÉTAIL PAR SEGMENT (sections muscle = "Analyse segmentaire de la masse musculaire" ou "Équilibre musculaire" ; graisse = "Analyse segmentaire de la masse grasse" ou "Analyse segmentaire de la graisse" — valeurs en kg, affichées à gauche=G/L et à droite=D/R du schéma corporel) :\n'
