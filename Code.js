...
// 🔐 EMPREINTE REGÉNÉRÉE LE 30/08/2026 — Michel était devant son PC, on arme enfin la
// serrure. L'empreinte du 25/08 était bien là, mais sa clé en clair n'existait NULLE PART
// (elle ne doit jamais toucher ce dépôt public) : une empreinte ne se remonte pas, donc la
// seule issue était d'en refaire une paire. La clé a été transmise à Michel dans la
// conversation, et posée par lui dans `FT_COUNT_TOKEN` chez Cloudflare.
...
var _COUNT_TOKEN_HASH_ = '0882b64cea5a100f52ef8d8e7b02e688dde6f94a47bdbe66524eb048bf94630a';
...
// ───────────────────────────────────────────────────────────
/* ═══ 📚 LE CATALOGUE PART AVEC LE DOCUMENT — ft-v1164 (07/09/2026) ══════════════════════════
   ⛔⛔ LE DÉFAUT, MESURÉ AVANT D'ÉCRIRE UNE LIGNE : le prompt d'import fait 8 577 caractères et
   contient **ZÉRO** nom du catalogue, et l'app envoyait `{action:'importProgram', images}` — la
   liste n'était **jamais transmise**. 👉 ***Le modèle écrit « Presse 45 degrés » parce que c'est
   ce qu'il y a sur la feuille, et personne ne lui a dit que l'app appelle ça « Press Jambes 45° ».***

   ⛔ CE QUE ÇA COÛTAIT, sur l'import réel de Michel : **4 exercices créés en douce** (Presse 45
   degrés · SDT roumain · Cardio léger · Elliptique/cardio léger). Un exercice hors catalogue n'a
   **ni photo, ni figurine, ni historique** — il a lu *« aucun repère dans ton historique »* sur
   une presse où il avait fait **280 kg le 4 septembre**.

   ⭐⭐ ET C'EST **R8**, DÉJÀ CORRIGÉ AILLEURS : `coach.js:_catalogueContext()` envoie la liste à
   Milo **depuis ft-v713**, et la règle est écrite mot pour mot dans `CLAUDE.md` — *« une consigne
   qui NOMME une source sans que cette source soit dans le contexte »*. **Personne ne l'avait fait
   pour l'import.** Un correctif posé sur une porte et pas sur sa jumelle.

   ⛔⛔ LA CONSIGNE N'EST PAS « UNIQUEMENT CES NOMS », ET C'EST LE CŒUR DE LA PRÉCAUTION.
   Un programme peut légitimement contenir un exercice que l'app ne connaît pas. Forcer un nom du
   catalogue reviendrait à **remplacer un exercice par un autre** — bien pire que d'en créer un
   nouveau (**R29** : le coût de l'erreur décide). On dit donc : *emploie le nom EXACT quand c'est
   manifestement le même exercice, sinon garde le nom du document.*

   ⚠️ LE MODÈLE PROPOSE, LE CODE VALIDE : la liste est bornée (600 noms, 80 caractères chacun) et
   nettoyée ici. Une charge utile absente ou vide rend `''` → le prompt est **exactement** celui
   d'avant, donc un client pas encore à jour se comporte comme aujourd'hui. */
function _blocCatalogue_(body) {
  try {
    var l = (body && body.catalogue) || [];
    if (!Array.isArray(l) || !l.length) return '';
    var vus = {}, noms = [];
    for (var i = 0; i < l.length && noms.length < 600; i++) {
      var n = String(l[i] == null ? '' : l[i]).trim().slice(0, 80);
      if (!n || vus[n]) continue;
      vus[n] = 1; noms.push(n);
    }
    if (!noms.length) return '';
    return 'LISTE DES EXERCICES QUE L\'APPLICATION CONNAÎT (' + noms.length + ') :\n'
      + noms.join(' · ')
      + '\n\n⛔ RÈGLE DE NOMMAGE — elle s\'applique à CHAQUE champ "name" que tu produiras :\n'
      + '- Quand un exercice du document est MANIFESTEMENT le même que l\'un de ceux ci-dessus, emploie le nom EXACT de la liste, caractère pour caractère (ex. « Presse 45 degrés » ou « presse à cuisses » → « Press Jambes 45° » ; « SDT roumain » → « Soulevé de Terre Roumain Barre »).\n'
      + '- ⛔ SINON, GARDE LE NOM DU DOCUMENT tel quel. Cette liste n\'est PAS une liste fermée : un programme peut contenir un exercice que l\'application ne connaît pas encore, et le forcer dans une case voisine remplacerait un exercice par un autre — c\'est pire que d\'en créer un nouveau.\n'
      + '- ⛔ Dans le doute, garde le nom du document. On préfère un exercice en trop à un exercice FAUX.\n';
  } catch (e) { return ''; }   // jamais bloquant : au pire, le prompt d'avant
}
...
function handleImportProgram_(body) {
  ...
  try {
    ...
    if (data.days) data.days.forEach(day => (day.exercises||[]).forEach(ex => {
      ...
      /* 🔥 LE TYPE PAR SÉRIE — ft-v1156 (07/09/2026).
         Michel importe son programme réel : son PDF porte une colonne « Type » (ECH / TRAV), et
         les 4 lignes d'échauffement du développé couché (50×5, 65×3, 80×2, 85×1) sont ressorties
         en 4 EXERCICES séparés nommés « Développé couché (ECH) ». 9 lignes → 9 « exercices », là
         où le document en contient 5 distincts.
         ⛔⛔ Ne pouvant pas écrire l'échauffement dans la SÉRIE (la règle 4 l'interdit), le modèle
         l'a écrit dans le NOM. C'est R4 : l'info existe, elle n'atteint pas la donnée — et un nom
         suffixé casse l'historique et les records de la personne.
         ⚠️ LA RÈGLE 4 N'EST PAS CASSÉE, et c'est le cœur de la précaution : elle interdit de
         DEVINER un échauffement depuis une PROSE (« à l'échec », « échauffement » dans une note
         de méthode) — décision @57, toujours juste. Ce champ-ci n'existe que quand le document
         porte une COLONNE qui classe CHAQUE LIGNE. Structure ≠ prose.
         ⛔ Deux valeurs seulement : 'W' (échauffement) ou '' (série normale). Ni 'E' ni 'D' ici —
         l'échec reste une méthode qui va en note, le dropset garde son `setType` d'exercice. */
      ex.setTypePerSet = Array.isArray(ex.setTypePerSet)
        ? ex.setTypePerSet.map(v=>String(v||'').toUpperCase()==='W' ? 'W' : '')
        : [];
    }));
    /* 📅 UNE DATE DE DÉBUT DÉJÀ PÉRIMÉE EST REFUSÉE — ft-v1157 (07/09/2026).
       ...
       l'exemple de structure du prompt, juste au-dessus (`"startDate":"2026-03-23"`).
       ⭐⭐ Un exemple qui RESSEMBLE à une vraie donnée se fait recopier : les autres champs de
       l'exemple sont des mots (« nom du programme »), impossibles à confondre — une date, elle,
       est plausible. L'exemple ne porte donc plus de date, ET on vérifie ici.
       ⛔ LE CRITÈRE EST DU SENS, PAS UNE BORNE ARBITRAIRE : on n'importe pas un programme dont
       le cycle s'est terminé AVANT le jour de l'import. Une date passée reste acceptée tant que
       le cycle court encore — quelqu'un qui importe un bloc commencé il y a deux semaines a
       raison de le dater ainsi.
       ⛔ On efface la DATE seulement, jamais la durée : `weeks` vient du document et reste vraie.
       Sans date, la carte affiche simplement « Semaine 1 / N » — l'app sait déjà vivre sans
       (`progPeriode` rend null), et la personne peut la poser à la main dans l'éditeur (R29 :
       mieux vaut ne rien dire que dire une date fausse). */
    if (data.startDate) {
      var _d0 = new Date(data.startDate);
      var _w  = parseInt(data.weeks) || 0;
      if (isNaN(_d0.getTime())) {
        data.startDate = '';
      } else if (_w > 0) {
        var _fin = new Date(_d0.getTime() + (_w * 7 - 1) * 86400000);
        if (_fin.getTime() < Date.now()) data.startDate = '';
      }
    }
...
function handleImportHistory_(body) {
  ...
  try {
    ...
    /* 📚 ft-v1164 — LA JUMELLE : l'import d'historique portait EXACTEMENT le même trou, et le
       réparer d'un seul côté aurait été la 5e fois de la journée qu'on oublie la porte d'à côté. */
    var _catH = _blocCatalogue_(body);
    if (_catH) userContent.push({ type: 'text', text: _catH });
    userContent.push({
      type: 'text',
