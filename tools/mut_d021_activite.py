...
MUT = [
    ...
    ('M-R34A-2', CO, "ni fourchette, ni calcul « par hypothèse » (pas de « disons 3 séances »), ",
    ...
    ('M-R34A-3', CO, "dis que ce niveau manque, que ses besoins en dépendent fortement, et demande-lui combien de séances il fait par semaine : le calcul viendra après sa réponse.",
    ...
    # ── D-024 (25/09) : la fuite « l'écart peut dépasser 500 kcal » ──
    ('M-R34A-6', CO, "ni ordre de grandeur, ni écart ou delta en kcal (pas de « l\\'écart peut dépasser 500 kcal »), ni exemple chiffré, ",
     "tu peux donner un ordre de grandeur de l\\'écart calorique, ", 'rouge',
     'LA FUITE D ORIGINE : un ordre de grandeur de l écart est de nouveau permis'),
    ('M-R34A-7', CO, "ni ordre de grandeur, ni écart ou delta en kcal (pas de « l\\'écart peut dépasser 500 kcal »), ni exemple chiffré, ",
     "ni ordre de grandeur, mais tu peux expliquer avec un exemple de 500 kcal, ", 'rouge',
     'DÉGUISÉE : l ordre de grandeur reste interdit mais « un exemple de 500 kcal » est permis'),
    ('M-R34A-9', CO, "ni estimation indirecte (pas de multiplicateur ou de pourcentage appliqué au BMR, pas de kcal brûlées par séance, pas de grammes de glucides ou de macros déduits d\\'une cible)",
     "tu peux dire « environ 1,5 fois ton BMR »", 'rouge',
     'DÉGUISÉE : aucun kcal écrit, mais un multiplicateur du BMR donne le TDEE en une ligne'),
    ('M-R34A-8', CO, "Tu peux citer ce qui n\\'en dépend pas (BMR, poids, taille, âge), en précisant que le BMR n\\'est ni sa dépense quotidienne ni une cible. ",
     "Ne cite aucun chiffre, pas même le BMR. ", 'rouge',
     'SUR-CORRECTION : le BMR (indépendant de l activité) devient interdit'),
