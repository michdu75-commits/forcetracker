...
MUT = [
    ...
    ('M05', SE, "const _act=_activiteValide(d.activityLevel); if(_act!=null){",
     "const _act=parseFloat(d.activityLevel); if(_act){", 'rouge', 'bornes de restauration de l activité retirées'),
    ...
    ('M17', SE, "  const act=(actEl&&actEl.dataset.touche==='1')?_activiteValide(actEl.value):null;",
     "  const act=(actEl&&actEl.dataset.touche==='1')?(numFR(actEl.value)||1.55):null;", 'rouge', 'DÉGUISÉE : « À choisir » enregistre 1,55'),
    ...
    ('M21', SE, "const _act=_activiteValide(d.activityLevel); if(_act!=null){",
     "const _act=(String(d.activityLevel)==='Infinity')?Infinity:_activiteValide(d.activityLevel); if(_act!=null){", 'rouge', 'M4b : la restauration contourne la garde pour Infinity'),
