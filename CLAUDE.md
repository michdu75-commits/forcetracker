...
| ft-v227 | graphique de poids — **étape 1/2** (le suivi masse grasse = étape 2 à venir) : **points cliquables** (tap un point → overlay `#ov-weigh-edit` → modifier date+kg ou supprimer, `openWeighEdit`/`saveWeighEdit`/`deleteWeighEntry` dans tracking.js) + **navigation par période** (chips `.wrange-chip` 1M/3M/6M/Tout, `_wRange`/`setWeightRange`, filtre `pts` sur 30/90/180j). Points rendus visibles (r=3.6 + ring `--bg2`, hit-target transparent r=12). Red dot `weight-edit` + Aide détaillée ← **actuel** |
