...
> **Version actuelle : `ft-v1118`** (prochaine : `ft-v1119`). Historique complet (ft-v128→574 + gouvernance
...
**⏭️ CE QUE ÇA NE FAIT PAS, ET IL FAUT LE LIRE** : ⛔ **le barème de récup du cardio n'est PAS corrigé.** Mesuré par la vraie fonction, à nuits identiques : aucune séance **76** · 45 min de tapis **71** · 20 min de tapis **71** · 4 séries de développé couché **70**. `_penaliteSeance` ne compte que des **séries validées**, donc un cardio tombe sur son **plancher de 6** — *45 minutes de tapis coûtent exactement autant que 20, et un peu moins que 4 séries*. ⚠️ L'app **connaît** pourtant le coût : `calcCardioKcal` rend **351 kcal** pour 45 min contre 156 pour 20 (famille **R4**). **Mais choisir combien 45 min de tapis doivent peser face à une séance de jambes est un jugement, pas une mesure** — et l'échelle actuelle va de −6 à −38. *J'attends l'ordre de grandeur de Michel plutôt que d'inventer un barème* (**R29**). ⛔ Le rappel ⏰ « pense à terminer ta séance » reste muet pour un cardio seul : il se mesure depuis la **dernière série validée**, et il n'y en a aucune — *lui inventer un point de départ serait deviner*. Le bouton de l'Accueil couvre le besoin. ⚠️ **Et Michel doit vérifier sur Safari/iPhone** : ce conteneur n'a que Chromium.

Tests : **parcours PARCOURS_N/PARCOURS_N** (+18, blocs **CCXXVI** et **CCXXVI bis**), calculs 266/266, muscles 241/241, croisés 50/50, dates 7/7, données classées 0 trou. ⭐⭐ **Le témoin qui porte la version rejoue le parcours exact de Michel** : noter 45 min de tapis, passer par l'Accueil, retaper le bouton rouge — *c'est le seul qui rougissait avant, et il rougissait en effaçant les données*. ⛔ **Un contrôle ouvre le bloc et vérifie DEUX choses** : que le cardio est vraiment noté, **et que la séance n'a AUCUN exercice** — sans la seconde, on mesurerait un cas qui n'a jamais été cassé. ⛔⛔ **Le symptôme rapporté est figé DANS LES DEUX SENS** : la carte « séance manquée » doit **sortir** quand rien n'a été fait, et **se taire** dès qu'un cardio seul a été enregistré — *un témoin qui ne vérifierait que le silence serait vert même si la carte ne sortait plus jamais*. ⛔⛔ **Et la non-régression qui compte le plus est celle qu'on aurait pu oublier** : on vient d'ÉLARGIR la définition de « séance ouverte », il faut donc prouver qu'on ne l'a pas élargie à tout — *ouvrir l'écran Séance sans rien faire crée déjà un `S.wkt` vide*, et si celui-là comptait, l'Accueil dirait « Reprendre » en permanence. **Mesuré : il ne compte pas.** ⛔ **Non-régression aussi** : une séance avec exercices garde son bouton, son rappel ⏰ **et** son brouillon de secours. ⭐ **Et un bloc à part rouvre l'app pour de vrai** (rechargement complet, brouillon relu depuis le stockage) : le message d'ouverture doit parler du **cardio**, jamais de « 0 exercice ». Fichiers : `log.js`, `screens.js`, `app.js`, `state.js`, `tracking.js`, `constants.js`, `coach.js`, `tests/parcours/runner.js`, `BUGS.md`, `sw.js`, `CLAUDE.md`, `docs/CONTEXTE-ACTUEL.md`, `docs/JOURNAL-ARCHIVE.md`, `docs/JOURNAL-DE-PARTAGE.md`. sw.js ft-v1118. |
