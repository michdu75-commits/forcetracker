...
> **Version actuelle : `ft-v1151`** (prochaine : `ft-v1152`). Historique complet (ft-v128→574 + gouvernance
...
**ft-v1151 — 🎚️ LE RIR NON NOTÉ N'ÉTAIT PAS COMPTÉ — MILO DEVAIT *DEVINER* L'ABSENCE AU LIEU DE LA *LIRE*** — Michel : *« pour le RIR, on a remarqué avec GPT que de ne rien mettre ne compte pas comme 0 mais comme rien du tout, donc Milo ne peut pas le comprendre »*.

**⭐⭐ MESURÉ SUR SON EXPORT RÉEL AVANT D'ÉCRIRE UNE LIGNE — 44 séances, 805 séries : 8 SÉRIES DE TRAVAIL SUR 591 PORTENT UN RIR, soit 1,4 %.** Et sur la fenêtre que Milo voit vraiment — les **5 dernières séances** — **3 sur 44 (6,8 %)**. 👉 ***Milo n'a donc, en pratique, presque aucune mesure de l'effort réel*** — et rien ne le lui disait.

**⛔⛔ LE PER-SÉRIE PARTAIT DÉJÀ (ft-v1038) ET LA RÈGLE ÉTAIT DANS LE PROMPT — c'est le COMPTE qui manquait, et c'est tout le sujet.** Le prompt dit noir sur blanc *« une série sans RIR n'est pas un RIR de 0 […] appuie-toi sur les RIR réellement notés, et **dis-le s'il n'y en a pas** »*. Pour obéir à cette dernière phrase, il fallait **balayer 44 lignes et faire l'inventaire** — un travail que le code fait sans se tromper et qu'un modèle fait au jugé. 👉 ***C'est R4/R8 une fois de plus : la consigne NOMME une source, le fait n'est jamais CALCULÉ.***

**⭐ COMPTÉ DANS LA PASSE QUI ÉCRIT DÉJÀ LES SÉRIES (R2)** — pas dans une seconde boucle : deux parcours du même historique finiraient par diverger, et c'est toujours le résumé qui aurait tort.

**⛔⛔ UN « (X) » COMPTE COMME NOTÉ, ET C'ÉTAIT LE PIÈGE LE PLUS FACILE DE LA VERSION.** L'échec **EST** un RIR de 0 (`_rirDeSet` le dit depuis ft-v1038), mais la ligne ne l'écrit pas deux fois — le tag le dit déjà (**R2**). Compter les *« RIRn »* écrits aurait donc **sous-compté exactement les séries les plus dures**, celles dont l'effort est le mieux connu. **Mesuré au contrôle négatif M1 : 5 notées au lieu de 10.**

**⭐⭐ LE VIDE EST DIT, ET L'INTERPRÉTATION EST INTERDITE.** À zéro noté, la ligne annonce **le nombre exact** de séries sans RIR et ferme la porte : *« ne déduis RIEN de ce vide — ni qu'il s'arrête loin de l'échec, ni qu'il y va — tu ne sais pas ; si l'effort compte pour ce que tu réponds, DEMANDE-LE »*. 👉 *Sans ce cas traité à part, on retombait sur « ne conclus rien de son absence » — une consigne que personne n'applique quand il ne sait pas combien il en manque.*

**⛔ ET RIEN QUAND IL N'Y A AUCUNE SÉRIE DE TRAVAIL** : *« 0 sur 0 »* n'est pas une information, c'est du bruit dans un contexte déjà plein (**R20**).

**⛔ LE COMPTE PORTE SUR LA FENÊTRE AFFICHÉE (5 séances), PAS SUR TOUT L'HISTORIQUE** — un chiffre qui contredirait les lignes juste au-dessus de lui serait **pire que pas de chiffre**. Un témoin sème **6** séances et vérifie que le dénominateur n'en compte que **5**.

**⛔⛔ ET R34 NE PEUT PAS ÊTRE TENUE ICI — je le dis plutôt que de faire semblant.** La règle veut *« banc avant → banc après »* ; le banc coûte des appels API que **ce conteneur n'a pas**. Ce que je peux prouver, c'est que **le fait est présent, exact et cohérent avec les lignes qu'il résume** ; ce que je ne peux pas prouver, c'est que Milo **s'en sert mieux**. **Le juge est Michel, dans ses propres conversations.** ⛔ Et **le prompt de fond n'a pas bougé** (**R7**) : on ajoute un FAIT, on ne durcit pas une consigne.

**📣 RÈGLE D'OR #11 — RIEN À ANNONCER.** Rien ne change à l'écran, rien n'est à faire, aucune saisie n'est modifiée : c'est **Milo** qui reçoit mieux. Une pop-up dirait *« Milo compte désormais vos RIR »* — du bruit (**R19/R25**). ⭐ L'aide détaillée sur le RIR dit déjà *« Milo le reçoit »*, et c'est toujours vrai.

**⏭️ CE QUE ÇA NE FAIT PAS, écrit pour ne pas le redécouvrir** : ⛔ **ça ne fait pas noter davantage de RIR** — à 1,4 %, le vrai levier est peut-être la SAISIE, pas le contexte ; *mesuré, noté, pas décidé* (ce serait une décision produit). ⛔ **Ça ne corrige pas la prescription trop lourde** : Milo saura qu'il ne sait pas, il ne saura pas pour autant à quel point c'était dur. ⛔ Et le compte **ne remonte pas** au-delà des 5 séances détaillées. ⚠️ **Michel doit vérifier sur Safari/iPhone.**

Tests : **parcours PARCOURS_N** (+10, bloc **CCL**), **calculs 339/339**, muscles 241/241, croisés 50/50, dates 9/9, données classées 0 trou. ⭐⭐ **Témoins FONCTIONNELS** : `buildCoachContext()` est **réellement appelée dans la page**, sur des séances semées exprès — *un `grep` dirait que le code compte, jamais qu'il compte JUSTE.* ⭐ **Le témoin le plus utile est un RECOUPEMENT INTERNE** : le chiffre annoncé doit égaler ce qu'on peut compter dans les lignes juste au-dessus (les `RIRn` écrits + les séries taguées `(X)`) — *un compteur qui contredit les données qu'il résume est pire qu'aucun compteur.* ⛔ **Contrôle négatif : 4 mutations, et chacune tombe où il faut** — ① **l'arbre d'avant : 5 rouges**, dont le contrôle d'ouverture ; ② **l'échec non compté : 2 rouges**, les deux qui portent sur le compte ; ③ **les échauffements dans le dénominateur : 4 rouges** ; ④ **la branche du vide retirée : 1 rouge**, exactement le cas de Michel. ⚠️ **Et les 2 témoins de non-régression restent verts partout** (la règle du prompt, le RIR par série) — *ce sont des gardes, pas des détecteurs, et les compter comme une preuve serait un vert qui ne peut pas rougir.* ⚠️⚠️ **LA SONDE ELLE-MÊME A DÛ ÊTRE CORRIGÉE, et c'est la leçon de méthode** : mon recoupement comptait les `RIRn` sur **tout** le contexte et ramassait des séries écrites dans d'autres blocs — **6 et 6 au lieu de 5 et 5**. *Une sonde qui déborde de sa cible ne rend pas « rien » : elle rend un chiffre faux et parfaitement crédible.* Fichiers : `coach.js`, `tests/parcours/runner.js`, `BUGS.md` (famille **2**), `sw.js`, `CLAUDE.md`, `docs/CONTEXTE-ACTUEL.md`, `docs/JOURNAL-DE-PARTAGE.md`, `docs/JOURNAL-ARCHIVE.md`. sw.js ft-v1151. |
