...
**⛔⛔ LA DÉCISION DE CONCEPTION EST QUE `coachMemory` RESTE UNE CHAÎNE — mesuré AVANT d'écrire une ligne.** Elle traverse **19 sites**, dont **deux contrats qui n'appartiennent pas au client** : `worker.js` la lit en `body.coachMemory || ''` et la **concatène dans le prompt de Milo**, et `Code.js` la passe au nettoyeur de **chaîne** d'Apps Script. 👉 ***En faire un objet aurait injecté « [object Object] » dans le prompt de Milo*** — c'est-à-dire exactement le recul que l'arbitrage interdit. La provenance vit donc **À CÔTÉ**, dans `coachMemoryMeta`. ⭐ **Ce n'est pas une duplication (R2)** : `coachMemory` porte **le texte**, `coachMemoryMeta` porte **d'où il vient** — deux informations, deux propriétaires.
