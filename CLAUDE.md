...
- 📋 **`docs/BRIEF-NUTRITION.md`** — **LE POINT D'ENTRÉE du chantier nutrition** (créé 19/08/2026 à la demande de Michel, pour une **autre instance** qui reprend le sujet). ⚠️ **Autonome** : lisible sans le dépôt. Il ne remplace ni le *moteur* (le comment) ni la *philosophie* (le pourquoi) — il dit **où on en est**, et c'est ce qui manquait : `NUTRITION-MOTEUR.md` a été écrit le 18/08, **six versions sont passées depuis** et une décision d'architecture avec, donc son tableau de briques annonçait comme « à faire » une brique **déjà livrée**. Porte : les **3 phrases qui décident** (levier jamais finalité · la nutrition IGNORE l'entraînement, le vrai défaut · calcul déterministe hors ligne) · **ce qui a été livré** ft-v906→915 avec sa raison (plancher kcal, provenance à 2 axes, barre de protéines branchée au journal, carte « où tu en es » et sa **moyenne honnête**, créatine libre à 2 seuils, repas habituels **observés jamais déclarés**, cru/cuit **écrit jamais converti**) · ⭐⭐ **la frontière cerveau/cervelet appliquée à la nutrition** et **le 2ᵉ critère** venu d'une relecture extérieure (*« transformation vérifiable ou JUGEMENT ? »* — sans lui, composer une assiette végane sans arachide partirait au cervelet, car ça n'a pas besoin de savoir qui est la personne) · ⛔ **la règle de sécurité** (*une contrainte vérifiable par du code ne dépend JAMAIS d'un seul LLM* : le modèle **propose**, le code **valide**) · ⚠️ **la dérive à interdire** (le cervelet qui finit par conseiller — verrouillée par un test, pas par une consigne) · les **DEUX bases** (journal = couverture, générateur = sûreté) · **7 pièges déjà payés** · et **§6 la suite avec son état honnête**, y compris ce qui est **bloqué et pourquoi**.
