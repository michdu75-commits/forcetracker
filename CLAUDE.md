...
| ft-v263 | **Suivi photos aussi pour Michel** : `michdu75@gmail.com` ajouté à `SUPER_TESTER_EMAILS` → accès au suivi photos via un bouton dans le **panneau Admin** (`openBodySeries`). **Sans** carte « Testeur Fondateur » (réservée à `TESTER_EMAILS`) ni message « Michel te remercie » (`checkSuperTesterWelcome` exige désormais `_isSuperTester()` **et** `_isTester()`). ← **actuel** |
