...
Tests : **blocs B-CCCXXXVIII (10 témoins de source) et B-CCCXXXIX (18 conduits dans le navigateur)**, dans `tests/parcours/mensurations.js` — le cas exact de Michel, les trois états de poids qui bloquaient, le point **et** la virgule comparés l'un à l'autre, un champ vidé qui ne détruit rien, deux jours qui coexistent, et **le rechargement complet**. ⛔ **CONTRÔLE NÉGATIF : 17 mutations sur un arbre CLONÉ, 17 conformes**, contrôle sain **28 OK / 0 rouge avant ET après**, dont **deux qui doivent RESTER VERTES** (les mots cherchés cités dans un commentaire — indispensable ici, les commentaires du correctif citent `persist`, `numFR` et `S.bw` en toutes lettres). ⭐ **Passe complète : 4506 ✅ / 0 ❌**, les 4 conditions vertes sur l'arbre `4c6e6207`.

Fichiers : `tracking.js`, `tests/parcours/mensurations.js` (nouveau), `tests/parcours/runner.js`, `tools/banc_mensurations.js` et `tools/mut_mensurations.py` (nouveaux), `sw.js`, `CLAUDE.md`, `docs/CONTEXTE-ACTUEL.md`, `docs/JOURNAL-DE-PARTAGE.md`, `docs/INVENTAIRE.md`. sw.js ft-v1228. |
