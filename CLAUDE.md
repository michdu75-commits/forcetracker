...
**ft-v1151 — 🎚️ DEUX CHOSES SUR LE RIR — ET LA SECONDE EST UNE CORRECTION DE FOND QUE MICHEL M'A IMPOSÉE EN COURS DE ROUTE**
...
**⛔⛔ ET LA MÊME PHRASE FAUSSE S'ÉTAIT PROPAGÉE À QUATRE ENDROITS**, corrigés ensemble : ① `_rirDeSet` rend désormais **`null`** sur un `X` — *le TAG porte l'échec, `rir` porte la réserve : deux questions, deux propriétaires* (**R2**) ; ② le **prompt de Milo** dit la différence **avec l'exemple** au lieu de *« c'est-à-dire RIR 0 »* ; ③ l'**aide détaillée** pareil ; ④ et l'**export CSV** laisse la colonne RIR **vide** sur un `X` — la colonne `type` porte déjà le X, et *écrire « 0 » dans un fichier qui se relit ailleurs serait faux d'un cran, sans personne pour rattraper la nuance*.
