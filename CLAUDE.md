...
**ft-v457 → v461 — Projet « DOSSIER ATHLÈTE / MILO »** (mémoire durable + personnalité — **détail complet dans `DOSSIER-ATHLETE-SUIVI.md`**, principes dans `CONSTITUTION-MILO.md` v1.1) : **brique 0** = ton de Milo Cool/Classique/Dynamique/Scientifique (v457) puis « 🎯 Laisse Milo choisir » (auto) devient le défaut (v458) · **brique 1** = socle « Registre Athlète » `S.registre` (v459, mémoire durable persistée local+cloud, injectée dans le briefing de Milo, vide au départ) · **brique 2** = faits mesurés `computeRegistreFacts()` (v460, 7 faits calculés : séances, régularité, durée moyenne, exos préférés, groupe le plus/moins travaillé, sommeil moyen, ancienneté — règle d'or « un fait = une décision ») · **amélioration Milo** « comprendre avant de conseiller » (v461, sur une rupture d'habitude Milo cherche d'abord à comprendre avec une question douce sans jugement, la personne avant le programme, ne jamais inventer ce qui a été fait récemment). Méthode : une brique à la fois (Objectif/Critère/Hors périmètre), branche + backup, tests Playwright, journal + Constitution à jour. ← **actuel** |
