...
| ft-v280 | **Fix séance : l'appui long ne « fait plus glisser » l'image de l'exercice** (retour Michel — comportement natif du navigateur, une balise `<img>` est glissable par défaut, l'appui long attrapait la vignette « fantôme »). Règle CSS **globale** `img{-webkit-user-drag:none}` (style.css, après les `@font-face`) → plus de drag-ghost sur aucune image de l'app. Le **tap** (ouvrir la photo, toggle gif) et le zoom restent normaux ; l'appui long « Enregistrer l'image » n'est pas touché. + **clone resynchronisé** sur la prod ft-v279/280 (chemins `../`, shim `cl_`, badge CLONE conservés ; ajoute vignettes accessoires + guide-film qui manquaient). Testé (Playwright HTTP : `-webkit-user-drag:none` calculé sur img, 0 erreur JS). ← **actuel** |
