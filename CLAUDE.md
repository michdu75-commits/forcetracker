...
> **👉 POUR OUVRIR À TOUT LE MONDE** (quand Michel dit « on balance à tout le monde ») : ① `_isNutriBeta()` → `return true;` ; ② dans `constants.js`, remettre les 3 entrées `WHATS_NEW` v15/16/17 (voir commentaire) + `WHATS_NEW_MAX=17` et les 3 red dots `manual-kcal`/`goal-recomp`/`reps-maxi` ; ③ bump `sw.js`. Les aides `?`/détaillées décrivent déjà ces features (laissées en place, opt-in).
