...
| ft-v289 | **Sous-menus (Menu → 1RM, guide, protéines…) : fermeture en glissé doux** (retour Michel : « le slide qui ferme la fenêtre 1RM se ferme super vite »). Cause : `#ov-drawer-cnt` (`.overlay` en `display:none/flex`) ouvrait/fermait **instantanément** — seul le *swipe* glissait, le bouton ✕ coupait net (« ça claque »). Fix : anim CSS `dcUp` (ouverture) + `dcDown .26s` (fermeture) scopées à `#ov-drawer-cnt`. `closeDrawerContent()` ajoute `.dc-closing` (joue `dcDown`) puis retire `.open` après 250 ms. Le *swipe* garde sa propre anim → on lui passe un closer immédiat (pas de double animation). Testé Playwright (ouverture dcUp, fermeture dcDown, `.open`/`.dc-closing` retirés après 300 ms, 0 erreur JS). ← **actuel** |
