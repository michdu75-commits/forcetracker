...
// ── 📊 Compteur d'appels IA (08/08/2026) ────────────────────────────────────
...
function _plafondAtteint(){ return _capDate === _jourParis(); }
// Envoie le coup à Apps Script, qui tient le compteur du jour (déjà écrit et déjà lu par le
// panneau Admin → IA : on REBRANCHE l'existant, on ne recrée pas un second compteur — R2).
...
async function _compterIA(action, email, env){
  try{
    const r = await fetch(APPS_SCRIPT_URL, {
      ...
      body: JSON.stringify({ action: 'aiCount', act: action, email: email,
                             token: (env && env.FT_COUNT_TOKEN) || '' })
    });
    const d = await r.json();
    if (d && d.blocked) { _capDate = _jourParis(); _capScope = d.scope || 'global'; }
  }catch(e){ /* repli ouvert : on ne bloque jamais sur une panne de comptage */ }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    // comptait plus rien depuis la migration 4G du 13/07, et le panneau Admin → IA
    ...
    if (_ACTIONS_IA.has(body.action)) {
      ...
      // (plusieurs isolats), et c'est assumé : l'objectif écrit dans Code.js est de
      // « borner le coût en cas d'abus », pas de tenir une comptabilité exacte.
      if (_plafondAtteint()) {
        return json({ status: 'error', error: 'quota', scope: _capScope,
          reply: _capScope === 'global'
