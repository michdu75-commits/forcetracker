 ...
 */
const CACHE = 'ft-v964'; // 🔤 ft-v964 = CES MOTS-LA NE S'ECRIVENT PAS. Michel, juste apres ft-v963 : « oui j'ai mis ca, apres je voulais mettre coquilette ». ⛔⛔ IL L'ECRIT AVEC UN SEUL L, et ma liste de synonymes avait « coquillette » : SA graphie a lui rendait ZERO resultat. La correction de la veille marchait donc pour l'orthographe parfaite, c'est-a-dire pour ceux qui n'en avaient pas besoin. ⭐ MESURE, PAS DEVINE : 6 autres graphies plausibles echouaient aussi — spagetti, tagliatele, farfale, fusili, linguini, pene. Toutes des variantes de CONSONNE DOUBLEE ou de H MUET, exactement la ou ces mots italiens se trompent. ⛔ LA TOLERANCE NE S'APPLIQUE QU'A LA LISTE FERMEE DE 12 FORMES, jamais a la base : on compare la frappe aux 12 mots connus, donc aucun rapprochement hasardeux possible sur 3 341 aliments. ⚠️⚠️ ET DEUX PIEGES TROUVES EN LE MESURANT, PAS EN LE RELISANT : ① ma 1re version retirait aussi la VOYELLE FINALE, et « macaroni » devenait « macaron » — LA PATISSERIE SERAIT PARTIE SUR LES PATES. Le retrait de la voyelle finale a saute ; « linguini » (graphie anglaise) est simplement AJOUTE a la liste, ce qui est plus honnete qu'une regle qui rabote au hasard. ② « torsade » est RETIRE de la liste (R30, un retrait s'ecrit) : CIQUAL l'emploie pour un BISCUIT APERITIF feuillete, usage au moins aussi courant que la pate — entre detourner un vrai aliment et rater une forme rare, on rate la forme rare. ⭐ Verifie sur les 2 261 mots distincts de CIQUAL : la seule collision restante est « spaghetti » (la COURGE), et elle est voulue — la courge garde sa correspondance EXACTE donc reste trouvable.
const PRECACHE = [
