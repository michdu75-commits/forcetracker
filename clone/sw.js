 ...
 */
const CACHE = 'ft-v975'; // ⚖️ ft-v975 = « JE NE PEUX PAS METTRE DE POIDS » — LA QUANTITE SUR UNE PHRASE LIBRE. Michel, capture a l'appui, devant une huile d'olive estimee par l'IA (135 kcal / 15 g de lipides). ⛔⛔ C'EST LE MOTIF DE ft-v973, DEUX JOURS DE SUITE : le mecanisme EXISTAIT, pose d'un seul cote — ft-v972 a donne le rescale par proportion a la modale « MODIFIER l'aliment », mais le formulaire d'AJOUT n'a de champ « Quantite » que si un pour-100 g est connu (scan, CIQUAL, recherche). Une phrase estimee par l'IA n'en a pas : aucun reglage, et 4 chiffres a recalculer soi-meme (R8 : une correction faite d'un cote et pas de l'autre est un oubli, pas un arbitrage). ⭐⭐ CE QUI REND LA CHOSE POSSIBLE EST COTE SERVEUR, ET C'EST UNE LIGNE DE CONSIGNE : le modele CHOISISSAIT DEJA une portion, mais EN SILENCE. Il l'annonce desormais (`g`), et une estimation aveugle devient une estimation ANCREE — R4 au mot pres, l'information ne descendait pas jusqu'a la DONNEE. 👉 TROIS SOURCES POUR LA REFERENCE : le per100 connu (bloc du scan, inchange) · le poids suppose par l'IA · le nombre ecrit dans la phrase. ⚠️⚠️ ET L'ORDRE ENTRE LES DEUX DERNIERES N'EST PAS UN DETAIL : sur « 3 oeufs et 200 g de riz », le nombre ecrit ne designe QU'UN COMPOSANT — le prendre pour reference reviendrait a dire que toute l'assiette pese 200 g. Le poids de l'IA porte sur le TOTAL. ⛔ Mais il n'appartient qu'a LA PHRASE QUI A ETE ESTIMEE : si la phrase change il est perime — une reference qui survit a son sujet est pire que pas de reference, elle a l'air d'un fait. ⛔ SANS AUCUN ANCRAGE, AUCUN POIDS INVENTE (R29) : des PORTIONS (x0,5 x1,5 x2 x3), vraies quelle que soit la portion de depart, et l'ecran dit POURQUOI. ⚠️ Le poids de l'IA est presente comme une ESTIMATION, pas comme une mesure (R32), et il est BORNE cote serveur : au-dela de 5 kg pour un repas c'est une hallucination, pas une portion — mieux vaut aucun repere qu'un repere faux.
const PRECACHE = [
