 ...
 */
const CACHE = 'ft-v961'; // 📅 ft-v961 = NAVIGUER DANS LE JOURNAL — voir ET modifier un autre jour. Michel : « on ne sait pas ce que l'on a mange dans la journee et on ne peut meme pas le modifier de ce fait ». ⭐ VERIFIE AVANT DE CODER (R28) : le Journal etait cable en dur sur today(), sans aucune navigation — impossible de voir OU de modifier un autre jour que celui du moment, exactement ce qu'il decrivait. ⭐ MEME REPERE VISUEL que le calendrier de l'Accueil (fleches ‹ ›, R13) : Aujourd'hui / Hier / date complete, et navigation illimitee vers le passe. ⛔⛔ ET ON NE VA JAMAIS DANS LE FUTUR : demain n'a rien a montrer, y naviguer donnerait l'impression qu'on peut noter un repas a l'avance — la fleche avant est desactivee des qu'on est sur aujourd'hui. ⭐⭐ LE TEMOIN CENTRAL : un jour passe est MODIFIABLE, pas seulement consultable — on peut editer et supprimer une entree d'hier exactement comme aujourd'hui, sans toucher a l'entree du jour present. ⭐ ET AJOUTER UN ALIMENT EN CONSULTANT LE PASSE LE DATE SUR CE JOUR-LA (backfill), jamais sur aujourd'hui — sinon la navigation servirait a regarder mais pas a corriger un oubli, ce qui aurait rate la moitie de la demande. ⚠️ UN JOUR CLOS N'A PLUS DE « restantes » : le libelle passe en simple comparaison a l'objectif (on ne va pas manger davantage hier), et l'objectif affiche reste explicitement celui d'AUJOURD'HUI — recalculer un objectif historique aurait ete un faux-precis qu'on n'a pas les moyens de garantir (R29).
const PRECACHE = [
