 ...
 */
const CACHE = 'ft-v947'; // 🔬 ft-v947 = LES 4 DRAPEAUX RESTANTS, LUS UN PAR UN — et 3 etaient des FAUX POSITIFS. Michel : « regarde les 3 diagnostic et le lien, et dit si les testeurs testent milo ». ⭐⭐ LES 3 « DIAGNOSTIC » VIENNENT TOUS DU MEME DEFAUT : le motif attrapait « tu es (en |atteint) » — or « TU ES EN » tout seul est une tournure francaise ordinaire. Les trois cas : « tu es en Jour 2 de ton programme », « tu es en plein dans la zone » (une TSH normale), « tu es en phase de charge initiale ? ». Aucun rapport avec la medecine. ⚠️ Le meme defaut dormait dans « tu fais (une |un |de l) » — « tu fais une belle seance » l'aurait declenche ; il n'a pas tire sur ces 129 reponses, c'est un hasard. 👉 Les deux tournures exigent desormais une PATHOLOGIE derriere ; « je te diagnostique » et « tu souffres de » restent seuls, eux ne peuvent pas etre anodins. Verifie dans les DEUX SENS : 6/6 vrais diagnostics vus, 0/5 faux positifs, et « ca PEUT etre une sciatique » reste vert (hypothese nommee, Constitution). ⚠️⚠️ ET LE RESSERRAGE A D'ABORD RENDU LE GARDE-FOU MUET : `\\b` au lieu de `\b` dans la chaine — une barre oblique de trop et il n'attrapait plus RIEN. Cousin du piege deja paye ici (le `\b` apres un accent) : un motif construit en chaine se verifie en le JOUANT, jamais en le relisant. ⭐ LE 4e DRAPEAU (un lien claude.ai) est un faux positif LEGER et il est GARDE tel quel : 1 sur 129, dans une conversation debridee sur le prompt lui-meme. Resserrer risquerait de rater une vraie source fabriquee — R19 coupe dans les deux sens. 👉 BILAN SUR SES 25 JOURS : 4 drapeaux, dont 3 VRAIES promesses de memoire non tenues. ⭐⭐ ET LA 2e QUESTION A REVELE UN TROU : un testeur qui utilise Milo SANS deraper n'apparaissait pas du tout — impossible de distinguer « ne l'utilise pas » de « tout va bien ». Or `retro.messages` compte ses reponses de Milo : c'est LITTERALEMENT la mesure d'usage. La vue affiche desormais tous les comptes, avec « N reponses de Milo, AUCUNE derive ✅ ».
const PRECACHE = [
