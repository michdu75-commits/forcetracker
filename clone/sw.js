 ...
 */
const CACHE = 'ft-v929'; // 🧪 ft-v929 = LE BENCHMARK EXISTE — on ne SUPPOSE plus que Milo suit ses regles, on le MESURE. Michel : « ca me derange pas du tout qu'on teste Milo, par contre faut que je sois sur que ca soit utile, si je paye et que c'est pas utile c'est gaspiller de l'argent » — puis « au moins 10 ou 15 ». ⭐⭐ LE DECLENCHEUR EST UNE PREUVE, PAS UNE INTUITION : le §8 de ARCHITECTURE-CERVEAU-CERVELET.md posait que tests/milo prouve la PRESENCE d'une regle, jamais son OBEISSANCE — et ft-v926 l'a demontre le jour meme (la regle « regarde QUI a choisi ces charges, le marqueur te le dit OU TU LA RETROUVES DANS VOTRE ECHANGE » etait dans le prompt, la seance etait quelques messages plus haut, Milo a quand meme reproche ses propres paliers). 👉 15 scenarios (tests/milo/eval-scenarios.js) + un runner (eval.js) : 6 viennent des bugs vecus en salle du 15 au 20/08, 9 reprennent les regles de conversation/securite et les 3 personas fondateurs. ⛔ AUCUN JUGE IA, et c'est une DECISION : le framework posait lui-meme « qui juge le juge ? » — un juge double le cout ET ajoute une erreur non calibree, alors que les vrais bugs de ce projet sont MECANIQUEMENT verifiables (un exercice absent, 82,5 kg sur une barre, un « c'est note » sans bloc, un lien invente). Les 15 verificateurs sont du JavaScript. ⚠️⚠️ UN VERT VAUT MOINS QU'UN ROUGE, ecrit en tete du corpus : un rouge est une PREUVE, un vert dit seulement « aucune violation DETECTABLE ». ⚠️ DEUX GARDE-FOUS DE COUT (seul runner du depot qui depense) : sans --go il tourne A BLANC (0 appel) et sort un devis MESURE — le contexte reel est construit localement, ~70 k car./scenario, 0,23 a 0,95 € la passe ; et il n'est PAS branche sur la suite de livraison (un test qui depense a chaque push finit coupe). ⭐ R13 : rien de neuf cote navigateur — il reutilise _vcApplyPersona/_vcAsk du laboratoire VC ; le seul changement est que les ATTENDUS, coches a la main par un juge humain, deviennent du CODE. ⚠️ ET BRANCHER LE LABORATOIRE A REVELE DEUX DEFAUTS : _vcAsk appelait buildCoachContext() SANS le message, donc envoyait PLUS que le vrai chemin (une evaluation qui envoie plus que la realite mesure une autre dilution — un vert n'y prouve rien) ; et les personas annoncaient « Haiku (defaut) » alors que worker.js sert SONNET a tout le monde depuis le 10/08 — croire qu'on teste Haiku en testant Sonnet, c'est corriger le mauvais cerveau (R9).
const PRECACHE = [
