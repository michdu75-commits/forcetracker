 ...
 */
const CACHE = 'ft-v959'; // 🥚 ft-v959 = LE BUG DU BLANC D'OEUF — la ligature CASSAIT la recherche CIQUAL. Michel a montre une photo de blanc d'oeuf liquide en demandant comment le retrouver dans la base, apres avoir signale que « poulet » ne trouvait rien (ce 2e point etait juste une version pas encore rafraichie). ⭐⭐ MAIS EN VERIFIANT LE PRODUIT D'OEUF, UN VRAI BUG EST APPARU : `normalize('NFD')` decompose les ACCENTS (e -> e + accent) mais PAS les LIGATURES ê/ae — ce sont deux lettres fusionnees en UNE, pas une lettre accentuee. Or le clavier iPhone en francais CORRIGE AUTOMATIQUEMENT « oeuf » en « oeuf-ligature » pendant la frappe, et CIQUAL ecrit tous ses noms en oe SEPARE (« Oeuf, blanc... »). Mesure : taper le mot avec ligature rendait ZERO resultat pour oeuf, boeuf — donc quasiment TOUJOURS sur iPhone, pour un aliment qui existe bien dans la base. ⛔ CORRIGE en deux remplacements supplementaires dans `_afNorm` (oe-ligature -> oe, ae-ligature -> ae), AVANT le NFD qui gere les accents. Meme fonction que CIQUAL et les suggestions locales partagent deja (R2) : un seul endroit a corriger repare les deux recherches d'un coup. CONTROLE NEGATIF : 2 rouges exactement, oeuf et boeuf en ligature ; le mot sans ligature continue de marcher (pas de regression).
const PRECACHE = [
