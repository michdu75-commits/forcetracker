 ...
 */
const CACHE = 'ft-v962'; // ⚖️ ft-v962 = MODIFIER LE POIDS D'UNE ENTREE DU JOURNAL, au lieu de recalculer les 4 macros a la main. Michel, sur un « Oeuf cru » : « ya oeuf cru (lol) pas cuit. Et on ne peut pas modifier le poids ». ⭐ DEUX QUESTIONS, ET UNE SEULE EST UN DEFAUT — verifie avant de coder (R28). ① L'OEUF CRU N'EST PAS UN TROU DE LA BASE : « Oeuf dur » sort PREMIER quand on tape « oeuf », avant « Oeuf cru », et poche / a la coque / brouille / au plat existent aussi. Il a pris le 2e de la liste. ⚠️ Et l'ecart est minuscule sur un oeuf entier : 140 vs 134 kcal/100 g, soit 12 kcal sur ses 4 oeufs — le cru/cuit compte enormement pour les feculents (x3), presque pas ici. On ne corrige donc RIEN cote base : la donnee etait la, le choix aussi. ② ⛔ LE POIDS, LUI, ETAIT UN VRAI DEFAUT : la modale « Modifier l'aliment » ne montrait que les 4 macros brutes — pour passer de 200 a 150 g il fallait recalculer kcal, P, G et L soi-meme, a la main, sur un ecran qui sait pourtant faire ce calcul depuis toujours. ⭐ R13 — ON NE REINVENTE RIEN : `_bcApplyGrams()` fait exactement ca a l'AJOUT (scan, CIQUAL, recherche) depuis ft-v956/957. On branche la MEME logique sur `e.per100`, le pour-100 g que `_provFood` enregistre deja depuis la brique 0 (ft-v907). ⭐⭐ ET C'EST R4 QUI PAYE : cette donnee etait stockee depuis des semaines et n'atteignait AUCUN ecran — elle existait, elle ne servait a rien. ⛔⛔ LE TEMOIN CENTRAL EST UN REFUS : le champ n'apparait QUE si `per100` existe. Une entree tapee a la main n'a pas de pour-100 g, donc pas de champ — en fabriquer un supposerait de deviner a quel poids correspondent ses 80 kcal, et ce serait un faux-precis (R29). Sa modale ne bouge pas d'un pixel. ⚠️ Et la quantite se RE-ENREGISTRE (`q`/`u`) : sans ca, la modification suivante repartirait du poids d'origine et annulerait la precedente en silence.
const PRECACHE = [
