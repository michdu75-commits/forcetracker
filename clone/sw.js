 ...
 */
const CACHE = 'ft-v958'; // 🔍 ft-v958 = UN AUTRE COMPLEMENT, IDENTIFICATION SEULEMENT. Michel a fourni Compl'Alim (registre officiel data.gouv.fr, 5 fichiers, 142 928 declarations), puis : « je ne demande pas a ce que tout soit detaille, mais peut-etre simplifier l'approche ». ⛔⛔ APPROCHE VOLONTAIREMENT SIMPLIFIEE : on ne garde QUE l'identification — nom, marque, categorie declaree (129 033 produits AUTORISES, dedoublonnes). AUCUNE dose, AUCUNE mise en garde, AUCUNE composition : les afficher rapprocherait l'app du conseil sur des substances — seule la creatine a ce traitement aujourd'hui, avec des precautions ecrites a la main et sourcees ANSES. C'est une fiche D'IDENTIFICATION (« est-ce le bon produit ? »), pas un moteur de recommandation, et un temoin central verifie qu'AUCUN mot de dose ou de mise en garde n'apparait jamais a l'ecran. ⚠️ CE N'EST PAS UNE BASE NUTRITIONNELLE : Compl'Alim ne donne aucune valeur kcal/proteines/glucides/lipides (verifie dans le fichier) — Michel l'avait suppose, corrige avant de coder. Pour les valeurs nutritives d'un complement, c'est Open Food Facts (deja branche, ft-v956). ⛔ ET CE N'EST QU'UNE RECHERCHE, PAS UN JOURNAL : rien n'est enregistre — construire un suivi quotidien pour un complement arbitraire est un autre chantier, non demande. ⛔⛔ CHARGEE A LA DEMANDE, JAMAIS AU DEMARRAGE, meme regle que CIQUAL (regle d'or #4) : 6,5 Mo → 1,58 Mo gzippe, mesure avant de coder. ⚠️ SOURCE : Compl'Alim, data.gouv.fr — licence a confirmer, non verifiable depuis cette session (reseau bloque).
const PRECACHE = [
