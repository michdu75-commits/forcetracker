 ...
 */
const CACHE = 'ft-v932'; // 📋 ft-v932 = LE RAPPORT DU BENCHMARK SE COPIE — l'export rendait un fichier d'UNE LIGNE. Michel lance le benchmark, appuie sur « Rapport (texte) », et le fichier qui lui revient contient « Benchmark Milo » et rien d'autre : c'est le `title:` passe a navigator.share, PAS le contenu. La feuille de partage a garde le titre et jete le fichier. Un export qui perd son contenu sans rien dire, c'est un export qui MENT. 👉 On ne remplace pas le partage, on ajoute un chemin qui ne depend d'AUCUNE feuille : « 📋 Copier le rapport ». C'est celui dont Michel a reellement besoin — il colle le texte dans la conversation. ⭐ R13 : motif repris tel quel de copyAppLink (13/08) — presse-papier → repli execCommand → et si les deux tombent ON LE DIT, avec en dernier recours le rapport AFFICHE dans le chat. Un bouton muet, de l'autre cote de l'ecran, ca s'appelle « ca ne marche pas ». ⚠️⚠️ ET LA CORRECTION EST VOLONTAIREMENT ETROITE (R19) : 8 AUTRES exports du depot partagent un fichier AVEC un titre (PT-001, VC, VM, programme, etude du corps) et ceux-la FONCTIONNENT chez Michel. L'explication « le titre survit au fichier » n'est donc PAS demontree en general — elle est constatee ICI, une fois. On corrige ici et on ne touche pas a ce qui marche : deviner deux fois de suite a deja coute cher (BUGS.md 12ter). Le jour ou un 2e export perd son contenu, la famille sera prouvee et le temoin s'elargira. ⚠️ Et mon 1er temoin rougissait sur SA PROPRE EXPLICATION : il cherchait `title:` dans tout le corps de la fonction, or le commentaire qui explique pourquoi on l'a retire contient le mot. Un motif doit viser le CODE, pas le texte autour.
const PRECACHE = [
