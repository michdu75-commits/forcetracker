 ...
 */
const CACHE = 'ft-v935'; // 📊 ft-v935 = LE TOTAL NE BOUGE PAS, LA COMPOSITION SI — historique par scenario. 3e passe reelle de Michel, la premiere APRES un correctif. ⭐⭐ EV-012 (keto) EST PASSE AU VERT : c'est la premiere mesure AVANT/APRES du projet, et elle dit que le rappel de regime de ft-v933 a marche. ⚠️⚠️ MAIS LE PIEGE EST LA : 4 rouges hier, 4 rouges aujourd'hui. En ne regardant que le COMPTE on conclurait « rien n'a change ». C'est FAUX — ce n'est pas le meme 4 : un defaut corrige, deux autres apparus. Un total stable peut cacher une correction ET une regression qui se compensent. 👉 Le rapport garde desormais le verdict de CHAQUE scenario passe apres passe (localStorage admin, 8 dernieres, jamais synchronise) et affiche « ❌ ❌ ✅ » avec sa lecture : SYSTEMATIQUE · intermittent (n/m) · stable au vert. ⭐ Ca separe le vrai defaut du bruit SANS depenser un seul appel de plus. ⚠️ Une seule passe ne produit AUCUNE tendance (2 minimum) : une ligne d'historique a une entree ne dit rien. Etat mesure sur 3 passes : EV-003 (face pull) et EV-015 (coach humain) rouges 3/3 = SYSTEMATIQUES, a corriger · EV-009 et EV-007 intermittents, a re-mesurer avant de coder. ⚠️ Et mon temoin a rougi a tort : il exigeait qu'un scenario vu une seule fois soit ABSENT DE TOUT le rapport, alors qu'il figure legitimement dans la liste des resultats — on ne veut garantir que son absence du bloc HISTORIQUE. Un motif doit viser ce qu'on veut garantir, pas plus large.
const PRECACHE = [
