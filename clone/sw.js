 ...
 */
const CACHE = 'ft-v896'; // 🧮 ft-v896 = LES BLOCS QUI BOUGENT SONT RANGES EN BAS — valider une serie ne refacture plus 15 000 caracteres. Le cache du prompt est un cache de PREFIXE : tout ce qui precede le premier caractere qui change est reutilise, tout ce qui suit est repaye. La seance EN COURS etait rangee au MILIEU du bloc personnel, donc valider une serie (toutes les ~90 s) faisait repayer les 12 884 caracteres parfaitement STABLES ranges derriere : le catalogue d'exercices, la methode de coaching, les unilateraux. MESURE avant/apres avec le nouvel outil `node tools/cache-coupure.js` : valider une serie 15 253 -> 20 caracteres reecrits, noter une pesee 12 995 -> 2 329. Les blocs mutables sont desormais classes par mutabilite CROISSANTE, le plus volatil en dernier (POIDS, CHECK-IN, DERNIERES SEANCES, SEANCE EN COURS). ⚠️ RIEN N'A CHANGE DANS LE TEXTE ENVOYE A MILO : 258 lignes des deux cotes, verifiees une a une. La seule modification volontaire est un renvoi de position qui etait FAUX — le prompt disait « sa MEMOIRE LONGUE plus bas » alors qu'elle est 6 266 caracteres plus HAUT ; on nomme desormais le bloc au lieu de pointer une direction, ce qui ne peut plus pourrir. ⚠️⚠️ ET UN INCIDENT A GARDER : pendant le chantier, le bloc POIDS a atterri DANS un commentaire du gabarit et a disparu du prompt sans lever la moindre erreur — un commentaire JS ecrit au milieu d'un template literal n'est pas un commentaire, c'est du TEXTE (+754 caracteres envoyes au modele). Le temoin XLV verifie donc d'abord qu'aucun bloc n'a disparu, et seulement ensuite la position de la coupure.
const PRECACHE = [
