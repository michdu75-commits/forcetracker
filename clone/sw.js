 ...
 */
const CACHE = 'ft-v917'; // 🧹 ft-v917 = LE PROMPT DE MILO N'ETAIT PLUS QU'A 35 CARACTERES DE SON PLAFOND. Michel : « euh Milo la c'est le plus urgent ». Il avait raison : plus aucune regle generique ne pouvait entrer sans qu'on en sorte une. ⭐ LA METHODE QUI MARCHE, ET C'EST LA SEULE SURE : on retire du prompt ce que le CODE garantit de facon DETERMINISTE (R7 — le prompt est le dernier levier). Le barème des paliers (40-50 %, +10-15 %, reps 5>3>2>1, 85 % = 1-2 reps) est calcule par `_monteeEnCharge` + `_repsPalier` + `_pasCharge` : Milo n'a pas a le detailler. Idem pour l'explication physiologique du superset, dont le refus dur est deja dans `_supersetInterdit` (« pourquoi le code et pas le prompt », ecrit le 12/08). ⚠️⚠️ ET LA METHODE A UNE LIMITE QUE LE TEMOIN M'A APPRISE : j'avais aussi retire la regle de DOSE (4-5 paliers sur le premier mouvement, 2-4 ensuite). Le temoin de ft-v887 est passe au ROUGE — et il avait raison : le code COMPLETE une montee, il ne RETIRE JAMAIS un palier en trop. C'est donc le prompt, et lui seul, qui empeche Milo d'en ecrire cinq sur un accessoire. Regle restauree, et desormais ecrite AVEC sa raison. ⭐ Le critere devient net : ce que le code CALCULE peut sortir du prompt ; ce que le code ne CORRIGE PAS doit y rester. Gain : 46 465 > 46 259 caracteres, marge 35 > 241. Modeste, et c'est honnete — la vraie question n'est pas de compresser, c'est de decider quelles regles peuvent partir (180 lignes), et ca demande un arbitrage produit.
const PRECACHE = [
