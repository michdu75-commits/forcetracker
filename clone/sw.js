 ...
 */
const CACHE = 'ft-v949'; // 🏋️ ft-v949 = LE NIVEAU D'ACTIVITE CONTIENT DEJA L'ENTRAINEMENT. Michel : « bon la nutrition lol ? ». ⭐⭐ J'AI ANNONCE L'INVERSE ET LE CODE M'A CONTREDIT : j'ai dit « la nutrition ignore completement l'entrainement » — FAUX, l'ecran affichait deja « Total = depense + seance ». Le vrai defaut est le CONTRAIRE : cette addition COMPTE LA SEANCE DEUX FOIS, puisque le multiplicateur s'appelle litteralement « Modere (3-4j) » — les seances sont deja dedans, lissees sur la semaine. Et pire, elle CONTREDISAIT l'anneau juste en dessous, qui lui ne l'ajoute pas : deux chiffres qui se contredisent sur le meme ecran, sans rien pour dire lequel commande la cible. ⚠️⚠️ ET LE DEFAUT DE FOND EST PLUS GRAVE : `applyFreqContext` demande « tu t'entraines plutot 5 fois maintenant ? », la personne repond OUI, et seul `coachQuiz.answers.freq` est ecrit — `S.activityLevel` ne bouge pas. Le TDEE, les macros et l'anneau restent cales sur une frequence que la personne a elle-meme corrigee. C'est R4 dans sa forme la plus pure (l'info est collectee, VALIDEE, stockee, et n'atteint jamais le calcul) doublee de R2 (deux declarations du meme fait). 👉 La tuile dit desormais le nombre de seances de la semaine, et une carte propose de recaler le niveau — avec le gain en kcal CALCULE, jamais devine. ⛔ ON PROPOSE, ON N'APPLIQUE JAMAIS TOUT SEUL : changer une cible calorique dans le dos de la personne est « l'erreur qui la touche » (R29). ⚠️ R12 : il faut le meme rythme sur 3 semaines sur 4 — une semaine chargee ou des vacances ne deplacent pas ce que quelqu'un mange.
const PRECACHE = [
