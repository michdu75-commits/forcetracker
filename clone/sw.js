 ...
 */
const CACHE = 'ft-v955'; // 🧹 ft-v955 = LE MENAGE DU MENU ADMIN — et DEUX FOIS je me suis trompe sur ce qui etait inutile. Michel : « retire ce qui est inutile, par contre marque bien dans les journaux qu'on les a retires ET POURQUOI ILS ONT ETE NECESSAIRES, ca permet d'avoir une tracabilite ». ⭐⭐ VERIFICATION FAITE, RIEN N'ETAIT INUTILE — et c'est R28 paye deux fois dans la meme tache. ① PT-001 : j'avais annonce « le benchmark l'a remplace ». FAUX. Il rejoue TOUT l'historique dans l'ordre, fait debriefer chaque seance et verifie que Milo se souvient de l'objectif de la fois d'avant — c'est un test de MEMOIRE LONGUE, alors que le benchmark teste 16 messages ISOLES. Il mesure la promesse centrale du produit, rien d'autre ne le fait. ② LE RECALAGE des anciennes seances : je le croyais one-shot. Or l'IMPORT D'HISTORIQUE existe, et des seances importees n'ont pas l'heure de leurs series — elles auraient besoin d'etre recalees. La fonction est idempotente et dit « rien a recaler » quand il n'y a rien. 👉 ON RANGE, ON NE SUPPRIME PAS : 36 boutons a plat -> 6 sections <details> repliables, seule la SURVEILLANCE ouverte. 19 cartes deplacees, 34 boutons, ZERO perdu — c'est le temoin central, parce qu'un refactoring HTML perd un bouton sans rien casser de visible. ⭐⭐ LE SEUL VRAI NETTOYAGE EST AILLEURS : les personas VC portaient les PRENOMS de vrais testeurs (Tatiana, Christophe, Emma) pour des profils ENTIEREMENT INVENTES. Le 21/08 j'ai lu le champ `resume` de VC-002 comme un FAIT sur le vrai Christophe et je m'en suis servi comme ARGUMENT pour justifier une decision produit ; Michel a du me corriger deux fois. Le piege etait STRUCTUREL, pas une etourderie — un decor de test et une note sur une personne reelle se lisaient exactement pareil. Renommes en profil A/B/C, prenoms injectes a Milo compris.
const PRECACHE = [
