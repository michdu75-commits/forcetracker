 ...
 */
const CACHE = 'ft-v901'; // 🔄 ft-v901 = LE CARDIO D'AVANT COMPTE ENFIN DANS LA DUREE DE LA SEANCE. Michel, en pleine seance : « je viens de commencer la seance mais la seance n'a pas commence ; je fais du cardio avant, il faut que ce soit pris en compte dans la duree totale ». Cause : depuis le 14/08 le chrono demarre a la 1re SERIE VALIDEE (tres bonne decision, elle a corrige 254 min stockees pour 96 reelles) — mais tout ce qui se passe AVANT en est sorti, y compris un vrai cardio. On ne touche PAS au chrono (c'est la mesure de la musculation) : une seule fonction, _dureeTotaleMin (app.js), ajoute les minutes de cardio qui ne sont pas deja dedans — rien si la duree a ete SAISIE a la main, l'avant seul si la source est le chrono (l'apres y est deja), les deux si la source est l'horodatage. ⭐ ET UNE SEANCE DE CARDIO SEUL N'AFFICHAIT AUCUNE DUREE : valide depuis le 02/08, elle arrivait avec duration=0 et proposait « ajouter la duree » apres 30 min de velo. ⭐⭐ UN DEFAUT SILENCIEUX TROUVE AU PASSAGE : _rythmeSeance (coach.js), ecrite le 16/08, soustrayait encore le cardio d'AVANT d'un chrono qui ne le contenait plus depuis le 14/08 — le rythme paraissait plus RAPIDE qu'en vrai, donc Milo mettait TROP d'exercices dans une heure : exactement le bug que cette fonction repare. ⚠️ Avant la 1re serie, l'ecran annonce le cardio note au lieu d'un « 0:00 » qui se lit comme une panne. On ne demarre pas le chrono pour autant : noter 20 min n'est pas la preuve qu'on vient de les faire.
const PRECACHE = [
