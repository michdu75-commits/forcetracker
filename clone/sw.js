 ...
 */
const CACHE = 'ft-v898'; // 🧭 ft-v898 = LES RENVOIS DE POSITION DU PROMPT SONT VERIFIES — et j'en avais casse un la veille. Le prompt dit a Milo OU trouver les choses (« son cadre CHIFFRE est plus bas », « les consignes du Gardien plus haut », « son historique plus haut »). Chacune est une AFFIRMATION verifiable, et un renvoi faux envoie Milo chercher au mauvais endroit sans lever la moindre erreur. ⚠️⚠️ CELUI QUI COMPTE : « ce qu'elle a reellement fait est dans son historique, PLUS HAUT » — or ft-v896 a descendu DERNIERES SEANCES tout en bas, donc l'historique etait 14 000 caracteres PLUS BAS. C'est la phrase PLANIFIE vs REALISE, celle qui empeche Milo de feliciter quelqu'un pour une seance qu'il n'a JAMAIS faite (docs/MODELE-METIER.md). Je l'avais cassee moi-meme en reordonnant. ⭐ LE CORRECTIF N'EST PAS D'ECRIRE LA BONNE DIRECTION, C'EST DE NOMMER LE BLOC (« le bloc DERNIERES SEANCES ») : une position ecrite en toutes lettres se perime au premier deplacement, et personne ne va la relire. Deuxieme renvoi faux en deux jours, tous deux silencieux -> temoin permanent (bloc XLVII). ⛔ ET L'OPTION 2 (scinder le Gardien, 5 -> 2 empreintes) est ECARTEE, PAS ABANDONNEE : elle eloigne les zones de blessure de la regle qui dit quoi en faire, et aucun test local ne sait verifier ca. Decision de Michel : « du moment que Milo assure toujours ». Ce qu'il faudrait pour la ressortir est ecrit dans docs/AUDIT-CONTEXTE-MILO.md §13.
const PRECACHE = [
