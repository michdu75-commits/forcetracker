 ...
 */
const CACHE = 'ft-v956'; // 🔎 ft-v956 = DES PROPOSITIONS QUAND ON TAPE UN ALIMENT. Michel, apres son PREMIER vrai repas note : « pour rentrer les aliments il n'y a pas de choix de propositions donc je suis oblige de faire fonctionner l'IA ». ⭐⭐ IL A RAISON, ET C'EST LE TROU N°1 DU DOSSIER NUTRITION : le champ « a la main » etait un texte VIDE — soit on connait ses macros par coeur, soit on depense une estimation IA POUR UNE BANANE. Le code-barres, lui, ne sert que si on a l'emballage. ⭐ DEUX SOURCES, ET AUCUNE INVENTEE : ① CE QU'IL A DEJA NOTE (instantane, hors ligne, ZERO invention — ses propres entrees avec ses propres grammages ; dedoublonne, la plus RECENTE gagne) ; ② LA RECHERCHE OPEN FOOD FACTS (l'app lui parle DEJA pour les codes-barres : meme serveur, gratuit, sans cle ni quota, vraies valeurs avec leur provenance). ⛔⛔ LE TEMOIN CENTRAL : aucun essai IA n'est consomme par ces deux chemins — compte sur les appels reseau reels, pas suppose. ⭐ R2 : un resultat de RECHERCHE passe par le MEME chemin que le code-barres (`_offRemplirFormulaire` sorti de `_lookupBarcode`), donc grammes, provenance, score sante ET l'avertissement cru/cuit du ×2,7 marchent aussi pour lui — deux chemins separes auraient fini par perdre l'avertissement d'un cote sans que personne ne le voie. ⛔ RIEN AU DEMARRAGE : tout part d'une frappe (regle d'or #4). Les locales sortent SANS reseau, la recherche distante attend une pause de frappe. Hors ligne, elle rend une liste vide et les locales continuent. ⚠️ LIMITE ECRITE : Open Food Facts est une base de PRODUITS DE MARQUE — « banane » y rend des bananes de marque, pas l'aliment generique. CIQUAL (3 484 generiques) reste le bon outil et n'est pas la ; on ne fait pas semblant du contraire et on n'invente surtout pas de valeurs nous-memes (R29).
const PRECACHE = [
