 ...
 */
const CACHE = 'ft-v897'; // 🛡️ ft-v897 = LE GARDIEN NE CASSE PLUS LE CACHE PENDANT LA SEANCE. `_gardienRules()` n'ajoutait pas que des regles : elle collait une NOTE sur la seance du jour (« DANS SA SEANCE DU JOUR : Developpe Militaire -> sollicite ton epaule ») EN TETE du contexte, avant meme « Tu es Milo ». Or un cache de prefixe se coupe a la premiere difference : pour quelqu'un de blesse, ajouter ou retirer un exercice qui touche sa zone fragile coupait a la position 1 487 et refacturait 46 741 caracteres du bloc COMMUN — pendant la seance, c'est-a-dire quand la personne ecrit le plus a Milo. MESURE sur 8 profils de sante x 2 seances : 9 empreintes de cache sur 16 -> 5. Michel a choisi cette option (la moins risquee) apres avoir demande le gain de l'option 2 (scinder le Gardien : 5 -> 2, mais elle deplace les zones nommees loin de la regle, ce qu'aucun test local ne sait verifier). ⚠️⚠️ CE QUI N'A PAS BOUGE, ET C'EST TESTE EN PREMIER : la REGLE (« ADAPTER, jamais interdire ») et les ZONES fragiles NOMMEES restent en tete, avec leur priorite absolue (R11). Seule l'OBSERVATION du jour descend, aupres de la seance qu'elle commente — ce n'est pas une regle de securite, c'est un constat sur aujourd'hui. On n'a pas achete du cache avec de la securite. ⚠️ Le calcul des zones fragiles est desormais dans _gardienZones(), lue par les DEUX (R2 : une information, un seul proprietaire — les faire calculer deux fois, c'est se garantir qu'ils divergeront). VERIFIE : 266 lignes de contexte des deux cotes, zero perdue, zero ajoutee.
const PRECACHE = [
