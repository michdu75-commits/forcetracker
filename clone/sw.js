 ...
 */
const CACHE = 'ft-v933'; // 🥑 ft-v933 = LE BENCHMARK A TROUVE SON PREMIER VRAI DEFAUT — Milo proposait RIZ, PATES, PAIN a un profil KETO. 1re vraie passe, lancee par Michel : les quatre aliments que la regle interdit nommement, sur les DEUX modeles et aux DEUX passes. ⭐⭐ VERIFIE AVANT DE CODER (R7 — le prompt est le DERNIER levier) : S.keto etait bien a true, ET la regle « ne propose JAMAIS (riz, pates, pain...) » etait bien DANS le prompt. Ce n'est donc ni une donnee absente (R8) ni une regle manquante : c'est une regle PRESENTE et NON APPLIQUEE — exactement l'hypothese que le benchmark existait pour tester (§8 de ARCHITECTURE-CERVEAU-CERVELET.md). L'outil a paye sa premiere passe. ⭐ LE CHIFFRE QUI L'EXPLIQUE : la regle etait a 67 % du prompt, parmi 56 autres « JAMAIS ». C'est la dilution, avec enfin un cas concret. 👉 CORRECTIF = LE LEVIER §9 n°1 : un rappel COURT en toute fin de prompt (97 %), dans la zone JAMAIS mise en cache, declenche seulement quand la question porte sur l'alimentation (346 caracteres, 0 quand on parle seance). ⛔ ET LA REGLE D'ORIGINE N'EST PAS RETIREE : si la detection rate, on retombe sur le comportement d'hier, jamais sur une regle absente en silence. C'est la condition que §9 pose lui-meme, et le temoin qui la garde est le plus important du bloc. ⚠️⚠️ ET L'INSTRUMENT S'EST TROMPE DEUX FOIS, LES DEUX DE MOI : ① un FAUX ROUGE — Haiku ecrivait « on estime ton 1RM a env. 93 kg » et le temoin criait sur une charge impossible, or un 1RM estime n'est pas une charge a mettre sur une barre (R19 : un faux rouge ferait jeter l'outil) ; ② une CONCLUSION TROP FORTE — il annoncait « R9 est CONFIRME » des que Haiku avait UN rouge de plus, alors que deux passes du MEME modele ont donne 3 puis 4 rouges. La variation naturelle est de ±1 : il faut desormais 3 rouges d'ecart, et le rapport dit « pas concluant » en dessous. Un outil de mesure qui conclut plus fort que ses donnees est pire qu'une absence d'outil.
const PRECACHE = [
