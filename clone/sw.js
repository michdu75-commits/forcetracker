 ...
 */
const CACHE = 'ft-v950'; // 🍽️ ft-v950 = LES REPAS D'ENTRAINEMENT N'EXISTENT PLUS QUE LES JOURS D'ENTRAINEMENT. Michel : « ok maintenant le plan de repas les jours de seance ». ⭐⭐ LE DEFAUT ALLAIT DANS LES DEUX SENS, et c'est ce qui le rendait invisible : les plans muscle/force/endurance affichaient « Pre-entrainement » ET « Post-entrainement » TOUS LES JOURS — jusqu'a 40 % des calories d'un dimanche de repos (force : 15 % + 25 %) rangees autour d'une seance qui n'existe pas — pendant que le plan perte n'en a AUCUN, meme un jour de squat lourd. Un plan qui parle d'entrainement un jour de repos n'est pas juste inutile : il apprend a ne plus lire les intitules. ⛔ ET LE POINT QUI COMPTE LE PLUS N'EST PAS L'INTITULE : les calories du jour ne bougent pas d'un kcal — elles sont REDISTRIBUEES, jamais retirees. On corrige un libelle qui ment, on ne change pas ce que quelqu'un mange (R29). ⭐ R2 : la redistribution est la MEME fonction que celle du jeune intermittent, qui l'avait deja ecrite ; deux copies finiraient par ne plus redistribuer pareil. ⭐⭐ Un jour de seance, les repas NOMMENT L'HEURE REELLE (« avant ta seance de 18 h ») — et si l'heure est inconnue on n'en INVENTE pas une. 3 sources dans l'ordre de certitude : seance FAITE, EN COURS, ANNONCEE pour aujourd'hui ; jamais un jour de la semaine. ⚠️ DEUX LIMITES ECRITES ET EPINGLEES PAR UN TEMOIN : les plans perte/recomp n'ont pas de pre/post donc rien ne change pour eux, et la journee n'est PAS reordonnee selon l'heure (une seance a 7 h devrait placer le pre-entrainement avant le petit-dejeuner).
const PRECACHE = [
