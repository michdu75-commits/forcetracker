 ...
 */
const CACHE = 'ft-v903'; // 🔄 ft-v903 = UNE MISE A JOUR NE TOMBE PLUS PENDANT UNE SEANCE QUI COMMENCE PAR DU CARDIO. Michel, pour la 2e fois : « faut eviter de faire une mise a jour quand je suis en seance, ca me nique mon bilan de fin de seance » (la 1re fois, le 15/08, avait cree le garde-fou). ⚠️⚠️ LE TROU : « seance en cours » se mesurait au nombre d'EXERCICES (S.wkt.exs.length). Une seance commencee mais sans exercice encore saisi — typiquement 20 min de velo AVANT la muscu, exactement sa seance de ce matin — ne comptait donc pas comme une seance. Le seul rempart restant etait « on n'applique que sur l'Accueil » : un aller-retour par l'accueil pendant le cardio, et la mise a jour tombait au milieu. 👉 UNE SEULE DEFINITION, lue par tout le monde (R2) : _seanceOuverte() (log.js) = demarree (startTs), ou avec des exercices, ou avec un cardio note — PAUSE COMPRISE, parce qu'une seance en pause n'est pas une seance finie. ⚠️ Et elle ne se confond pas avec _wktEnCours() (« est-ce que je m'entraine LA ? », pause exclue), qui sert au verrou d'ecran : deux questions voisines, deux reponses, une seule base.
const PRECACHE = [
