 ...
 */
const CACHE = 'ft-v931'; // 🧪 ft-v931 = LE BENCHMARK A UN BOUTON DANS L'APP — Michel a choisi « un bouton dans l'app », et c'etait la bonne reponse. ⚠️ POURQUOI CE N'EST PAS DU CONFORT : la ligne de commande ne peut PAS partir d'une session Claude Code (reseau bloque vers workers.dev, CONNECT → 403) ni d'un serveur local (le Worker n'accepte que l'origine github.io, verrou anti-abus du 27/07). Un outil de mesure que personne ne peut lancer ne mesure rien. L'app, elle, EST deja sur la bonne origine. 👉 Profil → Admin → « 🧪 Lancer le benchmark (15 scenarios) » et « ⚖️ Comparer Sonnet et Haiku ». Le COUT est annonce AVANT (0,25-0,95 € · 0,30-1,30 €), demande explicite de Michel. ⭐ R13 PUR : rien de neuf — _vcApplyPersona, _vcAsk, le gel _demoMode et la restauration par load() existaient deja pour les cartes VC ; on ajoute la BOUCLE et l'execution des verificateurs. ⚠️⚠️ ET LE CORPUS N'EST PAS RECOPIE (R2) : les 15 scenarios vivent dans tests/milo/eval-scenarios.js, lu par la ligne de commande ET par le bouton. Les recopier garantirait qu'un jour les deux ne testent plus la meme chose, sans que rien ne le signale. ⛔ ET IL SE TELECHARGE A LA DEMANDE, jamais au demarrage : l'app doit s'ouvrir instantanement a la salle (regle d'or #4). ⭐⭐ LE TEMOIN LE PLUS IMPORTANT N'EST PAS QUE CA MARCHE, C'EST QUE LES DONNEES REVIENNENT : on injecte 15 personas a la place du profil ; si la restauration lachait, on effacerait un compte pour un test. Regle d'or #3, elle passe avant tout. ⚠️ Et mon 1er temoin lisait window._evReport alors qu'un `let` en tete de script n'est PAS une propriete de window — 4 rouges qui ne mesuraient rien : c'etait le TEST qui etait faux, pas le code.
const PRECACHE = [
