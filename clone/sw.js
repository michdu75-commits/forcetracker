 ...
 */
const CACHE = 'ft-v946'; // 🕰️ ft-v946 = L'HISTORIQUE DEJA STOCKE PASSE AU GARDIEN — plus besoin d'attendre. Michel : « attend une chose on ne pourra pas recuperer les anciennes conversations alors ». Il avait raison : le compteur de ft-v944 ne compte qu'a partir de son branchement. ⭐ MAIS LES CONVERSATIONS SONT LA, sur le telephone (30 rangees + le fil en cours), et VERIFIE : elles gardent le texte BRUT, blocs {"retiens"} compris — c'est ce qui rend la mesure JUSTE et pas approximative. Un scan local, 0 appel, quelques millisecondes, lance UNE fois APRES le demarrage (jamais pendant : regle d'or #4). ⭐⭐ C'EST UN INSTANTANE, PAS UNE ADDITION : on REMPLACE le bloc `retro` a chaque passage, donc rejouer dix fois donne le meme resultat — pas de drapeau « l'ai-je deja fait ? » a oublier, et un chiffre double ne se voit pas. ⛔ ET IL RESTE SEPARE DU DIRECT : l'historique couvre PLUSIEURS versions de Milo, dont des anterieures aux correctifs ; additionner les deux donnerait un total qui ne veut rien dire. Deux blocs, deux periodes datees. ⚠️⚠️ ET UN TEMOIN A TROUVE UN VRAI DEFAUT DE MESURE : `bloc_technique` se leve sur CHAQUE seance proposee, CHAQUE bloc de memoire, CHAQUE liste de reponses rapides — c'est du trafic NORMAL, pas une derive. Le compter l'aurait rendu majoritaire et aurait noye le signal : on aurait mesure le BON fonctionnement de Milo en croyant mesurer ses ecarts. Il reste affiche dans le badge, il n'entre plus dans les compteurs. Une seule liste `_GARDIEN_DERIVES`, lue par le direct ET par le retro (R2). ⭐ MESURE SUR LES 25 VRAIS JOURS DE MICHEL : 7 derives sur 129 reponses (diagnostic 3 · promesse_vide 3 · source_fabriquee 1). ⚠️ Ce sont des DRAPEAUX, pas des preuves : seules les 3 promesses ont ete verifiees a la main.
const PRECACHE = [
