 ...
 */
const CACHE = 'ft-v924'; // 🔘 ft-v924 = LE BOUTON N'APPARAISSAIT PAS SUR UNE VRAIE SEANCE — retour de TERRAIN de Michel, capture a l'appui : « J'ai pas le bouton lancer la séance », alors que Milo ecrit « clique sur ⚡ Commencer cette séance ». ⚠️⚠️ C'ETAIT MA LIVRAISON DE LA VEILLE (ft-v919). Mesure sur SON texte reel : `_ressembleASeance` rendait FALSE — le cervelet n'etait meme pas appele — et le filet rendait RIEN. DEUX causes : ① j'exigeais un NOM et des SERIES sur la MEME ligne, or Milo ecrit un BLOC (nom sur sa ligne, « Paliers : 60×5 → 80×3 » ensuite, puis « 3×3 à 130 kg — repos 3 min »). ⚠️ J'avais valide mon detecteur sur des textes que J'AVAIS ECRITS — le piege classique de tester ses propres exemples au lieu du reel. ② la regex du filet est ancree en FIN de ligne : le « — repos 3 min » qui traine apres les kg la faisait echouer completement. 👉 L'aiguillage accepte une ligne de series SANS nom (il ne fait qu'ORIENTER, R29) et le filet va chercher le nom sur la ligne PRECEDENTE, en sautant les lignes de paliers — sinon c'est « Paliers : 60×5 » qui deviendrait le nom de l'exercice. ⭐ SON TEXTE REEL DEVIENT UN TEMOIN PERMANENT (R17) : les 5 exercices, dans l'ordre, avec les bons noms. ⚠️ Et la regle des NOMS n'a pas bouge : ce qu'on lit est repris TEL QUEL, jamais rapproche « a peu pres » d'un exercice voisin (leçon du 04/08).
const PRECACHE = [
