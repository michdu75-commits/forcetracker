 ...
 */
const CACHE = 'ft-v944'; // 🛡️ ft-v944 = LE GARDIEN TOURNE ENFIN LA OU LES GENS VIVENT — et il a fallu le CALIBRER d'abord. Michel a exporte ses conversations (« Regarde ») : 258 messages, 4 discussions, 25 jours, 142 425 caracteres. ⭐⭐ ON Y A MESURE 3 VRAIES PROMESSES DE MEMOIRE NON TENUES (« c'est note », « je retiens pour la prochaine fois ») sans qu'un seul bloc soit enregistre — et RIEN ne les voyait passer, parce que le Gardien de sortie ne tournait QUE sur le clone. Un garde-fou qui ne tourne pas la ou les gens vivent ne garde rien. ⚠️⚠️ MAIS LE BRANCHER TEL QUEL AURAIT ETE PIRE : joue sur ces 129 vraies reponses, le motif criait 7 fois — 3 vraies et 4 phrases qui n'en sont pas (un debrief « ce que je retiens : », une explication de sa propre memoire, un accuse de reception, une offre conditionnelle). Un garde-fou juste une fois sur deux ne survit pas a son premier mois (R19). Il a donc ete CALIBRE sur ces vraies donnees AVANT d'etre branche : 7 alertes → 3, et ce sont exactement les 3 vraies. ⛔ LE TEXTE AFFICHE NE CHANGE PAS D'UN CARACTERE : `_gardienSortie` commence par `_stripCoachTech` (ce que faisait deja la prod) et se contente de lever des drapeaux — on ajoute une MESURE, pas un filtre. C'est le temoin le plus important du bloc. ⚠️ ON MESURE CHEZ TOUT LE MONDE, ON AFFICHE CHEZ NOUS : le badge reste reserve (clone + admin), parce que « promesse de memoire sans rien enregistrer » sous une reponse ferait douter n'importe qui de son coach pour un defaut qui nous regarde. Chez les autres, la derive est COMPTEE. ⛔ ET LE COMPTEUR NE GARDE QUE DES NOMBRES : aucune phrase de Milo, aucun mot de la personne — une derive se mesure par sa FREQUENCE, et stocker le contenu fabriquerait un journal de conversation que personne n'a demande (Constitution P3). Lisible dans Profil → Admin. ⚠️ Les 3 cas trouves sont tous ANTERIEURS au correctif du 20/08 (ou dans le fil en cours, non datable) : rien ne prouve que ft-v923 a echoue — c'est desormais mesurable, ce qui est le vrai apport.
const PRECACHE = [
