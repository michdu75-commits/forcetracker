 ...
 */
const CACHE = 'ft-v915'; // ⚖️ ft-v915 = LE CRU/CUIT EST ECRIT, JAMAIS CONVERTI. Defaut mesure le 18/08 et corrige ce soir : la table `_PORTIONS` melangeait les deux SANS LE DIRE — riz 350 kcal/100 g (cru), pates 350 (seches), quinoa 368 (sec), mais legumineuses 116 (CUITES). Une ligne « Riz 80 g + lentilles 120 g » demandait donc de peser l'un cru et l'autre cuit, sans un mot. ⭐ ET CE N'EST PAS DU BRUIT QUI S'ANNULE SUR LA SEMAINE : c'est un biais SYSTEMATIQUE, toujours dans le meme sens — la seule classe d'erreur que « coherence > reactivite » ne peut pas absorber. ⛔ ON NE CONVERTIT PAS, ON NOMME : le ratio d'absorption d'eau depend de la cuisson de chacun, le calculer serait inventer un chiffre (R29). Chaque ligne porte son etat, et l'affichage dit « pese cru » / « pese cuit ». ⚠️ LA CONVENTION SUIT L'ALIMENT, PAS UNE REGLE GLOBALE : le riz s'achete sec, les lentilles arrivent souvent cuites en boite — forcer une convention unique obligerait a mentir sur l'un des deux. ⭐⭐ ET LE MEME PIEGE EXISTAIT COTE JOURNAL : Open Food Facts donne les valeurs « telles que vendues », donc un paquet de pates scanne puis pese CUIT compte x2,7. Une note previent, sans bloquer (R24), sur les seuls aliments qui gonflent vraiment. Et `etat` descend enfin jusqu'a la donnee : le champ existait depuis la brique 0 (ft-v907) et valait TOUJOURS null — R4, dans le fichier qui documente R4.
const PRECACHE = [
