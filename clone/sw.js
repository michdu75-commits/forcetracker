 ...
 */
const CACHE = 'ft-v930'; // ⚖️ ft-v930 = LE BENCHMARK SAIT COMPARER DEUX MODELES — Michel : « par la meme occasion test en haiku non ? ». ⭐⭐ OUI, ET C'EST LA SEULE FACON DE FERMER UNE QUESTION QUI REVIENT : « Sonnet pour tout le monde » (10/08) tient sur R9 — un raisonnement JUSTE, mais jamais MESURE sur ce prompt-ci. `--compare` joue les 15 scenarios sur les deux et met les verdicts cote a cote. ⚠️ CE N'EST PAS UNE RE-PROPOSITION de changer de modele (R30) : c'est la mesure qu'il faudrait pour avoir le droit de rouvrir le sujet. ⚠️⚠️ ET LA LECTURE EST ASYMETRIQUE, ecrite dans le worker, le runner ET le rapport : Haiku nettement plus rouge → R9 CONFIRME par un chiffre, question close · Haiku aussi vert → ca ne ROUVRE RIEN (un vert ne dit que « aucune violation detectable sur 15 pieges », et ce qui fait Milo — le ton, le naturel, le refus d'insister — n'est dans AUCUN de ces motifs ; l'argument du 10/08 n'etait pas technique : « si les gens trouvent Milo nul ils ne vont pas le prendre »). Ce test peut CONFIRMER la decision, pas la renverser. ⛔ ACCEPTER UN MODELE VENU DU CLIENT SANS OUVRIR DE TROU : la liste blanche du worker ne contient QUE des modeles MOINS CHERS que le defaut — le pire qu'un curieux puisse faire est de se rendre son PROPRE Milo plus bete et moins cher, ca ne touche personne d'autre et ca ne peut pas faire monter la facture ; tout le reste est IGNORE (repli silencieux, jamais une erreur). Le temoin compare les PRIX, pas les noms. ⚠️ ET LE WORKER REND LE MODELE QUI A REELLEMENT SERVI (_model), pas celui demande : sans ca un rapport annoncerait « teste en Haiku » une passe entierement jouee en Sonnet — l'erreur exacte des personas VC, et une comparaison d'un modele avec lui-meme ne se voit pas a l'oeil. Devis mesure : Sonnet 0,23-0,95 € · Haiku 0,08-0,32 € · comparaison ≈ la somme.
const PRECACHE = [
