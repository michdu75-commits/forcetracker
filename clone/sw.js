 ...
 */
const CACHE = 'ft-v952'; // 🔋 ft-v952 = OU ON ARRIVE A 100. Michel : « on a le score de recuperation mais il faudrait rajouter la donnee ou on arrive a 100 (bon sauf moi qui suis fumeur) ». ⭐ LE SCORE DONNAIT UN NOMBRE SANS DIRE CE QUI COUTE LES POINTS MANQUANTS — or c'est la seule chose sur laquelle on peut agir : un 72 sans explication est un jugement, « 72, il te manque surtout du sommeil » est un levier. Le detail par facteur EXISTAIT deja dans le calcul, il n'etait simplement pas exploite. ⭐⭐ ET SA PARENTHESE ETAIT LE POINT PRINCIPAL, PAS UNE BLAGUE — et elle est CALCULABLE : l'age et le tabac sont les deux facteurs PERMANENTS, donc a 48 ans et fumeur le maximum atteignable n'est pas 100 mais 93. Un plafond invisible transforme un outil de progres en reproche quotidien : on vise chaque jour un 100 qui n'existe pas, et on ne comprend pas pourquoi on n'y arrive jamais (Constitution P21). ⛔⛔ MAIS ON NE RE-BAREME PAS LE SCORE POUR AUTANT : ramener le chiffre « sur 93 » aurait reecrit silencieusement TOUT l'historique — un 85 d'il y a trois mois n'aurait plus voulu dire la meme chose et les courbes seraient devenues fausses sans que rien ne le signale. On garde l'echelle absolue et on AJOUTE le plafond. ⛔ Les permanents ne sont PAS comptes comme un « manque » : ils deplacent la ligne d'arrivee, ils ne sont pas un retard rattrapable ce soir. ⛔ ET AUCUN CONSEIL D'ARRETER DE FUMER : on nomme le fait, sans le commenter et sans le repeter (Constitution P13, accompagnement jamais therapie) — un temoin le verifie. ⛔ Temoin d'exactitude : plafond moins la somme des manques doit redonner LE SCORE EXACT, sinon un point fantome se promene.
const PRECACHE = [
