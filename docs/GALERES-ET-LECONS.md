...
| 🕳️ **4 trous de données connus** (garde-fou, 28/07) | ① **`nextPlanned`** — l'Accueil affiche « je m'en souviens » et le chat ne l'a pas : *Milo affirme se souvenir de ce qu'il n'a pas* ② `programmes` (il ne connaît pas ton planning) ③ `customExercises` ④ `exRestPref` (240 s au squat, ignoré) | Les brancher un par un. Le garde-fou `node tests/donnees/runner.js` les compte à chaque livraison → ils ne peuvent plus être oubliés |
