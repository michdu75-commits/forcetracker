...
|---|---|---|
| 🚨 **Worker IA GRAND OUVERT** (audit 27/07) | Aucune vérif d'origine (CORS `*`), aucun quota sur ce chemin (celui d'Apps Script est contourné) → **n'importe qui avec l'URL peut consommer l'API Anthropic aux frais de Michel** (coach = Sonnet, imports 8192 tokens). Le quota « 10 questions » est côté client seulement | **Patch PRÊT** : `docs/worker-securise-PROPOSITION.txt` (allowlist d'origines + procédure 2 min + rollback 1 ligne). À activer **AVEC Michel** (test device obligatoire — worker.js se déploie auto au push). + règle de rate-limiting dans le dashboard Cloudflare |
...
| 🧠 Prompt Milo ~18k tokens (audit 27/07) | Latence de chaque réponse + coût par message. Choix qualité assumé, mais ça grossit | Jardiner (« une consigne entre, une sort ») ; mesurer avant/après |
