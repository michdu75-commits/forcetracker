...
|---|---|---|---|---|---|
| 🟡 | 26/08 21:05 | session-B (claude-md-docs) | **LE TEMPO DESCEND JUSQU'À LA DONNÉE** — suite directe du relais 📣 de session-A (6 programmes de coach, §3bis). ⚠️ **Correction de ma propre annonce à Michel** : j'avais dit *« le tempo n'existe nulle part »*, c'est trop absolu — la consigne libre par exercice (`ex.note`) existe, s'affiche en séance et part chez Milo. **Le trou est ailleurs** : `SEC_PAR_REP = 3` est écrit EN DUR dans `app.js` et son propre commentaire dit *« le tempo courant »*. 👉 *Le tempo est de la PROSE dans une note et une CONSTANTE dans le calcul — les deux ne se rencontrent jamais* (**R4**). | `app.js`, `log.js`, `tests/parcours/runner.js`, `sw.js` | ft-v1028 (réservée) |
