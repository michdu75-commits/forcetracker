...
|---|---|---|---|---|---|
| 🟡 | 24/09 16:50 | session-A (project-status) | **🧠 MILO — provenance structurelle du débrief : supersets / ordre / RIR / cardio.** Retour réel de Michel (24/09) : Larsen « 70→90 » absent des séries affichées · cardio de FIN qualifié d'échauffement · superset Oiseau + Marteau aplati · RIR de l'Oiseau peut-être jamais proposé. Les 4 symptômes sont des **hypothèses à revalider**. ⛔ Pas de merge `master`, pas de publication, pas de bump, aucune règle de coaching inventée. | `log.js`, `coach.js` *(selon la cartographie)*, `tests/parcours/`, `tools/`, `RETOURS-TESTEURS.md` | — |
