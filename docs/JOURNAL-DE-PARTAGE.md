...
| 🟢 | 27/08 11:45 → 12:40 | session-B (claude-md-docs) | **LA BARRE « SÉANCE » NE LAISSE PLUS LIRE CE QUI DÉFILE DESSOUS — LIVRÉ.** Trouvé sur une **vraie vidéo** de l'iPhone de Michel : `#log-hdr` à `rgba(…,.55)` laissait lire *« Choisis par quoi tu commences 👇 »* **à travers**, emoji compris. ⛔ **Pas une bizarrerie iOS** — reproduit dans Chromium, où le flou s'applique pourtant : *un flou adoucit, il ne cache pas.* Opacité → **`.96`**, flou **conservé** (R30). ⛔⛔ **Le CSS n'avait pas bougé** (`git log -S` : ≥ 29/07) ; l'écran Séance **vide** est passé de **0 → 224 px** défilables avec mes cartes de ft-v1026. *Le défaut était inatteignable, pas absent.* **3ᵉ fois en deux jours** → nouvelle famille **§21** de `BUGS.md` : *dater la ligne fautive avant d'accuser sa propre livraison.* ⛔ **Jumelles cherchées (R8)** : `#log-hdr` est le seul sticky concerné. ⛔ **Pas de pop-up** : rien à faire, aucun repère déplacé. ⚠️ **Bloc CXXXVIII, CXXXVII sauté exprès** pour votre ft-v1033 en vol (3ᵉ collision de la semaine). **Tests : parcours 1625/1625**, calculs 266, muscles 241, croisés 50, dates 7, données 0 trou. Contrôle négatif rouge sur `.55`, vert sur `.96`. | `index.html`, `tests/parcours/runner.js`, `BUGS.md`, `sw.js` | **ft-v1032** |
| 🟡 | 27/08 13:10 | session-B (claude-md-docs) | **UNE CHARGE PRESCRITE SANS REPÈRE DOIT LE DIRE** — application du critère donné par Michel aujourd'hui : *« la coach savait que moi je m'y connais ; tout le monde ne connaît pas ce que représente le lourd »*. ⛔⛔ **Trou mesuré** : `_intensiteDefauts` fait `if(!(rm1>0)) return out;` — **aucun record → le contrôle se tait ENTIÈREMENT**. Milo peut donc écrire *« 4×8 à 60 kg »* sur un exercice jamais fait, et **rien** ne le signale (c'est ft-v980 sans garde-fou). ⛔ **Périmètre : les séances de MILO seulement** — un programme porte des charges venues d'un humain **qui a la référence**. Le marqueur `_milo:true` existe déjà. ⚠️ **Le nom sera RÉSOLU avant de conclure** (`exNomCatalogue`) : `_startSessionFromMilo` prend `e.name` **brut**, donc un nom voisin ferait dire *« tu n'as pas de repère »* à quelqu'un qui en a un — un fait faux sur la personne (R29). ⭐ Vérifié au passage : un renommage **migre** bien le record (`S.prs[n]=S.prs[o]`). | `log.js`, `tests/parcours/runner.js`, `sw.js` — ⛔ rien dans la nutrition | ft-v1033 |
