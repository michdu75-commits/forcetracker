...
|---|---|---|---|---|---|
| 🟡 | 26/08 19:05 | session-A (project-status) | **ONGLET MACROS RÉORGANISÉ** (`docs/MACROS-A.md`, variante A) — l'onglet fait 3,3 écrans, la cible est de savoir où on en est **sans défiler**. Réordonnancement des enfants de `#nu-macros` **uniquement** + 2 accordéons repliés (réglages, « comment c'est calculé »). ⚠️ **Je touche `index.html` (zone Nutrition l. 635-745) et `screens.js`** — dites-moi si ça vous gêne. | `index.html`, `screens.js`, `tests/parcours/runner.js`, `sw.js` | ft-v1025 (réservée) |
