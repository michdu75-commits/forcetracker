...
|---|---|---|---|---|---|
| 🟡 | 27/08 07:15 | session-B (claude-md-docs) | **LE REPOS DEVIENT UN MAXIMUM — et la 1ʳᵉ mesure a démenti ce que je croyais.** Décision de Michel relayée par session-A. ⛔⛔ **Mesuré dans un navigateur, deux fois : le chrono NE continue PAS en négatif** — à zéro la barre disparaît, `restIv` est vidé, `restStartTs=0`. ⚠️ **ft-v851 (14/08) a retiré les bornes `Math.max(0,…)` des DEUX fonctions d'affichage (`updRest`, `_updPill`) sans jamais toucher à `_restTick`**, qui appelle `stopRest()` à `left<=0`. 👉 *Les afficheurs savent montrer du négatif ; plus personne ne les appelle.* **La fonctionnalité n'a jamais tourné** — famille « le correctif posé d'un seul côté ». ⛔ Donc ce que je donnais pour acquis hier soir EST le travail. ⚠️ Point de vigilance : `_restDoneCb` (l'enchaînement superset) se déclenche aujourd'hui DANS ce `stopRest()` — il devra partir exactement une fois. | `log.js` (chrono de repos) — ⛔ **rien dans `app.js` nutrition ni `screens.js`**, hors du chemin de ft-v1029 | ft-v1030 (1029 pris) |
