...
|---|---|---|---|---|---|
| 🟡 | 26/08 09:08 | session-B (claude-md-docs) | un ÉCHEC de sync Google Sheets était compté comme un SUCCÈS : `finishWorkout` fait `if(ok)` sur l'OBJET `{ok:false,…}` (toujours vrai) → toast menteur + `synced=true` posé, donc la file de rattrapage ne la reprend jamais | `log.js`, `tests/parcours/runner.js`, `sw.js` | ft-v1015 (réservée) |
