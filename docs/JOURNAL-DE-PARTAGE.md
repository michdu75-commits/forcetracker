...
|---|---|---|---|---|---|
| 🟡 | 27/08 11:45 | session-B (claude-md-docs) | **LA BARRE « SÉANCE » LAISSE LIRE LE TEXTE QUI DÉFILE DESSOUS** — trouvé sur une **vraie vidéo** de l'iPhone de Michel, en production. `#log-hdr` est en `rgba(12,13,17,.55)` + `blur(12px)` : on lit *« Choisis par quoi tu commences 👇 »* **à travers**, emoji compris. ⚠️ **Reproduit dans Chromium** — ce n'est donc PAS une bizarrerie iOS : le flou s'applique et le texte passe quand même. *0,55 est trop transparent, point.* ⛔⛔ **ET LA DATE EST MESURÉE, pas supposée** : le CSS est ancien et **n'a pas bougé** ; avant ft-v1026 l'écran Séance vide était défilable de **0 px**, il l'est de **224 px** depuis mes cartes de types. *Le défaut était inatteignable, pas absent — il attendait son premier cas* (même famille que le gras de `.sw-feat` hier). ⛔ **Jumelles cherchées (R8)** : `#log-hdr` est le SEUL sticky avec ce motif ; les 2 autres `backdrop-filter` sont des voiles d'overlay, où la translucidité est voulue. | `index.html` (`#log-hdr`), `tests/parcours/runner.js`, `sw.js` — ⛔ rien dans la nutrition | ft-v1032 (1031 pris par session-A) |
