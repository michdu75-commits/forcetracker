...
| 🟡 | 27/08 15:20 | session-B (claude-md-docs) | **LE RIR — « il t'en restait combien ? »** (demande de Michel : *« on ajoute le RIR alors, et voir RPE, mais là c'est du lourd »*). ⛔⛔ **Trou mesuré** : `DISC_CADRE.echec` dit la règle par discipline (*« JAMAIS à l'échec »* en force athlétique, *« 1 à 3 en réserve »* en musculation…) et **Milo la reçoit** — mais **RIR/RPE n'existent NULLE PART dans l'app** (0 occurrence réelle). *On lui demande de juger une réserve qu'on ne mesure jamais* : **R8** dans sa forme la plus pure. ⭐ **`X` (échec) EXISTE DÉJÀ et vaut RIR 0** — donc **un seul propriétaire** (`_rirDeSet`), pas de 2ᵉ source qui divergerait (R2). ⭐ **Saisie dans la BARRE DE REPOS**, pas dans la ligne de série : valider une série démarre le repos (mesuré), c'est donc le bon moment — et la ligne a déjà **6 colonnes sur 430 px**, on n'en ajoute pas une 7ᵉ. ⛔ **Facultatif**, rien ne bloque. ⭐ Restitué dans la colonne « précédent » (là où il sert) + envoyé à Milo. ⛔ **RPE différé**, décision de Michel. | `log.js`, `index.html`, `style.css`, `coach.js`, `tests/parcours/runner.js`, `sw.js` — ⛔ rien dans la nutrition | ft-v1038 (1036 en ligne, 1037 réservé par session-A) |
