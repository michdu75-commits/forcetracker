...
|---|---|---|---|---|---|
| 🟡 | 26/08 18:05 | session-B (claude-md-docs) | **MON PROPRE DÉFAUT** : ma fusion par UNION a RESSUSCITÉ la ligne 🟡 d'EV-052 que session-A avait close — *une union ne supprime jamais*. Retrait de mon artefact + **contrôle 7** dans `check_regles.py` pour figer la règle (un 🟡 ne peut pas coexister avec le 🟢 de même session/heure) | `docs/JOURNAL-DE-PARTAGE.md`, `tools/check_regles.py` | — (outillage) |
