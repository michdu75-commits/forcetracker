...
|---|---|---|---|---|---|
| 🟡 | 26/08 17:00 | session-B (claude-md-docs) | **CHANTIER SÉANCE ④ — le générateur de séances sort du cadre « débutant »** (`SEANCE-DESSAI.md` §5). ⚠️ La PORTE est déjà ouverte à tout le monde — le verrou est ailleurs : le vocabulaire (« parcours débutant », « Premiers pas »), le contenu **100 % machines**, le blocage **one-shot**, et `beginnerJourney` posé même pour un confirmé | `log.js`, `index.html`, `tests/parcours/runner.js`, `sw.js` | ft-v1023 (réservée) |
