...
|---|---|---|---|---|---|
| 🟢 | 27/08 13:10 → 14:35 | session-B (claude-md-docs) | **UNE CHARGE PRESCRITE SANS REPÈRE LE DIT — LIVRÉ** (application du critère de Michel). ⛔⛔ Trou : `_intensiteDefauts` se tait **entièrement** sans record → Milo pouvait chiffrer sur un exercice jamais noté sans un mot (**ft-v980** privé de garde-fou). 👉 `_repereDefauts` le dit, **sans retirer le nombre** (R24). ⚠️⚠️ **Le résolveur de noms NE SUFFISAIT PAS** — mesuré : `exNomCatalogue('Developpe Couche')` rend la chaîne telle quelle ; sans comparaison normalisée (`_normEx`, réutilisée) on affirmait un **fait faux sur la personne** (R29). ⛔ L'**historique** compte comme repère, pas seulement le 1RM. ⚠️ Corrigé au passage : `_validationSeance` faisait `o.seanceWarn=w`, une **affectation** qui aurait effacé la nouvelle ligne — champ partagé = on **concatène**. ⛔⛔ **Et Michel a corrigé le LIBELLÉ en cours de route** (*« pas de record ne veut pas dire jamais fait »*) : le code était juste, **la phrase était fausse** — *« Aucun repère **dans ton historique** »* au lieu de *« sur cet exercice »*. **Une mesure juste peut produire une phrase fausse.** 📣 Règle #11 : points 2 à 5 (point rouge · aide `?` · aide détaillée · 30ᵉ diapo **sans image**), ⛔ **pas de pop-up** (rien à faire, aucun repère déplacé). ⚠️ Bloc **CXXXIX** (CXXXVII laissé libre pour votre ft-v1031). **Tests : parcours 1634/1634**, calculs 266, muscles 241, croisés 50, dates 7, données 0 trou. Contrôle négatif ciblé : contre ft-v1032, `["🔁 Déjà présent…"]` seul — **silence complet sur le repère**. | `log.js`, `constants.js`, `screens.js`, `coach.js`, `app.js`, `tests/parcours/runner.js`, `sw.js` | **ft-v1033** |
