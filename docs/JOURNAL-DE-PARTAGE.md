...
|---|---|---|---|---|---|
| 🟡 | 26/08 20:30 | session-B (claude-md-docs) | **LES TYPES DE SÉANCES sur l'écran Séance** (`SEANCE-DESSAI.md` §2.1) — remplir les ~700 px de vide. ⭐ Le vocabulaire existe (`DISC_LABELS`/`DISC_CADRE`, 5 types) : on ne crée PAS de 2ᵉ liste (R2). ⛔ Le vrai travail est **R4** : `coeur` dit « SQUAT · DC · SDT » en PROSE — aucun code ne peut agir dessus. | `constants.js`, `log.js`, `index.html`, `setup.js`, `tests/parcours/runner.js`, `sw.js` | ft-v1026 (réservée) |
