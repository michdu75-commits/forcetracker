...
| D-021 | 24/09/2026 → tranchée le 25/09/2026 | que faire d'un `ft4_act` **déjà stocké** à `1.55` sans preuve de provenance (l'ancienne app l'écrivait d'office, choisi ou non) ? | **Michel** | ⭐ **UNE CONFIRMATION, UNE SEULE FOIS** : ni tenu pour un choix, ni effacé. Carte « Niveau d'activité enregistré : Modéré (3-4j) — **Confirmer** / **Modifier** » (onglet Nutrition), « À confirmer » sous le sélecteur du Profil. Après réponse : provenance explicite `activitySrc = 'choisi'` (stockage `ft4_act_src`, cloud, restauration), on ne redemande plus. *Comment (Claude, dans la direction validée)* : un 1,55 à confirmer reste **calculable** (mêmes chiffres) mais il est **dit** à l'écran et à Milo (« À CONFIRMER ») ; un **autre** ancien niveau sans provenance (« hérité ») est utilisé comme avant sans être écrit « choisi » ; la provenance **ne survit jamais à sa valeur** (restauration) ; « Enregistrer » le Profil sans **toucher** au sélecteur ne vaut pas confirmation. Témoins `tests/parcours/d021_activite.js`, contrôle négatif `tools/mut_d021_activite.py`. Faits de la nuit du 24→25/09 : `docs/NUTRITION-GLUCIDES-2026-09-24.md` §10-11. | **garder silencieusement** tous les 1,55 comme des choix — écartée : l'app les a écrits d'office pendant des mois · **effacer** tous les 1,55 — écartée : on retirerait un vrai choix à ceux qui l'avaient fait, et le plan de tous les comptes existants d'un coup | cohérent | VALIDÉ |
| D-022 | 25/09/2026 | quand le niveau d'activité **nécessaire au calcul** n'est pas renseigné, Milo peut-il donner un TDEE ou une cible **par hypothèse** (même annoncée comme telle) ? (complète `D-016` / `D-020` ; ⚠️ ≠ le « D-022 » **local** de `docs/DOSSIER-V9-APPRENTISSAGE.md`, numérotation non officielle déjà signalée dans `docs/NUTRITION-GLUCIDES-2026-09-24.md`) | **Michel** | ⭐ **NON — MILO DEMANDE, IL N'INVENTE PAS.** Il peut dire que le niveau manque, expliquer qualitativement que les besoins en dépendent, demander le nombre de séances et annoncer que le calcul viendra après la réponse. ⛔ Il ne donne **ni TDEE, ni cible, ni fourchette**, ni scénario « supposons 3 séances », **même annoncés comme hypothèse**. Principe : *information manquante nécessaire au calcul → demander d'abord → calculer ensuite.* *Comment (Claude)* : cause mesurée au banc réel (run 36111949918) — trois règles du bloc commun poussaient à chiffrer (hypothèses par défaut sur la **fréquence** avec l'exemple « 3 séances/sem », ANTI-FAUX-PRÉCIS « donne des FOURCHETTES », l'anti-interrogatoire prioritaire sur « non renseigné → demande »). La consigne vit **dans la ligne d'activité** du contexte (`_MILO_ACT_ABSENTE`, coach.js), n'apparaît que sans activité, nomme les règles dont elle est l'exception, et ne touche pas le bloc commun. Périmètre : **l'activité nécessaire au TDEE/cible, rien d'autre**. Témoins B-CCCLXXI (`tests/parcours/d021_activite.js`), mutations M-R34A-1…5 (`tools/mut_d021_activite.py`). | **une fourchette étiquetée « hypothèse »** — écartée par Michel : elle devient un pseudo-calcul personnalisé, et le « modérée » de Milo (≈ 2 300–2 500) contredisait le Modéré de l'app (2 663) · **une règle générale « toute donnée manquante → ne chiffre pas »** — écartée : non auditée pour les autres données, hors périmètre · **une exception dans le bloc commun** — non retenue en premier : plafond de 46 500 caractères, et une consigne loin de la donnée perd la hiérarchie (R11) | cohérent | VALIDÉ |
| D-023 | 25/09/2026 | règle d'or **#11** pour `D-021` (anciens 1,55 à confirmer) : faut-il la pop-up `WHATS_NEW` (point 1) ? | **Michel** | ⛔ **PAS DE POP-UP.** Les mécanismes déjà livrés suffisent : la **carte** « Confirmer / Modifier » de l'onglet Nutrition, le **point rouge** `NEW_FEATURES`, l'**aide `?`** de l'onglet, l'**aide détaillée** et la **diapo du Guide** (points 2 à 5). *La pop-up se mérite* : la personne n'a rien à faire ailleurs que sur la carte qu'elle voit déjà. | **activer la pop-up** — écartée : elle répéterait la carte, qui porte déjà l'action à faire | cohérent | VALIDÉ |
| D-024 | 25/09/2026 | après `D-022`, le micro-banc réel (run 36118203950) montre Milo sans TDEE ni cible… mais écrivant *« l'écart peut dépasser 500 kcal »* : un chiffre **dépendant de l'activité inconnue** est-il permis s'il n'est ni un TDEE ni une cible ? (**clarification / extension de `D-022`**, qui reste la décision historique initiale, VALIDÉE et non réécrite) | **Michel** | ⛔ **NON. Quand l'activité nécessaire au calcul manque, Milo peut expliquer avec des mots ; il ne remplace pas le calcul interdit par un autre chiffre.** Autorisé : dire que l'activité manque, expliquer **qualitativement** qu'elle influence fortement les besoins, demander le nombre de séances, citer ce qui **ne dépend pas** de l'activité (BMR, poids, taille, âge) en précisant que le BMR n'est **ni une dépense quotidienne ni une cible**. Interdit : TDEE, cible, fourchettes, hypothèse chiffrée, estimation indirecte, **ordre de grandeur, écart ou delta en kcal, exemple numérique**, toute quantité calorique dont la valeur dépend de l'activité absente. *Chronologie (append-only)* : ① `D-022` (TDEE/cible/fourchette/hypothèse interdits) → ② banc réel run 36118203950 → ③ fuite numérique découverte : « l'écart peut dépasser 500 kcal » (un **écart** n'entrait dans aucune case de D-022) → ④ clarification de Michel le 25/09 = cette ligne, **qui s'ajoute à D-022 sans la remplacer**. Relecture contradictoire avant l'appel réel : « estimation indirecte » précisée (multiplicateur/pourcentage du BMR, kcal par séance, grammes de macros déduits d'une cible). *Comment (Claude)* : même consigne `_MILO_ACT_ABSENTE` (coach.js), élargie à **tout chiffre calorique dépendant**, avec autorisation **explicite** du BMR ; pas de post-traitement. Témoins B-CCCLXXII (`tests/parcours/d021_activite.js`), mutations M-R34A-6 à 9 (`tools/mut_d021_activite.py`). | **interdire tout chiffre de la réponse** — écartée : le BMR et les données indépendantes de l'activité restent vrais et utiles · **filtrer les nombres après génération** — écartée : post-traitement de prose, fragile, contraire à la contrainte « en amont » | cohérent | VALIDÉ |
| D-025 | 25/09/2026 | MILO-PDF1B : une réponse de Milo **non confirmée complète** (coupée par la limite de longueur, raison d'arrêt inconnue, serveur sans le signal) peut-elle être transformée en séance ? Démontré par la contre-vérification : « Leg curl 3×12 » coupé en « 3×1 » entrait dans la séance du jour. | **Michel** | ⛔ **NON. Une réponse incomplète ne peut pas être transformée en séance** : ni bouton « Commencer », ni question « on démarre ? », ni import — à l'arrivée comme au rechargement. Le texte reste lisible. Détail : `docs/MILO-PDF1.md` §B4. | **garder le bouton avec le marqueur juste au-dessus** (état de MILO-PDF1) — écarté : une séance fausse démarre sans qu'on relise | cohérent | VALIDÉ |
| D-026 | 25/09/2026 | MILO-PDF1B : où afficher le statut d'une réponse incomplète dans la bulle du chat, alors que Milo peut lui-même titrer « Analyse complète » ? | **Michel** | ⭐ **AVANT le texte de Milo**, comme premier élément de la bulle ; le texte de Milo reste intact, le PDF garde son bandeau en tête. | **sous le texte** (état de MILO-PDF1) — écarté : on lisait « Analyse complète » avant d'apprendre qu'elle ne l'est pas | cohérent | VALIDÉ |
| D-027 | 26/09/2026 | Publication MILO-PDF1 : le bouton « Enregistrer ce programme » peut-il rester sous une réponse globalement incomplète (coupée ou fin non confirmée) ? | **Michel** | ⭐ **OUI, UNIQUEMENT si** le bloc JSON du programme est lui-même complet, accepté par le parseur, de schéma valide, et qu'**aucun JSON partiel n'est réparé ni enregistré**. Mesuré avant publication : **c'était déjà exactement le comportement** (`JSON.parse` strict, programme vide refusé, rien d'enregistré avant le clic) → rien n'a changé, le comportement est **figé** par `B-CCCLXXXIV`/`B-CCCLXXXV`. Détail : `docs/MILO-PDF1.md` §C. | **retirer le bouton sous toute réponse incomplète** (aligné sur D-025) — écarté : un programme dont le bloc est complet et lisible n'a rien de coupé | cohérent | VALIDÉ |
| D-028 | 26/09/2026 | Publication MILO-PDF1 : l'« annonce de la prochaine séance » sous une réponse incomplète est-elle informative ou une écriture ? Mesuré : **une écriture** — un bloc « prevu » complet sous une réponse coupée ou non confirmée écrivait `S.nextPlanned` en mémoire et sur le disque ~~et vers le cloud~~ *(↪️ rectifié le 26/09 : une synchronisation était ensuite déclenchée, mais `nextPlanned` ne figure dans aucun paquet cloud vérifié — `setup.js`, `Code.js`, `supabase.js`)* (déjà le cas en production ft-v1234). | **Michel** | ⛔ **BLOQUER D'ABORD, même règle que la séance (D-025)** : une réponse non confirmée terminée **n'enregistre pas** l'annonce (ni ne remplace ni n'efface celle déjà enregistrée). Le bloc reste retiré de l'affichage. Appliqué avant de publier l'app. | **publier tel quel** (aucune régression, date déjà contrôlée 0-14 j) — écarté ; **arrêter la publication** — écarté | cohérent | VALIDÉ |
