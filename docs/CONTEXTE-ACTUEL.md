...
- **Version en ligne (live) :** `ft-v669`. Déploiement Pages via GitHub Actions (`.github/workflows/deploy-pages.yml`) — fiable + relançable à la main.
...
>
> **⏭️ LAISSÉ OUVERT par l'audit des tables (ft-v669) — 27 exercices (11 %) sans SCHÉMA DE MOUVEMENT.** Conséquences : ils sont appelés « accessoire » **par défaut** dans le contexte de Milo, et le garde-fou anti-fusion de l'import est **désactivé** pour eux (`if(qPat)` dans `_matchExercise`). Sur les 27, ~17 sont légitimes (gainage, abduction, corde à sauter, poignets). **Les vrais mal classés** : `Tirage en Rack` (hip-hinge), `Développé Haltères Assis` + `Développé Landmine` (poussée verticale), `Tirage Iso-Latéral Hammer` (tirage horizontal). **Et un cas de conception** : `Arraché`, `Clean & Jerk`, `Turkish Get-Up`, `Burpees`, `Box Jump`, `Farmer's Walk` n'ont **aucun** schéma adapté parmi les 6 actuels (squat · hip-hinge · poussée h/v · tirage h/v) — il faudrait en créer (porté ? explosif ?), ce qui est une décision de Michel, pas un correctif.
>
...
- **Branche de travail :** `claude/claude-md-docs-ytabnv` — **publiée aussi sur `master`** (donc live = ft-v669). *(session Claude Code web)*
