...
- **Version en ligne (live) :** `ft-v655`. Déploiement Pages via GitHub Actions (`.github/workflows/deploy-pages.yml`) — fiable + relançable à la main.
...
>
> **⏭️ 4 CHANTIERS OUVERTS, notés avec Michel les 28-29/07 (par ordre de dégât)**
> ① **La conversation coupée à 20 messages** — fermer l'app perd silencieusement le début d'une longue discussion, et ce qui est coupé n'est rangé nulle part. *Le seul des trois où on perd vraiment quelque chose.* Au passage : la discussion **en cours** doit apparaître en tête de « Mes discussions » (marquée « en cours »), et le bouton ne doit s'afficher que s'il y a quelque chose à ouvrir (aujourd'hui il s'ouvre sur du vide — c'est ce qui a fait croire Michel à un bug).
> ② **« C'était celle-là ✓ »** — cas réel du 28/07 : Michel annonce « bas du corps demain », puis la fait **le jour même**. La règle `plannedSession()` compare des **dates**, pas ce qui a été fait → l'Accueil et Milo annoncent une séance déjà faite. ⚠️ **Choix assumé** : on ne devine PAS en rapprochant le libellé des muscles travaillés (ça se tromperait un jour sur deux) — on propose **un bouton en fin de séance**, c'est la personne qui dit.
> ③ **Le questionnaire « Milo te connaît » REPART DE LA QUESTION 1** (trouvé par Michel le 29/07 à 00 h 01) — la carte de l'écran Coach dit *« tape pour revoir tes réponses »* et **repromène dans les 13 questions**, même quand tout est déjà rempli. Michel a tout refait. ✅ **Vérifié : ce n'est PAS une régression de ft-v655** — testé, le questionnaire ne s'ouvre jamais seul (ni à 00 h 01 ni à midi), et la fonction d'enregistrement modifiée fonctionne (réponses sauvées, `done:true`, bonne date, 0 erreur). C'est un défaut de **navigation** : `openCoachQuiz('free')` force `_cqIdx=0` (coach.js ~164) alors que la version « pro » reprend, elle, à la 1ʳᵉ question non répondue. **Le mot de Michel, qui donne le vrai poids** : *« pour moi c'est pas grave mais ça fait pas très sérieux avec des clients »* — c'est un enjeu de **crédibilité produit**, pas un confort. Une app qui semble avoir oublié ce qu'on lui a dit détruit exactement la promesse (*« il se souvient de qui tu es devenu »*).
> ④ **Le check-in DÉPLIÉ n'est plus raccord avec le replié** (signalé par Michel) — le replié est passé en 3 tuiles propres (lit violet · éclair orange · visage), le déplié est resté sur des **rangées de boutons emoji** (« les petits bonhommes »). Deux langages visuels dans la même carte. Concerne `energie`/`moral` dans `_renderDayStateCard` (screens.js ~976).
>
...
- **Branche de travail :** `claude/claude-md-docs-ytabnv` — **publiée aussi sur `master`** (donc live = ft-v655). *(session Claude Code web)*
