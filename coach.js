...
/* 🧮 R34-A — ACTIVITÉ ABSENTE : MILO DEMANDE, IL N'INVENTE PAS (décision de Michel, 25/09/2026, D-022).
   Mesuré au banc réel (run 36111949918) : avec « NON RENSEIGNÉ » seul, Milo disait bien que le niveau
   manquait… puis chiffrait quand même « disons 3 séances/sem → TDEE 2 300–2 500 ». Ce n'était pas une
   lubie du modèle : trois règles du BLOC COMMUN l'y poussaient — les hypothèses par défaut sur la
   FRÉQUENCE (avec l'exemple « 3 séances/sem »), ANTI-FAUX-PRÉCIS (« donne des FOURCHETTES ») et
   l'anti-interrogatoire, qui se déclare prioritaire sur « profil non renseigné → demande ».
...
function buildCoachContext(msg) {
  ...
  /* 🎚️ TROIS COMPTEURS, TROIS CONCEPTS — jamais fondus (14/09/2026).
     ⛔⛔ AVANT, LE TEXTE MENTAIT D'UN CRAN : le compteur mesurait `_effortConnu()` (« sait-on ce
     que la série a coûté ? », un `X` y compte) pendant que le libellé disait « portent un RIR ».
     Une séance à 3 séries notées + 1 `X` annonçait donc « 4 sur 4 portent un RIR » — et la
     quatrième n'en a aucun. *Un calcul A sous un texte B est un mensonge poli.*
     ⭐ Décision : on garde les DEUX informations, parce que les deux sont vraies et utiles — mais
     chacune sous son nom. La couverture RIR répond à « sur combien de séries ai-je une mesure de
     réserve ? » ; l'échec répond à « combien sont allées au bout ? ». *Les fondre reviendrait à
     reprendre le défaut qu'on vient de corriger, par l'autre bout.*
     ⛔ ET LE DÉNOMINATEUR EXCLUT LES `X` : une série allée à l'échec n'a pas de RIR **par
...
${(()=>{
  ...
  const _ech = _rirEchec ? ` ⭐ À part : ${_rirEchec} série${_rirEchec>1?'s':''} ${_rirEchec>1?'sont allées':'est allée'} à l'ÉCHEC (« (X) ») — l'effort y est connu, mais ce n'est PAS un RIR et ${_rirEchec>1?'elles ne sont':'elle n\'est'} pas compté${_rirEchec>1?'es':'e'} ci-dessus.` : '';
  /* ⛔ On parle quand même si la seule chose qu'on sait est un échec : se taire ferait croire
     qu'on ne sait rien de l'effort, alors qu'un `X` est l'effort le MIEUX connu. */
  if(!_rirTrav) return _rirEchec ? `\n→ 🎚️ RIR — LE COMPTE EXACT : aucune série de travail notable ci-dessus.${_ech}` : '';
  const pct = Math.round(100*_rirNotes/_rirTrav);
  if(_rirNotes===0) return `\n→ 🎚️ RIR — LE COMPTE EXACT : AUCUNE des ${_rirTrav} séries de travail ci-dessus n'a de RIR noté. Tu n'as donc AUCUNE mesure de l'effort réel sur cette période. ⛔ Ne déduis RIEN de ce vide : ni qu'il/elle s'arrête loin de l'échec, ni qu'il/elle y va — tu ne sais pas. Si l'effort compte pour ce que tu réponds, DEMANDE-LE au lieu de le supposer.${_ech}`;
  if(pct<50) return `\n→ 🎚️ RIR — LE COMPTE EXACT : ${_rirNotes} série${_rirNotes>1?'s':''} de travail sur ${_rirTrav} (${pct} %) porte${_rirNotes>1?'nt':''} un RIR. ⛔ C'est trop peu pour lire une tendance : parle de CES séries-là, jamais de « ses séances » en général, et ne conclus rien des ${_rirTrav-_rirNotes} autres.${_ech}`;
  return `\n→ 🎚️ RIR — LE COMPTE EXACT : ${_rirNotes} série${_rirNotes>1?'s':''} de travail sur ${_rirTrav} (${pct} %) porte${_rirNotes>1?'nt':''} un RIR.${_ech}`;
  ...
  help: {
    ...
    html:`<div style="display:flex;flex-direction:column;gap:10px;padding:0 2px 8px;">
      ${[
        ...
        /* 📊 ft-v1138 — L'AIDE DÉTAILLÉE EST L'ENDROIT DU *POURQUOI*. La pop-up annonce que deux
           tuiles sont parties et donne l'adresse de renvoi ; ici on dit la RÈGLE qui a décidé du
           retrait, parce que c'est elle qui vaut au-delà de ces deux tuiles-là — et parce que
           sans elle, une rangée de 2 ressemble à un travail inachevé. */
        /* 🗂️ ft-v1139 — L'AIDE DÉTAILLÉE EST L'ENDROIT DU *POURQUOI*. La pop-up dit que le
           menu a bougé ; ici on dit CE QUI ÉTAIT CASSÉ et la règle qui décide où va la
        ...
        /* 🏁 AIDE DÉTAILLÉE (règle d'or #11) — le cycle de force vit dans Menu → Tes outils, on
        ...
        {ic:'🔗',t:'La colonne « précédent » est vide ? c\'est un NOM, pas une perte',d:'La colonne <b>précédent</b> cherche tes anciennes séries <b>au nom exact de l\'exercice</b>. Si une séance proposée par Milo écrit <i>« Développé Épaules Assis Machine »</i> alors que ton historique porte <i>« Développé Épaules Assis Machine <b>(Shoulder Press)</b> »</i>, l\'app y voit <b>deux exercices différents</b> — d\'où un tiret à la place de tes charges, et une progression qui repart de zéro. ⛔ <b>Rien n\'est perdu dans ce cas</b> : tes séries sont toujours là, rangées sous l\'autre nom. ⭐ <b>Depuis le 06/09, la porte est fermée</b> : un nom qui ne diffère que par le suffixe entre parenthèses est <b>recollé au nom du catalogue</b> avant que la séance ne démarre. ⚠️ <b>Et seulement celui-là</b> : <i>« Développé Épaules Machine »</i> et <i>« Développé Épaules Assis Machine »</i> restent deux machines <b>différentes</b> — mélanger leurs charges serait pire que le tiret, parce que tu les croirais. 👉 <b>Si le tiret persiste</b>, c\'est que tu fais vraiment cet exercice pour la première fois — ou qu\'il est rangé sous un nom que le catalogue ne connaît pas (un exercice que tu as créé).'},
        ...
        {ic:'📏',t:'Tes mensurations, gardées dans le temps',d:'Onglet Progrès → Corps &amp; santé, carte « Masse grasse ». <b>Cou, taille et hanches</b> sont visibles ; les <b>6 autres</b> (poitrine, épaules, bras, avant-bras, cuisse, mollet) sont sous « <b>Autres mensurations</b> » — replié exprès, pour qu\'une saisie reste une saisie et pas un formulaire. ⭐ <b>Chaque valeur est datée</b> : tu peux enfin revoir d\'où tu es parti, ce que le seul % de graisse ne disait pas. ⭐ <b>Une seule mesure suffit</b> — tu n\'es plus obligé de remplir cou <b>et</b> taille pour que ta saisie soit gardée (avant, elle était jetée). ⛔ Se remesurer le même jour <b>corrige</b> la valeur, ça n\'empile pas : deux chiffres du même tour de taille le même jour, c\'est une hésitation, pas une tendance. ⚠️ <b>Une seule valeur par zone</b> (pas de droite/gauche). 💡 Seuls cou + taille (+ hanches chez la femme) servent au calcul du % ; les autres ne servent qu\'à suivre. ⭐ <b>Les deux écrans marchent</b> : le formulaire du <b>Profil</b> (Composition) date ta mesure exactement comme cette carte-ci — avant, lui seul ne le faisait pas, et la même mesure comptait ou ne comptait pas selon la porte par laquelle elle entrait. ⚠️ <b>Mais enregistrer ton profil ne re-date rien</b> si tu n\'as touché à aucune mesure : corriger ton poids ne doit pas faire croire que tu t\'es mesuré aujourd\'hui. ⛔ <b>Et une valeur entrée avant le 4 septembre 2026 n\'a pas de date</b> — elle n\'a jamais été enregistrée avec son jour, et on ne lui en invente pas une. Retape-la une fois pour qu\'elle compte. ☁️ <b>Tes mensurations sont sauvegardées</b> : elles partent avec le reste de ton compte et reviennent si tu changes de téléphone ou si tu restaures. ⭐ Et quand elles reviennent, elles se <b>mélangent</b> avec celles de ton appareil au lieu de les remplacer — une mesure prise sur ton téléphone et pas encore envoyée n\'est jamais écrasée par le serveur.'},
        ...
        /* 🏃 RÈGLE D'OR #11, POINT 4 — D-020 + D-021 (24-25/09/2026). */
        {ic:'🏃',t:'Nutrition — ton niveau d\'activité',d:'Tes calories partent de ton <b>niveau d\'activité</b> (Profil → Activité). ⭐ <b>Il doit venir de toi</b> : tant que tu ne l\'as pas choisi, l\'app n\'invente <b>aucun plan</b> et te dit ce qui manque. ⚠️ Avant, l\'app réglait ce niveau <b>toute seule</b> sur « Modéré (3-4j) » — impossible de savoir aujourd\'hui qui l\'a choisi. Si c\'est ton cas, une carte en haut de l\'onglet Nutrition te le montre : <b>Confirmer</b> le garde tel quel, <b>Modifier</b> t\'emmène le changer. Tant que tu n\'as pas répondu, tes chiffres l\'utilisent quand même, et Milo sait qu\'il reste à confirmer. On ne te le redemande plus ensuite.'},
