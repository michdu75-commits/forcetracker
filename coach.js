    ...
    const exStr = (s.exs||s.exercises||[]).map(e => {
      ...
      const setsStr = ds.length
        ? ds.map(x => {
            ...
            if(!ech){ _rirTrav++; if(typeof _effortConnu==='function' ? _effortConnu(x) : (_r!==null||x.type==='X')) _rirNotes++; }
