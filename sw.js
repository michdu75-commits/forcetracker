 ...
 */
const CACHE = 'ft-v926'; // 🏷️ ft-v926 = MILO REPROCHAIT LES PALIERS QU'IL AVAIT LUI-MEME PRESCRITS — 3e fois, et cette fois c'est l'auteur INCONNU. Michel, apres sa seance : « il y a un truc qui va pas apres avoir fini ma seance ». Dans le debrief, Milo ecrit « Lat Pulldown : 47 kg c'etait trop haut pour demarrer » — alors qu'il avait prescrit « Palier : 45×5 » quelques messages plus haut, dans la meme conversation. ⚠️⚠️ ET LA CAUSE EST ENCHAINEE AU BUG DU BOUTON : le bouton ne sortait pas (ft-v924/925), donc la seance a ete saisie A LA MAIN — donc sans le marqueur `_milo`, qui n'existe que sur le chemin du bouton. Les deux gardes-fous existants ont tenu (montee ecrite par l'app, 15/08 ; montee prescrite par Milo, 18/08) ; c'est le TROISIEME auteur possible — inconnu — qui n'etait pas couvert. ⭐ LE COMMENTAIRE DU 18/08 ANNONCAIT LA LIMITE MOT POUR MOT (« ce marqueur n'existe que si la seance a ete chargee depuis le chat ; ailleurs, seule la consigne du prompt peut rattraper ») — et la consigne du prompt EXISTE, elle dit d'aller verifier dans l'echange. **Elle n'a pas ete suivie.** C'est un cas reel du prerequis ecrit au §8 de ARCHITECTURE-CERVEAU-CERVELET : *une regle presente n'est pas une regle appliquee*. 👉 On ne se tait pas (le defaut de montee est vrai et utile) et on ne devine pas : on NOMME l'incertitude dans la DONNEE, pour que Milo cesse de supposer que la personne a choisi. Une information absente laisse la place a une supposition ; une information presente, non (R4).
const PRECACHE = [
