 ...
 */
const CACHE = 'ft-v992'; // 🧠 ft-v992 = LA MEMOIRE ELARGIE OUVERTE A TOUT LE MONDE. Priorite ④, tranchee par Michel apres mesure. ⛔⛔ LA RAISON D'AVANT RESTE ECRITE (R30) : reservee a michdu75 + christophe depuis le 03/08 pour « mesurer le cout reel sur deux comptes bien remplis avant d'ouvrir ». ⭐⭐ CE COUT A ETE MESURE, et c'est lui qui a permis de trancher — il est AUTO-DEGRESSIF, parce que la fonction ne resume QUE ce qui a ete vecu : 3 seances → 0 car. · 5 → 0 · 8 → 665 · 12 → 967 · 20 → 1 551 · 35 → 2 622 (borne MAX=30 lignes). ⛔⛔ ET LA CRAINTE DU PLAFOND NE TENAIT PAS, mesure a l'appui : ces caracteres tombent integralement dans le bloc PERSONNEL — le bloc commun est identique AU CARACTERE PRES (45 362 des deux cotes). Ce n'etait pas le bon bloc. ⭐ POURQUOI ON OUVRE (R9) : la memoire longue EST la promesse du produit (« le sportif ne repart jamais de zero »). La reserver revenait a ce que Michel juge Milo sur une memoire que PERSONNE d'autre n'a — donc a corriger le mauvais Milo. Un debutant ne paie rien. ⭐⭐ ET LA VRAIE TROUVAILLE EST AILLEURS : le banc d'essai n'avait AUCUN scenario de memoire longue (aucun scenario n'avait plus d'UNE seance). L'avant/apres exige par R34 aurait donc compare deux contextes IDENTIQUES et rendu « aucune regression » — un faux vert. D'ou EV-022, qui verifie que Milo retrouve une seance d'il y a 27 jours ET n'en invente pas la charge. La promesse centrale du produit n'etait verifiee par aucun scenario. ⚠️ CE QUI N'EST PAS PROUVE, ET C'EST ECRIT : le benchmark n'a PAS ete joue (il demande une vraie cle API, indisponible ici). On livre de quoi le jouer, pas son resultat — R34 n'est honore que quand Michel le lance. ⚠️ ET MA MESURE A ETE FAUSSE DEUX FOIS AVANT D'ETRE JUSTE : `_vcApplyPersona` attend le SCENARIO entier (elle fait `p.apply` elle-meme), je lui passais le sous-objet — donc « aucun scenario n'a de seance » etait faux (EV-017 en a une). La conclusion tenait, le chiffre non. 3e fois cette session qu'un levier qui n'est pas celui du code donne une mesure propre et fausse.
const PRECACHE = [
