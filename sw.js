 ...
 */
const CACHE = 'ft-v982'; // 🩹 ft-v982 = UNE BLESSURE DITE A MILO ATTEINT ENFIN LE GARDIEN. Michel : « je veux que Milo soit fiable ». ⛔⛔ LE CHEMIN ETAIT ETEINT EN PRODUCTION, derriere `window.__FT_CLONE__`, et le clone a ete retire en ft-v976 : une personne pouvait dire sa blessure a Milo, l'accepter en memoire, et le Gardien determinist n'en savait RIEN. Mesure contre l'ancien code : Profil Sante vide (`""`), Gardien vide (`[]`), consigne absente du contexte. ⚠️ L'audit exterieur y voyait une REGRESSION du retrait du clone — c'est FAUX, et la nuance compte : essai JAMAIS PROMU, liste comme tel le jour meme. Personne n'avait rien casse ; une decision n'avait jamais ete prise. ⭐⭐ ET EN LA PRENANT, ON A TROUVE POURQUOI L'ESSAI ETAIT PARQUE — c'est la decouverte de la soiree. `_gardienZonesFromText` detecte des NOMS DE MUSCLES, pas des blessures : mesure sur 9 formes de memoire anodines, **7 FAUX POSITIFS**. « Michel veut prioriser le dos et les epaules » produisait DEUX zones fragiles ; « Travaille les biceps le jeudi » en produisait une. Promouvoir le garde tel quel aurait ete PIRE que ne rien faire : Milo se serait mis a proteger des zones parfaitement saines. ⛔ L'essai n'etait pas OUBLIE, il etait INCOMPLET — il lui manquait la moitie qui distingue « parler de son dos » de « avoir mal au dos ». 👉 D'ou `_texteDitUneLimitation()` : il faut DEUX choses, une zone ET un mot de limitation (douleur, blessure, fragile, tendinite, hernie, opere, evite, menage…). Apres : **0 faux positif et 0 rate sur 17 phrases**. ⭐ Au passage, « talon » est ajoute a la zone cheville — c'est le mot que Michel emploie pour sa propre gene, et RIEN ne l'attrapait. ⭐⭐ ET LA SECONDE MOITIE ETAIT ETEINTE AUSSI : la consigne du prompt « nomme toujours la ZONE » vivait derriere le MEME garde. Sans elle Milo ne nomme pas la zone, et le pont n'a rien a lire — un garde-fou dont la moitie amont est debranchee n'est pas a moitie utile, il est inutile. ⛔ R2 : le filtre vit dans le PONT, pas dans `_gardienZonesFromText` — l'autre lecteur de cette fonction (les notes du Profil Sante) ne contient QUE des blessures par construction, y filtrer ferait rater de vraies limitations declarees a la main. ⚠️ Le mode d'echec choisi est la SUR-protection, jamais la sous-protection (R29). ⭐ Un temoin existant a rougi au passage et il avait raison : la regle « accident de moto » se retrouvait ecrite DEUX fois dans le prompt. Exemple dedoublonne plutot que temoin desarme — le prompt y gagne (R20).
const PRECACHE = [
