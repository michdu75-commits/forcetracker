 ...
 */
const CACHE = 'ft-v1049'; // 🆘 ft-v1049 = BLOQUANT : LE BOUTON « LANCER LA SEANCE » NE SORTAIT PLUS. Trouve par Michel EN SALLE, capture a l'appui : Milo ecrit la seance, dit « le bouton devrait apparaitre sous mon message »... et il n'y en a pas. ⛔⛔ MESURE SUR SON TEXTE : les TROIS voies echouaient pour la MEME raison (① bloc cache, ② `_ressembleASeance` false, ③ `_extractDaySession` null). Toutes lisaient « 95×3 » comme « 95 SERIES de 3 reps » : 95 depasse la borne des 12, la ligne est jetee — ses 4 lignes jetees, il en faut 2 valides, il en restait 0. 👉 Milo n'ecrit plus « 3×3 a 95 kg », il ecrit UNE LIGNE PAR SERIE : « S1 : 95×3 — repos 1 min 30 ». *Ce n'est pas un chemin casse, c'est une hypothese de FORMAT partagee par les trois* — 2e fois (le 20/08, meme symptome, autre format). ⭐⭐ CE QUI REND LA LECTURE SURE, C'EST LE PREFIXE S1/S2/S3 : dans « 3×3 » le 1er nombre est un COMPTE de series ; dans « S1 : 95×3 » la serie est deja nommee, donc c'est la CHARGE — les bornes ne pouvaient pas lever cette ambiguite, le prefixe si. ⚠️ Et une ligne S2/S3 HERITE du nom de la S1 au-dessus : sans ca `nomAvant` (3 lignes max, sautant celles qui portent des nombres) rendait vide sur S3 et on PERDAIT une serie — 2 au lieu de 3, mesure.
const PRECACHE = [
