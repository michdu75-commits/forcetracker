 ...
 */
const CACHE = 'ft-v1065'; // 0️⃣ ft-v1065 = LE GARDE DU BLOC QUANTITE NE REGARDE PLUS QUE LES CALORIES. Michel : « meme quand je mets zero il marque deja 5 g de prot ». ⚠️ LE CAS DECRIT N'A PAS ETE REPRODUIT — mesure sur les deux ecrans : un 0 pose a la main TIENT, il survit a un changement de quantite et s'enregistre a 0 (3 temoins le figent). ⛔⛔ MAIS LA MESURE A TROUVE AUTRE CHOSE : le garde testait `base.kcal>0`, donc mettre les CALORIES a 0 faisait disparaitre tout le reglage de quantite — alors que 35 g de proteines restaient a l'ecran, parfaitement rescalables. Ca touche les cas reels (boisson zero, aliment dont on ne connait que les proteines) et surtout la FRAPPE : on efface les calories pour les retaper, et le reglage s'evapore sous les doigts. *Un proxy commode — les calories pour « il y a des valeurs » — devient faux des qu'une valeur legitime vaut zero.*
const PRECACHE = [
