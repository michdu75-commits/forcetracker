 ...
 */
const CACHE = 'ft-v1030'; // ⏳ ft-v1030 = LE REPOS EST UN MAXIMUM, PAS UN COMPTE A REBOURS. Decision de Michel (« on peut repartir avant, c'est autorise »), issue des 6 programmes de sa coach ou la colonne s'intitule « Repos maximum » et porte des PLAGES. ⛔⛔ MESURE AVANT DE CODER, et elle a dementi ce que j'avais annonce la veille : le chrono s'ARRETAIT a 0:00 (barre masquee, restIv vide) sur les deux chemins. ft-v851 (14/08) avait retire les bornes Math.max(0,…) des DEUX fonctions d'AFFICHAGE sans toucher a `_restTick`, qui appelait `stopRest()` — les afficheurs savaient montrer du negatif, plus personne ne les appelait. La fonctionnalite n'avait JAMAIS tourne (famille « le correctif pose d'un seul cote »). 👉 Le chrono continue desormais (+0:37), la barre reste, et un libelle DEDIE dit « au-delà de ton repos max » — il INFORME au lieu d'accuser (R29). ⛔⛔ ET IL A SON PROPRE EMPLACEMENT : ma 1re version l'ecrivait dans #rest-label, qui porte deja « Échauffement », « Récup. à l'échec », « 📈 Pyramide + » et « ⏭️ Ensuite : … » — `updRest` tournant a chaque tick, elle les aurait tous effaces en silence. Un element d'ecran a UN proprietaire (R2). ⛔ Le callback superset part exactement UNE fois, le tap sur l'ecran GO arrete tout, et un arret de securite coupe au-dela de 15 min (un seul proprietaire pour ce nombre).
const PRECACHE = [
