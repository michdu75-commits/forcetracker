 ...
 */
const CACHE = 'ft-v996'; // 🏷️ ft-v996 = UN NOM D'EXERCICE ABREGE RETROUVE SA FICHE DU CATALOGUE. Cas reel Michel 24/08, deux captures : sa seance portait « Hip Thrust Barre » et « Abduction Cuisses » (noms COURTS, sans la parenthese explicative du catalogue) et l'ecran affichait « Muscle principal devine » + « Ajouter la photo de ta machine » — alors que hip-thrust-barre.webp et leg-abduction-machine-v2.webp etaient DEJA dans le depot. L'app proposait d'ajouter une photo qu'elle avait sous la main. ⛔⛔ LE DEFAUT EST SILENCIEUX : le calcul des muscles s'en sortait (il retombe sur les regles _MEX qui DEVINENT), donc rien ne plantait, rien ne rougissait — seuls les lookups qui exigent le nom EXACT (animation, tutoriel, silhouette de groupe) echouaient sans bruit. ⭐⭐ LE MECANISME EXISTAIT DEJA, POSE D'UN SEUL COTE (R8/R13, 3e fois apres ft-v973 et ft-v975) : _matchExercise porte depuis le 09/08 une etape « exact sans la parenthese », ecrite pour exactement ce cas (Michel, sur un vrai programme de Milo) — elle ne servait qu'a l'IMPORT, l'affichage ne l'a jamais eue. ⛔ UN SEUL PROPRIETAIRE (R2) : exNomCatalogue() dans constants.js, a cote de exNomActuel — pose aux 6 lookups exacts de log.js. ⛔ DETERMINISTE SEULEMENT, aucun rapprochement flou (R29 : montrer l'animation d'un AUTRE exercice est pire que n'en montrer aucune — la personne apprendrait un mouvement qu'elle n'a pas prescrit). ⛔⛔ LE ZERO-COLLISION EST MESURE ET C'EST LA CONDITION DE LA TABLE : verifie sur les 324 exercices, 77 parentheses -> 77 bases distinctes, 0 collision, 0 base qui soit deja le nom complet d'un autre. Toute base ambigue est RETIREE plutot qu'arbitree : si le catalogue grandit et cree une collision, l'abreviation cesse d'etre resolue (retour a l'ancien comportement) — jamais elle ne pointe vers le mauvais exercice. ⚠⚠ ET LA MEME CAUSE A UN 2e EFFET, PLUS LARGE, NON CORRIGE ICI ET LAISSE OUVERT EXPRES : _mscScores appelle exMuscles(ex.name) en nom EXACT, donc un nom abrege rate la DONNEE ECRITE et retombe sur la DEVINETTE. Mesure : 55 des 77 abreviations donnent des muscles DIFFERENTS, et « Inclinaison Lombaire » n'en donne AUCUN (figurine entierement grise). Exemple qui coute : « Rowing Poitrine Appuyee » abrege recredite le bas du dos, alors que la fiche du 02/08 l'avait retire EXPRES (poitrine appuyee = colonne non chargee). Non livre parce que ca change la figurine, la couleur du calendrier et les calories de seances PASSEES — c'est un arbitrage de Michel (R29/regle d'or #10), pas un detail technique. Voir docs/CONTEXTE-ACTUEL.md.
const PRECACHE = [
