 ...
 */
const CACHE = 'ft-v980'; // ⚡ ft-v980 = LE CONTROLE D'INTENSITE EN CODE. Michel : « comment il a pu deduire que je pouvais faire 3 series de 5 reps a 95, c'est impossible je ne suis pas encore assez fort ». ⭐⭐ ET MILO NE L'AVAIT PAS DEDUIT — IL L'AVAIT LUI-MEME DEMENTI : questionne, il repond « 105x2 → 1RM ~108 · 95x5 ≈ 88 %, tres lourd pour 3 series de 5, on vise 80-85 %, soit 85-90 kg. Je corrige : 3x5 a 90 kg. » Michel a dit « ne corrige pas », Milo a obei — et c'est le BON comportement. Le defaut n'est donc pas son jugement : c'est que son controle ne se declenche QUE SI ON LE QUESTIONNE. Il verifie APRES, jamais AVANT. ⭐ LA REALITE A TRANCHE, dans les donnees : 95x3 avec « pose de la barre a la rep. 2 », DEUX FOIS, puis 90x3 — Michel avait raison (infaisable) et le 90 corrige de Milo est exactement la ou il a fini. ⛔⛔ POURQUOI EN CODE ET PAS DANS LE PROMPT (R7) : « 88 % du 1RM sur 3x5, est-ce tenable ? » est ARITHMETIQUE — la confier a un modele, c'est la faire dependre d'un jour de fatigue (R9 : on evalue sur le modele des VRAIS utilisateurs). ⭐⭐ ET LA FORMULE REPRODUIT LA CORRECTION DE MILO INDEPENDAMMENT : on inverse `bz()` (Brzycki, R2 — jamais une 2e formule de 1RM) pour obtenir la charge d'une serie MAXIMALE a R reps, puis un coefficient de tenue de 0,93 parce que 3 series ne sont pas une serie. Resultat : 89,5 kg conseille la ou Milo disait 90. Le coefficient tient sur toute la plage contre les baremes (3 reps → 88 %, 5 reps → 83 % soit le chiffre cite par Milo, 8 reps → 75 %). ⛔⛔ ON SIGNALE, ON NE CORRIGE JAMAIS TOUT SEUL (R29) : Michel VOULAIT ses 95 kg pour tester son max, il en avait le droit — les charges partent INTACTES, l'avertissement est attache a l'exercice et lisible pendant la seance. ⛔ ET SANS RECORD CONNU, LA FONCTION SE TAIT : jamais un 1RM invente. Une seule serie a 95 ne declenche rien non plus (tester un max est legitime). ⛔ Le repos suit la meme regle, et c'est MICHEL qui l'a tranchee : « un 3x5 avec 90 secondes de repos c'est IMPOSSIBLE ». ⭐ R4 : le calcul ATTEINT le contexte de Milo (jumeau de `_verdictMontee`, pose LE MEME JOUR pour ne pas repeter le correctif d'un seul cote), avec l'AUTEUR nomme et un 4e cas de figure ecrit — une charge assumee en connaissance de cause ne se juge pas. ⚠️⚠️ ET UN TEMOIN M'A FAIT CORRIGER MA PROPRE POSE : j'avais branche le controle sur `_applyMiloSession` seul, la porte « une seance tourne deja » — il n'aurait JAMAIS tourne dans le cas normal. Il vit desormais dans `_appliqueMiloSession`, le seul point que les DEUX portes traversent (R2). ⛔ Et ca a revele un 2e defaut : `_milo:true` manquait sur cette porte, donc une seance chargee en mode « remplacer » perdait son AUTEUR — l'incident du 18/08 par une porte qu'on n'avait pas regardee.
const PRECACHE = [
