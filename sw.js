 ...
 */
const CACHE = 'ft-v948'; // 📤 ft-v948 = LE COMPTEUR PARTAIT SEULEMENT SI ON FAISAIT QUELQUE CHOSE. Michel : « a partir de quel moment tu pourras lire le Milo a Eline ? ». ⭐⭐ REPONSE HONNETE AVANT LE CORRECTIF : peut-etre jamais. Le scan retro de ft-v946 tourne bien au demarrage, mais la sauvegarde ne part que sur une ACTION (seance, reglage, message). Quelqu'un qui OUVRE l'app, LIT ses conversations et REFERME n'envoyait rien — et on aurait lu ce silence comme « elle ne s'en sert pas », alors qu'on n'avait simplement pas le chiffre. 👉 Le scan pousse desormais lui-meme la sauvegarde. ⛔ MAIS PAS A CHAQUE OUVERTURE : l'instantane est stable (memes conversations → meme resultat), donc il ne part que quand il y a du NOUVEAU. Une ecriture par nouveaute, pas une par demarrage — le stockage a deja sature une fois (29/07, 102 %). ⚠️⚠️ ET LE PREMIER JET AURAIT ENVOYE UNE SAUVEGARDE PAR JOUR : je comparais l'instantane ENTIER, `faitLe` compris — or `faitLe` est la date du SCAN, pas une mesure : elle change toute seule a minuit. La signature ignore desormais l'horodatage et ne compare que ce qu'on MESURE (reponses vues, derives, codes, periode). Un temoin simule le lendemain. ⭐ CONTRE-TEMOIN INDISPENSABLE : une VRAIE nouvelle conversation repart bien — sans lui, rendre le compteur muet aurait passe tous les autres temoins. ⛔ ET LE CONTENU NE BOUGE PAS D'UN MOT : ce qui part reste ~150 octets de NOMBRES, aucune phrase de Milo, aucun mot de la personne.
const PRECACHE = [
