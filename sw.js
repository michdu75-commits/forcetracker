 ...
 */
const CACHE = 'ft-v1107'; // 🍽️ ft-v1107 = L'APPORT ENTRE DANS LE MOTEUR DE TRAJECTOIRE. Suite du contre-audit de la proposition GPT (docs/NUTRITION-CONTRE-AUDIT-TRAJECTOIRE.md), Michel : « il va falloir ameliorer tout ca ». ⛔⛔ LE TROU ETAIT UNIQUE, ET MESURE : `tendance14j()` COMPTAIT les jours de repas (`alim:{jours:7}`) et ne lisait JAMAIS les calories. La chaine apport->cible->poids->objectif que GPT demande avait DEJA 4 maillons sur 5 en production ; il manquait le premier. ⛔⛔ ET UN JOUR MAL NOTE DEFORMAIT LA MOYENNE DE 269 kcal/j : 6 jours a 4 repas + 1 jour a 1 repas donnaient 1 798 kcal/j au lieu de 2 067, et l'ecart a la cible passait de -1 085 a -1 354. ⭐ Le probleme avait DEJA ete vu : `entreesParJour` est calcule depuis ft-v1021 avec le commentaire « une moyenne de 1,2 entree par jour ne decrit pas une journee » — et lu NULLE PART (donnee morte, R5). ⛔⛔ AUCUN SEUIL INVENTE, ET C'ETAIT LE POINT DUR : « 3 repas », « 80 % » sont des chiffres qu'il faudrait CHOISIR, et quelqu'un qui mange 2 fois par jour n'a pas tort. La barre est la MEDIANE des moments notes par jour DE LA PERSONNE — elle ne se choisit pas, elle se constate (meme logique que _GOAL_TREND en ft-v1100). Mediane et non moyenne : un seul jour a 6 entrees ne doit pas relever la barre des autres. ⛔⛔ ET LA JUMELLE COMPTAIT DOUBLE (R8) : la distorsion de 269 kcal avait ete chiffree SUR « Ta semaine », pas sur la carte neuve. La corriger d'un seul cote aurait laisse le defaut a l'endroit exact ou il avait ete mesure, et fabrique deux moyennes calculees differemment a 400 px d'ecart. Un seul proprietaire (`_joursAlimComplets`, R2) : les deux cartes ne different plus que par leur FENETRE (7 j / 14 j), ce qui est dit a l'ecran. ⛔ LES JOURS ECARTES SONT DITS DES DEUX COTES — un jour mis de cote en silence est une moyenne truquee — et le libelle SUIT le calcul (« jours complets » seulement si on a ecarte). ⛔ L'APPORT EST UN FAIT, PAS UN SIGNAL JUGE : ni vert ni orange dessus, et les 4 etats ne bougent pas. Un apport sous sa cible n'est PAS un echec (Constitution P21, anti-TCA) ; le faire basculer l'etat transformerait l'ecran en jugement quotidien sur ce qu'on mange. ⭐ MESURE, MEME FIXTURE DES DEUX COTES : carte 188 -> 210 px (+22), « Noter ce que je mange » INCHANGE (556), onglet 1 877 -> 1 899 (plafond 2 600). ⚠️ Et j'ai failli annoncer une regression qui n'existe pas : ma nouvelle fixture donnait 476 px de carte du jour contre 445 mesures en ft-v1102 — c'est la FIXTURE qui differe, verifie en rejouant le code de ft-v1102. ⏭️ NON FAIT, EXPRES : la recence de la memoire alimentaire. Elle change ce que MILO recoit, donc elle passe par un avant/apres au banc d'essai (R34) — decision de Michel. MESUREE sans etre appliquee : quelqu'un qui a change d'alimentation il y a 4 mois annonce a Milo « Riz basmati (60x) » comme dejeuner habituel, un aliment qu'il ne mange plus ; avec une fenetre de 60 jours ce serait « Patate douce (21x) ». Tests : parcours 2456 (bloc CCXVI, +12 de moi, +16 de session-B, arbre fusionne), calculs 266, muscles 241, croises 50, dates 7, donnees 0 trou.
