 ...
 */
const CACHE = 'ft-v999'; // 🏋️ ft-v999 = DEUX ANIMATIONS QUI MANQUAIENT, ET UN RETRAIT QUI S'EST REFERME TOUT SEUL. Michel : « j'ai encore des figurines qui n'apparaissent pas... le tirage horizontal prise serree, il y a le pull-over aussi ». ⛔⛔ CE N'EST PAS LE BUG DE ft-v996/997 : la, le fichier existait et l'app ne le trouvait pas. ICI LE FICHIER N'EXISTE PAS. Mesure : 306 images sur disque, 302 rattachees, 4 orphelines seulement — et aucune des 4 ne correspondait. 21 exercices sur 324 n'ont jamais eu d'animation ; le repli (image du muscle + « Ajouter la photo de ta machine ») est le comportement PREVU, pas une panne. ⭐⭐ LE PLUS IMPORTANT DE CETTE VERSION EST UN COMMENTAIRE DE 2026-08-02 QUI A DIT QUAND LE REBRANCHER. Il disait : « Ecarte Halteres affichait l'animation de l'ecarte DECLINE — les deux fiches pointaient le meme fichier. Aucune animation vaut mieux qu'une fausse ; A REBRANCHER LE JOUR OU ON A UNE VRAIE DEMO D'ECARTE A PLAT. » Ce jour est arrive. *Un retrait dont la CONDITION DE RETOUR est ecrite se referme tout seul le jour venu ; sans cette phrase, on aurait cru a un oubli et remis n'importe quoi* (R30). ⛔ VERIFIE IMAGE PAR IMAGE AVANT DE RATTACHER (R29 : montrer l'animation d'un AUTRE exercice est pire que n'en montrer aucune) — planche de 3 poses extraite de chaque GIF et regardee. Le tirage est bien la poulie BASSE assis poignee en V, pas la poulie HAUTE (`tirage-vertical-prise-serree.webp`, qui existe deja et est un AUTRE exercice). L'ecarte est bien un banc PLAT, pas decline. ⛔ LES 4 ENDROITS TENUS ALIGNES, et un temoin les epingle : fichier sur disque · ligne EX_YT · ligne du cache SW (sinon l'image manque HORS LIGNE, regle d'or #4) · et l'absence de collision de fichier entre les deux ecartes et entre les deux prises serrees. ⛔ CONVERTIS A LA CONVENTION DU DEPOT, pas colles tels quels : 700x700 GIF ~800 Ko -> 480x480 webp anime 12 images (126 et 165 Ko), mediane du dossier 96 Ko. ⚠️ LE PULL-OVER N'EST PAS CORRIGE ICI, ET CE N'EST PAS UN OUBLI : ce n'est pas une animation manquante mais un DOUBLON DE CATALOGUE. 5 entrees pull-over, dont une generique « Pull-over » rangee dans le bac BARRE juste au-dessus de « Pull-over Barre » — deux lignes pour la meme chose. Le GIF envoye est d'ailleurs l'image DEJA en place sur « Pull-over Haltere ». Fusionner ou retirer l'entree generique touche l'historique (records, seances) : c'est un arbitrage de Michel, pas un correctif technique.
const PRECACHE = [
