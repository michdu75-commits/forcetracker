 ...
 */
const CACHE = 'ft-v1094'; // 4e passe de chasse aux incoherences — six familles NEUVES, UNE SEULE a mordu, et elle touche la REGLE D'OR #3. ⛔⛔ DEUX ONGLETS OUVERTS, ET LE DERNIER QUI ECRIT EFFACE L'AUTRE : `persist()` ecrit TOUT `S` depuis la memoire de l'onglet qui l'appelle, or l'app est une PWA — elle est tres souvent ouverte a DEUX endroits (l'icone de l'ecran d'accueil ET un onglet du navigateur), qui partagent le meme stockage. MESURE par les vraies fonctions dans deux pages du meme contexte : l'onglet B termine une seance (1 seance + 1 record sur le disque), l'onglet A regle son temps de repos — le geste le plus banal qui soit — et il reste 0 SEANCE, 0 RECORD, que le rechargement ne ramene pas. Une seance terminee disparait sans un mot. ⚠️ Rien ne protegeait : aucun ecouteur `storage`, aucun BroadcastChannel dans le depot. 👉 LE CORRECTIF : quand un AUTRE onglet a ecrit, `persist()` prend l'UNION au lieu de remplacer — le navigateur nous le dit lui-meme, l'evenement `storage` ne se declenche QUE dans les autres onglets. ⛔ ET LE CAS NORMAL NE CHANGE PAS D'UN CARACTERE : un seul onglet, le drapeau reste faux, aucune fusion — c'est ce qui rend sur un correctif dans la fonction la plus critique de l'app, et deux temoins l'epinglent (dont la SUPPRESSION volontaire, qui doit rester possible). ⚠️ LES CINQ AUTRES FAMILLES ONT RENDU DU VIDE, ecrit pour ne pas refaire la chasse : hors ligne rien ne se perd (seance sur le disque, mise en file, 5 ecrans qui rendent), les 8 dates rares (1er janvier, 31 decembre, 1er du mois, lundi, 29 fevrier, changement d'heure, minuit) ne cassent rien, et les « seuils ecrits deux fois » etaient TOUS des nombres egaux par hasard (180 s contre 180 min, 120 kg contre 120 cm). ⚠️⚠️ ET QUATRE FAUX POSITIFS VENAIENT DE MES PROPRES SONDES : une cle inventee (`ft4_weight` au lieu de `ft4_wlog`) qui rendait toujours zero, `goScreen('s-home')` alors que la fonction prefixe elle-meme, un detecteur qui SUPPRIMAIT les commentaires et decalait donc tous les numeros de ligne, et un temoin « en ligne » qui n'en etait pas un (le vrai serveur repond 403 ici, donc synced etait faux DES DEUX COTES).
