 ...
 */
const CACHE = 'ft-v1044'; // 📉 ft-v1044 = L'EVOLUTION ATTENDUE EN RECOMPOSITION. La carte « tendance du poids » disait « l'evolution attendue est VARIABLE » pour l'objectif recomp — un repli ecrit pour un objectif INCONNU, servi a quelqu'un dont l'objectif est parfaitement defini. ⛔⛔ ET LE JUMEAU ETAIT PIRE (R8) : `onTrack` dans coach.js n'avait pas non plus `recomp` et retombait sur Math.abs(x)<0.2 — donc quelqu'un a -0.21 kg/sem, PILE dans sa cible, arrivait chez Milo en « a ajuster selon objectif ». Un fait faux sur la personne. ⭐ LE CHIFFRE VIENT DE L'APP ELLE-MEME, pas d'une valeur importee : _GOAL_DELTA_KCAL.recomp = -250 kcal/j → -1750 kcal/sem → ~ -0.23 kg si tout venait du gras. Bande 0 a -0.3 kg/sem. ⚠️ AUCUN POURCENTAGE A L'ECRAN (question de Michel) : le repere en % du poids de corps est celui d'une SECHE (Garthe 2011, norvegien ; Helms 2014), une recomposition n'en est pas une, et la balance affiche des kilos. ⛔ ET LA BALANCE EST LE MAUVAIS INSTRUMENT ICI, l'ecran le DIT : le gras qui part et le muscle qui vient s'annulent dessus.
const PRECACHE = [
