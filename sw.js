 ...
 */
const CACHE = 'ft-v979'; // 📋 ft-v979 = LE DEBRIEF NE SE PERD PLUS. Michel : « je n'ai pas eu de briefing parce qu'il y a eu la mise a jour de l'application » — IL AVAIT RAISON, et le mecanisme etait dans le code. ⛔⛔ LE DEBRIEF ETAIT DECROCHE AVANT D'ETRE LIVRE : l'ancien code retirait le jeton AVANT l'appel et ne le remettait que `if(!ok)`, donc SEULEMENT quand l'appel echoue proprement. Un rechargement pendant ces quelques secondes et le debrief n'est pas reporte, il est PERDU, sans message et sans trace. ⚠️ ET LE MOMENT N'EST PAS UN HASARD : une mise a jour en attente refuse de s'appliquer pendant une seance (`_majPeutSAppliquer`) et s'applique des l'ACCUEIL — ou `finishWorkout` depose justement la personne. La mise a jour attend la fin de la seance pour tomber exactement dans la fenetre du debrief ; 6 versions ont ete deployees le 23/08. ⭐ MESURE DANS SES DONNEES : 5 seances sur 36 sans AUCUN debrief (08, 10, 15, 18 et 23/08), toutes completes de 18 a 29 series — et une seance d'UN exercice et 3 series, elle, debriefee. R4a : l'oubli est SILENCIEUX. ⛔ ON NE POUVAIT PAS SIMPLEMENT « retirer le jeton apres la reponse » : la prise en amont est un correctif VOULU (le double debrief du 22/08, deux objectifs memorises contradictoires) — le defaire l'aurait fait revenir de l'autre cote (R30). Le jeton n'est donc plus DETRUIT : il passe « en cours » horodate, et un « en cours » retrouve au demarrage retourne dans la file. ⛔ ET C'EST UNE FILE : `setItem` n'avait QU'UNE place, deux seances sans ouvrir Milo et la premiere disparaissait en silence. ⭐⭐ 3e FILET, SANS AUCUN DRAPEAU : on compare ce qu'on a FAIT a ce qui a ete DEBRIEFE. ⛔ UNE SEULE seance, la plus recente, et PERIMEE a 36 h — la consigne dit « je viens de terminer ma seance », le faire dire d'une seance de deux semaines serait un mensonge sur le QUAND (R29) ; les seances du 08 au 18/08 ne reviendront donc pas, c'est delibere. ⭐⭐ ET UN TEMOIN EXISTANT A ATTRAPE UN DEFAUT DE MA PROPRE CONCEPTION : le rattrapage prenait `sessionLog` pour preuve, or il n'est ecrit que si Milo termine par son bloc technique — une reponse sans bloc aurait fait re-debriefer la MEME seance a CHAQUE lancement, en payant un appel a chaque fois. « Un debrief a ete LIVRE » et « Milo a produit une memoire » sont deux faits differents, donc deux proprietaires (R2). ⛔ Corrige au passage : la deduplication comparait `sessId===sid` en STRICT alors que les deux chemins ne passent pas le meme TYPE (id numerique cote seance, chaine cote Coach) — 4 dates en double dans son registre (30/07, 31/07, 03/08, 05/08), et c'est ce qui lui a fait dire « on avait pas dit samedi les pecs ? ».
const PRECACHE = [
