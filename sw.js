 ...
 */
const CACHE = 'ft-v1051'; // ⚖️ ft-v1051 = LA QUANTITE AU CHOIX : GRAMMES *OU* PORTIONS. Michel, capture a l'appui : « toujours ce probleme de quantite, il faut que je puisse mettre les grammes », puis la correction qui a decide de la FORME : « je ne prends pas toujours le meme poids... tu prends la ratatouille, il y a differentes boites de different poids ». ⛔⛔ CE QUI BLOQUAIT N'ETAIT PAS UN MANQUE DE MECANISME, C'ETAIT UN REFUS : `_afMajAncre` imposait l'un ou l'autre et affichait « on ne peut pas inventer un poids ». VRAI, ET A COTE DE LA QUESTION — l'APP ne peut pas l'inventer, la PERSONNE le connait. Il fallait le DEMANDER (R29). ⭐⭐ RIEN N'EST REINVENTE (R13) : le bloc « portion » posait DEJA `_afRef={q:1}` — grammes et portions sont LE MEME calcul avec une reference differente, l'unite n'est qu'une etiquette. Un seul champ actif (R2). ⛔ CE QUI EST RETENU EST LE POUR-100 g, JAMAIS LA BOITE : 100 g de ratatouille est stable, la boite non. `q` n'est qu'un pre-remplissage. ⛔ DECLARER N'EST PAS RESCALER : dire « ca pese 40 g » ne change pas les 4 valeurs, ca dit a quoi elles correspondent. Et le champ est VIDE, jamais pre-rempli a 100 (R29). ⚠️⚠️ LE PIEGE ETAIT LE DIVISEUR, ecrit faux au 1er jet : apres avoir declare 40 g puis tape 80, les champs affichent le DOUBLE — diviser par 40 donnerait un pour-100 g deux fois trop gros, et l'aliment resterait faux pour toujours. Les valeurs affichees et la quantite affichee vont TOUJOURS ensemble. ⭐ R4 : le poids descend jusqu'a la donnee, ce qui rebranche ft-v1042 — a la reprise, le champ en grammes s'ouvre tout seul.
const PRECACHE = [
