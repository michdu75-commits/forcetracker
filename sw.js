 ...
 */
const CACHE = 'ft-v983'; // 🩺 ft-v983 = LE DIAGNOSTIC MEDICAL NE PASSE PLUS SEUL. 3e et dernier bloquant de la contre-analyse. ⛔⛔ AUDIT DU GARDIEN DE SORTIE, MESURE : sur ses 5 controles, UN SEUL retire vraiment quelque chose (le bloc technique, via `_stripCoachTech`). Les quatre autres — interrogatoire, DIAGNOSTIC MEDICAL, promesse vide, source douteuse — sont COMPTES puis AFFICHES TELS QUELS. Detecte n'est pas empeche. 👉 Pour trois d'entre eux un compteur suffit : ils nous regardent, nous. PAS pour le diagnostic — la Constitution (P13/P22) dit que Milo ne diagnostique jamais et renvoie au medecin ; si la phrase sort quand meme, l'app doit poser le renvoi elle-meme. ⛔⛔ ON N'A PAS REECRIT LA REPONSE, ET C'EST DELIBERE : le code disait deja, a propos de ce meme controle, « il attrape la FORMULE, pas l'intention — donc il SIGNALE, il ne reecrit pas (on ne charcute pas une phrase) ». Charcuter produirait des phrases incomprehensibles sur les faux positifs, et un faux positif est ici certain a terme. On AJOUTE donc une ligne sous la reponse, sans en modifier un caractere — additif, visible, reversible. Si le motif se trompe, le pire est un rappel de bon sens en trop, pas une phrase mutilee (R29 : le cout de l'erreur decide). ⚠️ ET LE MOTIF EST DEJA CALIBRE : resserre le 21/08 apres 3 faux positifs sur 3 sur de vraies reponses (« tu es en Jour 2 », « tu es en phase de charge »). Il exige desormais une PATHOLOGIE nommee et ne tirait sur AUCUNE des 129 reponses mesurees — c'est ce qui rend cet affichage supportable : il est rare. ⭐⭐ LES DEUX TEMOINS QUI COMPTENT LE PLUS SONT VERTS DES DEUX COTES : le texte de Milo et son `dataset.raw` (partage/PDF) sont INTACTS avant comme apres. C'est exactement ce qui prouve qu'on a ajoute et non charcute.
const PRECACHE = [
