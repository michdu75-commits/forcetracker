 ...
 */
const CACHE = 'ft-v965'; // ⚖️ ft-v965 = LA QUANTITE ETAIT AU-DESSUS DU CHAMP OU L'ON TAPE. Michel : « j'ai mis 30 grammes de proteine mais en fait c'est pas ca, j'ai voulu mettre 30 grammes de POUDRE de proteine, et ca fait 88 grammes de proteine ». ⭐⭐ LE CHIFFRE DIT LA CAUSE : 88 g de proteine = EXACTEMENT 100 g de poudre. La quantite n'avait jamais ete changee, elle etait restee a sa valeur par defaut. ⛔⛔ ET LE CALCUL N'A JAMAIS ETE FAUX — c'etait le PLACEMENT. Le bloc « Quantite » vivait AVANT le champ de recherche : concu pour le CODE-BARRES (on scanne, PUIS on ajuste), il est devenu a contresens quand la recherche par nom est arrivee (ft-v956/957). On tape en bas, on choisit en bas, les macros se remplissent en bas — et le reglage qui COMMANDE tout ca restait hors du regard, plus haut dans la page. ⭐ Deplace juste AU-DESSUS des macros qu'il pilote, il sert desormais les DEUX chemins : apres un scan comme apres une recherche, il apparait la ou l'attention est deja. ⚠️⚠️ ET LE COUT ETAIT REEL ET SILENCIEUX : 100 g de whey au lieu de 30, c'est +64 g de proteine et ~250 kcal enregistres sans que rien ne le signale — la meme famille que le pluriel de ft-v963, une valeur PLAUSIBLE mais fausse, qu'on enregistre sans se mefier. ⭐ Le temoin epingle l'ORDRE DU DOM lui-meme (recherche < suggestions < quantite < macros) : une disposition ne casse aucun test fonctionnel, donc rien d'autre ne l'aurait vue. ⚠️ Rien d'autre ne bouge : ni le calcul, ni la valeur par defaut de 100 g — la corriger demanderait de DEVINER une portion, et ce serait un faux-precis (R29).
const PRECACHE = [
