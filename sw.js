 ...
 */
const CACHE = 'ft-v1089'; // 🩹 ft-v1089 = « null » DANS LE CHAMP KG — ET C'EST LE CORRECTIF DE LA VIRGULE QUI A OUVERT LE TROU. Michel, capture du compte d'Eline : son Pec Deck affichait `10 reps x null kg` sur deux series — et SA lecture est la bonne : « Eline avait mis des valeurs, et si je dis pas de betises c'est en mettant la virgule ». 🔗 LA CHAINE, MAILLON PAR MAILLON : ① ft-v1057 (30/08) fait passer CE champ de `type="number"` a `type="text"`... ⛔⛔ EN LAISSANT `+this.value`. Or `type="number"` etait la SEULE chose qui le protegeait : le navigateur refusait la virgule, `.value` rendait '', donc `+'' = 0`. ② A partir de la, `+'62,5'` = NaN. ③ `persist()` fait `JSON.stringify`, et ***`JSON.stringify(NaN)` vaut `null`***. ④ Au rechargement, le mot « null » s'affiche. ⚠️ SA VALEUR EST PERDUE, PAS MASQUEE — elle a tape un poids, il n'existe plus ; on ne le devine pas a sa place (R29). ⭐ LA LECON, ET C'EST LA FAMILLE §30 DE BUGS.md : *un correctif qui retire un garde-fou du navigateur sans mettre le sien fabrique un bug PIRE que celui qu'il repare* — un champ oublie garde son ancien comportement, celui-la a ete rendu plus fragile qu'avant. Et se mefier d'un correctif qui annonce un NOMBRE dans son titre (« 22 champs ») : le nombre dit ce qui a ete traite, jamais ce qui a ete manque. ⛔ Le champ passe desormais par `numFR`, un champ vide rend `null` (« je ne sais pas ») et jamais 0, et le temoin ajoute protege la REGLE et non le cas : aucun champ decimal en `type="text"` ne peut lire par `+this.value`.
// ⬆️ 15e collision de la semaine : session-A a publie SA ft-v1088 (la suite du lot d'audit)
// pendant ce travail. Ma version devient ft-v1089 — UN NUMERO DE CACHE NE RECULE JAMAIS, et
// on ne renumerote pas le travail de l'autre : leur ft-v1088 reste la leur, avec son entree
// dans CLAUDE.md.
const PRECACHE = [
