 ...
 */
const CACHE = 'ft-v972'; // ⚖️ ft-v972 = LA QUANTITE POUR TOUTES LES ENTREES + LES CALORIES QUI NE COLLENT PAS. Michel : « on ne peut pas modifier le poids, je modifie le nom ca ne change pas la valeur, il faut rajouter une ligne poids qui va modifier les calories et les autres lignes », puis devant une ligne a 1117 kcal pour 26 P / 1 G / 1 L : « putain je ne l'avais meme pas vu, j'etais axe sur les calories », puis « et en direct, pas au moment de l'enregistrer ». ⭐ LE RESCALE NE DEMANDE AUCUN per100 : on ne part pas d'une composition, on rescale PAR PROPORTION — X x (nouvelle / reference). Trois sources pour la reference : per100 connu (grammes absolus, ft-v962 inchange) · `q` enregistre · ⭐ LE NOM LUI-MEME (« 30g de proteines » porte son ancrage — on lit ce que Michel a deja ecrit au lieu de le redemander). ⛔ Sans aucun ancrage : aucun poids invente, des PORTIONS (x0,5 x1,5 x2, x3) qui sont vraies quelle que soit la portion de depart. ⛔⛔ ET LES CALORIES DOIVENT COLLER A LEURS PROPRES MACROS : 4P+4G+9L = 117 pour sa ligne a 1117 — un « 1 » de trop qui gonflait sa journee de 1 000 kcal, et que RIEN ne signalait. L'etiquette reelle de sa whey l'a confirme au dixieme (116,6 kcal / 26,4 g pour 30 g). ⛔ ON NE CORRIGE JAMAIS TOUT SEUL (R29) : on montre l'ecart, la personne tranche. ⭐ EN DIRECT A CINQ MOMENTS : chaque frappe, apres un scan, apres une estimation IA, en reprenant une entree du journal, a l'ouverture de la modale — attraper a la SAISIE vaut mieux qu'a la relecture, ou la journee est deja faussee. ⭐ R2 : UNE SEULE definition de « ces calories sont impossibles », partagee par la saisie et la modification. ⛔⛔ ET L'ALCOOL NE DECLENCHE RIEN : 7 kcal/g sans champ dedie — une biere reelle afficherait 69 % d'ecart, un verre de vin 87 %. Trouve en testant les cas limites sur son etiquette, pas apres coup (R19). La liste se TAIT seulement : le meme ecart sur un aliment ordinaire crie toujours, et des calories MANQUANTES crient meme sur une biere (l'alcool ajoute, il ne retire pas).
const PRECACHE = [
