 ...
 */
const CACHE = 'ft-v1068'; // ⚖️ ft-v1068 = UN SEUL PROPRIETAIRE DE « COMBIEN J'EN AI PRIS ? ». Michel : « que ce soit le code-barres, manuel, avec l'IA ou avec l'etiquette, il faut qu'il y ait une COHERENCE quand on change la dose, peu importe le produit — meme s'il faut qu'on cree un algorithme expres ». ⭐⭐ L'ALGORITHME EXISTAIT DEJA, QUATRE FOIS, ET C'EST BIEN LE PROBLEME : `valeurs = base × (saisie / reference)` — un pour-100 g n'est pas un autre calcul, c'est CE calcul avec une reference de 100. ⛔⛔ MESURE AVANT : le MEME geste (vider le champ) donnait TROIS resultats — pour-100 g (2 routes) tombait a ZERO, proportion-ajout revenait a la reference, proportion-modifier laissait des valeurs ORPHELINES (la jumelle de ft-v1061, encore vivante : 4e fois du jour qu'un correctif etait pose d'un seul cote). Et `af-bc-grams` etait le SEUL champ de quantite reste en `type=number`, donc le seul a jeter « 62,5 » — la route du code-barres, ratee par ft-v1057. 👉 UNE REGLE, QUATRE ROUTES : le champ vide ramene a la REFERENCE (le seul nombre encore ecrit a l'ecran), meme arrondi, meme lecture de la virgule, meme rafraichissement du controle. ⛔ Nuance de ft-v966 rattrapee par un temoin : les VALEURS se replient, la phrase « pour tes n g » se TAIT.
const PRECACHE = [
