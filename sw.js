 ...
 */
const CACHE = 'ft-v1064'; // ⚖️ ft-v1064 = LE CHOIX D'UNITE DANS « MODIFIER L'ALIMENT ». Michel, capture a l'appui : « quand j'ajoute il ne me donne que le choix de la quantite ». Son ecran de modification n'offrait que des multiplicateurs et un cul-de-sac : « Cette ligne n'a pas de quantite connue — on ne peut pas inventer un poids. Mets la quantite dans le nom. » ⛔⛔ CETTE PHRASE EST, A DEUX MOTS PRES, CELLE QUE ft-v1056 A SUPPRIMEE DE L'ECRAN D'AJOUT : meme refus, l'autre ecran — le correctif avait ete pose D'UN SEUL COTE (R8). *Demander a quelqu'un de reecrire le NOM de son aliment pour en changer le poids, c'est lui faire faire le travail de l'app.* ⭐ R13 : rien de reinvente, c'est `_afMajAncre` transpose — deux onglets, un seul champ actif, et une fois le poids declare on retombe sur le champ proportionnel qui existait deja depuis ft-v972. ⭐⭐ R4, la moitie qui manquait : le poids DESCEND jusqu'a la donnee (`q`/`u`) — sans ca l'app le redemande a chaque ouverture et le cul-de-sac revient. ⛔ Declarer n'est pas rescaler, et le champ reste VIDE (R29).
const PRECACHE = [
