 ...
 */
const CACHE = 'ft-v969'; // 🧮 ft-v969 = D'OU VIENT LE CHIFFRE DE PROTEINES. Michel, devant l'onglet Supplements : « sur cette image c'est la portion ou juste le nombre de proteine ? ». ⭐⭐ LA QUESTION ELLE-MEME MONTRAIT LE TROU : le champ dit « Proteines mangees (g) » sans jamais dire QUI le remplit. Quand le Journal porte des proteines, le champ reste VIDE avec un placeholder « 0 » pendant que la barre affiche le vrai total — deux nombres qui se contredisent, et rien ne dit lequel commande. ⛔ MEME FAMILLE QUE LE « 88 g » DE ft-v966 : aucun des deux n'est faux, c'est leur VOISINAGE MUET qui trompe. ⭐ R2 — LA LIGNE NE CALCULE RIEN : elle relit `eaten` et NOMME sa source, au lieu de refaire le calcul (deux calculs du meme nombre finiraient par diverger). Trois etats : rien de note -> on explique l'UNITE (grammes de proteines, pas poids d'aliment, ce qui repond exactement a sa question) ; le Journal remplit -> « 46 g lus dans ton Journal du jour » + comment le remplacer ; saisie manuelle -> « chiffre que TU as tape » + comment revenir au Journal. ⛔ ET ON NE DIT PAS « 0 g lus dans ton Journal » quand rien n'est note : ca se lirait comme un constat alors que c'est une journee qui commence (R24). ⛔⛔ LE TEMOIN QUI PROTEGE LE PLUS RESTE CELUI D'AVANT : la saisie manuelle n'est JAMAIS ecrasee par le Journal (R29, meme arbitrage que `manualKcal`) — il est vert des DEUX cotes, et c'est le but.
const PRECACHE = [
