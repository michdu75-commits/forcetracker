 ...
 */
const CACHE = 'ft-v1092'; // 🔐 ft-v1092 = LES DONNEES DE SANTE VIVENT DANS LEUR PROPRE CLE DE STOCKAGE. Idee de Michel : « on peut pas creer une section sante pour eviter justement que tout se trouve dans le meme JSON ? ». ⭐⭐ ELLE EST PLUS FORTE QUE LA PROMESSE QU'ON VENAIT D'ECRIRE : la politique dit que les outils de diagnostic ne montrent que le necessaire — garantie de COMPORTEMENT, l'outil CHOISIT de ne pas montrer. Avec deux cles (`u_` et `h_`) elle devient une garantie de CONSTRUCTION : ***l'outil ne l'a pas en main***. C'est toujours la seconde qui tient. ⛔⛔ LE GARDE-FOU TIENT EN UNE PHRASE : la sante est ecrite dans `h_` **ET RELUE** avant d'etre retiree de `u_`. Script Properties n'a pas de transaction — `h_` echoue → on ne retire rien ; `u_` echoue apres → la sante est aux DEUX endroits et la lecture prefere `h_`. *Il n'existe aucun ordre ou la sante est retiree avant d'etre confirmee ailleurs.* ⛔ REPLI SUR L'ANCIEN : un compte pas encore migre garde sa sante dans `u_` et se charge exactement comme avant (regle d'or #3). ⛔⛔ ET TROIS ENDROITS SERAIENT DEVENUS FAUX SANS ETRE TOUCHES, c'est la qu'etait le vrai risque : la SUPPRESSION DE COMPTE (sans `h_`, la separation CREAIT une fuite en pretendant en fermer une), la SAUVEGARDE DRIVE (elle aurait perdu bilans sanguins et corporels des demain, en silence) et la COMPRESSION (le reservoir du 29/07 regrossit). ⚠️ DEUX VERIFICATIONS ONT CORRIGE LA LISTE AVANT ECRITURE : `cycle` est le cycle de FORCE et non le menstruel — le deplacer aurait casse l'ecran Cycle de force ; `exPhotos` et `morpho` ne sont pas des donnees de sante. *Une liste ecrite de memoire aurait emporte le cycle de force et laisse le tabac.* 🎯 FAMILLE §31 de BUGS.md : un temoin vise sur une FORME se perime au premier remaniement — 4 fois cette semaine.
const PRECACHE = [
