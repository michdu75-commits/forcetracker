 ...
 */
const CACHE = 'ft-v991'; // ⚖️ ft-v991 = LE VOCABULAIRE KATCH DE MILO — « MESURÉE » devient « ESTIMÉE », et la PROVENANCE descend jusqu'à la donnée. Priorité ③ tranchée par Michel. ⛔⛔ MESURÉ AVANT DE TOUCHER AU CODE, dans un vrai navigateur : les TROIS provenances de la masse maigre (lue sur un rapport · DÉDUITE par soustraction · calculée depuis un %% de gras TAPÉ AU CLAVIER) produisaient une phrase IDENTIQUE MOT POUR MOT dans le prompt : « CALCULÉ SUR SA MASSE MAIGRE MESURÉE … C'est un chiffre SOLIDE … Tu peux t'appuyer dessus sans réserve. » ⚠️ Le brief de départ annonçait un « motif regex qui capture trop tôt » : c'était faux, la mesure l'a dit — c'est R4 (l'information reste dans la donnée et n'atteint jamais la sortie). ⛔ R32 : une balance MESURE un poids et une impédance, elle ESTIME tout le reste ; dire « mesurée » d'un %% de gras tapé au clavier est un fait faux sur la santé de quelqu'un. ⭐⭐ ET LE TÉMOIN PROTÉGEAIT LA MAUVAISE PHRASE : il épinglait le mot « MESURÉE », donc toute correction de R32 le faisait rougir et passait pour une régression — c'est pour ça que docs/SUIVI-AUDIT.md disait de le corriger D'ABORD. ⭐ KATCH N'EST PAS DÉVALUÉ POUR AUTANT : le prompt garde « un MEILLEUR point de départ que la formule habituelle » et l'écart chiffré — corriger un excès par l'excès inverse aurait fait douter Milo d'un calcul qui reste le bon (+180 kcal/j mesurés en ft-v833). ⭐ LE DRAPEAU `lmDeduite` DE ft-v978 ÉTAIT ÉCRIT PUIS JAMAIS LU : son commentaire annonçait « il existe pour la correction du vocabulaire de Milo » — c'est fait, et le commentaire est mis à jour pour ne pas faire dire une bêtise au suivant (R23). ⚠️ COMPTÉ LES ENDROITS (famille #3) : la phrase n'existait qu'à UN seul endroit en production, pas de jumelle ailleurs.
const PRECACHE = [
