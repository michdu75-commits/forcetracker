 ...
 */
const CACHE = 'ft-v1104'; // ♻️ ft-v1104 = UNE VALEUR FAUSSE QUI SE RECOPIE — LE MECANISME DU « TOUJOURS ». Michel, etiquette du pot a l'appui : « c'est cette prot la, toujours le meme souci ». La verite du tableau : 30 g -> 116,6 kcal / 26,4 g de proteines (88 g pour 100 g). L'app avait 156 / 35, soit EXACTEMENT 1,333x sur les DEUX nombres independamment — 1,333 = 40/30. ⛔⛔ LE « TOUJOURS » A UN MECANISME, ET IL EST DANS LE CODE : les suggestions de l'ecran d'ajout ont pour source ① CE QUE LA PERSONNE A DEJA NOTE. *Une estimation fausse notee une fois devient une proposition qu'on reprend en un tap, indefiniment* — et une valeur fausse qui se RECOPIE coute plus cher que la valeur fausse d'origine. ⛔⛔ ET LE GARDE-FOU DE ft-v1103 NE LA VOYAIT PAS SUR CE CHEMIN, pour DEUX raisons distinctes, mesurees : ① `af-bc-grams` porte `value="100"` dans le HTML et garde cette valeur MEME CACHE — le controle comparait donc 37 g a une portion de 100 g que personne ne voyait (*il ne rougissait pas : il mesurait un champ invisible*) ; ② et `e.q` (30 g, bien enregistre) n'etait lu QUE dans la branche `per100` : a la reprise d'un aliment sans pour-100 g, la quantite n'atteignait pas l'ecran — R4, a trois lignes du commentaire qui l'explique. ⛔⛔ ET UNE JUMELLE MANQUEE PAR MOI, LA VEILLE (R8) : le champ s'appelle `ef-grams` dans la modale et `af-bc-grams` dans le formulaire d'ajout ; mon premier jet ne lisait que le premier, donc le controle etait AVEUGLE sur tout le chemin code-barres / etiquette — c'est-a-dire precisement la ou arrivent les valeurs d'un produit emballe. *J'ai ecrit la regle de la jumelle dans le journal la veille et je l'ai manquee le lendemain.* ⭐ R13 : rien n'est reinvente, on emprunte `_afPoidsDeclare`, et le libelle « que tu as indique » reste VRAI — elle l'a indique la fois d'avant. ⭐⭐ ET LE CHEMIN FIABLE EST MESURE, PAS SUPPOSE : le pour-100 g de la vraie etiquette (88 g/100 g) sur 30 g rend 117 kcal / 26 g / 1 / 1, au chiffre pres, et EN SILENCE. C'est R33 (l'echelle des sources) verifiee sur son pot : le code-barres ou la photo de l'etiquette battent l'estimation. Tests : parcours 2421 (bloc CCXIII, +9), calculs 266, muscles 241, croises 50, dates 7, donnees 0 trou.
