 ...
 */
const CACHE = 'ft-v904'; // 🔄 ft-v904 = L'EXERCICE SUIVANT S'OUVRE ENFIN QUAND ON A FINI LE PRECEDENT. Michel : « je finis ma derniere serie de developpe couche et apres je fais les epaules ; ca devrait se reduire et l'exercice d'epaule s'ouvrir en grand » — et « je n'ai pas vu le message » (le « Ensuite : ... » du chrono de repos). ⚠️⚠️ LA CAUSE : « termine » se lisait « TOUTES les lignes cochees » (every(s=>s.done)). Or depuis ft-v887 l'app AJOUTE elle-meme des paliers d'ECHAUFFEMENT : quelqu'un qui attaque directement a sa charge de travail, ou qui echauffe sans le noter, laisse forcement des lignes vides — donc l'exercice n'etait JAMAIS considere comme fini. C'est l'app qui cree les lignes qui l'empechent ensuite de conclure. 👉 On regarde desormais les SERIES DE TRAVAIL, pas les lignes : l'exercice est fini quand il n'en reste aucune a faire. C'est deja la definition employee partout ailleurs (finishWorkout exclut E et W du decompte comme des records). ⚠️ Une SERIE DE TRAVAIL non cochee bloque toujours (elle peut encore etre faite), et un exercice qui n'aurait QUE des paliers n'avance pas non plus. ⭐ REPRODUIT AVANT CORRECTION : avec toutes les lignes cochees ca marchait deja ; en laissant les paliers vides, ni message ni bascule.
const PRECACHE = [
