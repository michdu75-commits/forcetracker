 ...
 */
const CACHE = 'ft-v993'; // 🧠 ft-v993 = LA COURSE `_saveCoachMemory` — PROUVEE, PUIS CORRIGEE. ⛔⛔ PROUVEE AVANT DE TOUCHER AU CODE (consigne explicite du suivi d'audit, lecon de ft-v979) : mesuree dans un navigateur en remplacant le RESEAU et rien d'autre. Deux resumes declenches a 20 ms d'ecart envoient TOUS LES DEUX `existingMemory:"MEMOIRE DE DEPART"`, et le dernier REVENU ecrase l'autre. Mesure : « FAIT-B » perdu, sans erreur, sans trace. ⭐ CE QUI REND LA COURSE POSSIBLE : `S.coachMemory` est lu au DEPART de l'appel et reecrit au RETOUR ; entre les deux, n'importe quel autre appel lit la meme valeur perimee. L'appelant (coach.js:4251) ne fait pas `await` — c'est VOULU, l'UI ne doit jamais attendre le reseau (regle d'or #3). ⛔ LE CORRECTIF NE REND DONC PAS L'APPEL BLOQUANT : on SERIALISE dans une file, exactement comme le debrief de ft-v979 (R13/R2 — pas un 2e mecanisme d'attente). Chaque resume part quand le precedent est fini et relit `S.coachMemory` A CE MOMENT-LA. Mesure apres correctif : l'appel 2 recoit « DEPART | FAIT-A » et les deux faits survivent. ⚠️ LA FILE NE SE CASSE JAMAIS : un appel en echec passe la main au suivant (2e argument de `.then`), sinon UNE panne reseau gelerait la memoire pour tout le reste de la session — un temoin l'epingle. ⭐⭐ ET LE ⑤ DE L'AUDIT (caches par lieu) EST MESURE PUIS NON CONSTRUIT, ce qui est la bonne reponse : les 5 variantes sont bien distinctes (salle 11 446 · basique 8 544 · maison 6 493 · poids du corps 2 136 · non renseigne 11 475 car.), mais l'arithmetique du cache ne donne AUCUN gain sous ~6 personnes actives dans la meme heure ET sur le meme lieu. Le projet a une poignee de testeurs. R19/R34 disaient « ne pas commencer sans donnees d'usage » — la mesure le confirme, on ne paie pas de la complexite pour un gain qui n'existe pas encore.
const PRECACHE = [
