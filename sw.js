 ...
 */
const CACHE = 'ft-v1091'; // 👆 ft-v1091 = CE QU'UNE FERMETURE AU DOIGT EMPORTE — 2e passe de chasse aux incoherences. ⛔⛔ ① LA CAMERA RESTAIT ALLUMEE : `closeBarcodeScanner()` est la SEULE chose qui coupe le flux video, et `#ov-bc-scan` n'etait declare nulle part dans `_OVERLAY_CLOSERS` (ni fermeture au clic sur le fond). Ferme en glissant, l'ecran disparaissait et la camera continuait de tourner — voyant vert allume, batterie qui file, rien a l'ecran pour le dire. 4e fois pour cette famille (S26 de BUGS.md), et la 1re qui touche a un capteur. ⛔⛔ ② LE BANDEAU « SAUVEGARDE EN LIGNE EN PAUSE » ETAIT EFFACE PAR L'ECRAN QUI LE PORTE : deux fonctions ecrivaient dans `#email-verify-card` (R2), et `renderSetup()` appelle la mauvaise — ouvrir l'onglet Profil remplacait l'alerte rouge par le rappel jaune « confirme ton e-mail », qui envoie en plus chercher au mauvais endroit. ft-v788 avait rendu ce refus visible ; il redevenait muet au 1er changement d'ecran. ⭐ Le drapeau `ft4_auth_refus`, ECRIT depuis ft-v788 et relu par PERSONNE (R5), sert enfin — avec sa guerison, sinon on remplacerait un silence par un cri permanent. ③ LA REPONSE « comment as-tu dormi ? » etait perdue si on refermait avant la 2e question — et perdue de facon ALEATOIRE, ce qui est pire. ④ Le guide de l'app ferme au doigt n'enchainait plus la proposition d'installation, definitivement.
// ⬆️ 17e collision (leur compte, pas le mien : ils ont numerote la 16e juste en dessous —
// deux tallies concurrents finiraient par se contredire, R2). Session-B a publie SA ft-v1090
// pendant ce travail (les series abimees
// par la virgule). Ma version devient ft-v1091 — UN NUMERO DE CACHE NE RECULE JAMAIS, et on
// ne renumerote pas le travail de l'autre. Leur recit est garde juste en dessous.
// 🩹 ft-v1090 = « null » DANS LE CHAMP KG + LE DETECTEUR CHEZ TOUT LE MONDE + LA POLITIQUE MISE A JOUR. ① Capture du compte d'Eline : `10 reps x null kg`. LA CAUSE VIENT DE MICHEL (« elle avait mis des valeurs, c'est en mettant la virgule ») et la chaine est etablie dans le code : ft-v1057 fait passer CE champ de `type="number"` a `type="text"` EN LAISSANT `+this.value` — or `type="number"` etait la SEULE protection (le navigateur refusait la virgule) ; `+'62,5'` = NaN ; `JSON.stringify(NaN)` vaut **null** ; le mot « null » s'affiche. ⚠️ SA VALEUR EST PERDUE, PAS MASQUEE. ⭐ FAMILLE §30 de BUGS.md : *un correctif qui retire un garde-fou sans mettre le sien fabrique un bug PIRE que celui qu'il repare* — et se mefier d'un correctif qui annonce un NOMBRE (« 22 champs ») : il dit ce qui a ete traite, jamais ce qui a ete manque. ② LE DETECTEUR CHEZ TOUT LE MONDE, demande de Michel : « ce que j'ai eu moi les autres peuvent l'avoir aussi ». Route `setsCorrompus`, LECTURE SEULE : 0 ecriture de compte, ni charges ni profil ni sante — juste un nombre, une date, un nom d'exercice. Elle ne repare pas : la valeur d'origine est perdue, seule la personne la connait (R29). ③ POLITIQUE DE CONFIDENTIALITE, section 4 bis. Elle promettait que les donnees servent « UNIQUEMENT a faire fonctionner l'app pour toi » — or on lit les comptes pour diagnostiquer. *Une promesse qu'on ne tient plus est pire qu'une promesse qu'on n'a jamais faite.* ⚠️ ft-v1089 = le lot de session-A (les 166 boutons) — son recit est dans CLAUDE.md.
// ⬆️ 16e collision de la semaine : session-A a publie SA ft-v1089 pendant ce travail. Ma
// version devient ft-v1090 — UN NUMERO DE CACHE NE RECULE JAMAIS, et on ne renumerote pas
// le travail de l'autre.
// 🧹 ft-v1088 = LA SUITE DU LOT D'AUDIT — et l'un des quatre est REFUTE. ① LA PROJECTION DE FORCE NE REGARDAIT QUE LE SQUAT : mesure, un souleve de terre a 180 kg (2,25 x le poids de corps) etait projete a **+10,8 %**, exactement comme un debutant total ; l'indice 4 de la table etait MORT (la branche ne rendait que 0..3). `getLevel` reveillee — par exercice, par SEXE, corrigee par l'age — ⚠️ apres avoir cherche POURQUOI elle dormait (R30) : debranchee en ft-v385 parce que l'AFFICHAGE du niveau a quitte l'Accueil, pas parce qu'elle etait fausse. ② « FEMME = FESSIERS » : le generateur ajoutait une abduction sur le seul test `gender==='F'`, alors que `S.priorities` existe, part au cloud et atteint meme Milo (R4). Une priorite DECLAREE decide desormais quel que soit le sexe. ⛔ Ce que recoit une femme qui n'a RIEN declare n'est pas tranche — arbitrage produit. ③ LA COURSE `_saveCoachMemory` EST REFUTEE, aucune ligne touchee : 3 appels concurrents sont serialises, un echec reseau ne detruit pas la memoire, et le jeton a deja deux proprietaires. 3 temoins figent la decision (R30). ④ L'ETAPE 1 NE FINISSAIT JAMAIS : `phase` posee a 1, rien ne l'avance — Milo redemandait de « tenir 3 semaines » a quelqu'un qui s'entraine depuis douze, et le preparait a une suite qui n'existe pas. ⚠️ Et mon temoin a rougi sur du code JUSTE : il cherchait « tenir 3 semaines » et attrapait SA PROPRE NEGATION.
// ⬆️ 14e collision : session-B a publie SA ft-v1087 pendant ce travail. Ma version devient
// ft-v1089 — un numero de cache ne recule jamais, et on ne renumerote pas leur travail.
const PRECACHE = [
