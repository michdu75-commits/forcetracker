 ...
 */
const CACHE = 'ft-v914'; // ⚖️ ft-v914 = MILO ECRIVAIT 82,5 kg, UNE CHARGE QUI N'EXISTE PAS DANS UNE SALLE. Michel, pour la DEUXIEME fois (1re le 15/08) : « il ne compte pas le deplacement dans la salle, quand il me met 82,5 faut le trouver les poids de 2,5 ». ⚠️⚠️ LE 15/08 AVAIT PRODUIT `_pasCharge` (log.js), calibree sur ses 31 seances — mais sa definition disait « ne s'applique QU'AUX CHARGES QUE L'APP FABRIQUE ». Milo n'a jamais recu la table : 0 occurrence de `_pasCharge` dans coach.js. C'est BUGS.md famille 15 (la regle juste, definie trop etroit) et R4 (l'info existe, elle n'atteint pas le prompt). ⏵ Le correctif n'est PAS d'arrondir Milo apres coup — PB-008 tient — mais de lui DONNER la table pour qu'il ecrive 80 ou 85 du premier coup. UNE seule table, `_PAS_CHARGE_TABLE`, lue par l'app ET le prompt (R2). ⭐⭐ 2e DEFAUT, STRUCTUREL : « toutes les ancres d'abord » FABRIQUE des zigzags de salle — squat > militaire > leg extension > elevations = 3 traversees pour une seance qui n'en demande qu'une. On groupe par zone, l'ancre avant ses accessoires DANS chaque zone, sans toucher a « l'ancre la plus lourde reste 1re » ni au superset antagoniste. ⚠️ ET LE GARDE-FOU DE TAILLE A REFUSE LA 1re VERSION : la regle de zone, ecrite dans le bloc commun, l'a fait passer a 47 286 caracteres pour un plafond de 46 500. Le seuil N'A PAS ete releve — il porte la consigne « une relecture dediee, PAS un relevement de plus ». La regle a demenage aupres du BUDGET DE TEMPS, ou elle a d'ailleurs plus de sens : une traversee coute des minutes.
const PRECACHE = [
