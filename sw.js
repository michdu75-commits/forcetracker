 ...
 */
const CACHE = 'ft-v967'; // 🛡️ ft-v967 = LA NOTE HONOREE DANS LA MEME REPONSE — 4e faux positif du Gardien, trouve par Michel. Il envoie LA REPONSE EXACTE qui levait le drapeau : « Et le Butterfly (Pec Deck) en debut de seance — JE NOTE, c'est ton choix, je le respecte »… et le Pec Deck EST dans la seance qu'il reconstruit dix lignes plus bas. ⛔ Il note ET APPLIQUE dans le meme message : il n'y a rien a differer, donc rien a enregistrer. Ce n'est pas une promesse vide, c'est un accuse de reception suivi de son execution. ⭐ LE CRITERE EST OBSERVABLE, pas une devinette : une SEANCE est-elle produite ici (>=3 blocs NxN) ET aucun mot de report ? alors la note est HONOREE. ⛔⛔ ET LE REPORT L'EMPORTE TOUJOURS : « je note POUR LA PROCHAINE FOIS » suivi d'une seance reste un drapeau — sinon il suffirait de joindre un tableau pour desarmer le garde-fou. ⭐ MESURE DANS LES DEUX SENS AVANT D'ETRE ECRIT : sur ses 119 reponses, 3 drapeaux -> 2, et les 2 gardes sont exactement les vrais. Les 3 vraies de ft-v944 restent gardees, dont « le Leg Curl avant le Face Pull, c'est note » qui n'a AUCUN mot de report — parce qu'elle ne produit pas de seance. ⚠️⚠️ ET CETTE VERSION CORRIGE UN VERDICT QUE J'AVAIS ECRIT UNE HEURE PLUS TOT : « ft-v923 NE TIENT PAS — mesure ». C'ETAIT FAUX, et fonde sur un compteur dont je n'avais lu AUCUN des 5 textes — exactement l'erreur BUGS.md 12quater, refaite DEUX HEURES apres avoir ecrit la famille qui la decrit. Ce qui est etabli : 2 vraies promesses non tenues, toutes deux ANTERIEURES aux correctifs. ⚠️ 2e calibration de ce motif sur de VRAIES conversations, 2e fois qu'elle vient de Michel (R19 : un garde-fou juste une fois sur deux ne survit pas a son premier mois).
const PRECACHE = [
