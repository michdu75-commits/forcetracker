 ...
 */
const CACHE = 'ft-v976'; // 🧪 ft-v976 = LE CLONE DE TEST EST RETIRE. Michel : « plus besoin des clones, ca permettra de gagner du temps ». ⭐ MESURE AVANT DE COUPER (R30 : un retrait se decide, il ne se constate pas) : sur les 60 DERNIERES VERSIONS, le dossier clone/ a change a chaque fois... et ZERO FOIS TOUT SEUL. Il n'a donc jamais servi de bac a sable sur cette periode — il RECOPIAIT la prod. ⛔ Ce qu'il coutait etait bien reel : 8 fichiers a dupliquer par version, un correctif propre au clone a re-poser (chemins ../data/), son sw.js a synchroniser, 2,8 Mo, et une deuxieme source de verite qui peut diverger sans rien casser. ⚠️⚠️ ET LE PIEGE S'EST DECLENCHE LE JOUR MEME : un `cp` un peu vite a efface 91 LIGNES de clone/index.html — le shim qui isole ses donnees des vraies. Repere et restaure. ⭐ Or un script de synchro existait (build_clone.py) AVEC un garde-fou « shim clone introuvable — abandon, rien ecrase » : un outil qu'on contourne ne protege rien. ⛔⛔ LES GARDES `window.__FT_CLONE__` SONT CONSERVEES EXPRES, ce n'est pas un oubli : plusieurs essais vivent derriere elles (zones sante lues dans le texte, header compacte, promesse d'inscription, questions illimitees) et les debrancher les rendrait soit UNIVERSELS soit PERDUS. On a supprime le clone, pas arbitre ses experiences — decision separee, listee dans le journal. 👉 Un bac a sable se refabrique depuis la prod en quelques minutes, exactement comme il est ne le 04/07/2026.
const PRECACHE = [
