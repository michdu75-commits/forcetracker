 ...
 */
const CACHE = 'ft-v1010'; // 🎯 ft-v1010 (developpee comme ft-v1008) = L'OBJECTIF GARDE SON HISTOIRE, LES MESSAGES LEUR DATE. Michel, le 19/08, a Milo : « As-tu vu que j'avais change d'objectif ? » → « Non, je ne vois pas de changement d'objectif dans ce que j'ai sous la main. » Il a fallu qu'il ecrive « j'etais en force max avant » pour que Milo reagisse. ⭐⭐ ET MILO DISAIT VRAI — mesure dans son export du 25/08 : `goal` valait `recomp`, et « force max » n'apparaissait NULLE PART ailleurs que dans la conversation elle-meme. Aucun journal, rien dans le registre. L'app gardait la valeur du JOUR, jamais son histoire. C'est R8 dans sa forme la plus pure — un prompt ne compense jamais une donnee absente — donc le correctif est dans la DONNEE, pas dans le prompt (R7). ⛔ UN SEUL PROPRIETAIRE (R2) : `_goalSet(g,src)` dans state.js ecrit `S.goal` ET journalise ; les 4 appelants reels y passent (profil · observation · les 2 de l'inscription). ⛔ L'INSCRIPTION N'EST PAS UN CHANGEMENT : la journaliser fabriquerait un faux « passe de muscle a force » le jour de la creation du compte, juste parce que `load()` pose « muscle » par defaut. ⛔ RESTAURER NON PLUS : setup.js ecrit `S.goal` directement, expres, et restaure le journal avec. ⛔ ON N'INVENTE AUCUN PASSE (R29) : un compte existant demarre avec un journal VIDE, et sans changement enregistre le bloc N'EXISTE PAS dans le contexte — pas d'en-tete vide. ⭐ Et la consigne d'agir n'apparait que sur un changement de moins de 30 jours : rappeler un virage vieux de six mois serait du bruit qu'on finit par ne plus lire (R19). ⏱️ HORODATAGE DES CONVERSATIONS (demande de Michel dans la foulee) : les messages naissaient sans date, donc AUCUNE phrase de Milo n'etait datable — c'est ce qui m'a empeche de situer sa conversation dans le temps. ⛔⛔ ET LE DEFAUT ETAIT A DEUX ENDROITS, PAS UN : `_convLightMsgs` reconstruisait `{role, content}` A LA MAIN et jetait la date (et `_silent` avec) — or c'est ELLE qui alimente `S.coachConversations` ET l'export ; et `loadCoachConv` la jetait aussi, si bien que rouvrir une vieille conversation EFFACAIT ses dates definitivement. ⚠️⚠️ MON PREMIER TEMOIN ETAIT VERT PENDANT CE TEMPS : il appelait `_lightMsg` DIRECTEMENT au lieu d'archiver puis de relire. *Un test qui n'emprunte pas le chemin de la production ne teste rien, il rassure* — lecon n°1 de SUIVI-AUDIT, payee une fois de plus. Mesure par le vrai chemin : archive `[0,1500,null]`, apres reouverture `[0,1500,null]`, stockage `[0,1500,null]` — les dates exactes survivent, et le vieux message reste SANS date. ⚠️ RENUMEROTEE 1008 → 1010 A LA FUSION : session-B a livre ft-v1009 pendant ce travail et a pousse la premiere. Livrer 1008 apres 1009 ferait RECULER le numero de cache chez les gens deja passes en 1009 — on ne recule jamais, on saute. Le trou dans la serie est le prix, et il est ecrit ici pour qu'on ne le prenne pas pour une version perdue.
const PRECACHE = [
