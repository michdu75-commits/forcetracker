 ...
 */
const CACHE = 'ft-v902'; // 🔄 ft-v902 = L'ECRAN NE S'ETEINT PLUS QUAND ON VA PARLER A MILO PENDANT UNE SEANCE. Michel : « l'ecran s'eteint pendant la seance ». ⚠️⚠️ LA CAUSE N'ETAIT PAS LE VERROU, C'EST CE A QUOI IL ETAIT ATTACHE : goScreen le relachait pour TOUT ecran autre que Seance — or pendant une seance on va justement parler a Milo, regarder ses records, noter une pesee. On revenait a un ecran eteint au milieu d'un repos. 👉 Le verrou appartient desormais a l'ETAT « une seance tourne » (_syncWakeLock/_wktEnCours, log.js), pas a l'ecran affiche (R2). ⚠️ Il se REND quand la seance est en pause, annulee ou terminee : une seance en pause n'est pas un entrainement, et tenir l'ecran allume sans raison se paie en batterie. ⚠️ Deux defauts voisins corriges au passage : le systeme reprend le verrou en arriere-plan sans prevenir et il ne revient jamais seul (on ecoute l'evenement 'release' pour pouvoir le redemander, sinon il ne serait plus jamais repris) ; et « Terminer » sur une seance vide relachait l'ecran AVANT les controles, donc affichait l'erreur ET laissait l'ecran s'eteindre.
const PRECACHE = [
