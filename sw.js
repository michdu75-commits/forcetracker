 ...
 */
const CACHE = 'ft-v1065'; // 📊 ft-v1065 = LES CARTES D'ENTRAINEMENT NE S'AFFICHENT QUE SUR « EXERCICES ». Michel, video a l'appui : « le haut de l'onglet exercice ou l'on montre les seances moyennes de la semaine, mais sur le poids et le record on s'en fout un peu de ca non ? », puis « meme les badges ». ⭐ IL A RAISON, et la raison est de fond : « Ce que ton histoire montre » et « Ce que tu travailles, par semaine » parlent d'ENTRAINEMENT — sur Poids (une pesee) et sur Badges (des recompenses) elles ne repondent a aucune question qu'on vient s'y poser, elles repoussent seulement le contenu hors de l'ecran. ⭐ MESURE : les 2 cartes pesent 216 + 191 = 407 px ; sur Poids les sous-onglets remontent de y=545 a y=118 (427 px rendus) et la pesee du jour de ~600 a 170. ⚠️⚠️ C'EST UN RETOUR SUR MA PROPRE DECISION de ft-v1041, qui disait « elle SURVIT au changement de sous-onglet » et posait un temoin dessus : l'INTENTION etait bonne (ne pas la faire clignoter), la PORTEE trop large — *ne pas clignoter ne veut pas dire etre partout*. Le temoin est RE-VISE, pas desarme : il verifie maintenant qu'elle est MASQUEE hors d'Exercices ET qu'elle REVIENT INTACTE (elle ne se perd pas). ⛔ On masque, on ne vide ni ne recalcule : le contenu est deja peint par `renderProgress` (R19). 🧾 Au passage, `CLAUDE.md` corrige : `S.weightLog` porte `kg`, PAS `bw` — la doc mentait depuis toujours, et c'est elle qui m'a fait ecrire une fixture rendant `value="undefined"` dans le champ de pesee (R23).
const PRECACHE = [
