 ...
 */
const CACHE = 'ft-v940'; // 🟢 ft-v940 = LA PASSE REELLE : 13 VERTS, 2 ROUGES — ET LES DEUX ETAIENT DES FAUX ROUGES. Michel a lance la passe et colle les REPONSES BRUTES (le bouton de ft-v938). Rejeu offline : 0 appel, 0 €. ⭐⭐ ET LE DIAGNOSTIC CHANGE TOUT : ① EV-003 (face pull) etait un FAUX ROUGE, et c'est la FAMILLE DE BUGS n°1 DU PROJET — LE PREMIER MATCH GAGNANT (BUGS.md, ≥12 fois). `findIndex` prenait la PREMIERE ligne contenant « face pull » : la phrase d'accueil ou Milo REPREND les mots de Michel (« haut du corps tirage + face pull — bonne idee pour l'epaule droite »). La vraie PRESCRIPTION etait 14 lignes plus bas, en avant-dernier, EXACTEMENT A SA PLACE. Donc le correctif de ft-v936 AVAIT MARCHE et mon motif le cachait — on cherche desormais une PRESCRIPTION (une ligne qui porte des series), plus une MENTION. ② EV-015 : mon verificateur etait PLUS STRICT QUE LE JUGE HUMAIN, et le depot en garde la preuve — le 25/07 (ft-v510) un juge humain avait note 5/5 ce comportement exact en ecrivant « propose de COMPLETER (pas remplacer)… comportement ideal ». Proposer d'ANALYSER le programme EST le role de complement. ⚠️⚠️ ET RELACHER DEUX MOTIFS JUSQU'A CE QUE TOUT SOIT VERT SERAIT PIRE QUE DE NE RIEN MESURER : des temoins prouvent qu'ils attrapent TOUJOURS la vraie violation (face pull place avant du lourd sans explication → rouge · un Milo qui n'offre aucun regard sur le programme → rouge). ⚠️ Un cas restait rate : le lourd SUR LA MEME LIGNE que le face pull — corrige, en retirant d'abord les mots du face pull puisque « TIRAGE visage » contient « tirage » et se denoncait lui-meme. ⛔ ET 15/15 VERT NE VEUT PAS DIRE « Milo est parfait » : un vert dit « aucune violation DETECTABLE », rien de plus — d'autant que deux motifs viennent de changer.
const PRECACHE = [
