 ...
 */
const CACHE = 'ft-v1103'; // ⚖️ ft-v1103 = LES MACROS NE PEUVENT PAS PESER PLUS QUE LA PORTION. Michel, capture a l'appui : « encore le souci avec la prot » — une ligne Iso zero protein a 35 g de proteines + 1 + 1 = 37 g de matiere dans une portion de 30 g. ⛔⛔ LE CONTROLE QUI EXISTAIT NE POUVAIT PAS LE VOIR, ET IL N'ETAIT PAS EN FAUTE : `_coherenceKcal` compare les CALORIES aux MACROS, et elles collent parfaitement ici (4x35 + 4x1 + 9x1 = 153 contre 156 affichees, 2 % d'ecart). *Une valeur inventee peut etre parfaitement coherente avec elle-meme* — famille §34 de BUGS.md. Il manquait la SECONDE question : est-ce que ca TIENT dans la portion ? ⭐ REPRODUIT PAR LE VRAI CHEMIN avant d'ecrire une ligne (§12quater — la fois ou j'ai livre une version entiere sur une cause deduite d'un seul nombre, sans regarder l'ecran) : la reponse du modele entre telle quelle dans `S.foodLog`, sans une alerte ; une reponse a 150 P / 80 G / 70 L pour 100 g y entrait aussi, soit 300 g dans 100 g. ⛔ UN SEUL PROPRIETAIRE (`_masseImpossible`, R2) dans la boite de coherence qui existe deja — donc les 3 surfaces d'un coup : estimation IA, formulaire d'ajout, modale de modification. ⛔ GRAMMES SEULEMENT, et ce n'est pas un detail : 100 ml de miel pesent ~140 g et portent ~116 g de glucides. La regle en ml ferait hurler l'app sur un sirop — c'est la lecon de la biere du controle voisin (R19). ⛔ LA TOLERANCE EST DERIVEE, PAS INVENTEE : les 4 champs sont arrondis a l'entier, donc 3 macros gonflees de <0,5 g et une quantite rabotee d'autant = 2 g au pire. Mesure : une huile (10 g pour 10 g de lipides) et du sucre (20 g pour 20 g de glucides) restent muettes alors qu'elles sont a 100 % d'une seule macro — *la limite physique est l'EGALITE, pas un pourcentage qu'il faudrait choisir*. ⛔⛔ ET AUCUN BOUTON DE CORRECTION, CONTRAIREMENT AU CONTROLE DES CALORIES : c'est une decision, pas un oubli. Quand les calories ne collent pas, la valeur juste SE CALCULE ; ici l'app sait que l'un des deux est faux et NE SAIT PAS LEQUEL. Chez Michel la PORTION est juste (30 g d'isolat portent ~26 g de proteines) et les macros sont fausses — un bouton « mettre 37 g » aurait AGGRAVE sa ligne au lieu de la reparer (R29). ⚠️ ET UN AVEU : le correctif est parti sur master dans un commit intitule « journal de partage », parce qu'un `git add -A` l'a emporte — donc sans bump de cache. Corrige ici. Tests : parcours 2413 (bloc CCXII, +12), calculs 266, muscles 241, croises 50, dates 7, donnees 0 trou. CONTROLE NEGATIF : 3 rouges contre l'ancien code, et les 5 cas legitimes verts des DEUX cotes.
