 ...
 */
const CACHE = 'ft-v1021'; // 🧠 ft-v1021 = L'APP APPREND TON ALIMENTATION, SANS UN SEUL APPEL. Michel : « il faut que l'application (pas Milo) apprenne du sportif cote nutrition sans que ca me coute un seul appel api ». La veille, le constat qui l'a amene la : « on connait l'athlete sportivement en lui posant des questions, mais pas du tout en alimentation, alors que c'est 80 % au moins de l'evolution physique ». MESURE : 6 questions sur l'entrainement, ZERO sur la nourriture. ⭐⭐ TOUT EST DE L'ARITHMETIQUE SUR `S.foodLog` — pas un octet de reseau, pas de modele, pas de cervelet, et ca marche hors ligne. *La donnee etait deja la ; ce qui manquait, c'est que quelqu'un la regarde.* Mesure du cout : 20 passages de l'observateur + 2 rendus + 1 contexte = **0 appel sortant de plus** (les 2 du demarrage, QR de partage et ping Apps Script, existent depuis toujours). ⭐ CE QU'IL APPREND : ses aliments PAR REPAS avec leur frequence, ses HORAIRES reels (mediane, pas moyenne — un diner a 2 h du matin ne doit pas deplacer l'heure habituelle), ses moyennes reelles, et le nombre de jours notes. ⛔ LA MOYENNE DIVISE PAR LES JOURS NOTES, pas par les jours ecoules : sinon un oubli de saisie ressemblerait a un jour de jeune et la moyenne mentirait vers le bas. ⛔⛔ ET CE QU'ON N'INVENTE PAS : une ABSENCE NE PROUVE RIEN. Michel n'a jamais note de macadamia (« j'en mange pas, et en plus c'est degueulasse ») mais quelqu'un d'autre peut simplement ne pas y avoir pense — Milo recoit donc la liste AVEC l'interdiction d'en conclure un degout (R29, Constitution P4). Les degouts, le budget et le temps de preparation devront etre DEMANDES ; c'est ecrit dans IDEES-FUTURES. ⛔ AUCUN SCORE DE FIABILITE INVENTE (R32) mais des ETATS NOMMES — insuffisant / partiel / solide : « 4 jours notes » n'autorise pas la meme phrase que « 40 jours ». Sous 3 jours, la carte le DIT (elle ne se tait pas, un silence laisserait croire qu'il n'y a rien a apprendre) et Milo, lui, se TAIT. ⭐ DEUX CONSOMMATEURS, donc pas de donnee morte (R3) : une carte visible — *un profil qu'on ne voit pas ne peut pas etre corrige* — et le contexte de Milo, SOUS le marqueur de l'instant, cache intact. ⚠️ Deux temoins EXISTANTS ont rougi et avaient raison : celui des anciens prix a attrape « 2 mois » dans un de mes commentaires (reformule plutot qu'affaibli), et celui du journal a vu la nouvelle carte nommer des aliments d'autres jours (etendu a la LISTE DES ENTREES, sa vraie promesse).
const PRECACHE = [
