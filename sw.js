 ...
 */
const CACHE = 'ft-v986'; // ✏️ ft-v986 = « A LA MAIN » EN PREMIER ET EN ROUGE, ET DEUX TEMOINS QUI ROUGISSAIENT A MINUIT. Michel : « intervertis, a la main en premier et en rouge ». ⚠️⚠️ LE CHANGEMENT REMPLACE UNE DECISION QUI AVAIT SA RAISON ECRITE (R30) : le code-barres etait premier depuis le 15/08 parce qu'il est GRATUIT, ILLIMITE et pas cache derriere l'IA. La raison d'avant reste ecrite dans le code ET dans le temoin, pour que personne ne « repare » ca dans six mois. ⛔ ET LA DONNEE MESUREE VA DANS LE SENS DE L'ANCIEN ORDRE, autant l'ecrire : sur les 23 entrees reelles de son journal, scan 6 · ciqual 4 · historique 4 · ia-texte 3 · recherche 1 · MANUEL 1. Le chemin le plus emprunte passe donc en 3e position, et le moins emprunte en rouge. On applique quand meme : c'est un arbitrage d'USAGE, et c'est Michel qui decide — la mesure est la pour revenir en arriere en connaissance de cause, pas pour contredire la consigne. ⚠️ LE VRAI COUT EST AILLEURS ET IL EST ECRIT : « a la main » N'EST PAS gratuit — le champ libre part en estimation IA (le chemin de son huile d'olive a 135 kcal). On met donc en rouge le bouton qui consomme du quota a la place de celui qui n'en consomme pas ; a surveiller dans `origine`. ⭐⭐ ET LE PLUS IMPORTANT DE CETTE VERSION N'EST PAS LE BOUTON : DEUX TEMOINS SONT PASSES AU ROUGE TOUT SEULS, a 00 h 34, SANS QU'AUCUN CODE APPLICATIF AIT BOUGE. Cause : `today()` de l'app calcule le jour en heure LOCALE (state.js:529, « l'heure du telephone, pas Greenwich ») ; SIX fixtures de test le calculaient en UTC. Entre 22 h UTC et minuit, les deux ne designent plus le meme jour — et « demain » en UTC vaut « aujourd'hui » a Paris. ⭐ L'APP EST JUSTE, CE SONT LES TEMOINS QUI MENTAIENT : ils etaient verts 22 heures par jour et rouges 2 heures. Un temoin qui depend de l'heure a laquelle on le lance ne protege rien — il rassure. C'est la famille « fuseaux horaires » de BUGS.md, cette fois DANS l'outil de mesure. ⛔ COMPTER LES ENDROITS (6e fois cette semaine) : 5 fixtures dans parcours + 1 dans calculs, dont UNE SEULE rougissait ce soir. Les 5 autres etaient latentes. Toutes repartent desormais du `today()` de l'app et marchent A MIDI, comme `journalNav` — une seule definition du jour (R2). 🧪 Benchmark : 16 → 21 scenarios, 5 pieges promus depuis JOURNAL-DE-TEST, tous VECUS en salle.
const PRECACHE = [
