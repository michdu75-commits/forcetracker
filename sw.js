 ...
 */
const CACHE = 'ft-v1027'; // 🧾 ft-v1027 = DEUX MOYENNES JUSTES QUI SE CONTREDISAIENT A L'ECRAN. Trouve sur une VRAIE capture de Michel, pas en relisant le code : la carte « ce que l'app a appris » annoncait « en moyenne 1920 kcal » et, 40 px plus bas, « Ta semaine · 2 495 kcal/j ». ⭐⭐ LES DEUX SONT EXACTS — la premiere porte sur l'HISTORIQUE ENTIER (7 jours notes etales sur 50), la seconde sur les 7 DERNIERS jours — mais RIEN ne le disait, et personne ne peut le deviner. C'est la famille « deux sources qui se contredisent » de BUGS.md, et elle est pire que l'absence : la personne VOIT les deux. 👉 La carte NOMME desormais sa fenetre (« Observe sur tout ton journal »). ⚠️ Deux autres defauts de la meme carte, vus sur la meme capture : « repartis sur 50 » etait un nombre NU (50 quoi ? l'unite etait dans ma tete) → « etales sur 50 jours » ; et « 1920 kcal » n'avait pas son separateur de milliers a cote d'un « 2 495 » qui en a un — ca se lit comme une coquille, pas comme une mesure. ⛔ AUCUN CHIFFRE N'A CHANGE : on ne corrige que ce que l'ecran DIT de ses chiffres.
const PRECACHE = [
