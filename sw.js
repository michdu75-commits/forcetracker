 ...
 */
const CACHE = 'ft-v1087'; // 🛡️ ft-v1087 = UN RECORD PLUS VIEUX QUE L'HISTORIQUE DE SON EXERCICE EST INTOUCHABLE. TROUVE PAR MICHEL SUR SES VRAIES DONNEES, apercu a l'appui, deux heures apres la livraison de l'outil (ft-v1085). ⛔⛔ LA REGLE ③ NE PROTEGEAIT QUE « AUCUNE SEANCE DU TOUT » — trop etroit. Son `Developpe Decline` porte un record du **14/06**, or l'app est nee le **17 juin** : la seance qui l'a fait ne PEUT PAS etre dans l'historique, elle vient d'un import ou d'une saisie. Mais l'exercice, lui, a des seances plus RECENTES → il echappait a la protection et tombait dans « baisse ». ***Appliquer aurait efface un vrai record.*** 👉 UN RECORD ANTERIEUR A LA PLUS ANCIENNE SEANCE DE SON EXERCICE — OU SANS DATE — EST INVERIFIABLE PAR CONSTRUCTION. Il est garde tel quel et affiche a part, avec sa date ET celle de la 1re seance connue : *un refus muet ressemble a une absence de probleme.* ⛔ ET LA PROTECTION NE VAUT QUE POUR UNE BAISSE : une montee est prouvee par l'historique lui-meme, elle ne detruit rien — le Pec Deck, tout aussi « ancien », monte toujours. ⚠️ La plus ancienne apparition se compte sur TOUTES les series, echauffement compris : on ne cherche pas une performance, on cherche depuis quand l'app VOIT cet exercice. ⚠️ ft-v1086 = le lot de session-A (les quatre petits defauts de l'audit) — son recit est dans CLAUDE.md.
const PRECACHE = [
