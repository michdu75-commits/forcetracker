 ...
 */
const CACHE = 'ft-v981'; // 🧮 ft-v981 = LES DEUX BUGS DE CALCUL DE L'AUDIT. Michel : « je vois qu'il y a pas mal de problemes que je n'avais pas vu », puis « fais tout ce que tu peux, je veux que Milo soit fiable ». ⛔⛔ 1er BUG — L'OBJECTIF « EQUILIBRE » RECEVAIT +350 KCAL : la ligne s'ecrivait `{…equilibre:0…}[goal]||350`, et en JavaScript **0 est faux**, donc `0||350` rend 350. MESURE sur un profil a 2 740 kcal de TDEE : « equilibre » rendait **3 190 kcal, exactement la valeur de « prise de muscle »** — quelqu'un qui choisit « maintien » recevait une cible de prise de masse, +350 kcal/jour, sans que rien ne le signale. Corrige a **2 840** = TDEE + phase, ecart 0. ⚠️ ET LA TABLE ETAIT DUPLIQUEE (state.js:820 ET screens.js:1963, mot pour mot) : corriger celle que l'audit nomme aurait laisse l'ecran annoncer un ecart et le moteur en appliquer un autre. Un seul proprietaire desormais, `goalDeltaKcal()` (R2). ⛔ Le repli a 350 est CONSERVE pour un objectif inconnu : on teste l'APPARTENANCE, ce qui distingue « absent » de « vaut zero ». ⛔⛔ 2e BUG — KATCH LISAIT UNE CLE QUI N'EXISTE PAS : `leanMassRecente()` cherchait `w.bw` alors que TOUS les producteurs ecrivent `kg` (tracking.js 421·668·729·938·1504). Mesure : avec `kg` → mifflin, « aucune mesure de composition corporelle » ; avec `bw` → katch. La branche « pesee + % de gras → masse maigre » n'a JAMAIS tourne en production. ⚠️ Ce n'etait pas un chiffre faux, c'etait un meilleur calcul jamais active — une donnee morte ne plante pas, elle appauvrit en silence (R5). ⭐⭐ ET LES DEUX BUGS ETAIENT PROTEGES PAR DES FIXTURES FAUSSES : les tests ecrivaient `bw`, la production ecrit `kg`. Les fixtures ont ete corrigees AVANT le code, et elles ont ROUGI — c'est ce rouge qui prouve le bug, pas une relecture. Un test qui n'emploie pas le schema de la production ne teste rien, il rassure. ⭐ TROUVE EN VERIFIANT L'AUDIT, QUI NE L'AVAIT PAS VU : un 2e lecteur cassé, `_bilanMois()` (app.js:3816), lisait `pesees[0].bw` — la ligne « Poids de corps 85 → 84 kg » du bilan mensuel ne s'affichait donc JAMAIS, protegee elle aussi par sa propre fixture fausse. Une cle fausse ne se trouve jamais toute seule (R8). ⛔ Le repli `bw` reste lu : une sauvegarde cloud ancienne ne doit pas perdre sa mesure en corrigeant un bug.
const PRECACHE = [
