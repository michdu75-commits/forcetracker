 ...
 */
const CACHE = 'ft-v921'; // 🦵 ft-v921 = LES ADDUCTEURS EXISTENT ENFIN — l'adduction comptait pour du FESSIER. Michel, en me reprenant : « Abducteur/Adducteur ce n'est pas pareil hein ». Exact, et sa remarque a ouvert un vrai bug : mesure faite, `Adduction Cuisses` rendait `glutes:2 quads:1` — la machine qui ramene la cuisse vers l'INTERIEUR colorait les FESSIERS. ⚠️⚠️ ET LE MANQUE ETAIT ECRIT, DATE, ET IL ATTENDAIT MICHEL : le 02/08, la relecture des 58 fiches Jambes s'etait arretee dessus en toutes lettres — « les ADDUCTEURS n'existent pas dans la figurine (17 muscles)… ⏭️ Ajouter un muscle change ce que voit l'utilisateur, donc c'est l'arbitrage de Michel (R29) ». ⭐ La bonne decision d'alors : on n'a PAS bricole, on a NOMME le manque et on a attendu. ⚠️ Et le DESSIN les connaissait deja (`front_adductor_left/right`, etiquette « Adducteurs ») — ils etaient juste rattaches au groupe « Flechisseurs », donc AUCUN exercice ne pouvait les allumer. C'est R31 : la figurine est le VOCABULAIRE, un muscle absent du vocabulaire est un muscle dont aucun module ne peut parler. 👉 Groupe `adductors` cree (18 → 19 codes), 5 fiches reprises (adduction en MOTEUR ; sumo, cossack, fentes laterales en SECONDAIRE — les moteurs restent quadriceps et fessiers). ⚠️ L'ABDUCTION ne bouge PAS : mouvement OPPOSE, moyen fessier, et un temoin jumeau le verifie. ⭐⭐ ET J'AVAIS D'ABORD AJOUTE MA REGLE DERRIERE UNE REGLE FAUSSE : `_MEX` s'arrete au PREMIER match, la regle 29 disait deja `glutes` et gagnait. C'est la famille de bugs n°1 du projet (≥12 fois). Corrige EN PLACE. ⭐⭐ ET UN TEMOIN A FAIT SON TRAVAIL EN ROUGISSANT : il FIGEAIT le manque (« l'ADDUCTION reste fausse ») — un test protege une ABSENCE aussi bien qu'une presence (R30) ; quand l'absence est comblee, il change de camp.
const PRECACHE = [
