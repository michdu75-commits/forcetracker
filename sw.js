 ...
 */
const CACHE = 'ft-v1026'; // 🏋️ ft-v1026 = LES TYPES DE SEANCES REMPLISSENT L'ECRAN VIDE. §2.1 du parcours de decouverte (docs/SEANCE-DESSAI.md), et la plainte d'ORIGINE de Michel : « quand on arrive c'est vide » — que ranger les imports (ft-v1024) avait mecaniquement AGRANDIE. ⛔⛔ LE VRAI TRAVAIL EST R4, DANS SA FORME LA PLUS PURE : `DISC_CADRE.coeur` dit « SQUAT · DEVELOPPE COUCHE · SOULEVE DE TERRE — les 3 mouvements passent avant tout le reste ». C'est exact, utile, et TOTALEMENT INEXPLOITABLE : de la PROSE, dont aucun code ne peut tirer une seance. *L'information restait dans le TEXTE et n'atteignait jamais la DONNEE.* `DISC_SEANCE` est la descente manquante. ⛔ AUCUNE 2e LISTE DE TYPES (R2) : les 5 viennent de DISC_LABELS, le cadre chiffre de DISC_CADRE — le doc de cadrage l'interdit nommement, elles divergeraient et Milo lirait l'une pendant que l'ecran afficherait l'autre. ⭐⭐ MICHEL A TRANCHE « LES 2 CARREMENT » entre (a) un tap → la seance et (c) un ecran qui explique : la carte porte la ligne CHIFFREE du cadre (l'info de c) et le tap CREE la seance (le geste de a) ; le cadre COMPLET reste a un tap du « ⓘ » (R25). ⭐ Les reps ET le repos de chaque exercice SUIVENT la discipline — c'est toute la difference entre les types : deux seances peuvent partager un exercice, jamais le nombre de repetitions. Mesure : powerlifting → 3 reps, 300 s. ⛔ Les 23 exercices sont VERIFIES CONTRE LE CATALOGUE, pas ecrits de memoire (la lecon de ft-v1023, ou 7 cibles sur 16 etaient fausses). ⚠️ RESERVE ECRITE : le catalogue n'a PAS l'arrache complet, seulement `Arrache Debout (Muscle Snatch)` — la seance halterophilie est une seance de TRAVAIL, pas de competition. On ne fait pas semblant d'avoir un mouvement qu'on n'a pas. ⛔ ON N'ECRASE JAMAIS UNE SEANCE EN COURS (regle d'or #3) et les cartes DISPARAISSENT des qu'un exercice est pose : elles proposent un depart, elles ne commentent pas un travail en cours. ⚠️⚠️ ET LA CAPTURE A MONTRE UN DEFAUT QUE LA MESURE TEXTE NE VOYAIT PAS : ma 1re ligne courte tronquait la prose a 42 caracteres et sortait « jusqu'a 15- » et « 90 a 1 ». *Une phrase coupee en plein milieu n'est pas une information courte, c'est une information FAUSSE.* On extrait desormais le MOTIF CHIFFRE, exact quelle que soit la phrase autour. ⛔ Le bouton central « + » n'a pas bouge d'un pixel : [152,880,63] avant et apres.
const PRECACHE = [
