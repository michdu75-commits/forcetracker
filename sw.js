 ...
 */
const CACHE = 'ft-v1045'; // 📊 ft-v1045 = LE VOLUME PAR GROUPE MUSCULAIRE ET PAR SEMAINE. Dernier morceau du relais de session-A. ⛔⛔ R8 dans sa forme la plus pure : `DISC_CADRE.volume` dit a Milo « 10 a 20 series de travail par groupe musculaire et par semaine », il recoit cette regle depuis toujours et n'a JAMAIS su combien la personne en avait fait — une consigne qui NOMME une source absente du contexte. ⛔ Rien de neuf n'est invente (R2/R31) : `_MG` porte deja les ~22 GROUPES et `exMuscles` la donnee ecrite ; on credite le muscle PRIMAIRE seulement (compter le secondaire donnerait 4 series de triceps pour un developpe couche). ⛔⛔ ET LA DECISION CENTRALE EST UN SILENCE : aucune cible a l'ECRAN — un mercredi, tout le monde est sous son cadre, ce serait un reproche sur une semaine inachevee (le defaut refuse en ft-v1022 et ft-v1029). Milo, lui, recoit les deux cotes et sait quel jour on est. ⚠️ Sous-comptage silencieux trouve a la mesure : « Traction » et « Presse a Cuisses » sont des PREFIXES ambigus qui ne resolvent pas — leurs series sont desormais comptees A PART et nommees, jamais perdues. ⚠️ Et seules muscu/bodybuilding expriment le volume par muscle et par semaine : les 3 autres disent autre chose, Milo en est prevenu.
const PRECACHE = [
