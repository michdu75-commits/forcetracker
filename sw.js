 ...
 */
const CACHE = 'ft-v994'; // 🧪 ft-v994 = LE BANC D'ESSAI PASSE DE 21 A 50 SCENARIOS. Michel : « il n'y a pas assez de controle, on le monte a 50 » — puis, aussitot : « et que les scenarios soient VIABLES hein, pas mettre tout et n'importe quoi ». ⛔⛔ CETTE SECONDE PHRASE EST LE VRAI CAHIER DES CHARGES, et le journal de test le disait deja : « remplir pour atteindre le chiffre → des entrees inventees ». Les 29 nouveaux viennent donc TOUS du vecu (docs/JOURNAL-DE-TEST.md), aucun invente. ⭐⭐ ET CHACUN A ETE EPROUVE UN PAR UN contre une BONNE et une MAUVAISE reponse avant livraison — *un scenario qui ne peut pas rougir ne mesure rien*. Resultat du 1er jet : **6 sur 23 NE MORDAIENT PAS**. ⛔⛔ LE DEFAUT LE PLUS GRAVE ETAIT PLUS ANCIEN QUE MES SCENARIOS : l'APOSTROPHE COURBE (U+2019), celle que Milo ecrit naturellement en francais, n'etait pas normalisee par `U.norm` — `normalize('NFD')` ne la convertit pas. Un motif ecrit `c'est note` ne matchait donc JAMAIS. **8 motifs du fichier** portent une apostrophe : ils ne rougissaient pas, ils NE VOYAIENT RIEN. Corrige dans `U.norm` et nulle part ailleurs (R2) — les reprendre un par un aurait laisse passer le suivant. ⭐ 2e DEFAUT : trois verificateurs comptaient des LIGNES, alors que Milo ecrit souvent toute la seance SUR UNE SEULE LIGNE — le temoin des « 30 exercices » voyait alors UN exercice et restait vert sur dix. ⚠️ ET 4 DES 6 ECHECS VENAIENT DE MES PROPRES ESSAIS, pas du code : je testais avec « C est note » (espace au lieu d'apostrophe) et des noms d'exercices tronques que le lexique refuse VOLONTAIREMENT (« uniquement des noms non ambigus, pour ne jamais rougir a tort »). ⚠️ COUT A CONNAITRE : 50 scenarios = 50 appels API par passe, contre 21 avant. ⚠️ ET LA LIMITE NE BOUGE PAS : ces verificateurs mesurent ce qui est mesurable PAR DU CODE. Le ton, le naturel, « est-ce que Milo est agreable » restent au juge humain — aucun de ces 50 ne le remplace.
const PRECACHE = [
