 ...
 */
const CACHE = 'ft-v1059'; // 📤 ft-v1059 = L'EXPORT DE L'HISTORIQUE NE MENT PLUS SUR SA REUSSITE. Michel : « le bouton exporter dans historique ne fonctionne pas ». ⛔⛔ MESURE D'ABORD : toute la chaine marche en Chromium (bouton visible et tappable, modale ouverte, CSV telecharge, 0 erreur JS) — donc c'etait specifique a son iPhone, et il fallait le savoir avant de chercher. ⭐⭐ LA CAUSE EST MESURABLE DANS LE CODE, pas devinee : sur les 7 endroits de l'app qui livrent un fichier, `_donnerFichier` (ft-v1048) etait LE SEUL a passer un type portant `;charset=utf-8`. Les six autres — le CSV du banc d'essai, les PDF de seance et de programme — utilisent des types PROPRES, et eux marchent sur son telephone. *`navigator.canShare` d'iOS ne reconnait pas un type parametre et rend false* → repli sur `<a download>`, qu'iOS N'HONORE PAS en PWA plein ecran : rien ne se passe, aucune erreur. ⛔ Et le charset y etait DECORATIF : c'est le BOM dans les octets qui fait lire l'UTF-8 a Excel, pas le type MIME d'un fichier qu'on partage — il n'y a aucun en-tete HTTP dans cette histoire. Un temoin lit le VRAI fichier telecharge et fige le BOM. ⛔⛔ ET LE 3e DEFAUT EST INDEPENDANT DE LA PLATEFORME : `return true` apres `a.click()`, sans rien savoir — donc le toast annoncait « 2 series exportees » alors que rien n'etait parti. *Un succes menteur est pire qu'un echec* : il fait chercher le probleme au mauvais endroit. Quatre issues distinctes maintenant (reussite · annulation MUETTE · incertitude nommee · echec). ⛔ R2 : le PDF recopiait la livraison, donc il portait les MEMES 3 defauts — il passe par le meme proprietaire, et un temoin refuse toute 2e livraison recopiee a cote.
const PRECACHE = [
