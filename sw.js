 ...
 */
const CACHE = 'ft-v1085'; // 🏅 ft-v1085 = RECALCULER LES RECORDS DEPUIS L'HISTORIQUE · ET LE 429 QUI DISAIT « VERIFIE TA CONNEXION ». ⭐ UN FAUX RECORD MESURE DANS SES DONNEES : `Rowing Hammer Strength` portait 66 kg x 8 -> 81,9 kg, c'est-a-dire EXACTEMENT les chiffres de son Tirage Poulie Haute, meme date — le fantome laisse par le renommage de ft-v1073. Il a repare la seance, le bon record du Tirage est revenu, MAIS LE FANTOME EST RESTE. Et comme il est faux VERS LE HAUT, il ne sera jamais battu : il resterait pour toujours, en servant de reference aux charges que Milo propose, a la montee en charge que l'app ajoute et au % du max dans le calcul des calories. ⛔⛔ L'OUTIL NE SUPPRIME JAMAIS UN RECORD QU'IL NE PEUT PAS VERIFIER : un exercice sans aucune serie dans l'historique (import ancien, autre appareil, seance effacee) garde son record TEL QUEL et est liste a part. *Recalculer, c'est corriger ce qu'on peut prouver, pas effacer ce qu'on ne retrouve pas.* ⛔ APERCU D'ABORD, le bouton qui ecrit naît de l'apercu (R29), et la regle d'eligibilite est celle de la PRODUCTION (`_serieFaitFoiPourPR`, R2) — un recalcul avec sa propre regle fabriquerait un 2e historique, pas une correction. ⚠️⚠️ ET LE 429 : Michel, en pleine seance, « Erreur : HTTP 429. Verifie ta connexion et reessaie » avec 5G et 97 % de batterie. Le worker ecrivait la vraie raison DANS LE CORPS de la reponse et l'app la jetait pour n'afficher que le code HTTP (R4). ⚠️⚠️ J'AI D'ABORD ANNONCE « TON QUOTA DU JOUR EST EPUISE » — FAUX, prouve par trois mots de Michel : « il a repondu apres ». Le plafond du worker est un drapeau tenu EN MEMOIRE DE CHAQUE ISOLAT Cloudflare, et le worker l'ecrit lui-meme : « approximatif par construction ». ⛔ CE ROUGE A PRODUIT UN GARDE-FOU : le corps porte aussi des jetons ecrits pour le CODE (`quota`, `rate_limit`) — les afficher aurait remplace un message faux par un message incomprehensible. `_phraseServeur` est le seul proprietaire de « ce texte est-il adresse a un humain ? ».
const PRECACHE = [
