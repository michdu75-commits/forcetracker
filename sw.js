 ...
 */
const CACHE = 'ft-v900'; // 🔄 ft-v900 = LE PLAN ALIMENTAIRE CHANGE TOUS LES JOURS — et le garde-fou a trouve DEUX bugs de securite qui existaient DEJA. Chaque repas porte desormais 3 variantes, choisies par la DATE : ca change chaque jour et c'est stable dans la journee. ⚠️⚠️ AUCUN APPEL IA, et c'est un choix : le bloc est calcule par l'app, HORS LIGNE et gratuitement (le « Plan de repas IA » existe deja, separement). ⚠️⚠️ LA REGLE DE SECURITE DE LA BRIQUE : les variantes n'emploient QUE des aliments deja presents dans les plans. Chaque aliment doit etre connu des tables de substitution (_DIET_SWAPS) ET d'allergenes — sinon un vegan voit de la viande. En n'introduisant AUCUN mot nouveau, le risque est nul par construction. ⭐⭐ ET LE GARDE-FOU A TROUVE DEUX DEFAUTS PRE-EXISTANTS, pas les miens : « Thon » SEUL n'etait couvert par AUCUNE substitution (un vegetarien voyait du thon dans « Pates completes + thon », depuis toujours) et le MIEL n'etait remplace nulle part alors que ce n'est pas vegan. Corriges : Thon rejoint la regle POISSON (pas la regle viande, sinon « Poulet/thon » serait touche deux fois), et le miel devient des dattes — un aliment deja connu des tables, donc aucun mot nouveau. ⚠️ LE TEST PARCOURT LES 3 VARIANTES DE TOUS LES PLANS, sur 40 jours x 6 objectifs : sans ca il ne verifierait que la variante du JOUR OU IL TOURNE, et une variante dangereuse ne sortirait que certains jours. C'est le bug d'Emma du 02/08 avec un calendrier par-dessus. getMeals() accepte un jour force, qui n'existe QUE pour ca.
const PRECACHE = [
