 ...
 */
const CACHE = 'ft-v1053'; // ⚡ ft-v1053 = « CETTE SEANCE TE CONVIENT ? » — LE LANCEMENT EST FIGE. Michel apres TROIS pannes du bouton en 8 jours : « il faut absolument figer le fait d'avoir toujours la seance lancee quand on lui demande une seance ». ⛔⛔ LES 3 PANNES ONT LA MEME CAUSE DE FOND : la presence du bouton dependait de la FORME de ce que Milo ecrivait (le nom sur la meme ligne le 20/08 · « 95×3 » lu comme 95 series en ft-v1049 · le `break` de ft-v1051). On ne peut pas enumerer d'avance toutes les facons dont un modele ecrit une seance — donc tant que le declencheur est la REPONSE, il y aura une 4e panne. ⭐⭐ LE DECLENCHEUR DEVIENT LA DEMANDE DE LA PERSONNE, une chose qu'on connait avec certitude parce qu'elle l'a tapee. ⛔ ZERO GESTE EN PLUS (condition de Michel) : « Oui, on demarre » est le meme bouton rouge, meme place, pleine largeur — ce qu'on GAGNE est le « Non, retravaille » et ses raisons en un tap. ⚠️ VOCABULAIRE CORRIGE PAR MICHEL : « un programme c'est une chose et juste la seance en est une autre » — le libelle dit SEANCE. ⛔ Et l'echec se DIT : si la seance reste illisible, l'app le nomme et propose de la faire reecrire, jamais un bouton mort.
const PRECACHE = [
