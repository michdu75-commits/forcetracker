 ...
 */
const CACHE = 'ft-v1102'; // 🏁 ft-v1102 = LA REFONTE DE L'ONGLET NUTRITION + LE MOTEUR DE TENDANCE DETERMINISTE. Decision produit relayee par Michel (20 points). ⛔⛔ « X KCAL RESTANTES » CESSE D'ETRE L'INFORMATION DOMINANTE : le calcul ne change pas d'une virgule, il est traduit en aliments reels juste dessous. ⭐ Sous chaque anneau, un mot dit POURQUOI cette cible-la aujourd'hui (cible haute/basse selon le jour, « la meme tous les jours » pour les proteines, « atteint ») — et AUCUN « pratiquement atteint » : ce mot exigerait un seuil qu'aucune source du projet ne fournit. 📈 NOUVELLE CARTE « TON EVOLUTION » : poids + charges + repas notes sur 14 jours, 4 etats, ZERO appel API (mesure : 0 requete sortante au rendu dans les 4 etats). ⛔ Dans « donnees insuffisantes » : aucune fleche, aucun pourcentage — une fleche EST deja une conclusion. ⛔ Milo n'apparait que dans l'etat AMBIGU, sur appui volontaire, et il recoit les tendances jamais le journal alimentaire. ⛔⛔ ET UN VIEUX DEFAUT MAJEUR TROUVE EN CHEMIN : `linearRegression` recevait l'INDEX du point comme abscisse, donc « slope*7 » voulait dire « sur 7 PESEES » et non 7 jours — la meme evolution reelle (+0,20 kg/sem) sortait a +0,20 / +0,40 / +0,60 / +1,40 selon la frequence de pesee. Quelqu'un qui se pesait le dimanche voyait sa tendance MULTIPLIEE PAR SEPT, et Milo la recevait telle quelle. ⭐ ft-v1100 n'a pas cree ce defaut, il l'a RENDU VISIBLE : un garde-fou juste revele les mesures fausses qu'il consomme. ⭐ MESURE avant/apres : onglet 1939 -> 2140 px (plafond 2600), « Noter ce que je mange » INCHANGE AU PIXEL (525 px), carte du jour inchangee (445 px) en portant 4 informations de plus. Tests : parcours 2389 ✅ (bloc CCX, +18), calculs 266, muscles 241, croises 50, dates 7, donnees 0 trou.
