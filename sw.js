 ...
 */
const CACHE = 'ft-v1050'; // 📅 ft-v1050 = LA SEANCE PREVUE QUI N'A PAS EU LIEU. Methode de la coach de Michel, dans ses mots : « si elle est loupee elle est loupee, c'est pas grave sur une semaine. PLUTOT ELLE DEMANDE CE QUI S'EST PASSE — fatigue, travail, empechement, ca peut arriver. » ⭐⭐ Sa reponse nomme un TROISIEME comportement : ni rattraper (culpabilisant), ni se taire (indifferent) — POSER UNE QUESTION, avec ses reponses deja legitimees dedans. ⛔⛔ Et l'app faisait PIRE que « ne rien dire » : l'Accueil executait `S.nextPlanned=null; persist()` des que la date etait passee — ELLE EFFACAIT LA TRACE EN SILENCE. Rien pour la personne, rien pour Milo (R5). ⛔ ZERO APPEL API (consigne de Michel : « pas d'IA surtout ») — mesure : le compteur reste arme depuis le debut, il voit les 2 appels du DEMARRAGE (ce qui prouve qu'il ecoute) et la fonctionnalite n'en ajoute AUCUN. ⛔ Jamais de rattrapage propose, aucun total affiche, jamais deux fois pour la meme date (R24, R12, P21). La croix vaut « je ne veux pas le dire » : le FAIT est garde, aucun motif invente (R29). ⭐⭐ Et le temoin d'anti-fuite du banc d'essai (ft-v1014) a REFUSE la livraison : `missedLog` n'etait pas remis a zero dans `_vcApplyPersona`. En cherchant sa jumelle (R8), `nextPlanned` fuitait DEJA depuis ft-v654 — un garde-fou pose apres coup ne voit pas ce qui est passe avant lui. ⚠️ Defaut trouve A LA CAPTURE : « Pas la tete » DEBORDAIT de sa pastille de 47 px, invisible dans le texte rendu.
const PRECACHE = [
