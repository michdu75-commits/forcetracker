 ...
 */
const CACHE = 'ft-v990'; // 💰 ft-v990 = INSTRUMENTATION DU COUT REEL PAR APPEL API. Priorite 3 tranchee par Michel apres le contre-audit du 24/08, EN PARALLELE de la validation unique (①②, ft-v989) : « instrumentation fine du cout reel par appel API ». Le prompt de Milo n'avait jamais ete mesure qu'en CARACTERES (AUDIT-CONTEXTE-MILO.md) — jamais en tokens reels, faute de cle API dans l'environnement d'analyse. C'est la seule facon de savoir un jour si un chantier de cache (④⑤, R34) rapporte quoi que ce soit. ⛔⛔ NE CHANGE STRICTEMENT RIEN AU COMPORTEMENT DE MILO : lecture seule de `data.usage`, un champ que l'API Anthropic renvoie deja a CHAQUE appel et que worker.js jetait jusqu'ici. Capture au SEUL point commun (callClaude + callClaudeDiag — cette derniere est la fonction de PRODUCTION de la conversation, coach() l'appelle TOUJOURS, malgre son nom qui laisse croire a un chemin de test). ⛔ MEME CHEMIN QUE _compterIA (R2, pas un 2e canal de telemetrie) : fire-and-forget vers Apps Script, repli OUVERT — une panne de mesure ne bloque jamais Milo (regle d'or #3). ⛔⛔ COTE Code.js, MEME MECANIQUE QUE ai_quota (R2/R13) : une propriete JSON BORNEE, remise a zero chaque jour — jamais un historique qui grossit, c'est exactement la lecon du reservoir plein a 102 % du 29/07. Le jeton de securite est FACTORISE (_countTokenArme_) plutot que duplique : la meme empreinte protege desormais aiCount ET aiUsageLog. 💶 LE COUT EN EUROS EST UNE ESTIMATION, ECRIT COMME TEL (R29) : les tarifs sont repris de tests/milo/eval.js (seule source de prix du depot), et le coefficient de cache-ecriture (1,25×) suppose une fenetre 5 min — le bloc commun utilise parfois 1h, donc l'estimation sous-estime legerement les jours ou il est reecrit. Les TOKENS, eux, sont exacts. ⭐⭐ VERIFIE FONCTIONNELLEMENT, PAS SEULEMENT EN LECTURE : le vrai Code.js execute dans un bac a sable Node (PropertiesService/Utilities/Session stubbes) confirme l'accumulation, la remise a zero quotidienne, le rejet d'un modele inconnu (jamais de prix invente) et le refus d'un mauvais jeton. ⚠️ CE QUI NE PEUT PAS ETRE VERIFIE ICI, ET C'EST ECRIT : un vrai appel facture a l'API Anthropic, indisponible dans cet environnement — la premiere vraie donnee arrivera au premier appel de Michel en prod.
const PRECACHE = [
