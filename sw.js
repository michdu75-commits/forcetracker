 ...
 */
const CACHE = 'ft-v927'; // 📋 ft-v927 = UN DEBRIEF COUVRE TOUS LES EXERCICES. Michel, apres sa seance : Milo a debriefe 3 exercices sur 5, en sautant le FACE PULL — qu'il avait pourtant prescrit lui-meme « indispensable pour l'epaule droite » — et le crunch. ⭐ VERIFIE AVANT DE CORRIGER : les 5 etaient bien TRANSMIS dans son contexte. Ce n'est donc pas une perte de donnee (R4), c'est un choix de redaction. ⚠️ SON ARGUMENT EST IMPARABLE : « un debrief c'est un debrief », et surtout « j'ai eu le debrief de fin de seance avec tout ce qui a ete fait » — l'app en montre 5, Milo en montre 3, et les deux se contredisent (R2). 👉 Regle ajoutee : un debrief couvre TOUS les exercices, une ligne suffit quand il n'y a rien a dire, mais l'exercice doit APPARAITRE ; et un exercice qui PROTEGE une zone fragile declaree ne se saute JAMAIS. ⭐ ET ON LUI DONNE LE COMPTE (« 5 exercices ») au lieu de lui demander de compter dans une ligne dense : une consigne qui dit « n'en saute aucun » sans le nombre est un prompt qui compense une donnee absente (R8). ⚠️ Le motif est le meme que ft-v923 : Milo traite les petits accessoires comme negligeables — il les range au hasard dans l'ordre, et il les oublie au debrief.
const PRECACHE = [
