 ...
 */
const CACHE = 'ft-v1082'; // 🔥 ft-v1082 = LE REPOS D'UN PALIER SUIT SA CHARGE, ET LE PLAFOND DE PALIERS AUSSI. Document de revue des echauffements relaye par Michel, puis « vas-y go ». ⛔⛔ LES 4 CAS « PRIORITE HAUTE » DU DOCUMENT SONT DEJA REPARES — rejoues dans un navigateur, pas relus dans le journal : Tirage Poulie Haute du 16/08, Pec Deck (0 palier, *accessoire*), Developpe Epaules (1 approche, pas 3), RDL apres squat (0 ajout). *La question centrale — « pourquoi 5 chauffes le 16 aout et 1 le 20 ? » — a pour reponse une DATE, pas une regle instable : ft-v887, le 17/08.* ⛔ CE QUI RESTAIT, MESURE AVANT DE TOUCHER AU CODE : ① `restByType` rendait **45 s a plat** entre paliers — sur une montee vers 130 kg, `100x2 -> 115x1` (**88 %** de la charge du jour) donnait 45 secondes ; ② et le 2e defaut est sorti de la MESURE, pas du document — le plafond de `_monteeCompletee` etait un **5 en dur**, donc **5 paliers et 19 repetitions d'echauffement de 60 kg a 150 kg, a l'identique**. *Un squat a 60 kg recevait le protocole d'un squat a 150.* ⭐ AUCUN CHIFFRE NEUF (R2/R13) : le repos reutilise les zones des repetitions (`_PALIER_ZONES`) et le plafond APPELLE le bareme — un seul proprietaire de chaque question. ⛔ DEUX BORNES : jamais plus court qu'avant (45 s reste le plancher), jamais plus long que le repos de travail REGLE par la personne. Et le nombre de series ne diminue jamais (invariant du 11/08). ⚠️ ET `startRest()` VIDE `#rest-label` : les libelles « Echauffement » / « Recup. a l'echec » sont muets DEPUIS TOUJOURS sur ce chemin — non repare expres, c'est un chantier a part (R30).
// ⬆️ 13e collision de la semaine : session-B a publie SA ft-v1081 (les doublons du classeur)
// pendant ce travail. Ma version devient ft-v1082 — UN NUMERO DE CACHE NE RECULE JAMAIS, et
// on ne renumerote pas le travail de l'autre : leur ft-v1081 reste la leur, avec son entree.
const PRECACHE = [
