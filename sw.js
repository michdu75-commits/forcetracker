 ...
 */
const CACHE = 'ft-v1047'; // 📅 ft-v1047 = LA SEANCE PREVUE QUI N'A PAS EU LIEU. Methode de la coach de Michel, dans ses mots : « si elle est loupee elle est loupee, c'est pas grave sur une semaine. PLUTOT ELLE DEMANDE CE QUI S'EST PASSE — fatigue, travail, empechement, ca peut arriver. » ⭐⭐ Sa reponse nomme un TROISIEME comportement : ni rattraper (culpabilisant), ni se taire (indifferent) — POSER UNE QUESTION, avec ses reponses deja legitimees dedans. ⛔⛔ Et l'app faisait PIRE que « ne rien dire » : `plannedSession()` rend null des que la date est passee, puis l'Accueil executait `S.nextPlanned=null; persist()` — ELLE EFFACAIT LA TRACE EN SILENCE. Rien pour la personne, rien pour Milo (R5). ⛔ ZERO APPEL API (consigne de Michel : « pas d'IA surtout ») — tout est deterministe et local, un temoin compte les appels sortants. ⛔ Jamais de rattrapage propose, aucun total affiche, jamais deux fois pour la meme date (R24, P21). La croix vaut « je ne veux pas le dire » : le FAIT est garde, aucun motif invente (R29). ⚠️ Defaut trouve A LA CAPTURE : « Pas la tete » DEBORDAIT de sa pastille de 47 px — invisible dans le texte rendu.
const PRECACHE = [
