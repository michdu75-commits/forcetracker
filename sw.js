 ...
 */
const CACHE = 'ft-v1062'; // 🪟 ft-v1062 = UNE MODALE ENFERMEE DANS UN ECRAN QUI DEFILE S'OUVRE HORS DE L'ECRAN. Michel : « ca clique bien mais rien ne se passe » sur le bouton Exporter. ⛔⛔ LA CAUSE, MESUREE : `.overlay` est en **position:absolute**, donc elle se cale sur son ancetre POSITIONNE. A la racine c'est `.screens`, et `inset:0` couvre la fenetre. Mais un `.screen` est lui aussi `position:absolute` ET `overflow-y:auto` — enfermee dedans, `inset:0` designe le HAUT DU CONTENU, pas la fenetre. Ecran Progres defile a fond : la modale s'ouvrait a **y = −3102**, soit **2 800 px AU-DESSUS**. La classe `open` etait posee, la modale rendue, et il ne se passait RIEN. ⚠️⚠️ CE QUI REND CETTE PANNE COUTEUSE : elle est SILENCIEUSE (aucune erreur) et elle DEPEND DU DEFILEMENT — donc un test dont la fixture n'a qu'une seance ne defile pas et passe. *Mon propre test d'hier reproduisait le chemin heureux : c'est pour ca que ft-v1059 a corrige un vrai defaut, mais pas CELUI-LA.* ⚠️ JUMELLE TROUVEE EN LA CHERCHANT (R8) : `ov-rest-edit` dans `s-log` — le reglage du temps de repos, EN PLEINE SEANCE, meme cause, anterieure a moi. ⭐⭐ Mesure : sur **63 overlays, 61 etaient deja a la racine** — les 2 fautives etaient l'exception. Un temoin permanent refuse desormais toute `.overlay` a l'interieur d'un `.screen`, ET verifie le comportement ecran DEFILE A FOND (la condition qui manquait).
const PRECACHE = [
