 ...
 */
const CACHE = 'ft-v1000'; // 🔁 ft-v1000 = LE « PULL-OVER » GENERIQUE, ET UN TEST QUI M'A REPRIS. Michel renvoie la meme capture du selecteur sans un mot : la capture EST la demande. Le generique etait la seule ligne sans vignette, rangee en BARRE juste au-dessus de « Pull-over Barre ». ⛔⛔ MA PREMIERE SOLUTION A ETE REFUSEE PAR UN TEST DU PROJET, ET IL AVAIT RAISON. J'avais fait PARTAGER `pullover-haltere.webp` entre le generique et « Pull-over Haltere ». Le controle croise ② — « deux exercices ne partagent jamais la meme ANIMATION » — est passe au ROUGE. Cette regle est nee du bug du 02/08 (« Ecarte Halteres » et « Ecarte Decline » pointaient le meme fichier, l'app montrait le mauvais mouvement), et j'avais ecrit un temoin LE MATIN MEME disant « les deux ecartes gardent des fichiers differents ». *Un test permanent m'a empeche de refaire, deux heures plus tard, le bug que je venais de documenter.* C'est R17 qui paie : chaque bug devient un scenario permanent, y compris contre celui qui l'a ecrit. 👉 CE QUI EST LIVRE EST DONC LE SEUL GESTE DEFENDABLE : le BAC. Le generique n'a PAS de materiel, « barre » venait d'une regle de classement (`pull-?over` figure dans la liste barre) ; il passe en POIDS LIBRE, la version classique du pull-over sans precision. 0 octet, aucune donnee touchee, une ligne. ⛔ ET IL RESTE SANS VIGNETTE, EXPRES : aucune animation vaut mieux qu'une animation qui affirme « Pull-over = Pull-over Haltere » sans que ce soit decide. C'est la phrase du 02/08, appliquee a moi-meme. ⛔⛔ LE VRAI DEFAUT RESTE ENTIER ET IL EST ECRIT : DOUBLON DE CATALOGUE, 5 entrees pull-over pour un exercice qui, sans precision, n'a pas de materiel. Fusionner RENOMMERAIT les seances et records passes de Michel, et on ne sait pas s'il les a faits a la barre ou a l'haltere — deviner ecrirait un fait faux dans son historique (R29). Une vignette qui manque coute un coup d'oeil ; un historique renomme a tort coute une donnee qu'on croyait juste. La fusion attend SA reponse. ⭐⭐ ET LE TEMOIN LE PLUS IMPORTANT GARDE UNE ABSENCE : « le generique n'emprunte l'animation de personne » — lui en donner une exigera de trancher le doublon, pas de contourner la regle.
const PRECACHE = [
