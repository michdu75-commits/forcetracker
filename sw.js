 ...
 */
const CACHE = 'ft-v1011'; // ⏱️ ft-v1011 = L'EXPORT DES CONVERSATIONS PORTE ENFIN SES DATES. Michel, en relisant son propre export : « il va falloir horodater les conversations ». ⭐ ft-v1010 avait pose le `ts` A LA CREATION et l'avait fait survivre au stockage — mais PERSONNE ne le lisait : *une donnee ecrite que rien ne relit n'existe pas* (R5). C'est tres exactement ce qui m'a empeche de dater sa conversation du 19/08 quand il me l'a envoyee. ⛔⛔ ET LE TITRE MENTAIT : il affichait la date de CREATION de la discussion — celle de Michel, ouverte le 19/08, s'est poursuivie jusqu'au 25, six jours d'ecart, donc en la relisant on datait tout son contenu du 19. *Un repere faux est pire qu'un repere absent : on s'y fie.* Il affiche desormais la PLAGE reelle, et un seul jour reste un seul jour (jamais « du X au X »). ⛔ LE JOUR NE SE REPETE PAS A CHAQUE LIGNE : il s'ecrit quand il CHANGE, l'heure seule ensuite — dater les 287 messages noierait la conversation sous l'horodatage, et ce qu'on veut est retrouver un moment, pas remplir des colonnes (R19). ⛔ RIEN N'EST INVENTE POUR LES ANCIENS (R29) : un message sans `ts` n'affiche AUCUNE heure, et tous ceux d'avant ft-v1010 sont dans ce cas — mieux vaut un trou visible qu'une heure fausse qui aurait l'air vraie. ⭐ UN SEUL CONSTRUCTEUR, VERIFIE AVANT DE CODER : Michel a signale que le fichier venait du cote ADMIN, donc on a compte les endroits (lecon de ft-v973/975/984/996/1006) — une seule fonction fabrique ce fichier, le correctif ne pouvait pas etre pose du mauvais cote.
const PRECACHE = [
