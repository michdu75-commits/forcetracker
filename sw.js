 ...
 */
const CACHE = 'ft-v1105'; // ♻️ ft-v1105 = LE BANC D'ESSAI RETROUVE SA MEMOIRE, ET CESSE DE FUIR. Michel : « corrige les 3 lignes du banc et les fuites » — suite du contre-audit du plan Milo Session Builder. ⛔⛔ ① TROIS CHAMPS ETAIENT FORCES A `null` EN DUR au milieu de 50 autres qui lisent la fixture (`S.wkt`, `S.cycle`, `S.dayState`) : un persona « il est fatigue, il a mal a l'epaule ce matin » etait IMPOSSIBLE A ECRIRE, un persona en cycle de force aussi, un persona avec une seance en cours aussi — c'est-a-dire les trois situations ou l'app en sait le PLUS sur la personne, donc celles ou sa memoire devrait le plus se voir. ⛔⛔ ② QUATRE DONNEES DE LA VRAIE PERSONNE PARTAIENT DANS CHAQUE PERSONA : `exSwaps` (les exercices qu'elle remplace ET LA RAISON qu'elle a donnee), `programmes`, `fasting`, `foodMode`. 3e fois pour cette famille (`foodLog` ft-v1014, `missedLog`/`nextPlanned` ft-v1050). ⚠️ Elles etaient CONNUES et epinglees depuis ft-v1014, laissees en place au nom de R34 — ⭐ or cette raison ne tenait pas, et c'est mesurable : `_vcApplyPersona` n'a AUCUN appelant hors du banc d'essai. *Un report prudent finit par proteger le defaut lui-meme.* ⚠️⚠️ ③ ET `foodMode` EST LE CAS QUI APPREND QUELQUE CHOSE : ma 1re sonde l'a declare PROPRE parce que je l'avais essaye avec `keto`, dont l'ALIAS `S.keto` etait bien remis a zero. Or la regle cetogene lit `S.keto` quand les regles paleo/low carb/mediterraneen lisent `S.foodMode` — avec `paleo`, la fuite etait grande ouverte. *Une fuite refermee par un alias n'est pas refermee : elle est masquee par la valeur qu'on a choisie pour l'essayer.* Un seul proprietaire (R2) : `keto` se DERIVE de `foodMode`, comme `load()` le fait. ⛔ AUCUN ECRAN, AUCUN CALCUL DE L'APP N'EST TOUCHE — c'est du code de TEST. Tests : parcours 2437 (bloc CCXIV, +16 dont 6 controles), calculs 266, muscles 241, croises 50, dates 7, donnees 0 trou.
