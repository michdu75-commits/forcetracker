 ...
 */
const CACHE = 'ft-v1018'; // 📉📇 ft-v1017 + ft-v1018 = L'HISTORIQUE DU SCORE DE RECUP, ET L'EMAIL DANS LE CLASSEUR. Michel : « sur accueil et recuperation… j'ai l'impression qu'il n'y a pas d'historique ou c'est moi ? » — il avait raison, le score etait calcule, affiche, puis JETE. ⛔⛔ ON NE STOCKE RIEN, ON REJOUE : `calcRecoveryDetail(refTs)` se replace a un instant, et le score est une fonction DETERMINISTE de donnees deja la. Un journal de scores serait une 2e source de verite pour un chiffre calculable (R2) et ne commencerait qu'aujourd'hui ; en rejouant, la courbe est RETROACTIVE. ⭐⭐ ET TOUS LES POINTS SONT PRIS A LA MEME HEURE — mesure : memes nuits, une seance la veille a 18 h, le score va de 44 a 6 h a 56 a 22 h. 12 POINTS D'ECART SANS QUE RIEN DU CORPS N'AIT CHANGE. Comparer un matin a un soir montrerait L'HEURE DE LA JOURNEE, pas la recuperation. ⛔⛔ « la derniere seance » depend de QUAND on regarde : rejouer un jeudi avec la seance du samedi suivant penaliserait le jeudi pour un effort pas encore fourni. Idem pour une nuit notee apres coup. ⛔ Avant la 1re donnee : `null`, jamais la base neutre de 70 — ce serait un score d'apparence normale pour des jours ou la personne n'utilisait pas encore l'app (R29). ⛔ Et l'historique du SOMMEIL, lui, RESTE CACHE : c'est la decision de ft-v547 (« ca prend trop de place », Michel), reconfirmee le 26/08 — la courbe se glisse dans une rangee DEJA la, pas dans une carte de plus (R30). 📇 ft-v1018 : la ligne du classeur Google Sheets porte enfin son EMAIL — les seances de tous les testeurs s'empilaient dans le meme onglet `Sessions` sans identifiant, il existait pour etre lu et ne pouvait pas l'etre. ⛔⛔ La colonne va A LA FIN, jamais au debut : inserer en tete decalerait des milliers de lignes d'un cran, et un tableur ne le signale pas. Les vieilles lignes gardent une cellule VIDE — on ne sait pas de qui elles viennent, on ne l'invente pas (R29). ⛔ L'email est pose sur l'ENVELOPPE, pas sur chaque serie (R2).
const PRECACHE = [
