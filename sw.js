 ...
 */
const CACHE = 'ft-v977'; // 📐 ft-v977 = LE HEADER COMPACTE SORT DU PLACARD. Michel, aussitot apres le retrait du clone : « le header compacte oui, promeus-le ». ⭐ PREMIER DES CINQ ESSAIS PARQUES A ETRE TRANCHE : il attendait derriere `html.is-clone` depuis ft-v610, marque « a promouvoir si Michel valide », et le retrait du clone (ft-v976) venait de lui enlever son dernier moyen d'etre essaye — un essai parque sans porte de sortie finit oublie, pas decide (R30). ⚠️⚠️ ET LA PROMOTION AURAIT ETE UN NO-OP SILENCIEUX : en retirant `html.is-clone` les 4 regles PERDENT LEUR SPECIFICITE, or TROIS d'entre elles sont redefinies PLUS BAS dans style.css (.coach-header l.798, .coach-header-sub l.800, .coach-quota l.897). Empilees en haut du fichier elles auraient ete ECRASEES, et « c'est promu » aurait ete faux SANS QU'AUCUN TEST NE ROUGISSE. Les 4 valeurs sont donc ecrites DANS LA REGLE D'ORIGINE (R2 : deux regles qui se disputent la meme propriete, c'est une de trop). ⭐⭐ D'OU LE TEMOIN : il ne verifie pas que les regles EXISTENT, il verifie qu'elles GAGNENT — il lit le style CALCULE par le navigateur, pas le fichier. 🔴 REGLE D'OR #9, MESURE ET PAS REGARDE : le bouton central « + » est a 792 px AVANT et 792 px APRES, hauteur 44 -> 44, et il ne bouge pas davantage apres une navigation ; la nav reste a 770. 👉 LE GAIN VA AU BON ENDROIT : le header de Milo passe de 83 a 50 px = +45 px pour le fil de discussion et +12 px pour l'Accueil, barre de saisie inchangee. ⛔ L'IDENTITE NE BOUGE PAS : titre 21 px, sous-titre 13,5 px — le gain vient des ESPACEMENTS seuls (temoin dedie). ⛔ La marge HAUTE n'est pas reductible davantage : au-dessus du logo c'est la barre d'etat de l'iPhone. ⛔⛔ UN SECOND ESSAI EST DELIBEREMENT NON PROMU (ft-v611), raison ecrite dans coach.js : il raccourcissait « 8 questions gratuites » en « 8 questions » — quelques pixels gagnes SUR LE DOS DE CE QUE LA PERSONNE COMPREND DE SON COMPTE, « 8 questions » se lisant comme un plafond definitif. Un maintien sous garde s'ecrit comme un retrait (R30).
const PRECACHE = [
