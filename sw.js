 ...
 */
const CACHE = 'ft-v1081'; // 🔁 ft-v1081 = VOIR LES DOUBLONS DU CLASSEUR, SANS RIEN SUPPRIMER. Michel : « comment verifier si ma seance a ete ecrite plusieurs fois ? ». ⚠️ POURQUOI ILS EXISTENT : avant ft-v1077, une grosse seance depassait les 8 s d'attente du telephone — qui abandonnait **pendant que le script Google finissait d'ecrire**. Chaque nouvel essai re-collait donc les memes lignes. La cause est corrigee ; ce qui a deja ete ecrit, non. ⛔⛔ LE TEMOIN CENTRAL N'EST PAS LE COMPTAGE, C'EST QUE LA ROUTE N'ECRIT RIEN (**R29**) : le classeur porte les donnees de la personne, on lui MONTRE ce qu'on voit, elle decide ensuite. *Un detecteur qui nettoie d'office est exactement le geste qu'on refuse.* ⛔ UNE SEULE LECTURE du classeur (`getDataRange`), pas une par ligne — la lecon de ft-v1077 appliquee au code neuf. ⛔ ET ON DIT CE QU'ON NE PEUT PAS VOIR : les lignes ecrites avant ft-v1018 n'ont pas d'email, donc elles ne sont ni comptees ni accusees — mais leur nombre est AFFICHE, sinon un total qui ne colle pas passerait pour une erreur de l'outil. ⛔ Jeton admin obligatoire : cette route lit l'entrainement de quelqu'un. ⚠️ Et un temoin de REGLE au passage : chaque groupe de l'Admin doit annoncer le VRAI nombre de cartes qu'il porte — j'ai failli livrer « 4 » sur un groupe qui en compte 5.
// ⬆️ 12e collision : session-A a publie SA ft-v1080 (la seance bizarre de Milo, deux
// charnieres de hanche) pendant ce travail. Ma version devient ft-v1081 — UN NUMERO DE
// CACHE NE RECULE JAMAIS, et on ne renumerote pas le travail de l'autre : leur ft-v1080
// reste la leur, avec son entree de journal. (Idem le 30/08 avec leur ft-v1068.)
const PRECACHE = [
