 ...
 */
const CACHE = 'ft-v1019'; // 🍽️ ft-v1019 = CE QU'IL TE RESTE A MANGER, TRADUIT EN TES ALIMENTS. Michel : « il faut montrer une estimation de ce qu'il nous reste a manger dans la journee (a 14h par exemple, il te reste 150 g de prot a manger, tu peux faire 1 shake de prot et 150 g de poulet, pareil pour les glucides et pareil pour les lipides) ». ⭐⭐ SA PRECISION EST TOUT LE SUJET : le chiffre « il te reste 150 g » etait DEJA a l'ecran, ligne par ligne, et il ne servait a rien — *personne ne sait a quoi ressemblent 150 g de proteines dans une assiette.* Ce qui manquait n'etait pas la DONNEE, c'etait sa TRADUCTION. C'est le trou 3.3 de NUTRITION-MOTEUR (« le Journal et le Plan ne se parlent pas »). ⛔⛔ LES ALIMENTS SONT LES SIENS, JAMAIS UNE BASE INVENTEE (R29) : ses favoris + ce qu'il a reellement mange, avec les macros QU'IL A LUI-MEME enregistrees. ⭐ Et ca evite entierement le trou 3.2 (« aucune base d'aliments ») — on n'a PAS besoin des 300 aliments `composable` pour ca. ⛔ R13/R2 : on ENRICHIT la carte existante (le reste reste a cote des chiffres qu'il explique), et `_resteDuJour` LIT `_foodTotals` et `calcMacros` sans rien recalculer. ⚠️⚠️ MA 1re VERSION PROPOSAIT UN SEUL ALIMENT et rendait « 5,5 × Blanc de poulet » / « 3,5 × Huile d'olive » — *ce ne sont pas des idees, ce sont des absurdites*. Et l'exemple de Michel disait DEJA pourquoi : « 1 shake ET 150 g de poulet », une COMBINAISON, parce qu'un aliment ne couvre pas un gros manque a dose raisonnable. C'est la MESURE qui l'a dit, pas une relecture. Desormais : 2 aliments au plus, 2 portions ou 250 g au plus, et on DIT ce que la combinaison couvre vraiment (« ≈ 108 g sur 167 ») plutot que de faire croire qu'elle tombe juste. ⛔⛔ LES TROIS GARDE-FOUS ANTI-TCA SONT LA MOITIE DU TRAVAIL (Constitution P21) : ① AUJOURD'HUI SEULEMENT — « il te manquait 40 g » sur hier est un reproche sur une journee qu'on ne peut plus changer ; ② AUCUNE idee ni commentaire quand la cible est DEPASSEE — on ne fait pas la morale ; ③ le texte dit « une idee, pas une consigne », parce que la portion enregistree est une estimation. *La nutrition ne doit jamais devenir une source de stress superieure au benefice qu'elle apporte.* ⭐ Detail de langue assume : l'elision (« d'Amandes », « d'Oeufs ») mais « de Yaourt » — le yaourt est plus frequent qu'un journal que les yeux.
const PRECACHE = [
