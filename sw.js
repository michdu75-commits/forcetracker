 ...
 */
const CACHE = 'ft-v1012'; // 📋 ft-v1012 = CREER UN PROGRAMME DEPUIS ZERO — la porte qui manquait (ecran Seance ②/5). AVANT, aucun chemin ne menait a la CREATION d'un programme : la modale « Mes Programmes » ne proposait que « Sauvegarder la seance actuelle », donc il fallait monter une SEANCE entiere a la main pour obtenir un PROGRAMME. ⭐⭐ L'EDITEUR EXISTAIT POURTANT EN ENTIER (`_renderProgEdit`) — atteignable seulement par le ✏️ d'un programme DEJA cree. *Une porte manquait, pas une fonctionnalite* (R13 : on ouvre celui qui est la, on n'en ecrit pas un 2e). ⛔⛔ RIEN N'EST ECRIT TANT QU'ON N'A PAS SAUVEGARDE : `_editProgIdx` pointe sur un index qui n'existe pas encore (la longueur du tableau) et `saveProgEdit` fait `S.programmes[idx]=…` — sur cet index-la, ca AJOUTE. Fermer sans sauvegarder ne laisse RIEN : pas de programme fantome a moitie rempli. ⚠️ LE NOM DEVIENT OBLIGATOIRE, et ca ne se voyait pas avant : on ne pouvait editer QUE des programmes deja nommes. Un nom vide aurait produit une ligne SANS TITRE dans la liste. On previent et on rend la main sur le champ, on ne detruit rien (R24). ⭐⭐ ET LA CAPTURE A REVELE UNE INCOHERENCE QUE JE VENAIS DE CREER : le selecteur restait ouvert cote SEANCE (ft-v1009) mais refermait encore a chaque exercice dans l'EDITEUR DE PROGRAMME — c'est-a-dire exactement la ou Michel monte ses listes (« je vais vouloir creer mon programme et il va falloir que ce soit rapide »). J'avais corrige le symptome du cote ou il genait le MOINS. Le mode 'prog' reste desormais ouvert lui aussi. ⛔⛔ LE TEMOIN LE PLUS IMPORTANT DU LOT VERIFIE QUE RIEN NE FUIT : si le mode retombait en 'workout' apres le 1er ajout, les 5 suivants partiraient SILENCIEUSEMENT dans la seance du jour au lieu du programme. Mesure : 6 ajouts d'affilee, mode tenu, seance restee vide, puis retour a 'workout' a la fermeture. ⛔ VOCABULAIRE : « + Ajouter » devient « + Creer ma seance » (Michel : « meme le bouton ajouter n'est pas top »), et le message de l'ecran vide disait « Appuie sur + Ajouter un exercice » — il designait un bouton qui ne s'appelait pas comme ca. Les deux disent enfin la meme chose. ⚠️ Et le message de la liste vide (« Cree une seance et utilise Sauvegarder ») est devenu FAUX le jour ou le bouton est arrive : il envoyait faire le detour juste au-dessous du raccourci. *Quand on ouvre une porte, on relit ce que disent les panneaux.*
const PRECACHE = [
