 ...
 */
const CACHE = 'ft-v1061'; // ⚖️ ft-v1061 = LA QUANTITE ET LES VALEURS NE SE DESAPPAIRENT PLUS. Michel, 4 captures dont son etiquette d'Iso Zero Protein : « ca sent le bug ». Il y en avait DEUX. ① LE CHAMP VIDE : il tape « 3 » (debut de 30), les valeurs tombent a 12 kcal, il efface pour recommencer — et 12 kcal RESTENT a l'ecran a cote d'un champ vide, au-dessus du bouton rouge « Ajouter au journal ». 4e fois que ce voisinage muet revient (ft-v966, v1042, v1056). ⛔⛔ ② LE VRAI : `_afMajAncre` relisait les 4 champs a CHAQUE appel pour en faire la nouvelle `base`, y compris quand elle n'etait rappelee que pour REDESSINER. Apres un rescale ces champs portent `base × facteur` : la reference devenait une valeur derivee d'elle-meme, et l'erreur se FIGEAIT. Mesure sur son etiquette : reference 30 g, il tape 40 (156 kcal, juste), un geste redessine, `base` devient 156 pendant que `q` reste 30 — et 40 g affiche 208 kcal / 47 g, c'est-a-dire EXACTEMENT sa capture, 1,33× l'etiquette. ⭐ L'en-tete de la fonction disait deja « appelee seulement quand la SOURCE des valeurs change » : vrai de l'intention, pas du code. Un argument `srcChange` rend la phrase executable. ⛔ Defaut de ft-v1056, donc de moi.
const PRECACHE = [
