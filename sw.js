 ...
 */
const CACHE = 'ft-v1058'; // 📊 ft-v1058 = LE VOLUME PAR MUSCLE PASSE A 14 JOURS, RAMENES A LA SEMAINE. Michel, capture a l'appui : « c'est super mais ca ne serait pas mieux sur 2 semaines ? » — et sa capture porte sa propre preuve : 2 seances dans la fenetre de 7 jours, quand la carte JUSTE AU-DESSUS dit 3,4 seances/semaine sur 78 jours. *Les deux cartes parlaient du meme bonhomme sans en donner la meme image* : 7 jours est un echantillon trop court pour un entrainement, une semaine chargee et une semaine creuse alternent normalement. ⛔⛔ MAIS LES CHIFFRES BRUTS NE SONT PAS DOUBLES, et c'est TOUT l'arbitrage : `DISC_CADRE.volume` dit « 10 a 20 series par groupe musculaire ET PAR SEMAINE » — afficher 24 sur 14 jours donnerait un nombre FAUX ET CREDIBLE face a un repere hebdomadaire. L'ECHANTILLON s'elargit, l'UNITE reste la semaine (Fessiers 12 → 6/sem). ⭐ Arrondi au DEMI, pas au dixieme (une serie est un entier divise par 2 : « 4,3 » serait une fausse precision). ⛔ Le nombre de seances reste affiche : une moyenne sur 14 jours NOIE une semaine a zero, et lui seul dit ce que la moyenne ne dit pas. ⛔ Milo recoit LA MEME grandeur que l'ecran (R2) — sinon l'ecran dirait 6 et lui 12 — plus l'avertissement de ne jamais parler de « cette semaine » a partir d'une moyenne.
const PRECACHE = [
