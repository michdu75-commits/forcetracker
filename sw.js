 ...
 */
const CACHE = 'ft-v905'; // 🔄 ft-v905 = LES DEUX ERREURS DE MILO DU 18/08, ET LES DEUX AVAIENT UNE CAUSE DANS LE CODE. ① IL S'EST TROMPE DE SERIE : la note « barre raque a la 4eme » etait sur la 2e serie, il a debriefe la 3e. La ligne envoyee etait une SUITE NON NUMEROTEE ou paliers et series de travail se ressemblent — on lui demandait un travail d'index que le code fait sans erreur. Les series de TRAVAIL sont desormais numerotees (S1, S2, S3) ; les paliers restent marques (E) sans numero. ② IL A REPROCHE UNE MONTEE EN CHARGE QU'IL AVAIT LUI-MEME PRESCRITE (48 kg de depart sur le Developpe Incline). Le garde-fou existait DEJA depuis le 15/08 — mais il ne couvrait que « montee ecrite par l'APP », pas « prescrite par MILO ». La jumelle manquait (R8). Une seance chargee depuis le chat porte maintenant _milo:true, et le marqueur dit d'ou viennent les paliers : la correction porte sur SA prescription, jamais sur la personne (R29 : un reproche injuste coute la confiance dans l'outil). ⚠️ PORTEE HONNETE : le marqueur n'existe que pour une seance chargee DEPUIS le chat ; passee par un programme enregistre, elle a perdu son auteur en route — la seule consigne du prompt peut alors rattraper.
const PRECACHE = [
