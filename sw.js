 ...
 */
const CACHE = 'ft-v989'; // 🛡️ ft-v989 = LA VALIDATION UNIQUE AVANT UNE SEANCE DE MILO. PRIORITE 1 tranchee par Michel apres le contre-audit du 24/08 : « une validation deterministe unique avant l'activation de la seance : blessures, exclusions, doublons ». ⛔⛔ POSEE AU SEUL POINT QUE LES DEUX PORTES TRAVERSENT — `_appliqueMiloSession`, exactement la meme raison que le controle d'intensite (ft-v980) : le poser ailleurs l'aurait fait manquer dans le cas normal (_startSessionFromMilo, aucune seance en cours). ⛔ ELLE NE REINVENTE RIEN (R2/R13) : DOUBLONS = comparaison directe des noms · EXCLUSIONS = S.exSwaps + _EX_SWAP_RAISONS (la case `durable` existait deja, elle filtre deja « il me gene »/« trop long » des raisons de circonstance comme « machine prise ») · BLESSURES = _gardienZones() + _GARDIEN_CONSTRAINTS (coach.js), la MEME mecanique qui construit deja la note « dans sa seance du jour » envoyee a Milo. ⚠️ ON SIGNALE, ON NE BLOQUE PAS (R24, Constitution P13 « adapter jamais interdire ») : la seance demarre normalement, l'avertissement reste attache a l'exercice (comme intensiteWarn) — un toast seul aurait disparu avant la 1re serie. Seul l'ACTIF ou AUJOURD'HUI declenche une blessure ici ; une fragilite durable mais calme reste couverte par le Gardien de la conversation (R19 — pas de bruit repete). ⛔ LES CHARGES DE MILO NE SONT JAMAIS MODIFIEES, verifie par un temoin dedie (R29). 🎨 AFFICHAGE : le bandeau existant (`_intensiteBandeau`) est ETENDU, pas duplique — meme mecanique, meme style, deux icones nouvelles (🚫 exclusion, 🛡️ blessure, 🔁 doublon) a cote de ⚡. Verifie a l'ecran, pas seulement en donnees : capture prise sur les 4 cas reunis, les 3 bandeaux s'affichent correctement, la charge de Milo (40 kg) part intacte, le bouton central + n'a pas bouge (regle d'or #9).
const PRECACHE = [
