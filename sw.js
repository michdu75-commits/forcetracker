 ...
 */
const CACHE = 'ft-v1100'; // 🏁 ft-v1100 = UN CYCLE TERMINE NE LE DISAIT JAMAIS, NI A L'ECRAN NI A MILO. Michel : « continue stp ». 2e passe, et elle commence par une DETTE : j'avais annonce le cycle « non mesure » en ft-v1099 parce que mes sondes s'etaient trompees de champ. Repris avec une fixture LUE dans `startCycle`. ⛔⛔ LE DEFAUT : `getCurrentCycleWeek` PLAFONNE a `weeks` — juste pour une barre de progression, FAUX pour tout le reste. MESURE : un cycle fini depuis 1 jour, 1 an et SIX ANS rend un ecran RIGOUREUSEMENT IDENTIQUE a celui de la derniere semaine (« Semaine 12/12 — Decharge, 55 % 1RM · 2x5 reps »), presente comme la consigne du jour. ⛔⛔ ET CE N'EST PAS QU'UN AFFICHAGE : la meme valeur partait dans le contexte de Milo (« Actif - Semaine 12/12 - Phase Decharge ») — ***il prescrivait une decharge sur la foi d'un fait faux sur la personne***, famille R4/R10. ⭐ UN SEUL PROPRIETAIRE (`cycleTermine`, R2), lu par l'ECRAN et par `buildCoachContext` : deux copies auraient diverge. ⚠️ ON NE CLOTURE PAS TOUT SEUL (R29) : `S.cycle` porte les 1RM de depart et les objectifs de la personne. On le DIT, elle decide — et le bandeau precise « rien n'est supprime tant que tu ne clotures pas ». ⛔⛔ LE TEMOIN QUI PORTE LE RISQUE N'EST PAS CELUI DU CYCLE FINI, c'est celui de la DERNIERE SEMAINE, qui ne doit RIEN afficher : un bandeau « termine » pendant la semaine de decharge serait faux et decourageant. ⏭️ CINQ FAMILLES ONT RENDU DU VIDE, ecrit pour ne pas refaire la chasse : le TEXTE LIBRE de la personne est bien echappe partout (nom d'exercice piege rendu sur 5 ecrans, 0 script execute — et le controle prouve que le nom etait bien AFFICHE) · le TIMER de repos s'appuie sur `Date.now()`, donc il resiste au sommeil de l'app · les SERIES SPECIALES comptent pareil partout (echauffement et 'W' hors volume, drop dedans, record inchange) · la FILE de synchro ne renvoie que `synced===false` · le QUOTA est garde par `_premiumPending` depuis ft-v446. ⚠️⚠️ ET QUATRE DE MES SONDES N'ONT RIEN MESURE AVANT D'ETRE REPAREES, toujours au meme signe — un resultat identique des deux cotes : `window._demoMode=true` court-circuite `syncSheets`, `S.connected` manquait pour l'envoi de fin de seance, la fixture du cycle utilisait `rm1s` au lieu de `exercises`, et j'ai cherche le MOT « cycle » dans le contexte de Milo — qui en est plein — au lieu de ses VALEURS.
