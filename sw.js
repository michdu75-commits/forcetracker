 ...
 */
const CACHE = 'ft-v1034'; // 🪟 ft-v1032 = LA BARRE « SEANCE » NE LAISSE PLUS LIRE CE QUI DEFILE DESSOUS. Trouve sur une VRAIE VIDEO de l'iPhone de Michel, en production : `#log-hdr` etait en rgba(12,13,17,.55) + blur(12px), et « Choisis par quoi tu commences 👇 » se lisait A TRAVERS la barre, emoji compris. ⛔ PAS une bizarrerie iOS — reproduit dans Chromium ou le flou s'applique pourtant : *un flou ADOUCIT, il ne CACHE pas*, aucune valeur de blur ne rattrape 0,55. Opacite portee a .96, le flou CONSERVE (on opacifie, on ne supprime pas l'effet). ⛔⛔ ET LE CSS N'AVAIT PAS BOUGE : mesure, l'ecran Seance VIDE etait defilable de 0 px avant ft-v1026 et l'est de 224 px depuis les cartes de types. *Le defaut etait INATTEIGNABLE, pas absent — il attendait son premier cas*, comme le gras de .sw-feat la veille. ⛔ Jumelles cherchees (R8) : #log-hdr est le SEUL sticky avec ce motif ; les 2 autres backdrop-filter sont des voiles d'overlay, ou la translucidite est voulue.
const PRECACHE = [
