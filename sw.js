 ...
 */
const CACHE = 'ft-v960'; // 🔤 ft-v960 = MEME BUG, SUR L'APOSTROPHE. Michel, apres la ligature du blanc d'oeuf : « faut aller voir aussi avec les accents, le E trema, tous les caracteres speciaux ». ⭐ VERIFIE SYSTEMATIQUEMENT sur les ~132 000 noms des deux bases (CIQUAL + Compl'Alim) plutot que de deviner. ⭐ Accents (e/e/e...), TREMA (e/i/u) et CEDILLE (c) sont deja CORRECTS : NFD les decompose tous en lettre + accent, deja retires. ⚠️⚠️ MAIS L'APOSTROPHE avait exactement le meme defaut que la ligature : le clavier iPhone convertit AUTOMATIQUEMENT l'apostrophe DROITE tapee en apostrophe COURBE pendant la frappe. 238 aliments CIQUAL en portent une (« Soupe a l'oignon », « Saute d'agneau »). Mesure : « aujourd'hui » tape (droite) et « aujourd'hui » stocke (courbe) ne se reconnaissaient PAS comme le meme mot. ⛔ CORRIGE en RETIRANT purement l'apostrophe et ses variantes (courbe droite/gauche, accent grave, accent aigu isole) — verifie au passage que certaines entrees utilisent MEME ces variantes comme coquille d'origine (« PROBIO´DIET »). Toujours dans la meme fonction partagee par CIQUAL et les suggestions locales (R2).
const PRECACHE = [
