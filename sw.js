 ...
 */
const CACHE = 'ft-v1055'; // 🩹 ft-v1055 = UNE CONSIGNE INTERNE N'EST PAS UNE DEMANDE DE LA PERSONNE. Capture de Michel a 15 h 16 : « Cette seance te convient ? » s'affichait sous un DEBRIEF DE FIN DE SEANCE — il venait de terminer, et l'app lui proposait d'en demarrer une. ⛔⛔ LA CAUSE EST DANS MON CODE DE ft-v1053 : le debrief auto envoie une consigne `_silent` qui commence par « Je viens de terminer MA SEANCE », et `_demandeUneSeance` y lisait une demande — *je pairais la reponse de Milo avec un texte que Michel n'a JAMAIS tape*. ⭐ La regle existait deja a DEUX endroits (l'affichage du fil, le bouton « Mes discussions ») : c'est moi qui ne l'ai pas reprise, pas l'app qui l'ignorait. ⛔ Et on ABANDONNE au lieu de remonter plus haut : repecher une demande plus ancienne collerait la question sous une reponse qui n'y repond pas — le defaut qu'on corrige. 🔍 JUMELLE trouvee en la cherchant (R8) : `_convTitle` aussi — une discussion rangee pouvait s'intituler « [DEBRIEF AUTO] Je viens de terminer ma seance… ».
const PRECACHE = [
