 ...
 */
const CACHE = 'ft-v1043'; // 🎽 ft-v1043 = LE CONTROLE D'INTENSITE CONNAIT ENFIN LA DISCIPLINE. Relais de session-A (« le trou le plus petit et le plus rentable »), choisi par Michel. ⛔⛔ Mesure : `_intensiteDefauts(nom, sets)` ne portait AUCUNE occurrence de `discipline` ni de `DISC_` — un seul chiffre, 150 s, pour tout le monde, quand `DISC_CADRE.repos` dit « 3 a 5 min entre les series lourdes » en force athletique et « 60 a 120 s » en bodybuilding. L'app AFFICHAIT donc un cadre et en VERIFIAIT un autre. Le cas qui passait : un powerlifter a 160 s entre deux series a 88 % ne recevait RIEN. ⛔⛔ Le plancher a 150 s RESTE : c'est la decision de Michel (« un 3x5 avec 90 secondes de repos c'est IMPOSSIBLE »), et le bas de plage de muscu vaut 90 s — lire le cadre sans plancher aurait rendu son propre cas SILENCIEUX. Le seuil ne peut donc que RESSERRER. ⛔ NON CONSTRUIT, verifie : le plafond de charge ne bouge pas (Brzycki est de la PHYSIOLOGIE, pas de la doctrine) · `_INT_LOURD`=0,80 est deja juste pour les 5 · et on n'avertit PAS sur une charge au-dessus du cadre, que les cadres eux-memes autorisent (« du lourd ponctuel ne se reproche pas »).
const PRECACHE = [
