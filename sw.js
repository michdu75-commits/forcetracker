 ...
 */
const CACHE = 'ft-v987'; // 🔢 ft-v987 = « CE N'ETAIT PAS UN SCAN, J'AI RENTRE LE CODE-BARRE MANUELLEMENT ». Michel avait raison, et l'app se contredisait elle-meme : son propre commentaire dit « `saisie` dit COMMENT c'est entre », et les QUATRE chemins de code-barres s'enregistraient tous en `'scan'` — la camera, la photo decodee par ZXing, la photo lue par l'IA, et les chiffres TAPES au clavier. ⭐ MESURE SUR SES 23 ENTREES REELLES : ses « 6 scans » comptaient des saisies clavier, donc la donnee censee trancher les questions produit (SUIVI-AUDIT) etait FAUSSE. Quatre valeurs desormais : scan · photo-code · photo-code-ia · code-tape. ⭐⭐ ET CE N'EST PAS COSMETIQUE — LES QUATRE N'ONT PAS LA MEME FIABILITE : `scan` et `photo-code` sont decodes par ZXing, qui VERIFIE la cle de controle ; `photo-code-ia` et `code-tape` sont des chiffres NON verifies. C'est l'echelle des sources de R33 appliquee a un seul champ. ⛔⛔ D'OU LE VRAI APPORT DE CETTE VERSION : `_eanValide()`, la cle de controle d'un EAN-8/12/13, de l'arithmetique pure, zero reseau. POURQUOI ELLE COMPTE : le seul mode d'echec de ce chemin est SILENCIEUX — un chiffre faux ne donne pas « introuvable », il donne LE PRODUIT DE QUELQU'UN D'AUTRE, avec un vrai nom et de vraies calories. Une erreur qui ressemble a un succes ne se voit jamais. Mesure : un seul chiffre change sur Nutella et sur Coca-Cola est refuse dans les deux cas. ⛔ ON NE BLOQUE PAS (R24) : les codes internes de magasin ne suivent pas la norme, donc on PREVIENT et on cherche quand meme ; une longueur hors norme rend `null` — « je ne sais pas » n'est pas « c'est faux » (R29). ⚠️ ET LE MESSAGE REMPLACE le « Recherche du produit… » au lieu de s'empiler dessus : sinon il serait ecrase en une demi-seconde et personne ne le lirait (lecon de ft-v985). ⚠️⚠️ ET MON PROPRE TEMOIN A ATTRAPE MON PROPRE DEFAUT : `_provFood` construit une LISTE BLANCHE, donc le drapeau `codeDouteux` s'arretait a cette fonction et n'atteignait JAMAIS l'entree enregistree — sans erreur, sans test rouge. C'est R4 DANS LA FONCTION QUI DOCUMENTE R4, deux fois au meme endroit (la 1re, c'etait `etat` le 19/08). ⚠️ Mon 1er essai de mesure n'a rien mesure non plus : j'ecrivais `window._bcNutr`, une variable de SCRIPT — 3e fois cette semaine (`_afSuggLoc`, `_miloPendingIdx`). Le temoin passe desormais par le VRAI chemin, en remplacant le RESEAU. ⭐⭐ CONTROLE NEGATIF INSTRUCTIF : `recu : scan,scan,scan,scan` — le defaut imprime noir sur blanc.
const PRECACHE = [
