 ...
 */
const CACHE = 'ft-v1022'; // 📊 ft-v1022 = LE DEBRIEF CHIFFRE EST CALCULE EN LOCAL, TOUJOURS. Brique ③ du chantier ecran Seance (docs/SEANCE-DESSAI.md §4). Michel : « pas de reseau, il faut absolument que la personne puisse avoir un debrief » ET « plus on code, moins on consomme d'API » — ⭐ les DEUX faces d'une seule ligne de code, c'est ARCHITECTURE-CERVEAU-CERVELET appliquee ici (« est-ce que ca a besoin de savoir QUI est la personne ? » non → le code, oui → Milo). ⛔⛔ LE DEFAUT ETAIT SYMETRIQUE ET FAUX DES DEUX COTES : hors ligne on ne rendait que « N exercices · N series · N kg » (mode degrade MUTILE, alors que le code sait calculer bien plus GRATUITEMENT), et EN LIGNE `slot.innerHTML=` REMPLACAIT ces chiffres par le texte de Milo — donc le JUGEMENT SANS LES FAITS. Desormais : les faits en local TOUJOURS, Milo AJOUTE par-dessus, separe par un trait. ⛔ RIEN N'EST CALCULE DE NEUF, tout est REBRANCHE (R13/R2) : _mscScores/_mscFocus (muscles), _calSessMix (region + %), _dureeTotaleMin (duree cardio comprise), _monteeDefauts (echauffement), _intensiteDefauts (charge vs 1RM), DISC_CADRE (le repere de la discipline). ⛔⛔ ET _validationSeance N'EST PAS UTILISEE, EXPRES : elle est ecrite pour ce que Milo PROPOSE (mode `add` compare a la seance EN COURS) — sur une seance FINIE ce serait R14. Un doublon peut etre voulu, un exercice « exclu » qu'on a quand meme fait est un CHOIX, et signaler apres coup une zone sensible est un reproche sans action possible (R29). ⛔ Les 2 detecteurs se taisent tout seuls quand ils ne savent pas, les points sont PLAFONNES a 3 (un debrief n'est pas un proces-verbal, R19), et le cadre est AFFICHE sans etre NOTE (DISC_CADRE est de la prose, pas des bornes calculables — pretendre verifier ce qu'on ne peut pas mesurer serait une fausse precision). ⚠️ ET LA CAPTURE A MONTRE UN DOUBLON QUE LE TEXTE NE VOYAIT PAS : les tuiles de l'ecran portent deja volume/series/kcal/records, le debrief les reecrivait 15 cm plus bas (R25).
const PRECACHE = [
