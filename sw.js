 ...
 */
const CACHE = 'ft-v1051'; // 🆘 ft-v1051 = 3e PANNE DU BOUTON DE LANCEMENT EN 8 JOURS. Michel : « pk il y a tjrs une couille avec le lancement de la seance ». ⛔⛔ CAUSE : `_renderCoachThread` ne re-analysait QUE le dernier message de Milo (`break`) — l'intention etait bonne (« une vieille seance ne doit pas ressurgir »), la mise en oeuvre trop stricte. Chez lui le dernier message est « la seance est ecrite au-dessus, le bouton devrait apparaitre » : AUCUNE seance dedans, donc null, donc plus aucun bouton — et ca restait vrai APRES ft-v1049. 👉 *Il suffit que Milo dise un mot apres avoir propose la seance pour perdre le bouton a jamais.* On remonte au plus 3 messages de Milo et on s'arrete au premier qui porte une seance ; la borne garde l'intention d'origine. ⭐⭐ LA LECON QUI SE REPLIQUE (relayee a session-A a la demande de Michel) : les 3 pannes viennent d'une hypothese sur la FORME de ce que Milo ecrit, et Milo change d'ecriture sans prevenir — un detecteur verifie sur nos propres exemples ne teste rien.
const PRECACHE = [
