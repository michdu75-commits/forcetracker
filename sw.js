 ...
 */
const CACHE = 'ft-v1020'; // 🥗 ft-v1020 = « CE QU'IL TE RESTE » PROPOSE CE QU'IL MANGE, PAS CE QUI EST OPTIMAL. Michel, devant la 1re version : « je pense qu'il faut rester simple, tout le monde ne bouffe pas de flocons d'avoine, moi le premier ». ⚠️ D'abord une clarification qui vaut d'etre notee : les flocons venaient de MON jeu de test, pas de son journal — l'app n'a jamais propose que SES aliments. Mais le fond etait juste : trois lignes de combinaisons, ca ne se lit pas. ⭐ REGLE 1 — ON CLASSE PAR CE QU'IL MANGE : un favori d'abord (il l'a choisi), puis la FREQUENCE reelle dans son journal ; la densite ne sert plus qu'a departager. *Un aliment parfait qu'on ne mange jamais est un mauvais conseil.* ⭐ REGLE 2 — UN SEUL ALIMENT PAR DEFAUT : un deuxieme n'est ajoute QUE si le premier couvre moins de la moitie du manque. ⛔⛔ ET LA REGLE 3 A ETE TROUVEE EN MESURANT LA REGLE 1 : mon 1er jet classait le favori en tete quelle que soit la macro, et a sorti « 2 × Shake proteine » sur la ligne GLUCIDES. *Un shake proteine pour des glucides, ca ne veut rien dire.* LA PERTINENCE PASSE DONC AVANT LES GOUTS, avec un critere PHYSIQUE et non arbitraire : la macro doit peser au moins 30 % des calories de l'aliment (riz 88 % de glucides, huile 100 % de lipides, shake 83 % de proteines mais 10 % de glucides — il ne sort jamais sur cette ligne-la). ⭐⭐ RESULTAT MESURE, et la ligne proteines est LITTERALEMENT la phrase de Michel : « 180 g de proteines — 2 × Shake proteine + 250 g de Blanc de poulet ». Les flocons d'avoine, manges UNE fois et peu proteines, ne sortent plus nulle part.
const PRECACHE = [
