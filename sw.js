 ...
 */
const CACHE = 'ft-v941'; // 🚪 ft-v941 = UN STOCKAGE QUI SURVIT DERRIERE UNE PORTE QUI NE SURVIT PAS NE SERT A RIEN. Michel : « je ne peux pas rejouer j'ai plus les cases ». Bug de ft-v938, et il est de moi : les deux boutons GRATUITS (🔬 rejouer les verificateurs · 📥 copier les reponses) ne vivaient QUE sur la carte de resultat — or cette carte vit dans le CHAT, donc elle MEURT au rechargement de l'app. Les reponses, elles, avaient ete stockees exprès pour y survivre. ⭐ TOUT L'INTERET de ft-v938 (« on paie une fois, on exploite dix fois ») tombait au premier rechargement : la matiere etait la, l'acces n'existait plus. 👉 Les deux boutons vivent desormais dans Profil → Admin, a cote de « Lancer le benchmark » — un endroit qui, lui, ne depend d'aucune session de chat. ⚠️ ET LE CONTROLE NEGATIF DIT EXACTEMENT LA BONNE CHOSE : 1 seul rouge, le chemin manquant. Le temoin « depart a froid » est vert des DEUX cotes, et c'est voulu — `rejouerVerifs()` a toujours fonctionne quand on l'appelait ; ce qui manquait, c'etait un moyen de l'appeler. Le defaut etait un POINT D'ENTREE, pas une fonction. ⚠️ Le temoin lit les DEUX fichiers HTML (app et clone) : un chemin present d'un seul cote finirait par diverger (R2).
const PRECACHE = [
