 ...
 */
const CACHE = 'ft-v938'; // 💾 ft-v938 = GARDER LES REPONSES : LE GISEMENT GRATUIT DU BENCHMARK. Michel : « on ne peut pas ameliorer le benchmark ou il faut plus de passes ? ». Les deux — mais le plus gros gain ne coutait rien et on le JETAIT. LE CONSTAT, mesure dans le code : une passe coute 0,25-0,95 € et produit 15 vraies reponses de Milo ; elles vivaient en memoire le temps de la session puis disparaissaient (le rapport ne gardait que les VERDICTS). Or les verificateurs sont du CODE : les rejouer ne coute AUCUN appel. On paie une fois, on exploite dix fois. ⭐ LE CAS REEL QUI L'A MOTIVE : le faux rouge EV-001 (« on estime ton 1RM a 93 kg » pris pour une charge a mettre sur une barre) a ete corrige A L'AVEUGLE, sur le texte que Michel avait colle dans la conversation. 👉 LIVRE : les reponses sont gardees en local (admin, jamais synchronise) · bouton « 🔬 Rejouer les verificateurs (0 €) » · bouton « 📥 Copier les reponses » · et en ligne de commande `node tests/milo/eval.js --rejouer <fichier>`, meme forme des deux cotes (R2). ⚠️⚠️ LE PIEGE DE CE BLOC EST SILENCIEUX : un REJEU N'EST PAS UNE NOUVELLE PASSE. Milo n'a pas reparle ; ce qu'on mesure, c'est le VERIFICATEUR. L'ecrire dans l'historique fabriquerait une mesure qui n'a jamais eu lieu et fausserait la lecture « systematique vs intermittent » — celle qui decide de ce qu'on corrige. D'ou le drapeau `sansHist`, et un rapport qui le DIT en toutes lettres. ⛔ REGLE D'OR n°3 AVANT LA FONCTIONNALITE : plafond par reponse (8 000 car.) ET au total (200 000) ; si le stockage refuse, on RETIRE la cle et on continue sans rien dire — un confort de diagnostic ne fait jamais tomber une sauvegarde. ⚠️ Et deux temoins existants ont rougi : ils lisaient le corps de `copyEvalText`, ou le code de copie n'est plus (il vit dans `_evCopier`, partage par les deux boutons — R2). Un temoin doit viser la fonction qui PORTE le comportement.
const PRECACHE = [
