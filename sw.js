 ...
 */
const CACHE = 'ft-v978'; // 🔍 ft-v978 = LES TROIS CORRECTIONS DE L'AUDIT DE 200 PAGES. Michel : « vas-y fais les 3 corrections de vingt minutes ». ⭐⭐ LE PDF DE MILO N'ETAIT PAS CASSE : mesure dans un vrai navigateur, `_coachPdfText` rend 81 caracteres sur 81, 323 sur 345 et 9 769 sur 9 769 selon le format — LE CONTENU EST INTACT, c'est la LIVRAISON qui echouait. Les DEUX hypotheses du dossier (« le PDF est vide » / « le PDF est trop pauvre ») etaient fausses. ⛔⛔ LA VRAIE CAUSE ETAIT LITTERALE : `navigator.share({files:[file], title:'Conseil de '+coach})` — « Conseil de » + « Milo » = MOT POUR MOT ce que Michel voyait. Il recevait le TITRE de la feuille de partage, le fichier ayant ete jete. ⭐⭐ ET LE CORRECTIF EXISTAIT DEJA DANS LE DEPOT, POSE D'UN SEUL COTE : le 20/08, meme symptome sur le benchmark (« Benchmark Milo », une ligne), avec la note « PAS DE title: DANS LE PARTAGE ». Un seul export l'avait recue, NEUF le gardaient. 3e fois en deux jours qu'un correctif juste ne vit que d'un cote (ft-v973, ft-v975) — d'ou la lecon appliquee ici : COMPTER LES ENDROITS, deux nombres et pas un. 0 des 10 partages de fichier garde un titre, contre 1 sur 10 avant ; les partages de LIEN gardent le leur, c'est la qu'un titre sert. ⚠️ CE QUI N'EST PAS PROUVE EST ECRIT AUSSI : l'echec lui-meme n'a pas pu etre reproduit (il faut un vrai Safari iOS) et d'autres exports fonctionnaient AVEC un titre — l'application choisie dans la feuille compte probablement. ⛔ 2e CORRECTION : la phrase « on y perd du muscle avant du gras » est retiree de l'ecran Nutrition. Le corps n'a pas d'interrupteur graisse->muscle. Remplacee par ce qui est VRAI (plus le deficit est fort et long, plus il devient difficile de GARDER le muscle) et SANS aucun seuil invente en echange (R29) — un temoin refuse tout chiffre du type « sous X kcal ». ⛔⛔ 3e CORRECTION : le marqueur « masse maigre DEDUITE » n'est plus jete. En ft-v974 je le supprimais moi-meme avant de remplir le formulaire : une masse maigre retrouvee par SOUSTRACTION arrivait indiscernable d'une valeur LUE. Il est desormais conserve et ENREGISTRE avec le bilan, a cote de sa provenance (R33). Comportement differe mais NOMME (R3) : il existe pour la correction suivante — le prompt de Milo presente encore cette valeur comme « MESUREE … chiffre SOLIDE … sans reserve », y compris quand elle vient d'une pesee dont le % de gras a ete tape a la main.
const PRECACHE = [
