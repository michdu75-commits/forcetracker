 ...
 */
const CACHE = 'ft-v1005'; // 🧪 ft-v1005 = LE LIBELLE DU BENCHMARK NE MENT PLUS. L'ecran admin annoncait « 16 scenarios », « 16 pieges », « 2 × 16 » — le benchmark en porte 53. Il a grandi 16 → 21 → 53 en trois semaines (c'est R35 : il grandit a chaque bug), les libelles ecrits en dur n'ont pas suivi. ⛔ ON N'A PAS MIS « 53 » A LA PLACE : ce serait la meme dette six semaines plus tard, sur l'ecran meme qui sert a decider d'une depense. Le nombre est RETIRE. ⭐ LE COMPTE ET LE PRIX EXISTAIENT DEJA, JUSTES, AILLEURS : `startEvalBench` calcule SC.length et _evPrix(n) et les annonce dans la confirmation AVANT le premier appel paye — le libelle statique portait une seconde source de verite pour rien (R2). ⭐⭐ ET LE TEMOIN INTERDIT LE NOMBRE AU LIEU DE L'EPINGLER : un temoin qui verifierait « 53 » rougirait a la 54e promotion et on l'ajusterait sans reflechir ; celui-ci refuse tout nombre a 2-3 chiffres dans le bloc benchmark de l'admin, donc la prochaine derive fait rougir la livraison au lieu de mentir en silence. // 🧹 ft-v1002 = « SQUAT SUMO » RETIRE DU CHOIX — Michel : « squat sumo on supprime, ca me soule ». 2e retrait de la journee, MEME FORME que le pull-over : RETRAIT, jamais fusion. L'identifiant `squat-sumo` reste dans EX_IDS + RETIRES_VOLONTAIREMENT, donc seances et records deja faits gardent nom, muscles (fessiers/quadriceps + adducteurs...) et MET 6,5. Verifie dans un vrai navigateur. *On retire du CHOIX, jamais de la MEMOIRE.* ⭐⭐ ET SON HISTOIRE ETAIT DEJA ECRITE, ce qui a evite de la redecouvrir (R30) : le 13/08 son illustration avait ete retiree parce qu'elle montrait un HALTERE entre les jambes — c'est-a-dire le geste du « Squat Gobelet », qui a sa propre photo. On affichait la photo d'un AUTRE exercice. Le commentaire d'alors disait qu'on gardait le fichier « le jour ou Michel trouve une figurine A LA BARRE » ; elle n'est jamais venue, et au bout de 12 jours il a prefere retirer l'exercice. Un retrait dont la condition de retour est ecrite se referme proprement — meme quand la reponse est « finalement non ». ⭐ LE FICHIER ORPHELIN DORMAIT DANS LE CACHE DU SERVICE WORKER : `squat-sumo-avec-haltere.webp` etait telecharge par TOUT LE MONDE pour rien depuis le 13/08. Retire du depot ET du cache (recuperable dans git si une variante haltere est creee un jour). ⛔ 7 ENDROITS TENUS ALIGNES : les 2 entrees EXLIB (Jambes ET Fessiers) · EX_IDS · RETIRES_VOLONTAIREMENT · les 2 equivalences d'import · EX_EN · le cache SW · A-FAIRE-SUR-PC (tache close, elle n'avait plus d'objet). ⛔ LES 2 EQUIVALENCES NE SONT PAS REDIRIGEES, EXPRES : `sumo squat` et `wide stance squat` ne visent plus rien, car AUCUNE autre fiche ne decrit ce geste. Un import « sumo squat » sera donc propose comme exercice NOUVEAU — ce qui est honnete — plutot que rattache de force au mauvais squat (R29). ⭐⭐ ET LE TEMOIN ECRIT LE MATIN MEME A SERVI DES LE CAS SUIVANT : la regle « aucune equivalence d'import ne vise un exercice introuvable » (ft-v1001) est restee VERTE, ce qui prouve que les 2 alias ont bien ete retires. C'est R17 qui paie deux fois en une matinee.
const PRECACHE = [
