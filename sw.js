 ...
 */
const CACHE = 'ft-v1095'; // 5e passe : LES IMPORTS — ce qu'un modele hallucine n'entre plus dans l'historique. La question qui decide (R33) : un document partiel, illisible ou aberrant produit-il un ECHEC PROPRE ou une DONNEE INVENTEE ? Mesure en rendant a l'app, par ses vraies fonctions, ce qu'un modele peut lui renvoyer. ⛔⛔ CE QUI PASSAIT : `500 kg x 50 reps` posait un record de 1 060 kg de 1RM · une charge NEGATIVE un record de -90 kg · les dates 1900-01-01, 2099-01-01 et meme la chaine « le mardi » entraient telles quelles · et un `exercises` rendu comme une CHAINE faisait planter l'import avec une trace technique a l'ecran. ⭐⭐ LE 500 x 50 EST LE CAS INSTRUCTIF : chaque valeur passait les bornes existantes (500 <= 500, 50 <= 100) — c'est la COMBINAISON qui est impossible, `bz()` plafonnant les repetitions a 20, toute charge x 20 vaut 2,1 fois elle-meme. On borne donc aussi ce qui SORT (1RM <= 600). ⭐ Un faux record vers le HAUT n'est jamais battu : il est eternel, et il sert de reference aux charges que Milo propose. ⛔ LES BORNES ONT DESORMAIS UN SEUL PROPRIETAIRE (`_serieValide`), partage avec `coach.js` qui les avait DEJA — le meme garde-fou sur un chemin et pas sur l'autre, c'est R8, 3e fois cette semaine. ⛔ ON ECARTE LA SERIE, PAS LA SEANCE : une ligne mal lue ne fait pas perdre les neuf autres (regle d'or #3) ; une date invalide, elle, ecarte la seance — l'apercu ne permet pas de la corriger et une date fausse pollue durablement graphes et contexte. ⭐ ET ON LE DIT : un rejet silencieux est indiscernable d'un import reussi (R29). ⚠️ Normalisation A L'ENTREE, une fois : une dizaine de lecteurs de `sess.exercises` en aval, les rustiner un par un c'est en oublier un. ⚠️⚠️ ET MA SONDE A MENTI DEUX FOIS AVANT DE MESURER : elle appelait `_histAnalyzeBatch` (qui ne fait que RENDRE le tableau) au lieu de `analyzeHistPhotos` (qui REMPLIT l'etat), et elle n'avait pas de cas VALIDE — les 14 reponses hostiles rendaient donc le meme resultat, la signature d'une sonde muette. Le temoin de controle est desormais le PREMIER cas du bloc.
