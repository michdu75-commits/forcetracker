 ...
 */
const CACHE = 'ft-v918'; // 🔗 ft-v918 = MILO NE PEUT PAS ALLER SUR INTERNET, MAIS IL POUVAIT PRETENDRE L'AVOIR FAIT. Michel, en relisant le prompt : « je n'ai pas vu de protection pour eviter a Milo d'aller sur internet ». ⭐ VERIFIE AVANT DE REPONDRE : il ne PEUT PAS y aller — l'appel API ne porte AUCUN champ `tools`, aucune recherche web (worker.js). C'est structurel, pas une consigne, et c'est la bonne facon de le garantir (R7). ⚠️ MAIS LE RISQUE VOISIN N'ETAIT PAS COUVERT : il peut tres bien INVENTER une etude, un organisme, un lien. Or la regle qui s'appelle « ne fabrique jamais de source » ne parlait que des sources INTERNES (« je vois ca dans tes antecedents »). C'est BUGS.md famille 15 — la regle juste, definie trop etroit — et c'est d'autant plus sensible que Milo parle de SANTE et de NUTRITION. 👉 DEUX ETAGES : le PROMPT empeche (regle elargie a l'exterieur, + « tu n'as aucun acces internet, tu ne peux rien verifier »), et le GARDIEN DE SORTIE attrape le cas detectable — un LIEN, que Milo n'a par construction jamais pu verifier. ⛔ ET ON NE TENTE PAS de reperer « selon l'ANSES » : l'app CITE de vraies sources (Open Food Facts, Nutri-Score) et Milo a le droit de les nommer — un garde-fou qui crie sur une phrase juste finit desactive (R19). Un temoin verifie ce faux-positif. ⚠️ Rien de neuf construit : le Gardien de sortie existait avec 4 controles, il en a 5 (R13). ⚠️ Marge du bloc commun retombee a 15 caracteres — la regle est passee devant (R11), et l'arbitrage sur les 140 regles n'est plus optionnel.
const PRECACHE = [
