 ...
 */
const CACHE = 'ft-v1001'; // 🧹 ft-v1001 = LE « PULL-OVER » GENERIQUE RETIRE DU CHOIX — decision Michel : « a l'haltere je fais beaucoup mais il y a aussi a la barre, mais le pull over tout seul on peut le retirer ». Fin d'un doublon qui a occupe deux versions. ⭐⭐ LE POINT QUI DECIDE DE TOUT EST LA FORME DU RETRAIT, PAS LE RETRAIT. Le projet distingue deja RETRAIT et FUSION, et c'est ecrit dans tests/croises : « on retire du CHOIX, jamais de la MEMOIRE ». Une FUSION aurait fait MIGRER l'historique — state.js:210 REECRIT EN DUR le nom des seances stockees. Or Michel fait le pull-over aux DEUX (haltere surtout, barre aussi) : renommer ses « Pull-over » vers une variante aurait ecrit un fait faux dans son historique ET melange ses records (R29). 👉 C'est donc un RETRAIT : l'entree sort de EXLIB (invisible au selecteur), son identifiant `pull-over` reste dans EX_IDS et rejoint RETIRES_VOLONTAIREMENT avec sa raison ecrite (R30). Verifie dans un vrai navigateur : seance passee et record gardent leur nom NON RENOMME, avec muscles (Grand dorsal + pec/triceps/dentele) et MET 5,5 intacts. ⭐⭐ ET UNE JUMELLE A ETE TROUVEE EN LA CHERCHANT (R8) : 4 equivalences d'import visaient encore « Pull-over » (`cable pullover`, `pullover poulie`, `straight arm lat pulldown`, `straight arm pulldown`) — un import s'y serait rattache a un exercice qui n'existe plus. ⭐ Et c'etait DEJA faux avant le retrait : les quatre decrivent la version POULIE. Le retrait n'a pas cree le defaut, il l'a rendu visible. ⭐⭐ LE TEMOIN QUI VAUT PLUS QUE LE CORRECTIF : une regle generale interdit desormais TOUTE cible d'equivalence introuvable. ⚠️ Et il a fallu la calibrer : « leg curl » vise « Curl Ischio-jambiers (Leg Curl) », absent du catalogue — et c'est VALIDE, c'est l'ancien nom de « Leg Curl Couche Machine ». J'ai failli reparer ce qui marchait (R28/R30). La regle exacte est donc : au catalogue, OU s'y ramenant par exNomActuel. ⚠️ ET UN TEMOIN D'HIER EXIGEAIT L'INVERSE : « les 5 entrees pull-over restent 5 ». Il etait juste TANT QUE LA DECISION N'ETAIT PAS PRISE. Retourne vers ce qu'il protegeait vraiment — que l'historique ne soit jamais reecrit — avec sa raison d'avant conservee. *Un temoin qui fige un ETAT rougit des qu'une decision est prise ; ce qu'on fige, c'est une REGLE.*
const PRECACHE = [
