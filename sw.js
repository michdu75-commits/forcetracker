 ...
 */
const CACHE = 'ft-v937'; // 🖊️ ft-v937 = UN DECOR DE TEST PRIS POUR UN FAIT REEL — corrige DEUX FOIS par Michel. Dans le journal de ft-v936 j'avais justifie la decision EV-015 par « Christophe, persona fondateur, a un vrai coach ». DEUX ERREURS EN DIX MOTS. ① « a un vrai coach » vient du champ `resume` du persona VC-002 dans coach.js — une BIOGRAPHIE DE FICTION ecrite le 25/07 pour un scenario de test ; Michel : « Christophe n'est pas coach, c'est un sportif qui fait du body ». ② « persona fondateur » vient de docs/PERSONAS-FONDATEURS.md, ou « fondateurs » qualifie les DIMENSIONS du projet et non un statut de personne (le doc dit lui-meme « Michel = le fondateur, a part ») ; Michel : « Christophe n'est pas un fondateur hein, c'est un testeur ». ⭐ MEME MECANIQUE DEUX FOIS : un mot du vocabulaire interne (un decor de test, un nom de dimension) relu comme un fait sur une personne reelle — puis servi comme ARGUMENT pour justifier d'ajouter une regle au prompt. C'est l'hypothese-presentee-comme-un-fait que la Constitution interdit a Milo, appliquee a celui qui ecrit le code. ⚠️ LE PIEGE EST STRUCTUREL : les personas portent le PRENOM de vrais testeurs (Tatiana, Christophe, Emma) parce qu'ils s'en inspirent, mais leurs champs sont INVENTES — et rien dans le code ne distingue une fiction d'une note reelle. L'avertissement est donc pose LA OU LE PIEGE SE TEND, en tete de VC_PERSONAS, pas seulement dans le journal (R27). Les vraies infos sur les testeurs vivent dans RETOURS-TESTEURS.md. ⭐⭐ Michel complete, et c'est le point qui compte : « comme Tatiana et Emma, ils n'ont aucune action directe sur l'application ». Ce sont des TESTEURS : ils remontent des retours, MICHEL DECIDE. L'avertissement est donc pose aussi en tete de docs/PERSONAS-FONDATEURS.md, le document qui m'a induit en erreur (R27), avec la regle qui ferme les deux trous : toute affirmation sur une personne reelle vient de RETOURS-TESTEURS.md ou de Michel — jamais d'un nom de dimension, jamais d'un champ de persona. ⏭️ CONSEQUENCE : la decision EV-015 ne repose plus sur rien et se rouvre sur ses seuls merites (« un utilisateur suivi par un coach a-t-il besoin que Milo se pose en complement ? »). Aucun cas d'usage reel ne l'appuie a ce jour. Correction de DOC + commentaires : zero changement de comportement.
const PRECACHE = [
