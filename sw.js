 ...
 */
const CACHE = 'ft-v984'; // ⚖️ ft-v984 = LA QUANTITE SUIT L'ALIMENT QUAND ON LE REPREND. Michel, captures a l'appui : « comment ca se fait que je ne peux pas mettre la quantite, serieux c'est relou ». ⛔⛔ REPRODUIT DANS UN NAVIGATEUR AVANT DE TOUCHER AU CODE (lecon 12quater, appliquee cette fois) : par le chemin CIQUAL, `blocQuantite: true`. Par le chemin de SON PROPRE JOURNAL — celui qu'on emprunte des la 2e fois — `blocQuantite: false`, ALORS QUE `per100` est bien present dans la source. ⛔ `_afSuggPrendreLocale` (app.js:1991) cachait le bloc SANS CONDITION… et transmettait `per100` deux lignes plus bas. C'est R4 a deux lignes d'ecart : l'information existait et n'atteignait pas l'ecran. ⚠️ CONSEQUENCE VECUE : le mecanisme de ft-v962/965 marchait la PREMIERE fois qu'on note un aliment et disparaissait toutes les suivantes — un defaut qui ne se voit qu'a la DEUXIEME saisie, donc jamais en testant une fois. ⭐ R13/R2 : on ne reinvente rien, on reconstruit `_bcNutr` depuis le `per100` deja enregistre et le bloc existant fait le reste, exactement comme apres un scan. Le libelle dit d'ou vient la reference (« ta derniere saisie ») et la quantite reprend celle de la fois d'avant. ⛔⛔ ET ON NE RECALCULE PAS LES MACROS EN ARRIVANT : elles sont deja justes, et la personne a pu les corriger a la main. Mesure : son 29 kcal corrige reste 29, pas 48 — les reecrire aurait efface sa correction sans le dire (R29). Le recalcul part au premier changement de quantite, quand elle le demande : 50 g → 24 kcal / 6 g. ⛔ Une entree tapee A LA MAIN n'a pas de pour-100 g, donc pas de bloc : aucun poids invente. ⚠️ CE QUI N'EST PAS EXPLIQUE EST ECRIT AUSSI : sa 3e capture montre une ratatouille SANS pour-100 g (« cette ligne n'a pas de quantite connue »). Ce correctif ne la repare pas retroactivement — une entree ancienne ou entree par un chemin qui n'enregistrait pas encore `per100` reste sans quantite. On ne pretend pas avoir tout couvert.
const PRECACHE = [
