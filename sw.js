 ...
 */
const CACHE = 'ft-v923'; // 🔀 ft-v923 = L'ORDRE DES ACCESSOIRES + « c'est noté » ne notait rien. Michel, deux captures : « regarde il a inverse encore ». Milo ecrit une seance qui descend en charge — 130 → 65 → 60 → 30 → 55 : un FACE PULL de 30 kg AVANT un LEG CURL de 55. ⚠️⚠️ ET C'EST MA REGLE DE LA VEILLE QUI ETAIT INCOMPLETE (ft-v914) : elle ordonne l'ANCRE par rapport a ses ACCESSOIRES dans une zone, et ne dit RIEN de l'ordre ENTRE accessoires. Michel a confirme qu'il n'avait JAMAIS demande ce placement — donc ce n'est pas la memoire, c'est la regle qui manquait. 👉 Ajoute : dans une zone, du plus LOURD au plus LEGER (on fait le plus demandant frais) · et le petit travail de SANTE/rotation (face pull, rotateurs, gainage, mollets) FINIT la seance. ⚠️ SANS INTERDIRE l'activation avant un lourd : c'est un choix de coach valable, on demande juste qu'il soit DIT — sinon on ne distingue pas une intention d'un oubli (Constitution : adapter, jamais interdire). ⭐⭐ ET EN CHERCHANT, UN TROU REEL DU GARDIEN : Milo ecrit « c'est noté » et RIEN n'est enregistre. Le controle ne cherchait que la forme PERSONNELLE (« je note »). Mesure : « je le note » → attrape · « c'est noté » → RIEN. BUGS.md famille 15, 5e fois cette semaine. ⚠️⚠️ DOUBLE PIEGE AU CORRECTIF : le prompt DEMANDE « super, c'est noté 💪 » pour une seance annoncee (legitime, il emet {prevu}) — donc on accepte la phrase quand un bloc enregistre vraiment ; ET `\b` apres un « é » n'existe pas en JavaScript, donc mon premier motif etait MUET. C'est le test qui l'a montre, pas ma relecture.
const PRECACHE = [
