 ...
 */
const CACHE = 'ft-v1013'; // 📤 ft-v1013 = L'EXPORT « AVEC MES DISCUSSIONS » PERDAIT LA DISCUSSION DU MOMENT. Michel : « compare la difference des exportations pour tout le monde et celle que je fais en admin. C'est quoi la difference, parce que s'il n'y en a pas autant supprimer. » ⭐ IL Y EN AVAIT — et les DEUX etaient au desavantage de l'export GENERAL. ① LE FIL EN COURS ETAIT ABSENT : la boucle recopie `S.coachConversations`, qui ne contient QUE les discussions RANGEES (fermees par le « + ») ; le fil qu'on est en train d'ecrire vit dans `ft4_coach_hist`, donc dans AUCUNE cle de `S`. *Quelqu'un qui exporte « avec mes discussions » repartait SANS la discussion du moment* — souvent celle qui l'interesse le plus — et le fichier avait l'air complet. Mesure : 3 messages exportes au lieu de 4. ② LES CONSIGNES INTERNES PARTAIENT AVEC : le debrief automatique injecte des messages `_silent` que la personne n'a JAMAIS vus ni ecrits ; l'export texte les filtre depuis toujours, celui-ci les livrait. ⭐ R2 — on ne reinvente rien : on emprunte les DEUX mecanismes deja eprouves de `exporterConversationsMilo` (lecture du fil courant + filtre `_silent`). ⛔⛔ REPONSE A LA QUESTION POSEE : ON NE SUPPRIME NI L'UN NI L'AUTRE. Mesure sur les memes donnees — l'un est un fichier LISIBLE (678 car., texte date, 6 messages, zero donnee de sante), l'autre une SAUVEGARDE JSON a reimporter (5 019 car., 96 cles, bilans compris). Ils ne font pas doublon. ⭐ CE QUI ETAIT MAL RANGE : le seul fichier lisible par un humain etait reserve a l'ADMIN depuis le 05/08, alors que son propre commentaire disait pourquoi il existe — « le fil vit UNIQUEMENT sur le telephone, un changement d'appareil l'efface, on donne donc un FICHIER que la personne emporte ». Il est desormais dans la modale d'export, ouverte a tout le monde. ⛔ ET L'ENTREE ADMIN N'EST PAS RETIREE (R30) : meme fonction, deux portes — un retrait silencieux ressemble a un oubli.
const PRECACHE = [
