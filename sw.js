 ...
 */
const CACHE = 'ft-v936'; // 🔀 ft-v936 = UN ROUGE A DEUX CAUSES OPPOSEES — et on ne les corrige pas pareil. Apres 3 passes reelles, deux scenarios sont rouges 3/3 : EV-003 et EV-015. ⭐⭐ EN CHERCHANT LA REGLE DANS LE PROMPT (R7, avant de coder), LES DEUX CAS SE SONT REVELES OPPOSES : · EV-003 (le face pull avant du lourd) : la regle EXISTE, a 74 % du prompt — exactement la zone de la regle keto (67 %) qui n'etait pas suivie non plus. Regle DILUEE → 2e usage du levier §9 n°1 : rappel court en fin de prompt, seulement quand on demande une seance. · EV-015 (respecter le coach humain) : la regle N'EXISTE PAS. Les seules occurrences de « coach humain » du depot sont dans la definition du persona VC-002 — c'est-a-dire dans le TEST, pas dans le produit. On ne peut pas reprocher a Milo une consigne jamais donnee : ce test mesurait un attendu que le produit n'a jamais promis. 👉 Un ROUGE ne dit pas laquelle des deux causes ; il faut aller voir. EV-015 est donc marque `specAbsente` et COMPTE A PART (⚠️ et non ❌) : un outil qui accuse a tort finit ignore (R19). La regle a ecrire ou non est une DECISION PRODUIT de Michel, pas un bug. ⛔ Et la regle de fond de ft-v923 n'est PAS retiree : une detection ratee retombe sur le comportement d'hier, jamais sur une regle absente en silence. ⚠️⚠️ ET DEUX TEMOINS EXISTANTS ONT ROUGI — les deux mesuraient un RACCOURCI devenu faux. ① Le temoin de cache comparait la TAILLE TOTALE du contexte ; or depuis ft-v933 la queue non cachee porte des rappels conditionnels, et c'est VOULU. Mesure avant de le corriger : le prefixe cache fait 66 959 caracteres dans les deux cas, IDENTIQUE octet pour octet — le cache n'etait pas casse. Le temoin mesure desormais le PREFIXE, ce qui est plus fort. ② L'autre exigeait qu'un appelant SANS message recoive EXACTEMENT autant qu'un message donne ; le contrat dit « on envoie TOUT », donc il doit en recevoir AU MOINS autant. L'egalite stricte interdisait par construction d'avoir plus d'un rappel conditionnel.
const PRECACHE = [
