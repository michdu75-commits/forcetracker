 ...
 */
const CACHE = 'ft-v919'; // 🫀 ft-v919 = LE CERVELET — MILO PARLE, LE CERVELET TRADUIT. 1re brique de l'architecture cerveau/cervelet (docs/ARCHITECTURE-CERVEAU-CERVELET.md), sur une idee de Michel : « milo est le cerveau et l'autre ia s'occupe du reste ». ⚠️ LE PROBLEME MESURE : Milo devait produire EN MEME TEMPS une reponse lisible ET un bloc JSON valide, dont la specification pesait ~3 700 caracteres dans le prompt COMMUN — envoye a tout le monde, a chaque conversation, y compris a quelqu'un qui parle nutrition. 👉 Il ecrit maintenant sa seance EN FRANCAIS ; une 2e IA (Haiku, action `seanceJson` dans le worker) la convertit. ⭐ ELLE NE SAIT RIEN DE LA PERSONNE — ni profil, ni records, ni email : c'est le critere qui la definit (« est-ce que ca a besoin de savoir QUI est la personne ? »), et ne pas lui donner l'info est la garantie la plus simple qu'elle ne s'en servira pas. ⚠️ ET ELLE NE PARLE JAMAIS (R6, une seule voix). ⭐⭐ CASCADE A 3 ETAGES, l'ordre compte : ① le bloc cache s'il est encore la (le prompt commun est en CACHE 1 H — pendant une heure Milo peut encore l'emettre) → ② le cervelet traduit → ③ la lecture deterministe du texte (04/08) reste le filet, gratuite et hors ligne. On ne remplace jamais un chemin qui marche, on en ajoute un meilleur devant. ⚠️ L'AIGUILLAGE est deterministe et PLUS PERMISSIF que le filet, expres : lui ne fait qu'orienter (au pire un appel pour rien), l'autre CONSTRUIT la seance (R29). ⚠️ Bloc commun 46 485 → 44 157 caracteres, marge 15 → 2 343. Et un bug latent attrape par son temoin : la traduction revient en differe, le bouton se serait colle sous le message SUIVANT.
const PRECACHE = [
