 ...
 */
const CACHE = 'ft-v995'; // 🏃 ft-v995 = LE CARDIO DE MILO VA DANS SON BLOC, PAS DANS LES EXERCICES. Michel, EN SALLE le 24/08, capture a l'appui : « il me rajoute le velo elliptique alors qu'on a un onglet expres pour le cardio ». Sur sa seance : « Elliptique — 0/1 serie », type E, note « 8 min leger », pose comme un exercice de MUSCULATION, pendant que le bloc Cardio restait VIDE. ⭐⭐ SA RAISON DECIDE DE TOUT : « si on fait une seance cardio toute seule, on veut qu'elle soit comptabilisee. Mais je ne veux pas que la course, le velo elliptique ou peu importe arrive dans un exercice de musculation, ca n'a strictement rien a voir. » ⛔⛔ CE N'ETAIT PAS UN DEFAUT DE JUGEMENT DE MILO — LES DEUX BOUTS DU CHEMIN MANQUAIENT (verifie dans le code, R28) : le prompt ne lui dit nulle part qu'un bloc cardio existe pour ce qu'il PROPOSE (il ne le LIT que pour raconter le passe), et `_appliqueMiloSession` ne regardait jamais un champ cardio. Meme forme que le pont blessure de ft-v982. ⭐ R13 — RIEN N'EST REINVENTE : `_exEquip()` range deja elliptique, tapis, rameur, corde a sauter, air bike dans un bac 'cardio' depuis ft-v712. ⛔⛔ ET C'EST POSE DANS `_appliqueMiloSession`, LE SEUL POINT QUE LES DEUX PORTES TRAVERSENT (lecon ft-v980) : une seance de Milo arrive soit par le bloc JSON (modeles capables), soit par le REPLI DE LECTURE DU TEXTE (modeles legers). *Corriger seulement le JSON n'aurait rien change pour ELINE* — biais R9 deja vecu avec le bouton « Commencer cette seance », que Michel avait et sa fille jamais. ⚠️ AVANT **ET** APRES (precision de Michel le meme soir) : on tranche par POSITION — avant le 1er exercice de muscu → echauffement, apres le dernier → cardio de fin. ⛔ Un cardio AU MILIEU reste un exercice, et sans duree lisible AUCUNE duree n'est inventee (R29). ⛔ Un cardio DEJA note par la personne n'est jamais ecrase. ⭐⭐ DEUX DEFAUTS TROUVES PAR LA MESURE, invisibles a la relecture : ① ma 1re pose ecrivait le cardio AVANT que `S.wkt` soit reconstruit en mode « start » — l'elliptique sortait donc des exercices et N'ARRIVAIT NULLE PART (R4 dans sa forme la plus bete, mesure : exercices corrects mais cardioAvant `null`) ; ② l'intensite tombait sur « modere » alors que la note dit « leger » — `_naz()` desaccentue le NOM, pas la NOTE. *Meme famille que l'apostrophe courbe du banc d'essai (ft-v994) : un caractere non normalise rend un motif aveugle sans que rien ne le signale.* Cout reel : 4,0 contre 6,0 MET, soit **50 % d'ecart** sur les calories de ce cardio. ⚠️ CE QUI N'EST PAS FAIT, ET C'EST ECRIT : le prompt ne NOMME toujours pas le bloc a Milo. Le correctif agit apres coup, donc les donnees sont justes — mais Milo continuera d'ECRIRE « Elliptique 1x10 » dans sa phrase. C'est le dernier levier (R7), a faire ensuite.
const PRECACHE = [
