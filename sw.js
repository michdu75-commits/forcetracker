 ...
 */
const CACHE = 'ft-v911'; // 🔄 ft-v911 = « TES REPAS HABITUELS » — UN APPUI, ZERO FORMULAIRE. Michel decrit sa vraie journee : shaker + banane le matin, steak-riz midi et soir, tous les jours. Trois fois par jour un formulaire a cinq champs, personne ne tient — et c'est la raison n°1 pour laquelle un suivi nutrition s'arrete. ⚠️ ON N'INVENTE RIEN ET ON NE STOCKE RIEN DE PLUS : un « repas habituel » n'est pas declare, il est OBSERVE dans le journal — les aliments notes ENSEMBLE, le meme jour, sur le meme repas, au moins DEUX fois. Une fois c'est un repas, deux fois c'est une habitude. ⚠️ ET QUI MANGE DIFFEREMMENT CHAQUE JOUR NE VOIT RIEN DU TOUT : pas de section vide, qui serait un reproche deguise (R24). C'est la limite que Michel a lui-meme posee — « ca c'est moi qui le fais, les autres peut-etre pas ». ⚠️ La provenance dit « reprise » (brique 0) : ni une mesure fraiche, ni une saisie manuelle — sans quoi un chiffre repris finirait par passer pour une mesure. Et un repas deja rejoue aujourd'hui ne se re-propose pas.
const PRECACHE = [
