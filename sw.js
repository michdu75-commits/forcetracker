 ...
 */
const CACHE = 'ft-v1084'; // 🦴 ft-v1084 = UNE SEANCE A UN SUJET — MILO EST ENFIN PREVENU. Michel : « j'ai pas envie que Milo me repropose une seance bizarre », « ca me fait depenser de l'API en lui disant », « si un mec demande une seance et qu'il sort ca, ca fait pas serieux ». ⛔⛔ R8 DANS SA FORME LA PLUS PURE : `_movPattern()` classe le souleve de terre ET le roumain en `hip-hinge` DEPUIS TOUJOURS (ca sert au calcul des calories), et `_validationSeance` le signale depuis ft-v1080 — mais APRES COUP. ***Rien, dans ce que Milo recoit, ne lui disait de ne pas l'ecrire.*** *On ne durcit pas un prompt tant qu'on ne lui a pas donne le fait.* ⛔ La regle est posee DANS LE BLOC PERSONNEL, la ou vit deja la regle d'ordre — le bloc commun est plafonne a 46 500 caracteres et on ne releve pas le seuil pour se faire de la place. 0 dilution, 0 appel API en plus. ⭐⭐ ET LA 3e LIGNE — CELLE QUI NOMME CE QUI RESTE PERMIS — VIENT D'UNE MESURE, PAS D'UNE PRUDENCE : sur **140 seances reelles de 8 comptes**, un critere « deux familles lourdes » sans filtre accusait **9 seances normales**, dont un `poussee×2 tirage×2` **cinq fois de suite chez la meme personne** (son programme). Sans elle, Milo sur-corrigerait et refuserait un dos complet. ⚠️⚠️ ET C'EST LA 2e FOIS : meme plainte le 22/08 (« on est retourne sur les jambes »). Le scenario EV-041 existait deja — mais il compte les BASCULES d'ORDRE, donc la seance REORDONNEE le passerait en gardant ses deux charnieres. D'ou EV-055, qui mesure la COMPOSITION.
