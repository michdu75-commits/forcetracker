 ...
 */
const CACHE = 'ft-v925'; // 🔁 ft-v925 = LE BOUTON SORT MAINTENANT DANS TOUS LES MODES DE PANNE. 2e retour de terrain le meme jour : « ca ne fonctionne toujours pas », app bien en ft-v924. ⭐ MESURE D'ABORD, ET ELLE A CHANGE LA RECHERCHE : son texte se lisait PARFAITEMENT en local — aiguillage true, filet 5 exercices, bons noms, bon ordre. Le defaut n'etait donc PAS la lecture, il etait APRES. DEUX TROUS, tous deux INVISIBLES : ① `_appendStartSessionBtn` sortait EN SILENCE quand la seance etait structurellement pauvre (exercices sans series) ; comme l'appelant ecrivait `_montee(s) || _filet`, une seance vide restait « truthy », le repli ne jouait pas, et il n'y avait NI bouton NI filet. ② `fetch` n'a AUCUN delai par defaut : sur une 5G capricieuse — donc a la salle — l'appel reste suspendu indefiniment, le `.then` ne part jamais, le repli n'est jamais atteint. *Une panne franche se rattrape ; une attente infinie, non.* 👉 La pose du bouton REND desormais si elle a reussi, l'appelant retombe sur le filet sinon, et l'appel est coupe a 12 s. ⚠️ ET MON PREMIER TEMOIN NE PROUVAIT RIEN : ecrit avec la NOUVELLE facon d'appeler, il passait aussi sur l'ancien code (ou la fonction rend `undefined`, donc « faux », donc le repli jouait quand meme). 8e fois ce piege — il est ecrit la ou il se produit.
const PRECACHE = [
