 ...
 */
const CACHE = 'ft-v945'; // 🌍 ft-v945 = LA MESURE CONTINUE, CHEZ DE VRAIS UTILISATEURS. Michel : « mais je veux une mesure continue ». ⭐⭐ LE FOND, ET C'EST LUI QUI L'A SOULEVE : « n'oublie pas Milo avec moi, il est debride ». Verifie dans le code — `_estSuperAdmin()` lui ouvre DEUX portes fermees a tout le monde : citer/analyser ses propres consignes, et parler de n'importe quel sujet (le filtre hors-sujet local est meme desactive pour lui). Le modele, lui, est le MEME (Sonnet 4.6 pour tous). ⚠️ CONSEQUENCE DIRECTE SUR ft-v944 : le Gardien venait d'etre calibre sur SES 129 reponses — l'echantillon le moins representatif du parc. L'un des 4 faux positifs (« Ce que je retiens : uniquement dans ton profil ») n'existe QUE parce qu'il est debride : Milo n'a pas le droit d'expliquer son fonctionnement aux autres. C'est le cousin de R9 : on evaluait le mauvais Milo. 👉 LE COMPTEUR REMONTE DESORMAIS avec la sauvegarde — vers Apps Script ET le miroir Supabase (le corps est construit une seule fois, R2). ⛔ DES NOMBRES SEULEMENT : ~150 octets, {depuis, dernier, total, codes}. Aucune phrase de Milo, aucun mot de la personne — les conversations ne quittent toujours pas le telephone. Le serveur RECONSTRUIT l'objet au lieu de le recopier : un client modifie ne peut pas glisser du texte par ce champ. ⚠️ ET ON NE LE CACHE PAS (« je ne veux pas leur cacher ») : la carte des conversations le DIT en toutes lettres. ⛔ Milo, lui, ne recoit PAS ce compteur : lui donner son propre score l'inviterait a le commenter — exactement la sortie de role qu'on traque. ⚠️ Le garde-fou R4a a REFUSE la livraison tant que `gardienStats` n'etait pas classee : elle est EXCLUE, avec la raison ecrite. ⚠️ Et mon temoin a d'abord attrape le miroir Supabase (corps enveloppe dans `p_data`) et rendait « absent » alors que le compteur partait bien — il deballe desormais, et couvre les deux chemins.
const PRECACHE = [
