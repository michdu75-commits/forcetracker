 ...
 */
const CACHE = 'ft-v1060'; // 👎 ft-v1060 = « MILO A REPONDU A COTE » — LE POUCE COMPTE, IL NE RACONTE RIEN. Michel : « j'aimerais savoir si Milo deconne quand les utilisateurs posent une question… 1 appel API qui sert a rien, et s'il met 2 ou 3 reponses avant de tomber juste. LA ca me coute de l'argent pour rien. J'appelle ca ameliorer le service. » ⛔⛔ ET IL A ECARTE LA SURVEILLANCE LUI-MEME : « je ne veux pas savoir ce qu'ils disent a Milo, je m'en fous » — d'ou toute la forme du correctif : on mesure la QUALITE, jamais le contenu. Un bouton discret « 👎 a cote » sous chaque reponse, 4 motifs fermes (a cote · trop vague · faux · il a oublie), compte EN LOCAL, ZERO appel reseau (mesure : 0). Le contenu ne part QUE si la personne coche « joindre l'echange » — DECOCHEE par defaut, parce qu'un consentement pre-coche n'en est pas un (Constitution P3). ⛔ PAS DE POUCE VERT, expres : un 👍/👎 sous chaque reponse transforme la conversation en formulaire de satisfaction, et le taux de clic dirait surtout qui est poli — on ne demande que ce dont on fera quelque chose (R19/R24). ⭐ R13 : zero composant neuf, zero CSS ajoute (le pied de bulle et .coach-share-btn existent depuis le bouton Partager ; les pastilles reutilisent .ck-opt du check-in).
const PRECACHE = [
