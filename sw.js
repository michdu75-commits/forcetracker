 ...
 */
const CACHE = 'ft-v1036'; // 📍 ft-v1035 = UNE CHARGE PRESCRITE SANS REPERE LE DIT. Applique le critere donne par Michel : « la coach savait que moi je m'y connais ; tout le monde ne connait pas ce que represente le lourd ». Elle ecrit « lourd » parce qu'un REFERENTIEL COMMUN existe entre eux ; Milo parle a des gens dont il ne sait parfois rien. ⛔⛔ TROU MESURE : `_intensiteDefauts` commence par `if(!(rm1>0)) return out;` — aucun record, le controle se tait ENTIEREMENT, et Milo pouvait ecrire « 4x8 a 60 kg » sur un exercice jamais fait sans qu'un mot ne soit dit (ft-v980 prive de garde-fou). ⛔ ON NE RETIRE PAS LE NOMBRE (R24) : une charge pre-remplie fait gagner du temps en salle ; ce qui etait faux, c'est qu'elle soit presentee comme calibree. ⚠️⚠️ LE PIEGE EST LE NOM : mesure, exNomCatalogue('Developpe Couche') rend la chaine TELLE QUELLE — sans comparaison normalisee (_normEx, reutilisee) on annoncait « pas de repere » a quelqu'un dont le record existe, un FAIT FAUX sur la personne (R29). ⛔ Et l'historique compte aussi comme repere, pas seulement le 1RM. ⚠️ Corrige au passage : `_validationSeance` faisait `o.seanceWarn=w`, une AFFECTATION qui aurait efface la nouvelle ligne — un champ partage se remplit par AJOUT.
const PRECACHE = [
