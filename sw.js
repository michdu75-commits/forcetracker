 ...
 */
const CACHE = 'ft-v1096'; // 📤 ft-v1096 = EXPORTS DATES (NUTRITION · POIDS) · LE DOUBLON DE SEANCE REPARE · L'ONGLET RENOMME. Michel : « il faudra creer un export date de la nutrition et aussi cote poids », puis « repare le dedoublement toi-meme ». 📤 LES DEUX EXPORTS reutilisent le patron de `exportHistoCsv` (R13) — meme echappement CSV, meme separateur `;` + BOM (Excel FR ouvre le `,` en UNE colonne et rend « Developpe » en « DÃ©veloppÃ© » sans BOM), meme feuille de partage iOS, meme toast qui sait dire « je ne sais pas ». UN SEUL proprietaire de l'echappement (R2) au lieu de trois copies. ⛔ La NUTRITION exporte aussi `saisie` et `source` : sans la provenance, on ne sait plus si une valeur a ete SCANNEE, tapee ou estimee par l'IA. ⚠️ Le POIDS lit `kg` et PAS `bw` — la doc du projet a dit `bw` pendant des mois alors que l'app ecrit `kg` partout (corrige le 30/08). 🩹 LE DOUBLON, ET IL VENAIT D'UN CORRECTIF : la fusion multi-onglets de ft-v1094 signait une seance `date|nb|volume` — or ***corriger une charge change le volume***, donc la version corrigee et celle du disque avaient deux signatures et l'union GARDAIT LES DEUX. Le correctif qui repare une perte de seance fabriquait un doublon, declenche par le geste le plus banal qui soit. ⭐ Le proprietaire existait deja (R2) : `s.ts || s.id`, ce qu'emploient `openSessDetail`, la suppression et la mise a jour. ⚠️⚠️ ET MON PROPRE CORRECTIF PORTAIT UN RISQUE SYMETRIQUE, MESURE AVANT D'ETRE ECRIT : avec un repli sur `date|nb`, DEUX VRAIES seances du meme jour fusionnaient en UNE — un doublon echange contre une PERTE, le mauvais sens (R29). Le nom du 1er exercice les separe : il ne change pas quand on corrige un poids, contrairement au volume. Six cas mesures, six verts. 🧭 L'ONGLET « Poids » devient « Corps & sante » (choix de Michel) : il porte aussi les correlations, le bilan corporel et le bilan sanguin. MESURE a 3 largeurs : identique a 390 et 375 px, +18 px de haut a 320 px (le libelle passe sur deux lignes, rien ne deborde). ⚠️ Ma 1re sonde mesurait 0 PARTOUT en annoncant « ne deborde pas » : `goScreen` prefixe lui-meme. Elle verifie desormais que l'ecran est ACTIF avant de mesurer — sinon c'est un vert qui ne mesure rien.
// ⬆️ 18e collision : session-A a publie ft-v1094 puis ft-v1095 pendant ce travail. Ma version
// devient ft-v1096 — un numero de cache ne recule jamais.
const PRECACHE = [
