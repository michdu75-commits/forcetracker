 ...
 */
const CACHE = 'ft-v1015'; // ☁️ ft-v1015 = UN ECHEC DE SYNC GOOGLE SHEETS ETAIT COMPTE COMME UN SUCCES. Trouve en creusant « j'ai l'impression qu'il n'y a pas d'historique » (Michel). ⛔⛔ `syncSheets` REND UN OBJET, PAS UN BOOLEEN — et un objet est TOUJOURS vrai : `const ok=await syncSheets(sess); if(ok)` prenait `{ok:false,error:'Timeout (8s)'}` pour un succes. ⚠️ DEUX DEGATS, ET LE SECOND EST LE VRAI : ① le toast annoncait « Seance synchronisee ! » alors que rien n'etait parti ; ② surtout, `synced=true` etait pose — or la file de rattrapage `_retrySheetQueue` filtre `s.synced===false`, donc **une seance perdue en route n'etait JAMAIS reprise**, ni au retour du reseau, ni au demarrage suivant. *Le seul mecanisme de secours etait desarme par la ligne censee constater le succes.* ⭐ L'AUTRE APPELANT LISAIT `res.ok` CORRECTEMENT DEPUIS TOUJOURS (`_retrySheetQueue`, tracking.js) : deux copies du meme geste, une juste, une fausse (R2) — et la fausse etait sur le chemin le plus frequent, la fin de seance. ⭐⭐ MESURE PAR LE VRAI CHEMIN, PAS RELUE : `finishWorkout` appelee pour de bon, seul `fetch` remplace. AVANT — toast « Seance synchronisee ! », `synced:true`, **0** seance en file. APRES — « Seance sauvegardee ! », `synced:false`, **1** en file. ⚠️ ET LE TEMOIN DOIT COUPER `_demoMode` : `seedScript` le pose a `true` et `syncSheets` rend alors `{ok:true}` sans toucher au reseau — sans cette ligne il serait vert en ne mesurant rien.
const PRECACHE = [
