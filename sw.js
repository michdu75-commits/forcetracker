 ...
 */
const CACHE = 'ft-v928'; // 📋 ft-v928 = LE RECAP DU DEBRIEF EST ECRIT PAR LE CODE, plus par Milo. Michel : « comme le debrief est automatique autant le faire en haiku, ca coute pas cher ». ⚠️ ON N'A PAS CHANGE DE MODELE, et le chiffre tranche : ~15 debriefs/mois, l'ecart Sonnet/Haiku vaut ~0,17 €/mois — pour ca on degraderait precisement le message dont il venait de se plaindre. Et R9 : un modele leger suit MAL les consignes fines, or on venait d'en ajouter une exigeante (« couvre les 5 exercices »). La decision « Sonnet pour tout le monde » est ecrite dans worker.js avec ses mots du 10/08, marquee a ne pas re-proposer (R30). ⭐⭐ MAIS SON INTUITION AVAIT UNE MOITIE JUSTE, et c'est EXACTEMENT sa frontiere cerveau/cervelet : LISTER est une TRANSFORMATION, COMMENTER est un JUGEMENT. La liste ne merite donc meme pas Haiku — elle merite du CODE. 👉 `_recapSeance()` ecrit desormais la liste complete (nom, series au format reps×poids, echauffements comptes) au-dessus de la reponse de Milo, gratuitement et hors ligne, comme le recap de fin de seance. Milo ne PEUT plus sauter un exercice, au lieu qu'on lui DEMANDE de ne pas le faire — et sa consigne lui dit de commenter sans recopier. ⚠️ PORTEE HONNETE : ca ne couvre que le debrief AUTOMATIQUE, dont le declenchement est deterministe ; quand la personne demande « que penses-tu de ma seance » en plein chat, seule la regle du prompt (ft-v927) reste, et elle est plus faible.
const PRECACHE = [
