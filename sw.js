 ...
 */
const CACHE = 'ft-v942'; // 🔐 ft-v942 = L'APP DEMANDE LE MOT DE PASSE D'UN PDF PROTEGE. Michel : « j'ai envie de mettre ma prise de sang mais c'est protege par un mot de passe, je vais comment ? ». Les laboratoires livrent tres souvent leurs bilans en PDF chiffre, et l'app rendait « Souci lecture fichier » — un message qui dit qu'il y a un probleme SANS dire lequel, donc sans dire quoi faire. La personne n'avait aucun moyen de deviner qu'il suffisait d'un mot de passe. ⭐ LE CORRECTIF VIT DANS `_pdfOuvrir` ET PAS DANS L'IMPORT DU BILAN (R2) : QUATRE imports lisent des PDF (bilan sanguin, programme, historique, repas) et heritent tous du meme comportement — un seul proprietaire de l'ouverture, donc aucune divergence possible. ⛔ LE MOT DE PASSE NE QUITTE JAMAIS LE TELEPHONE : pdf.js dechiffre EN LOCAL dans le navigateur, ce sont les IMAGES rendues qui partent ensuite ; il n'est ni stocke, ni synchronise, ni envoye — un temoin compte 0 appel reseau pendant toute l'ouverture. ⚠️ Honnetete ecrite dans le code : `prompt()` affiche ce qu'on tape en clair, ce n'est pas un champ masque. ⛔⛔ LES DEUX TEMOINS QUI PROTEGENT LE PLUS SONT DES SORTIES : annuler sort et ne redemande pas · TROIS mauvais mots de passe ARRETENT tout. Sans ce plafond, un mot de passe qu'on ne retrouve pas piegerait la personne dans une suite de fenetres sans fin. ⚠️ ET LE GARDE EST ETROIT (R19) : pdf.js signale le chiffrement par une exception NOMMEE, donc un fichier corrompu remonte tel quel et ne fait reclamer AUCUN mot de passe qui n'existe pas. ⚠️ Detail paye : une COPIE fraiche du tampon a chaque essai — pdf.js prend possession du buffer et le detache, le reutiliser ferait echouer la 2e tentative pour une raison sans rapport avec le mot de passe.
const PRECACHE = [
