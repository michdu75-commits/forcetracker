 ...
 */
const CACHE = 'ft-v1057'; // 🔢 ft-v1057 = LA VIRGULE DECIMALE — neuf mots d'ELINE, et une corruption de donnees. Retour de la fille de Michel dans la boite a idees : « impossible de mettre la virgule pour les poids ». ⛔⛔ ELLE DECRIT UN REFUS, LA MESURE DIT BIEN PIRE : dans un `type="number"`, taper « 62,5 » rend **"625"** — le navigateur JETTE la virgule et garde les chiffres. Mesure au clavier sur le champ le plus utilise : serie enregistree a 625 kg, 1RM a 776 kg — qui partait dans ses RECORDS et sa COURBE. Et ailleurs `parseFloat('62,5')` rend 62. Deux facons de se tromper, toutes deux silencieuses. ⭐⭐ LE MOT DE LA PERSONNE DECRIT LE SYMPTOME, JAMAIS LA CAUSE : s'arreter a « impossible » faisait chercher le clavier et rater les seances deja fausses. Nouvelle famille §23 de BUGS.md — un champ qui « refuse » peut en fait MUTILER, et un nombre faux mais CREDIBLE ne se voit jamais. ⭐⭐ ET L'APP SAVAIT DEJA LIRE UNE VIRGULE, A DIX ENDROITS — mais seulement pour du texte venu d'AILLEURS (une phrase, un rapport de balance, Milo). Jamais pour ce que la personne TAPE. 👉 22 champs decimaux en `type="text"` + `inputmode="decimal"` (le pave chiffre RESTE) et 41 lectures par un seul lecteur `numFR` (R2). ⚠️ Le plus critique echappait a la recherche : `upSet` (le kg de serie) recoit sa valeur EN ARGUMENT depuis le HTML, pas via `.value`.
const PRECACHE = [
