 ...
 */
const CACHE = 'ft-v1025'; // 🍽️ ft-v1025 = L'ONGLET MACROS REORGANISE (chantier docs/MACROS-A.md, variante A). ⭐ LE DEFAUT ETAIT MESURE : 2 649 px (3,1 ecrans) avant de savoir ou on en est, la CIBLE ecrite DEUX FOIS a 200 px d'ecart, l'action la plus frequente (« noter ce que je mange ») a 1 783 px du haut, et 647 px de FORMULAIRE DE REGLAGES poses au milieu du contenu quotidien. 👉 Ce qu'on fait AUJOURD'HUI monte en tete (carte du jour : le reel en gros, la cible une seule fois en petit, 3 anneaux SVG de 92 px, « ce qu'il te reste »), ce qui EXPLIQUE le calcul et ce qui se REGLE deux fois par an descend dans deux accordeons REPLIES. ⛔ RIEN N'EST SUPPRIME (R30) : BMR, TDEE, hydratation, repartition, charge/decharge, regime, jeune — tout est encore la, ailleurs. ⛔⛔ ET AUCUN `id` N'EST RENOMME : renderNutrition() remplit les memes trous qu'avant et ne sait pas qu'ils ont bouge — un `id` renomme casserait le remplissage EN SILENCE (l'ecran resterait sur « — »), d'ou le temoin qui verifie les 11 trous. ⭐ R13 : l'accordeon est `details.acc`, celui du Profil et du menu admin — natif, zero JS, clavier gratuit. ⚠️ DEUX ERREURS DU BRIEF CORRIGEES A LA MESURE : il renvoyait vers « le composant SVG existant, celui de l'anneau de recup » (qui est en conic-gradient + masques, pas en SVG — le vrai anneau SVG est celui de la bande des 7 jours), et il appelait `#nu-cycle` « charge/decharge » (c'est l'explication du cycle des glucides ; les boutons sont `.phase-row`) — le suivre au mot aurait range le mauvais bloc. ⚠️ ET SA FUSION carte-du-jour + semaine A ETE REFUSEE PAR LA MESURE : elle donnait 958 px pour les 5 premiers blocs, contre un objectif annonce de 844. La semaine reste une carte a part, avec le retrospectif. Mesure finale : 793 px, total 1 439 px (−46 %), « noter » a 415 px. 🔴 Bouton central « + » : 44x44, cx 166,8, top 792 — identique avant/apres, mesure a trois moments. 🧠 « Ce que l'app a appris » DEPLACEE du Journal vers Macros (decision Michel) : deplacee, pas dupliquee (R2).
const PRECACHE = [
