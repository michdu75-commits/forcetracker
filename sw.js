 ...
 */
const CACHE = 'ft-v1029'; // 🐢 ft-v1028 = LE TEMPO DESCEND JUSQU'A LA DONNEE. Vient des 6 programmes ecrits par la coach de Michel (2023) : le TEMPO y est une COLONNE (« 3 sec descente, 2 sec contraction »). ⚠️ J'avais dit « le tempo n'existe nulle part » : FAUX — la consigne libre par exercice existe, s'affiche en seance et part chez Milo. ⛔⛔ LE TROU EST AILLEURS, et c'est R4 dans sa forme la plus pure : le tempo est de la PROSE dans une note et une CONSTANTE (SEC_PAR_REP=3) dans le calcul — dont le commentaire dit lui-meme « le tempo courant ». Les deux ne se rencontraient jamais : une serie de 10 reps a 3+2 s dure 60 s de travail, l'app en comptait 40. ⛔ Le piege n'est pas le tempo, c'est le REPOS : « 45 sec max » est la colonne « Repos maximum » de la coach — deux garde-fous (le texte doit parler du MOUVEMENT, et les secondes de repos sont retirees avant de compter) + une borne 1-15 s/rep. Rien de chiffrable → on se TAIT, jamais « 3 par defaut ». ⛔⛔ ET LE VRAI DEFAUT DU JOUR EST AILLEURS ENCORE, trouve en lisant la ligne d'a cote : `ex.note ? '…' : '' + _intensiteBandeau(ex)` se lit `: ('' + bandeau)` — un exercice qui portait une CONSIGNE perdait son bandeau 🚫 exclusions / 🛡️ blessures, c'est-a-dire la sortie du Gardien. Et ce sont justement les exercices venus d'un PROGRAMME qui ont une consigne.
const PRECACHE = [
