 ...
 */
const CACHE = 'ft-v1024'; // 🧹 ft-v1024 = « SCANNER » ET « IMPORTER UN JOURNAL » : RANGES, PAS RETIRES. 5e et derniere brique du chantier ecran Seance (docs/SEANCE-DESSAI.md §8). ⛔⛔ MESURE AVANT DE TOUCHER AU CODE : ces deux actions servent UNE FOIS DANS UNE VIE, et elles prenaient 200 px PLEINE LARGEUR — contre ~110 px sur une DEMI-rangee pour « + Creer ma seance », qu'on fait a chaque seance. *La place visuelle disait exactement l'inverse de la frequence d'usage.* ⛔ ELLES NE SONT PAS SUPPRIMEES (R30) : un retrait silencieux ressemble a un oubli, et elles restent le chemin le plus rapide pour qui arrive d'une autre app ou d'un carnet papier. ⭐⭐ ET LE RANGEMENT SUIT L'USAGE, IL N'EST PAS UNIFORME : quelqu'un de VRAIMENT nouveau (0 programme ET 0 seance) les voit DEPLIEES — les replier lui cacherait precisement ce dont il a besoin (R29) ; tous les autres ont une ligne discrete de ~35 px qui les deplie en un tap. Mesure : 200 px → 35 px. ⚠️ L'etat n'est PAS persiste, comme `_checkinOpen` : c'est un confort de session, pas une preference — *une preference qu'on n'a jamais demandee est une preference inventee*. ⛔⛔ LE BOUTON CENTRAL « + » N'A PAS BOUGE D'UN PIXEL (regle d'or #9) : [152,880,63] avant ET apres, mesure. ⚠️ ET MON APPEL A `_renderImportRow` A D'ABORD ATTERRI DANS LE MAUVAIS BLOC — ancre sur la 1re occurrence de `_startWktChrono()`, qui vit AUSSI dans le gestionnaire de pause. 3e fois aujourd'hui : famille « premier match gagnant ». ⏭️ CE QUE CA NE REPARE PAS : le grand VIDE de l'ecran (~700 px sous les actions) — la plainte d'origine de Michel. Ranger l'a mecaniquement AGRANDI. Ce qui doit le remplir est le §2.1 du cadrage (les types de seances), pas encore construit.
const PRECACHE = [
