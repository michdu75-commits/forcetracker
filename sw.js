 ...
 */
const CACHE = 'ft-v943'; // 📈 ft-v943 = L'EVOLUTION DU BILAN SANGUIN ATTEINT MILO — mais il ne l'ouvre JAMAIS lui-meme. Michel : « qu'il voie l'evolution, comme la courbe du poids, et tous les marqueurs, mais il ne le dit que si on lui demande par contre ». ⚠️ AVANT, ON N'ENVOYAIT QUE LE DERNIER BILAN (`bt[0]`) et une SELECTION de marqueurs — alors que l'ECRAN, lui, comparait deja chaque marqueur au bilan precedent (fleches ▲/▼ chiffrees). L'app savait, Milo pas : R4/R8 dans sa forme la plus nette — la donnee existe et n'atteint pas celui qui en parle. 👉 Partent desormais : TOUS les marqueurs + jusqu'a 3 bilans anterieurs avec leurs dates. ⭐ R13 : le motif « valeur + ecart vs bilan precedent » est repris du BILAN CORPOREL juste au-dessus, pas reinvente. ⛔⛔ ET LE POINT LE PLUS DELICAT EST LE 3e DE SA PHRASE : donner PLUS de donnees medicales rend mecaniquement plus probable que Milo en parle tout seul — un modele commente ce qu'on lui donne. La regle « seulement si on demande » est donc posee JUSTE A COTE de la donnee qu'elle encadre (la ou la regle keto avait echoue en vivant a 67 % du prompt), et surtout rendue MESURABLE par un nouveau scenario du benchmark, EV-016 : la personne demande une SEANCE, Milo doit rester muet sur le bilan. Une consigne qu'on ne mesure pas n'est qu'un espoir. ⚠️ Le corpus passe a 16 scenarios — et le PRIX annonce se CALCULE desormais (`_EV_PRIX`, meme source que « rejouer les rouges ») au lieu d'etre ecrit en dur pour 15 : un cout annonce faux est pire qu'un cout non annonce, puisque Michel decide de depenser sur ce chiffre. ⚠️ Deux erreurs payees : une virgule en trop a fabrique un 17e element VIDE dans le tableau (le temoin epingle donc le nombre) · et mes 1ers temoins prenaient TSH et glycemie, que l'ANCIEN code envoyait deja — verts des deux cotes, ils ne prouvaient rien ; il a fallu un marqueur ni hors norme ni dans l'ancienne liste (Sodium) pour que la mesure discrimine.
const PRECACHE = [
