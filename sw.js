 ...
 */
const CACHE = 'ft-v953'; // ⏳ ft-v953 = QUAND SERAI-JE REVENU AU MAX. Michel : « peut-on rajouter un indicateur ou l'on peut retrouver 100 % de notre forme ? parce que la on ne sait pas quand on aura recupere au max ». ⭐ LE SCORE DIT OU ON EN EST, PAS QUAND CA SERA FINI — or c'est la question qu'on se pose vraiment le lendemain d'un gros leg day, et la reponse est CALCULABLE a la minute. ⛔⛔ MAIS ON NE PROJETTE AUCUN CHIFFRE : annoncer « tu seras a 93 jeudi » supposerait de connaitre les NUITS QUI N'ONT PAS ENCORE EU LIEU — or le sommeil EST la base du score. Un nombre projete serait une invention presentee comme un calcul (R29, Principe 18). On rend donc ce qui est EXACT (le moment ou la fatigue MECANIQUE sera partie) et on NOMME ce qui dependra de la personne, sans le chiffrer. ⚠️⚠️ ET CETTE VERSION CORRIGE UNE ERREUR DE ft-v952, LIVREE LA VEILLE : « ton maximum atteignable est 93 » etait FAUX tout court. Le bonus de REPOS (+12 apres 4 jours sans seance) compense les facteurs permanents — 100 de sommeil + 12 de repos − 3 d'age − 4 de tabac = 105, ramene a 100. Donc 100 EST atteignable, mais en ne s'entrainant pas. Le texte dit desormais « tant que tu t'entraines regulierement, ton maximum est 93 » et nomme l'autre a cote : un plafond annonce trop bas est aussi trompeur qu'un plafond invisible, il ferait renoncer a un chiffre reellement possible. Verifie par un temoin qui MESURE le 100 apres 4 jours de repos, pas qui le suppose. ⚠️ R2 : la penalite de fatigue est sortie en fonction commune, pour que la DATE annoncee lise exactement le meme chiffre que le SCORE — deux barèmes finiraient par annoncer une date qui ne correspond pas. ⚠️ Et un temoin a attrape un decalage d'une minute : a l'instant exact, Math.round(0,5) rend 1 et non 0 — on annoncait la fin de la fatigue une minute trop tot.
const PRECACHE = [
