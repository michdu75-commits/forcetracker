 ...
 */
const CACHE = 'ft-v1009'; // 🔁 ft-v1009 = LE SELECTEUR D'EXERCICES RESTE OUVERT APRES UN AJOUT. Premiere brique du chantier ecran Seance (docs/SEANCE-DESSAI.md §8, point 1), et le goulot que Michel a senti en premier en preparant la creation de programmes : « il va falloir ameliorer aussi l'acces aux exercices... et il va falloir que ce soit rapide ». ⛔ AVANT : `addExercise()` appelait `closeExPicker()` a CHAQUE ajout — monter une seance de 6 exercices demandait 6 ALLERS-RETOURS (rouvrir, retaper une recherche, six fois). Mesure : les 6 ajouts se font desormais d'affilee, la recherche est videe et prete entre chaque, et le titre dit combien on a ajoute. ⛔⛔ SEUL LE MODE « WORKOUT » RESTE OUVERT, et c'est la nuance qui compte : `replace`, `replaceSess`, `addSess`, `prog` et `addToGroup` designent UNE place precise et se ferment tout seuls (ils rendent plus haut dans la fonction, ils ne passent jamais par ce chemin). Un mode « remplacer » reste ouvert AJOUTERAIT au lieu de remplacer, en silence — un temoin l'epingle et verifie qu'il a bien remplace SANS rien ajouter. ⚠️ ET LE SCROLL A DU DEMENAGER AVEC : `scrollIntoView` faisait defiler l'ecran DERRIERE la modale — un mouvement invisible, sur une page qu'on ne regarde pas, et a la fermeture on se retrouvait n'importe ou. Il est reporte a `closeExPicker`, au moment ou l'ecran redevient visible, et seulement si on a reellement ajoute quelque chose. ⛔ LA SORTIE RESTE EVIDENTE (R24 : informer sans bloquer) : bouton « Fermer », poignee et tap a l'exterieur marchent tous. Le focus n'est PAS remis dans la recherche sur mobile — le clavier masquerait la liste. ⛔⛔ LE BOUTON CENTRAL « + » N'A PAS BOUGE D'UN PIXEL (regle d'or #9) : mesure avant/apres (152,880,63 des deux cotes), pas regarde.
const PRECACHE = [
