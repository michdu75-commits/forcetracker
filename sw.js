 ...
 */
const CACHE = 'ft-v968'; // 📋 ft-v968 = LE JOURNAL RANGE PAR REPAS + LA 2e COLLATION. Michel : « c'est un peu le foutoir la, il faudrait ranger tout ca. La c'est une liste, il faut les ranger et creer des lignes deroulantes pour chaque section » + « pouvoir rajouter une collation aussi, il y en a qui prennent une collation le matin et le soir ». ⭐⭐ LE VRAI PROBLEME N'ETAIT PAS L'AFFICHAGE, C'ETAIT LE TRI : la liste etait triee par HEURE DE SAISIE — on note son petit-dejeuner a midi et sa collation le soir, donc les repas se melangeaient. C'est desormais l'ordre du REPAS qui commande, et FOOD_MEALS suit l'ordre de la journee. ⭐ R13 — MEME MOTIF QUE LE MENU ADMIN (ft-v955) : `<details>` natif, ZERO JS, donc ca tient meme si un script tombe, et le clavier comme les lecteurs d'ecran le gerent gratuitement. Chaque section porte son total kcal et proteines. ⛔ UNE SECTION VIDE NE S'AFFICHE PAS : « Collation 2 — 0 aliment » chaque jour ferait de l'ecran la liste de ce qu'on n'a PAS mange (R24). ⭐⭐ ET L'ETAT PLIE SURVIT AU RE-RENDU : sans memoire, ajouter un aliment redeplierait tout ce que la personne vient de replier — un defaut qu'on ne voit qu'a la DEUXIEME action, donc jamais en testant une fois. ⛔⛔ LE PIEGE EVITE, ET IL ETAIT SILENCIEUX : le repli valait `FOOD_MEALS[1]`, qui DESIGNAIT le dejeuner ; avec le nouvel ordre l'index 1 devient la COLLATION, et toute entree au repas inconnu serait devenue une collation sans que rien ne le signale. *Un index qui depend de l'ordre d'un tableau devient faux le jour ou on trie ce tableau* (R14). ⛔ `collation` GARDE SA CLE : la renommer aurait orpheline toutes les entrees deja notees. ⚠️ ET LES LIBELLES RESTENT NEUTRES (« Collation 2 », pas « du soir ») : Michel dit matin et soir, quelqu'un d'autre prendra un gouter a 16 h — etiqueter l'heure a sa place serait un faux-precis (R29).
const PRECACHE = [
