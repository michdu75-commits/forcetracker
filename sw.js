 ...
 */
const CACHE = 'ft-v1056'; // ⏰ ft-v1056 = LE FAVORI DEMANDE LE MOMENT DE LA JOURNEE + LA QUANTITE DE LA DERNIERE FOIS EST PROPOSEE. Michel, 2 captures : « il y a les favoris c'est bien, mais il categorise direct en collation ou diner. C'est un bon principe, mais on doit donner le choix : on clique sur le favori et on demande a quel moment de la journee » ; et « je ne prends pas toujours le meme poids... tu prends la ratatouille, il y a differentes boites de different poids ». ⭐⭐ LA MEME REGLE DEUX FOIS : ON PROPOSE, ON N'IMPOSE PAS. ⛔ Un repas habituel est OBSERVE (memes aliments notes ensemble 2 fois) : le moment observe est un bon PARI, pas un fait sur aujourd'hui — le meme shaker peut etre un petit-dej un jour et une collation le lendemain. Il est desormais MARQUE « d'habitude », et rien ne part sans un tap. ⛔ Et ft-v1042 PRE-REMPLISSAIT la quantite du dernier repas a la reprise, avec « pour tes 250 g » affiche aussitot : juste pour une dose de whey, FAUX pour une boite de ratatouille. Champ vide + pastille « 250 g (la derniere fois) ». ⚠️ 3e « voisinage muet » : un total qui devance le choix se lit comme un fait sur le repas. ⚠️⚠️ ET DEUX TEMOINS NE POUVAIENT PAS VERDIR : l'un cherchait un ANTISLASH litteral (echappement double par le script qui a pose le bloc), l'autre une espace ORDINAIRE la ou l'app ecrit une espace INSECABLE (regle de ft-v1034). Les deux chaines sont identiques a l'oeil. Miroir du « vert qui ne peut pas rougir » de BUGS.md.
const PRECACHE = [
