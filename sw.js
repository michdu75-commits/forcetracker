 ...
 */
const CACHE = 'ft-v1014'; // 🍽️ ft-v1014 = MILO VOIT ENFIN CE QU'ELLE A MANGE. Michel a Milo : « As-tu assez de recul pour mon alimentation ? » → « Mon alimentation est DEJA dans l'application. » Milo, honnete : « Je n'ai pas acces au journal alimentaire (…) EN L'ETAT JE TRAVAILLE A L'AVEUGLE SUR LA NUTRITION. » ⭐ IL DISAIT VRAI : `foodLog` etait classe EXCLU avec la mention « DECISION A CONFIRMER » — jamais confirmee. Un « a confirmer » qui traine devient une limite que personne n'a choisie (R30). ⭐⭐ ET L'ARGUMENT D'ORIGINE ETAIT JUSTE, mesure sur son vrai journal : 13 126 caracteres BRUTS. Mais un resume par jour en fait 221 — 59 fois moins — et il repond aux QUATRE questions que Milo avait listees lui-meme. *Le sujet n'etait pas « transmettre ou pas », c'etait « transmettre QUOI ».* ⛔⛔ LE CHIFFRE QUI AUTORISE LE CHANGEMENT (R34) N'EST PAS LA TAILLE, C'EST LE CACHE : le bloc commun est IDENTIQUE OCTET POUR OCTET avant/apres, et il le reste APRES UN REPAS AJOUTE — parce que le bloc est pose SOUS le marqueur de l'instant, la ou le code dit lui-meme de mettre ce qui change. Le garde-fou de taille le confirme independamment : sain 45 362 · blesse 47 118, inchanges. Cout reel : +835 caracteres. ⛔ LE DETAIL PLAT RESTE DEHORS (c'est lui qui pesait 13 000 car.), 7 jours au maximum, et AUCUN journal → AUCUN bloc. ⛔⛔ ANTI-TCA (Constitution P21) : ces chiffres servent quand ON LES DEMANDE — la consigne interdit de commenter spontanement ce qu'elle a mange. *Un coach qui epluche les assiettes sans qu'on lui demande fabrique le stress qu'on veut eviter.* ⚠️ ET LE PLUS IMPORTANT N'EST PAS LA FONCTIONNALITE : en entrant dans le contexte, `foodLog` a cree une FUITE. `_vcApplyPersona` ne le remettait pas a zero — le VRAI journal alimentaire de Michel serait parti dans les 54 scenarios du banc d'essai. Son en-tete dit pourtant « anti-fuite : TOUT ce que lit le contexte ». Corrige, et un temoin GENERALISE la regle : 8 autres trous sont EPINGLES avec leur raison (non corriges — ca changerait ce que Milo recoit, donc son propre avant/apres), une NEUVIEME fera rougir la livraison. ⭐ EV-054 promu (il ne valait rien avant : son attendu dependait d'une donnee absente). ⚠️ Et un temoin epinglait « 53 scenarios » EN DUR — la dette meme que ft-v1005 avait retiree de l'ecran la veille, appliquee d'un seul cote (R8).
const PRECACHE = [
