 ...
 */
const CACHE = 'ft-v1048'; // 📤 ft-v1048 = EXPORTER L'HISTORIQUE EN CSV ET EN PDF (demande de Michel sur sa video). ⚠️⚠️ R23 D'ABORD : un export EXISTAIT deja (Menu -> Exporter mes donnees -> « mes seances seulement ») et j'ai failli le reconstruire — mais il sort du JSON. *Le trou n'etait pas l'export, c'etait le FORMAT.* ⛔ Le JSON reste ou il est : sauvegarder ses donnees et regarder son historique sont deux besoins differents. ⛔⛔ UN SEUL PRODUCTEUR DE LIGNES pour les deux formats (`_histoLignes`) — deux producteurs ne diraient pas la meme chose du meme historique (R2). ⛔ Memes colonnes que l'onglet Sessions du Google Sheet (R2), plus le nom de seance et le RIR. ⛔⛔ AUCUNE DONNEE DE SANTE : ni poids de corps, ni age, ni sexe, ni e-mail — ce fichier sort de l'app. ⚠️ CSV : separateur « ; » et BOM UTF-8, sinon Excel FR met tout dans une colonne et casse les accents ; echappement reel des guillemets et des virgules. ⚠️ Et un defaut trouve a la capture du PDF : `lastAutoTable` n'a PAS de `startY` — le titre partait a NaN et l'export echouait EN SILENCE dans un catch.
const PRECACHE = [
