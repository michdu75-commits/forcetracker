 ...
 */
const CACHE = 'ft-v1047'; // 🩹 ft-v1047 = 3 ERREURS DES CARTES DE PROGRES, TROUVEES SUR UNE VIDEO DE MICHEL (10 s, en prod) : « c'est pas mal mais je suis sur qu'on peut ameliorer le visuel ». ⛔⛔ LA PIRE EST DE MOI ET DANS LA FAMILLE QUE J'EVITE TOUTE LA SEMAINE : la carte annoncait « sur 37 seances etalees sur 74 jours » puis « 11 sur 27 » — DEUX DENOMINATEURS QUI SE CONTREDISENT, SANS UN MOT. Cause mesuree : `_calSessMix` ne classe pas toutes les seances. C'est ft-v1027 refait par moi. ⛔ On ne repare PAS en comptant sur 37 : les non classees deviendraient des seances « non dominees par le haut », un fait FAUX (R29) — la ligne NOMME sa fenetre et les deux nombres apparaissent ENSEMBLE. ⚠️ ② « le plus souvent dominees » a 11/27 = 41 % : c'est le MODE, mais la phrase se lit « la majorite ». ⚠️ ③ « Le tronc domine 1 fois » : 1 occurrence n'est pas une constante, c'est du bruit. 🎨 VISUEL (perimetre choisi par Michel : mes 2 cartes) : la barre prend la couleur MUSCLE de la figurine — ⛔ toutes de la MEME couleur, seule la LONGUEUR varie, donc ce n'est pas un statut ; le rail gris pleine largeur disparait ; un lisere distingue la synthese ; et le pied de 2 lignes passe a une ligne — l'exemple SURVIT dans l'aide (alleger n'est pas supprimer).
const PRECACHE = [
