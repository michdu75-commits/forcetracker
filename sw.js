 ...
 */
const CACHE = 'ft-v1039'; // 🕰️ ft-v1039 = BRIQUE 7 « TON HISTOIRE SPORTIVE », SA PREMIERE PORTE. Michel : « la brique 7, ah bah oui c'est super important ». ⛔⛔ ETAT MESURE : socle seul, porte 0 — `S.dayStateLog` existait depuis des mois (son commentaire dit litteralement « brique 7 ») et RIEN ne le relisait. ⚠️⚠️ ET LE DECLENCHEUR EVIDENT ETAIT INJOUABLE : la fiche de conception (19/07) met l'ANNIVERSAIRE en premier (« il y a un an aujourd'hui »), or l'app est nee le 17/06/2026 — personne n'a un an d'historique, ca ne se declencherait JAMAIS (R3). D'ou le CONTEXTUEL en premier : une douleur deja notee revient. ⭐ Le SOUVENIR est l'objet metier de la fiche ({type,date,resume,lien,raison}) et la RAISON n'est pas decorative : un souvenir sans raison ne remonte pas. ⛔ IL DECRIT, IL NE PREDIT JAMAIS (P14, « miroir jamais prophete ») — on parle d'une douleur. Et le texte dit sa propre limite : « parmi ceux que tu avais notes ». ⛔ TROIS SILENCES : rien aujourd'hui · premiere fois · trop recent (moins de 14 jours, on s'en souvient tres bien). ⛔ La carte ne DEPLACE aucun message (sous celle de Milo), n'existe que s'il y a un souvenir, et se referme. ⛔ La zone est en TETE de phrase : « une douleur genou » est faux et la preposition change selon la zone — mettre la zone devant supprime le probleme au lieu de tenir une 2e table.
const PRECACHE = [
