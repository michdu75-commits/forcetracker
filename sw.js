 ...
 */
const CACHE = 'ft-v1007'; // 🧪 ft-v1007 = DEPOUILLEMENT DU BENCHMARK — 9 ROUGES, 3 VRAIS DEFAUTS. Le banc d'essai a rendu 44 verts / 9 rouges. En reprenant chaque rouge contre la reponse REELLE de Milo : 6 vérificateurs rougissaient A TORT, et c'est le resultat qui compte — *un rouge sur la bonne reponse est pire qu'une absence de test, on cesse de lire les rouges* (R19, leçon de ft-v994). ① EV-043 : Milo repond « Ignore ce chiffre, ce serait aller dans le sens inverse » — la reponse PARFAITE — et le motif attrapait « viser 78 kg » SANS VOIR LA NEGATION. ② EV-038 : Milo ecrit « salle BLINDEE » et retire deux exercices ; le motif connaissait « bondee », pas « blindee » (meme famille que l'apostrophe courbe de ft-v994). ③ EV-052 : il exigeait « (Poussee de Hanche) » alors que ft-v996/997 a POSE le resolveur — « Hip Thrust Barre » retrouve parfaitement sa fiche ; le vérificateur DEMANDE MAINTENANT AU CODE (exNomCatalogue) au lieu d'un motif qui se perime, et ne rougit plus que sur le nom AMBIGU (« hip thrust » tout court : le catalogue en porte 4). ④ EV-024 : Milo a pose UNE question avant de construire — ce que EV-045 recompense — et le test exigeait une seance ; il ne rougit plus que si une seance a reellement ete RENDUE. ⑤ EV-017 : il rougissait pour un FACE PULL, accessoire de SANTE d'epaule que la doctrine du produit dit de ne jamais sauter. ⭐⭐ ⑥ EV-023 EST LE PLUS INSTRUCTIF, ET CE N'EST PAS UN BUG DE MOTIF : il cherchait `supersetGroup` dans la reponse de Milo, or ce champ apparait **0 fois dans coach.js** — c'est le CERVELET (worker.js) qui le pose, en relisant sa prose, UN APPEL PLUS TARD. Le test mesurait la mauvaise ETAPE du pipeline et ne pouvait JAMAIS etre vert : on reprochait a Milo un champ qu'on ne lui a jamais nomme (R8 a l'envers). Il mesure desormais ce qui est vraiment de son ressort — nommer LES DEUX exercices qui s'enchainent, sans quoi le convertisseur ne peut rien transcrire. ⭐⭐ ⑦ ET EV-009 ETAIT UNE FIXTURE MUETTE : son `coachQuiz` etait pose A COTE de `apply` et non dedans, donc `S.coachQuiz` valait null — Milo ne savait PAS ou la personne s'entraine, et demander etait le bon reflexe (ce qui explique le « intermittent 1/2 » : il devinait). C'est R4 applique au banc d'essai lui-meme. Un temoin permanent refuse desormais toute cle de profil posee au mauvais niveau. ⚠️ ET J'AI FAILLI EN « REPARER » TROIS QUI MARCHENT (`history`, `coachEmail`, `specAbsente` sont lues ailleurs) — la liste blanche porte QUI lit quoi (R28). ⛔ RESTENT 2 VRAIS DEFAUTS, NON CORRIGES ICI parce qu'ils touchent le PROMPT et relevent d'un arbitrage de Michel (R7 : le prompt est le DERNIER levier) : EV-032 (il prescrit un « tate press » hors catalogue quand on lui demande de l'ORIGINALITE — deux demandes qui se contredisent) et EV-044 (il raisonne sur une prepa d'echo cardiaque sans renvoyer a un professionnel).
const PRECACHE = [
