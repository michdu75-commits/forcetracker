 ...
 */
const CACHE = 'ft-v1046'; // 🎚️ ft-v1046 = LE RPE — UN CHANGEMENT DE VOCABULAIRE, PAS UN 2e SYSTEME. Michel avait differe le RPE avec son critere : « c'est des gens qui connaissent bien qui connaissent le RPE et ils utilisent bien » — donc OPTIONNEL et DISCRET, jamais une question posee a quelqu'un qui n'a pas demande. ⛔⛔ RIEN N'EST STOCKE EN PLUS : RPE et RIR sont la meme mesure inversee (RPE = 10 - RIR), donc `set.rir` reste le SEUL proprietaire et le RPE n'est qu'un AFFICHAGE (R2) — deux champs finiraient par se contredire sur la meme serie. Basculer ne convertit rien, n'efface rien : l'historique se relit dans la nouvelle langue. Un seul vocabulaire partage par les 3 endroits qui montrent la reserve (barre de repos, colonne « precedent », contexte de Milo), et Milo recoit la PREFERENCE en gardant la donnee en RIR — non ambigue. ⛔ L'ordre des boutons s'inverse en RPE (6 vers 10), parce que c'est comme ca que le bareme se lit. ⚠️ LIMITE ECRITE : pas de demi-points (8,5 · 9,5) — ils existent dans le bareme mais l'app ne les MESURE pas, et descendre le stockage au demi-point abimerait le RIR pour tout le monde (R29).
const PRECACHE = [
