 ...
 */
const CACHE = 'ft-v1093'; // 3e passe de chasse aux incoherences. ⛔⛔ 1 LE BOUTON RETOUR (Android) ETAIT LA 3e PORTE, ET LA SEULE ENCORE OUVERTE : ft-v1091 a fait passer le glissement du doigt par `_closeOverlayProper`, mais le gestionnaire `popstate` faisait `classList.remove('open')` en direct. Or sur Android le retour EST le geste de fermeture. Mesure : `closeBarcodeScanner` appelee 0 fois (LA CAMERA RESTE ALLUMEE), `ft4_wn_seen` toujours null (les pop-ups « une seule fois » reviennent a chaque demarrage, bug de ft-v629), les 3 ecrans `data-no-dismiss` fermes quand meme (dont « analyse en cours », dont le minuteur continue de repeindre un ecran cache), et il fermait le MAUVAIS ecran : `.pop()` rend le dernier du HTML, or `ov-confirm` porte z-index 10000 et se trouve AVANT `ov-sess-detail` -> le detail se fermait et la question « Supprimer ? » restait a flotter au-dessus de rien. ⛔⛔ 2 LE REGIME ALIMENTAIRE ET LE JEUNE N'ETAIENT STOCKES NULLE PART : envoyes depuis le 02/08, attendus au retour, et aucune ligne de Code.js ne les ecrivait. Aller-retour mesure : keto + 16/8 partent, reviennent VIDES, les glucides passent de 39 g a 432 g et Milo cesse de savoir qu'il ne faut proposer ni pain ni petit-dejeuner. ⛔ 3 `goalLog` casse DES DEUX COTES (serveur muet + lecture au mauvais endroit) : la panne que ft-v1010 disait avoir reparee. ⛔ 4 LES TEXTES QUI ANNONCAIENT UN CHIFFRE QUE LE CODE N'APPLIQUE PLUS : « Timer 45 s » (le chrono fait 45/90/120 depuis ft-v1082), « repos 20 s entre chaque palier » (il n'y en a AUCUN), « 7 derniers jours » pour une fenetre de 14, « 18 badges » pour 19, et un bouton « -10% » qui n'existe plus. ⚠️ ET MA PREMIERE SONDE MESURAIT AUTRE CHOSE : une pop-up de DEMARRAGE reste ouverte au chargement, elle etait « celle du dessus », et toutes mes mesures la visaient elle.
