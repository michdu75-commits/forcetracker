 ...
 */
const CACHE = 'ft-v954'; // 🧮 ft-v954 = LE TOTAL DU GARDIEN CONTREDISAIT SON PROPRE DETAIL. Michel, devant l'ecran apres la 1re remontee reelle : le bloc annoncait « TOTAL, tous comptes confondus » et n'agregeait en fait que le 📡 DIRECT — il affichait donc « bloc_technique : 2 » pendant que le detail juste en dessous montrait 2 promesses de memoire chez lui et 1 chez Eline. C'est la famille de bugs « deux sources qui se contredisent » (BUGS.md), et elle est plus vicieuse que l'absence parce que la personne VOIT les deux. ⛔ ET ON NE LES ADDITIONNE PAS POUR AUTANT : le direct et l'historique couvrent deux EPOQUES — l'historique remonte a des versions de Milo anterieures aux correctifs, les fondre donnerait un chiffre qui ne veut rien dire (decision de ft-v946). 👉 DEUX totaux NOMMES au lieu d'un total qui ment sur ce qu'il compte, et la ligne « ne s'additionne PAS au direct » ecrite a l'ecran. ⭐ Le rendu est SORTI de l'appel reseau (`_gardienStatsRendu`) pour qu'un temoin puisse le jouer sur des chiffres connus : le cas reel de la capture est desormais fige dans les tests, chiffres compris. ⚠️ Au passage, le « bloc_technique : 2 » visible chez Michel est un RESIDU d'avant ft-v946 (le correctif qui l'a sorti des derives) : le compteur est cumulatif, le code actuel ne le compte plus.
const PRECACHE = [
