 ...
 */
const CACHE = 'ft-v988'; // 🏋️ ft-v988 = EXPORTER SEULEMENT SES SEANCES — et l'export « normal » DIT enfin ce qu'il emporte. Michel demande l'option, puis, en decouvrant le contenu du fichier : « oui j'ai vu mes bilans dans l'export ». ⭐⭐ CE N'EST PAS UN CONFORT, C'EST DE LA CONFIDENTIALITE : le bouton « Exporter » emportait deja TOUT — bilan sanguin, bilan corporel, TRT, profil sante — et la modale n'avertissait QUE pour les conversations. Or le fichier existe POUR ETRE DONNE (a ChatGPT, a un coach, a moi pour deboguer), donc le seul geste possible poussait a partager beaucoup plus que necessaire. Un export tout-ou-rien n'est pas un probleme d'ergonomie. ⛔⛔ LISTE BLANCHE, PAS LISTE NOIRE, et c'est la seule forme acceptable ici : avec une liste noire, toute donnee ajoutee DEMAIN partirait toute seule dans un fichier cense etre etroit, sans que personne ne le decide. Avec une liste blanche, le pire cas devient « il manque quelque chose » au lieu de « on a divulgue quelque chose » (R29). ⛔ ET UN SEUL EXPORTEUR (R2) : meme fonction, meme format, donc fichier reimportable — et le retrait des photos d'exercices perso (31 % du fichier le 17/08) vaut GRATUITEMENT pour le nouveau mode. Un temoin l'epingle. ⭐ LE CHOIX ETROIT EST EN PREMIER ET EN ROUGE : l'option la moins exposante doit etre la plus facile a prendre. ⚠️ ET LA QUESTION SE POSE DESORMAIS MEME SANS CONVERSATION — changement volontaire, la raison d'avant reste ecrite : le temoin exigeait l'inverse au nom de R24 (« ne pas poser une question inutile »), ce qui etait juste TANT QU'IL N'Y AVAIT QU'UN SEUL VRAI CHOIX. Quelqu'un sans conversation n'avait AUCUN choix et repartait avec ses bilans dans le fichier. Ce qui reste de R24 : le bouton « avec mes discussions » DISPARAIT quand il n'y en a aucune. ⛔ LE POIDS DE CORPS N'Y EST PAS, et le fichier le DIT avec la raison (sans lui une charge ne se juge pas en relatif) — un export muet sur ses trous laisse croire qu'il est complet. Le nom du fichier dit ce qu'il contient. 📐 Au passage : le garde-fou de taille du bloc commun de Milo mesure DESORMAIS un profil BLESSE — mesure imprimee a chaque passe (sain 45 362 · blesse 47 118, plafond 46 500, depassement 618) et plafond blesse EPINGLE a 47 500 pour qu'il ne derive pas pendant que la decision de fond attend (docs/AUDIT-CONTEXTE-MILO.md §14.6).
const PRECACHE = [
