 ...
 */
const CACHE = 'ft-v1042'; // 🔭 ft-v1041 = BRIQUE 8 « SYNTHESE », SA PREMIERE PORTE. Michel : « la brique 8 alors ». ⛔⛔ Etat mesure avant : signal 1, porte 0 — et le seul signal (startPt001Test) est un outil ADMIN : la brique n'avait RIEN. ⭐⭐ LA FRONTIERE AVEC LA 7 EST ECRITE DANS LA VISION : la 7 repond a « que s'est-il passe ? » (le souvenir, ft-v1039), la 8 a « QU'EST-CE QUE CETTE HISTOIRE M'APPREND ? ». Tournures autorisees : « une constante apparait » — ⛔ JAMAIS « tu devrais ». 👉 Trois constantes calculees EN LOCAL en tete de Progres : rythme reel · l'exercice le plus fidele · la region dominante (via _calSessMix, reutilise). ⛔ Chaque ligne NOMME sa fenetre (sinon deux chiffres se contredisent sans explication — le defaut de ft-v1027) · aucun verbe de prescription (temoin dedie) · aucune comparaison a une norme · et sous 8 seances sur 21 jours elle DIT qu'elle ne sait pas encore, au lieu de laisser un cadre vide (lecon de ft-v1021). ⚠️⚠️ DEUX NOMS SUPPOSES ONT COUTE UN CAS CHACUN : `m.region` au lieu de `m.reg` (la constante d'equilibre ne sortait JAMAIS, en silence), et un libelle qui disait « le tronc revient 0 fois » — or _calSessMix rend la region DOMINANTE, donc ca se lit « tu ne travailles jamais ton tronc », ce qui est FAUX. *Une mesure juste peut produire une phrase fausse.*
const PRECACHE = [
