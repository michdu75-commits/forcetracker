 ...
 */
const CACHE = 'ft-v985'; // 🗑️ ft-v985 = LA CONFIRMATION PASSE DEVANT TOUT. Michel, capture a l'appui : « le bouton supprimer ne fonctionne pas ». ⛔⛔ IL FONCTIONNAIT PARFAITEMENT — mesure dans un navigateur : la question « Supprimer l'aliment ? » s'ouvrait bel et bien, MAIS DERRIERE la fenetre de modification. `document.elementsFromPoint` au centre de la confirmation rendait `["EDIT", "CONFIRM"]`. ⭐⭐ ET MICHEL L'A CONFIRME LUI-MEME SANS LE SAVOIR : « ca a fonctionne apres » — en fermant la modale d'edition, la confirmation restee dessous devient visible et repond. La preuve terrain et la mesure disent la meme chose. ⭐ LA CAUSE EXACTE : `#ov-confirm` valait 500 et `#ov-edit-food` vaut 500 AUSSI. A z-index EGAL, c'est l'ordre du DOM qui tranche — et `ov-edit-food` est cree dynamiquement puis ajoute A LA FIN du body, donc apres le `ov-confirm` statique. Deux regles a egalite, c'est le hasard du DOM qui decide. ⛔⛔ ET LE DEFAUT ETAIT SYSTEMIQUE, PAS PROPRE A CET ECRAN : compte, **25 appels a `showConfirm`** dans 5 fichiers, et **19 overlays** au-dessus ou a egalite — dont **13 a 9999**, plus le toast a 600. Corriger le seul `ov-edit-food` aurait laisse les DIX-HUIT autres. ⭐ R2 — UN SEUL ENDROIT : la confirmation est l'ecran le plus prioritaire de l'app par nature (elle INTERROMPT pour poser une question dont depend une suppression), elle passe donc au-dessus de tout le monde, toast compris. ⭐⭐ ET LE TEMOIN NE VERIFIE PAS LE CAS, IL VERIFIE LA REGLE : aucun overlay ne doit ATTEINDRE son niveau — un `>=` et non un `>`, parce que l'egalite est deja le defaut. ⚠️ CONTROLE NEGATIF : 3 rouges, et **3 VERTS DES DEUX COTES** — le bouton ouvre bien la question, la suppression aboutit, les fenetres se referment. C'est exactement ce qui prouve que le mecanisme n'a JAMAIS ete casse : seule la visibilite l'etait.
const PRECACHE = [
