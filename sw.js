 ...
 */
const CACHE = 'ft-v1077'; // ☁️ ft-v1077 = LA SEANCE QUI NE PARTAIT PAS, MEME EN WIFI. Michel : « J'ai teste en wifi », et l'Admin affichait toujours « 1 seance non synchronisee / 39 » ; son toast disait « Timeout (8s) ». ⛔⛔ CAUSE MESUREE DANS LE CODE : `handleLogSession_` faisait UN `appendRow` PAR SERIE — un aller-retour complet vers le classeur a chaque serie — et c'est le SEUL ecrivain de `Code.js` dont la duree grandit avec les donnees. Une seance de 25 series = 25 ecritures ; le telephone, lui, abandonne a 8 s. ⭐⭐ ET L'ABANDON DU TELEPHONE N'ARRETE PAS LE SCRIPT : les lignes s'ecrivaient quand meme, la seance restait marquee « non synchronisee », et CHAQUE nouvel essai re-collait les memes lignes. *Un delai depasse n'est pas un echec — c'est une reponse qu'on n'a pas attendue.* ⛔ Ecriture EN BLOC (`setValues`) : 1 seance = 1 ecriture, qu'elle fasse 3 ou 60 series. ⛔ Et la moitie frontend : « ❌ Sync : Timeout (8s) » se lisait comme une seance PERDUE — il n'y a plus ni croix ni rouge pour un reseau lent (« ta seance est gardee, on reessaiera »), mais une VRAIE erreur garde la sienne. ⚠️ HONNETE : la duree du serveur n'a PAS pu etre mesuree depuis ici (sortie reseau bloquee) — la cause est etablie par lecture du code, pas chronometree chez lui.
