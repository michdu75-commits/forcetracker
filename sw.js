 ...
 */
const CACHE = 'ft-v1038'; // 💪 ft-v1038 = LE RIR, « il t'en restait combien ? ». Demande de Michel apres la discussion sur le « lourd ». ⛔⛔ TROU MESURE : DISC_CADRE.echec dit la regle PAR DISCIPLINE (« JAMAIS a l'echec » en force athletique, « 1 a 3 en reserve » en musculation) et Milo la RECOIT — mais RIR/RPE n'existaient NULLE PART dans l'app. On lui demandait de juger une reserve qu'on ne mesure jamais : R8 dans sa forme la plus pure. ⭐ LE DEMI-SYSTEME EXISTAIT : le tag X = Echec EST un RIR de 0 → un SEUL proprietaire (_rirDeSet), pas de 2e systeme qui divergerait (R2). ⭐ Saisie dans la BARRE DE REPOS : valider une serie demarre le repos (mesure), c'est le seul moment ou la question tombe juste — et la ligne de serie a deja 6 colonnes sur 430 px. ⛔ RIEN N'EST OBLIGATOIRE et `null` n'est PAS 0 : une serie non notee ne doit jamais se lire « allee a l'echec » (R29). ⚠️⚠️ L'ORDRE M'A PIEGE : startRest COMMENCE par stopRest(), qui vide la cible — poser la cible avant l'appel ne servait a rien (mesure : 0 bouton). D'ou la cible EN ATTENTE, deposee par toggleSet et consommee par startRest : il y a CINQ startRest dans toggleSet, on n'en patche pas cinq. ⭐ Restitue dans la colonne « precedent » (8x80.2r) — c'est la qu'il sert : avant de refaire la serie. RPE differe, decision de Michel.
const PRECACHE = [
