 ...
 */
const CACHE = 'ft-v906'; // 🔄 ft-v906 = L'APP NE PRESCRIT PLUS UNE CIBLE QU'ELLE QUALIFIERAIT D'ALERTE. Trouve par un contre-audit exterieur (autre instance Claude, sans acces au code), VERIFIE ici dans le code : autoKcal etait une addition sans plancher (TDEE + objectif + phase + cycle) alors que le Gardien de Milo alerte sous 1500 kcal/j chez un homme et 1200 chez une femme. Les deux ne se parlaient pas — R2 sur une regle de SECURITE. Mesure refaite avec nos propres regles : femme 55 kg/160 cm/45 ans sedentaire en perte = 1047 kcal, et 847 en phase de decharge. ⚠️ L'audit annoncait 947 : il avait oublie le +100 de la phase de charge. Le defaut est reel mais plus etroit que decrit — il mord surtout en decharge et sur les profils sedentaires legers. ⚠️⚠️ ET L'ASYMETRIE EST PIRE QUE LE CHIFFRE : le Gardien ne s'allume que si la personne TIENT son journal, or le principe 4 assume qu'une bonne partie ne le tiendra pas. Ceux-la voyaient la cible et n'avaient AUCUN garde-fou : le Gardien protegeait la population qui en avait le moins besoin. ⚠️ LE PLANCHER NE TOUCHE PAS manualKcal : une cible saisie a la main est celle de la personne (R29 + Constitution — on adapte, on n'interdit pas). Il est EXPLIQUE a l'ecran, jamais applique en silence. + KETO : 15 % de proteines passait sous 0,8 g/kg des que le poids est eleve (100 kg a 1950 kcal = 0,73 g/kg) — l'app generait la repartition qui declenche sa propre alerte. Plancher a 0,8 g/kg, les lipides absorbent (5 % de glucides est la contrainte qui definit le regime, on n'y touche pas).
const PRECACHE = [
