 ...
 */
const CACHE = 'ft-v907'; // 🔄 ft-v907 = BRIQUE 0 DE LA NUTRITION — CHAQUE LIGNE DU JOURNAL PORTE ENFIN SA PROVENANCE. Une entree s'ecrivait { date, meal, name, kcal, prot, carbs, fat, ts } : un RESULTAT sans trace de son origine. ⚠️⚠️ C'EST LA SEULE BRIQUE QU'ON NE PEUT PAS RATTRAPER : tout le reste (base d'aliments, generateur, niveaux de precision) se construira sur les donnees existantes ; une entree ecrite sans ces champs ne les retrouvera jamais, et chaque jour en fabrique d'autres. ⚠️ ET LE CHAMP LE PLUS COUTEUX N'EST PAS LA SOURCE, C'EST LA QUANTITE : une ligne disait « 380 kcal » sans dire « 250 g de X » — meme en connaissant plus tard la bonne valeur au 100 g, on ne pouvait RIEN recalculer. Le scan et l'etiquette CONNAISSAIENT le poids, ils ne l'enregistraient pas. DEUX AXES, pas un (correction du contre-audit) : `saisie` dit COMMENT c'est entre (manuel/scan/photo-ia/ia-texte/liste), `origine` dit D'OU VIENT LE CHIFFRE (utilisateur/off/etiquette/ia/reprise). Les confondre perd l'information dans les deux sens. + `per100` garde les valeurs au 100 g quand la source les donne, `modifie` dit si la personne a retouche les macros apres un remplissage automatique (la source n'explique alors plus le chiffre). ⚠️ ON N'INVENTE RIEN : `etat` (cru/cuit) et `q` restent null quand on ne sait pas — l'etat viendra de la base d'aliments, il ne se devine pas. Et la provenance NE SURVIT PAS d'une saisie a l'autre : une provenance fausse est pire que pas de provenance, elle se presente comme un fait verifiable. ⚠️ RETROCOMPATIBLE : une entree sans `v` est une entree d'avant — on ne la reecrit pas et on ne lui suppose aucune provenance.
const PRECACHE = [
