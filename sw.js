 ...
 */
const CACHE = 'ft-v937'; // 🖊️ ft-v937 = UN DECOR DE TEST PRIS POUR UN FAIT REEL — corrige par Michel. Dans le journal de ft-v936 j'avais justifie la decision EV-015 par « Christophe, persona fondateur, a un vrai coach ». C'est FAUX : Michel corrige — « Christophe n'est pas coach, c'est un sportif qui fait du body ». La phrase venait du champ `resume` du persona VC-002 dans coach.js, une BIOGRAPHIE DE FICTION ecrite le 25/07 pour un scenario de test. J'ai pris un decor de test pour un fait sur une personne reelle, puis je m'en suis servi comme ARGUMENT pour justifier d'ajouter une regle au prompt. ⭐ C'est l'hypothese-presentee-comme-un-fait que la Constitution interdit a Milo, appliquee a celui qui ecrit le code. ⚠️ LE PIEGE EST STRUCTUREL, pas une etourderie : les personas portent le PRENOM de vrais testeurs (Tatiana, Christophe, Emma) parce qu'ils s'en inspirent, mais leurs champs sont INVENTES. L'avertissement est donc pose LA OU LE PIEGE SE TEND — en tete de VC_PERSONAS, a cote du resume qui m'a trompe — et pas seulement dans le journal. ⏭️ CONSEQUENCE : la decision EV-015 ne repose plus sur rien et se rouvre sur ses seuls merites (« un utilisateur suivi par un coach a-t-il besoin que Milo se pose en complement ? »). Aucun cas d'usage reel ne l'appuie a ce jour. Correction de DOC + commentaire : zero changement de comportement.
const PRECACHE = [
