 ...
 */
const CACHE = 'ft-v1006'; // 🔴 ft-v1006 = LE BOUTON ROUGE DE `showConfirm` DIT ENFIN CE QU'IL FAIT. Michel, capture a l'appui en lancant le benchmark : « Ya marque annuler ou supprimer lol ». La fenetre annoncait « 53 appels au Coach, environ 0,79 € a 3,45 € » et proposait [Annuler] [SUPPRIMER] — on ne supprime rien, on LANCE, et ca coute de l'argent. ⭐ LE MECANISME EXISTAIT DEJA, POSE D'UN SEUL COTE : showConfirm(title,msg,cb,okLabel) accepte un libelle depuis toujours, et 10 appels sur 24 ne le passaient pas alors que leur action n'etait PAS une suppression (lancer PT-001, lancer un test VC, lancer le benchmark, rejouer les rouges, importer des pesees, fusionner deux exercices, vider le cache, vider la seance, abandonner la seance, refaire l'inscription). 5e fois de la semaine apres ft-v973, v975, v984 et v996. ⛔⛔ LA JUMELLE, TROUVEE EN LA CHERCHANT (R8) : l'ecran « Fusionner les exercices » existe a DEUX endroits — log.js passait bien 'Fusionner', setup.js ne passait rien et affichait donc « Supprimer » pour un RENOMMAGE. Deux copies du meme ecran, une juste, une fausse (R2). ⭐⭐ ET LE TEMOIN FIGE UNE REGLE, PAS UNE LISTE : enumerer les 24 appels rougirait au 25e et on l'allongerait par reflexe ; la regle, elle, ne perime pas — le defaut de showConfirm est « Supprimer », DONC un appel sans libelle doit avoir un titre qui commence par « Supprimer ». ⚠️ Et mon decoupage d'arguments a menti d'abord : il sautait les chaines mais pas les COMMENTAIRES, si bien qu'une apostrophe francaise dans un commentaire ouvrait une fausse chaine et avalait la fin de l'appel — il accusait 5 appels deja corriges. Un seul lecteur pour les bornes ET pour les virgules.
const PRECACHE = [
