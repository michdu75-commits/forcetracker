 ...
 */
const CACHE = 'ft-v934'; // 🔁 ft-v934 = REJOUER LES ROUGES — un TAUX, plus un booleen. Michel : « sinon on passe a 20 passes non ? ». ⭐ L'INTUITION EST JUSTE : repeter est la SEULE facon de battre le bruit (deux passes du MEME modele ont donne 3 puis 4 rouges). ⛔ MAIS PAS 20 × 15 = 300 APPELS : c'est 4,60 a 19 €, et surtout c'est AU-DESSUS du plafond anti-abus (50 appels/jour/personne, 600 au total) — le run serait coupe en cours et on paierait un rapport tronque. Le runner refuse net au-dela de 45. 👉 ON REPETE CE QUI COMPTE : bouton « 🔁 Rejouer les rouges » qui ne rejoue QUE les scenarios rouges, avec une repetition qui S'ADAPTE a leur nombre pour tenir sous le plafond (3 rouges × 10 = 30 appels ≈ 0,45 €). ⭐⭐ ET LE VERDICT CHANGE DE NATURE : « rouge 5/10 » au lieu de « rouge ». La vraie question n'est pas rouge-ou-vert mais « ce defaut tombe-t-il A CHAQUE FOIS ou une fois sur cinq ? » — un defaut systematique et un defaut intermittent ne se corrigent pas pareil, et un outil qui ecrase l'intermittence en booleen ferait chercher un bug systematique la ou il n'y en a pas. ⚠️ Un defaut intermittent reste classe ROUGE : on mesure sa frequence, on ne l'excuse pas. En ligne de commande : `--repeat N`, a combiner avec `--only`. Chaque passe part d'un navigateur NEUF et d'un profil remis a neutre — sinon on mesurerait la memoire, pas la regle.
const PRECACHE = [
