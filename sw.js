 ...
 */
const CACHE = 'ft-v957'; // 🥗 ft-v957 = LA BASE CIQUAL — LES ALIMENTS GENERIQUES (brique 1). Michel a fourni Table_Ciqual_2025_complete.xlsx apres ft-v956 : Open Food Facts ne rend que des PRODUITS DE MARQUE, jamais « banane » tout court. 3 484 aliments de l'ANSES, convertis par tools/ciqual.py. ⚠️ SOURCE ET LICENCE : Licence Ouverte / Etalab — reutilisation libre A CONDITION de citer la source ; la mention est AFFICHEE dans la liste, ce n'est pas optionnel. ⛔⛔ LE POINT QUI DECIDE DE TOUT : chargee A LA DEMANDE, jamais au demarrage (250 Ko, 68 Ko gzippes). La regle d'or #4 dit que l'app s'ouvre instantanement a la salle — un temoin verifie qu'apres 2 s d'app la base n'est PAS chargee, et qu'elle arrive a la premiere frappe. Elle n'est pas non plus dans le PRECACHE du service worker : ce serait 250 Ko re-telecharges A CHAQUE version, pour une base que beaucoup n'ouvriront jamais. ⛔⛔ ET LE POINT DE RIGUEUR : dans CIQUAL, « - » veut dire NON DETERMINE, PAS zero. 143 aliments n'ont pas de calories determinees — le convertisseur les garde a `null` au lieu d'inventer un 0 (R29), et l'affichage les ECARTE parce qu'un aliment sans calories ne peut pas servir un journal. La donnee reste dans le fichier, c'est la vue qui filtre. « traces » et « < 0,55 » valent 0, eux, parce que c'est ce qu'ils veulent dire. ⭐ PROTEINES N×6.25 et non facteur de Jones (elles different sur 711 aliments) : la colonne d'energie retenue est celle du Reglement UE 1169/2011, celle que la personne lit sur ses emballages — melanger deux conventions donnerait des macros qui ne retombent pas sur leurs propres calories. ⭐ R2 : un aliment CIQUAL remplit le formulaire par le MEME chemin que le code-barres et la recherche OFF. ⛔ Pas d'etat « tel-que-vendu » : CIQUAL dit le cru/cuit EN TOUTES LETTRES dans le nom. ⚠️ Et aucune carte de score sante sur un aliment brut : une banane n'a ni Nutri-Score ni NOVA.
const PRECACHE = [
  './', './index.html', './style.css', './confidentialite.html',
  './constants.js', './state.js', './screens.js', './log.js',
  './setup.js', './tracking.js', './coach.js', './app.js', './food-health.js',
  './manifest.json', './logo.png', './female-body.png',
  /* ⛔ `data/ciqual.json` (250 Ko) N'EST VOLONTAIREMENT PAS ICI (22/08/2026). Le préchargement
