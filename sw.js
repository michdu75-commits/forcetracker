 ...
 */
const CACHE = 'ft-v1016'; // 🔬 ft-v1016 = LE REJEU GRATUIT A TROUVE UN FAUX ROUGE DE PLUS, ET IL ETAIT DE MOI. Michel a lance « Rejouer les verificateurs sur la derniere passe » (0 appel, 0 €) : sur les MEMES reponses de Milo du 25/08, **9 rouges → 4**. Les 5 verificateurs corriges en ft-v1007 sont donc PROUVES — meme texte, verdict different, sans depenser un centime. ⭐⭐ MAIS EV-052 ROUGISSAIT ENCORE, ET C'ETAIT MON MOTIF : il prenait la PREMIERE occurrence de « hip thrust » — or Milo ecrivait d'abord « une seance jambes/fessiers solide avec le hip thrust en tete de file » (une phrase de PROSE) puis, deux lignes plus bas, la prescription « **Hip Thrust Barre** ». Le motif rendait donc « hip thrust en tete de file », que personne ne peut resoudre. ⛔ ON REGARDE DESORMAIS TOUTES LES OCCURRENCES : il suffit qu'UNE seule soit resolvable par `exNomCatalogue`. *La regle voulue n'a jamais ete « ne parle jamais de hip thrust en passant », c'est « quand tu le PRESCRIS, dis LEQUEL » — et une mention en prose n'est pas une prescription.* ⭐⭐ LA LECON DE METHODE EST LA VRAIE LIVRAISON : ce faux rouge avait survecu a une correction ET a une epreuve dans les deux sens, parce que je l'avais eprouve sur des textes que J'AVAIS ECRITS. Il n'est tombe que confronte a la VRAIE reponse de Milo. *Un verificateur ne se juge que sur des reponses reelles ; teste sur ses propres exemples, il ne mesure que l'imagination de celui qui l'a ecrit.* ⭐ ET LE REJEU GRATUIT EST L'OUTIL QUI L'A PERMIS : il rejoue les motifs d'aujourd'hui sur des reponses deja payees — 0 €. A employer AVANT toute passe payante. ⚠️ Numerotee 1016 et non 1015 : session-B avait pris 1015 pendant ce travail (l'echec de sync Google Sheets compte comme un succes).
const PRECACHE = [
