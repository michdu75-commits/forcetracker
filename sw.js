 ...
 */
const CACHE = 'ft-v1063'; // 🍽️ ft-v1063 = « J'AI 2 FOIS LA MEME PROT ». Michel, capture a l'appui : son shaker apparait DEUX FOIS dans « Tes repas habituels », deux lignes identiques a l'ecran. ⛔⛔ LA CAUSE TIENT A UN `+` : la signature d'un repas habituel etait `meal + '::' + aliments`. Note 6 fois en Collation 2 et 2 fois en Petit-dej, le meme shaker faisait DEUX habitudes. ⚠️ REGRESSION DE ft-v1056, LA MIENNE : tant que la carte APPLIQUAIT le moment, il faisait partie de l'identite de ce qu'on rejoue — deux lignes, deux resultats. Depuis que le moment SE DEMANDE au tap, les deux lignes font exactement la meme chose. *Un critere de regroupement qui survit a la disparition de son motif fabrique des doublons.* ⛔ LE COUT N'EST PAS QUE VISUEL : la liste est bornee a 3, donc une variante en double CHASSE une vraie autre habitude — sur sa capture, ses 3 lignes ne sont que 2 repas. ⭐ ON FILTRE AVANT DE FUSIONNER, et c'est ce qui preserve son usage reel : il prend ce shaker le matin ET l'apres-midi ; fusionner d'abord ferait disparaitre l'habitude entiere des qu'il l'a notee une fois dans la journee. ⚠️ Et mon propre temoin m'a repris : le compte doit porter sur TOUTES les variantes, y compris celle ecartee — le filtre decide de ce qu'on PROPOSE, jamais de ce qu'on a COMPTE.
const PRECACHE = [
