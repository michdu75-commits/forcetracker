 ...
 */
const CACHE = 'ft-v1083'; // 🧹 ft-v1083 = NETTOYER LES DOUBLONS DU CLASSEUR — LA SEULE ROUTE QUI SUPPRIME. Go de Michel APRES avoir vu le constat (sa seance du 31/08 ecrite **7 fois**, celle du 28/08 en double, **157 lignes en trop**) — jamais avant (**R29**). ⛔⛔ TROIS GARDE-FOUS : ① **apercu par defaut** (sans `confirme=1` la route ne supprime RIEN) · ② **copie de l'onglet AVANT** la 1re suppression, et si la copie rate on s'arrete net — *la sauvegarde nocturne garde les COMPTES, pas le classeur* · ③ un plafond de surete. ⭐⭐ ET LE PLAFOND A ETE CORRIGE PAR UNE MESURE, PAS PAR UN AVIS : mon 1er jet refusait au-dela de **la moitie** des lignes. Or sa seance est ecrite 7 fois — **6 lignes sur 7 sont des doublons legitimes** : ***le garde-fou aurait refuse le seul cas pour lequel on ecrit cette route***. Un ratio ne distingue pas « je me trompe en grand » de « il y a vraiment beaucoup de doublons ». Plafond devenu ABSOLU ; la vraie protection est l'apercu + le fait qu'on ne supprime JAMAIS la 1re occurrence (garanti par construction). ⛔ On ne touche QUE les lignes portant son email : ni les autres testeurs, ni les ~3000 lignes sans email. ⚠️⚠️ Suppression PAR BLOCS, DE BAS EN HAUT : 157 `deleteRow` referaient exactement le bug de ft-v1077, et supprimer de haut en bas rendrait faux, EN SILENCE, tous les index d'en dessous.
