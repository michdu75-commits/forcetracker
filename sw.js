 ...
 */
const CACHE = 'ft-v559'; // Objectif de FORCE par exercice (idée maquettes Gemini/GPT) : fixe un 1RM visé par exercice → carte « 🎯 Objectif de force » (input + barre de progression % + « encore X kg »/« Atteint ! ») + ligne repère verte pointillée sur le graphe des Progrès (renderChart, setup.js). `S.strengthGoals` {nom:kg} (ft4_strgoals) persisté local + cloud (payload + restore + backend Code.js `_po_` — auto-déployé). `setStrengthGoal()`. Checklist #11 : WHATS_NEW v31 🎯 + red dot strength-goal (spot ptab-exo) + aides ?/détaillée. Testé Playwright (set/bar/ligne/persist/reload/remove, 0 erreur JS). Précédent ft-v558 : rollback Style Visuel. (ft-v553→557) — décision Michel : reproduire une maquette illustrée au pixel, à l'aveugle, ne convergeait pas. state.js/app.js/index.html/style.css/screens.js restaurés à l'état ft-v552 (fix calendrier + tout le reste conservés). Aucun impact prod (le Style Visuel était gaté sur le clone → jamais visible en prod). Leçon : pour un futur refresh visuel, partir d'une SPEC précise (valeurs hex/angles, ex. le widget Flutter fourni) ou d'un designer, pas d'une illustration à deviner.
const PRECACHE = [
