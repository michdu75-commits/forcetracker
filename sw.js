 ...
 */
const CACHE = 'ft-v560'; // Cartes de l'historique des séances ALLÉGÉES (retour GPT sur l'onglet Progrès) : le muscle principal (ou le nom de la séance du programme) devient le GROS TITRE de la carte ; la date + le volume (rouge) passent en ligne d'info discrète ; les calories rendues discrètes (petit, gris/orange) ; liste des exercices limitée à 2 lignes (`-webkit-line-clamp`). renderSessions (setup.js) restructuré (une seule colonne texte + mini-figurine à droite), CSS `.sess-title`/`.sess-meta`/`.sess-date2`/`.sess-vol2`/`.sess-cal2`/`.sess-exs2`. Aucune donnée touchée (mêmes onclick openSessDetail/showSessMuscleMap). Testé Playwright + captures jour/nuit (titre 15/800, calories en t3, 0 erreur JS).
const PRECACHE = [
