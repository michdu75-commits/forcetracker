 ...
 */
const CACHE = 'ft-v909'; // 🔄 ft-v909 = « OU TU EN ES » — LA NUTRITION REPOND ENFIN A LA VRAIE QUESTION. Michel, sur sa propre app : « meme moi ca me saoule de l'utiliser, c'est assez mal fait » · « ce n'est pas intuitif, je veux voir OU J'EN SUIS ». ⭐ LE DIAGNOSTIC N'EST PAS UN BUG, C'EST UNE QUESTION MAL POSEE : l'ecran repondait a « combien il te reste a manger aujourd'hui » — une question qui n'a de sens QUE si on a deja tout note. ⚠️⚠️ LA REGLE QUI TIENT TOUT : une semaine INCOMPLETE produit une moyenne HONNETE. On divise par le nombre de jours REELLEMENT notes, jamais par 7, et on ECRIT combien il y en a (« 3 jours notes sur 7 »). Diviser par 7 quand 3 jours sont notes affiche une sous-alimentation qui n'existe pas — exactement le chiffre faux qui fait abandonner un suivi (P21). ⚠️ ET ON N'AFFICHE RIEN QUAND ON NE SAIT RIEN : zero jour note → une invitation, pas un « 0 / 2600 kcal » qui ressemble a un reproche (R29). Aujourd'hui non note → on le dit, on n'invente pas un zero.
const PRECACHE = [
